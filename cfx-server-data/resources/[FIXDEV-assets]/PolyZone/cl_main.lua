...
RegisterNetEvent('FIXDEV-base:PolyZoneUpdate')
AddEventHandler('FIXDEV-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "FIXDEV-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('FIXDEV-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('FIXDEV-polyzone:ssadasdsadsda2')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('FIXDEV-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('FIXDEV-polyzone:stop2')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('FIXDEV-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('FIXDEV-polyzone:wedasdsadasdsadsadsa')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('FIXDEV-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('FIXDEV-polyzone:stop')
            end
        end
