...
toggleFaceWear = function(pWearType, pShouldRemove)
  ...
  if pWearType == 1 and not pShouldRemove and removedHatManually then
    local hasHat = exports["FIXDEV-inventory"]:hasEnoughOfItem("hat", 1, false, true, {hat = facialWear[PropIndex+1]["Prop"]})
    if not hasHat then
  ...
  if pWearType == 4 and not pShouldRemove and removedMaskManually then
    local hasMask = exports["FIXDEV-inventory"]:hasEnoughOfItem("mask", 1, false, true, {mask = facialWear[pWearType]["Prop"]})
    if not hasMask then
    ...
    if pWearType ~= 5 then
      ...
      if pWearType == 4 then
        ...
        if currentDrawable ~= -1 then
          ...
          local hasPropItem = exports["FIXDEV-inventory"]:hasEnoughOfItem("mask", 1, false, true, itemMeta)
        ...
        if pWearType ~= 2 then
          ...
          if pWearType == 1 and currentProp ~= -1 then
            ...
            local hasPropItem = exports["FIXDEV-inventory"]:hasEnoughOfItem("hat", 1, false, true, itemMeta)
            if not hasPropItem then
...
AddEventHandler("FIXDEV-jobmanager:playerBecameJob", function(job, name)
  ...
  if DoesEntityExist(objFound) then
    -- TriggerEvent("DoLongHudText","Press [E] to leave the bed at any time.",1)
      ...
      if not disableKeys then
        showInteraction = true
        exports["FIXDEV-ui"]:showInteraction("[E] Leave Bed; [F] Dismiss")
      end
...
Citizen.CreateThread(function()
  while true do
    local isSitting = exports["FIXDEV-flags"]:HasPedFlag(PlayerPedId(), "isSittingOnChair")
...
RegisterNetEvent('FIXDEV-voice:focus:set')
AddEventHandler('FIXDEV-voice:focus:set', function(hasFocus, hasKeyboard, hasMouse)
...
RegisterNetEvent('pFIXDEV:admin:maxstats')
AddEventHandler('pFIXDEV:admin:maxstats', function()
    currentValues["thirst"] = 100
    currentValues["hunger"] = 100 
...
function initHud(hasStarted)
  ...
  if hasStarted then
    --Wait(100)
    local initialHudSettings = exports["FIXDEV-hud"]:GetPreferences()
    if initialHudSettings ~= "{}" then
      exports["FIXDEV-hud"]:sendAppEvent("preferences", initialHudSettings)
      exports["FIXDEV-ui"]:sendAppEvent('preferences', initialHudSettings)
      TriggerEvent("FIXDEV-preferences:setPreferences", initialHudSettings)
    end
    exports["FIXDEV-hud"]:sendAppEvent("hud", {
      display = true,
    })
  end
end
  ...
  while true do
    ...
    if counter == 0 then
      ...
      if valueChanged then
        -- RPC.execute("FIXDEV-hud:UpdateNeeds", currentValues)
        exports["FIXDEV-hud"]:startHealthArmorUpdates()
        exports["FIXDEV-hud"]:sendAppEvent("hud", {
...
AddEventHandler("buffs:triggerBuff", function(item)
  ...
  TriggerEvent("FIXDEV-buffs:addUIBuff", { buffedHunger = true, buffedThirst = true })
  Citizen.CreateThread(function()
    Citizen.Wait(60000 * 60 * 6) -- 6hr
    buffStartTime = 0
    TriggerEvent("FIXDEV-buffs:addUIBuff", { buffedHunger = false, buffedThirst = false })
  end)
end)
...
AddEventHandler("buffs:triggerBuff", function(item)
  ...
  TriggerEvent("FIXDEV-buffs:addUIBuff", { buffedHealth = true, buffedArmor = true })
  Citizen.CreateThread(function()
    Citizen.Wait(60000 * 60 * 6) -- 3hr
    citrineStartTime = 0
    TriggerEvent("FIXDEV-buffs:addUIBuff", { buffedHealth = false, buffedArmor = false })
  end)
end)
Citizen.CreateThread(function()
  while true do
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(item)
  ...
  local finished = exports["FIXDEV-taskbar"]:taskBar(4000, "Using Chapstick")
...
RegisterNetEvent("FIXDEV-police:cuffs:state")
AddEventHandler("FIXDEV-police:cuffs:state", function(pIsHandcuffed, pIsHandcuffedAndWalking)
...
AddEventHandler("baseevents:enteredVehicle", function(pVehicle)
  ...
  local hasLicense = exports["FIXDEV-gov"]:HasLicense("pilot_license")
  if hasLicense then
    return
  end
