      ...
      if pWearType == 4 then
        ...
        if currentDrawable ~= -1 then
          ...
          if not hasPropItem then
            ...
            exports['FIXDEV-clothing']:SaveCurrentClothing(PlayerPedId())
          end
  ...
  while true do
    ...
    if counter == 0 then
      ...
      if valueChanged then
        -- TriggerEvent("FIXDEV-ui:hud:values", currentValues)
...
AddEventHandler("shoes:steal", function(pArgs, pEntity)
  ...
  Citizen.Wait(1600)
  ClearPedTasks(PlayerPedId())
  TriggerServerEvent("FIXDEV-clothing:removeShoes", GetPlayerServerId(NetworkGetPlayerIndexFromPed(pEntity)))
    ...
    while BLOCKING_LOOP and not dToggle do
      if IsEntityInWater(pVehicle) then
        if GetPedInVehicleSeat(pVehicle, -1) == PlayerPedId() then
          TriggerServerEvent("FIXDEV:vehicles:reduceFuel", NetworkGetNetworkIdFromEntity(pVehicle), 1)
        end
      end
      Wait(4000)
    end
