...
RegisterNetEvent('FIXDEV-meth:startMeth')
AddEventHandler('FIXDEV-meth:startMeth', function()
    local ped = PlayerPedId()
    if exports["FIXDEV-inventory"]:hasEnoughOfItem("methstick",1,false) then 
        TriggerEvent("animation:PlayAnimation","type")
        FreezeEntityPosition(ped, true)
        local finished = exports["FIXDEV-taskbar"]:taskBar(15000,"Starting meth production")
...
RegisterNetEvent('FIXDEV-meth:startp1')
AddEventHandler('FIXDEV-meth:startp1', function()
    local ped = PlayerPedId()
    if allowedtostart then
        if exports["FIXDEV-inventory"]:hasEnoughOfItem("chloroform",1,false) then
            TriggerEvent("animation:PlayAnimation","push")
            FreezeEntityPosition(ped, true)
            local finished = exports["FIXDEV-taskbar"]:taskBar(30000,"Placing chloroform into furnace")
            if finished == 100 then
                FreezeEntityPosition(ped, false)
...
RegisterNetEvent('FIXDEV-meth:PrepareOven')
AddEventHandler('FIXDEV-meth:PrepareOven', function()
    local ped = PlayerPedId()
    if donep1 then 
        TriggerEvent("animation:PlayAnimation","browse")
        FreezeEntityPosition(ped, true)
        local finished = exports["FIXDEV-taskbar"]:taskBar(30000,"Preparing oven")
        if finished == 100 then
            FreezeEntityPosition(ped, false)
...
AddEventHandler("dreams-meth:begin-cook", function()
	...
	if donep2 then
        if exports["FIXDEV-inventory"]:hasEnoughOfItem("methbrick",1) then
            Cooking()
            TriggerEvent("animation:meth")
            FreezeEntityPosition(PlayerPedId(),true)
            SetEntityHeading(PlayerPedId(), 179.97245788574)
            local finished = exports["FIXDEV-taskbar"]:taskBar(115000,"Mixing Chemicals")
            if (finished == 100) then
                donep2 = false
...
AddEventHandler("pack:crystal", function()
	...
	if exports["FIXDEV-inventory"]:hasEnoughOfItem("methraw",5,false) and exports["FIXDEV-inventory"]:hasEnoughOfItem("drugbaggy",1,false) and ongoing == false then
		FreezeEntityPosition(PlayerPedId(),true)
		SetEntityHeading(PlayerPedId(), 357.65960693359)
		Drying()
        TriggerEvent("animation:push")
		local finished = exports["FIXDEV-taskbar"]:taskBar(32000,"Drying crystals be careful!")
		if (finished == 100) then
			FreezeEntityPosition(PlayerPedId(),false)
