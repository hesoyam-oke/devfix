...
AddEventHandler("Animation:Set:Gait", function(pArgs)
    ...
    if setGait == "move_m@swagger" then
      if exports["FIXDEV-inventory"]:hasEnoughOfItem("pimpcane", 1) then
...
anims = {
    ...
    ["c"] = function(ped)
        if exports["FIXDEV-flags"]:HasPedFlag(PlayerPedId(), 'isSittingOnChair') then
          TriggerEvent("FIXDEV-emotes:sitOnChair")
        elseif exports["FIXDEV-flags"]:HasPedFlag(PlayerPedId(), 'isPoledancing') then
    ...
    ["umbrella"] = function(ped)
        if obj ~= 0 then
            ...
            TriggerEvent("destroyPropPerm")
            if exports["FIXDEV-inventory"]:hasEnoughOfItem("umbrella", 1) then

                TriggerEvent("actionbar:setEmptyHanded")

                local finished = exports["FIXDEV-taskbar"]:taskBar(2500,"Opening Umbrella")
    ...
    ["smoke"] = function(ped)
        ...
        if exports["FIXDEV-inventory"]:hasEnoughOfItem("ciggy", 1) then
            ClearPedTasks(PlayerPedId())
    ...
    ["cigar"] = function(ped)
        if exports["FIXDEV-inventory"]:hasEnoughOfItem("cigar", 1) then
            local animDict = "amb@world_human_smoking@male@male_a@enter"
            local animation = "enter"
            if IsPedArmed(ped, 7) then
                TriggerEvent("actionbar:setEmptyHanded")
    ...
    ["floss"] = function(ped)
        local animDict = "nikez@floss"
        local animation = "floss"
...
animsDog = {
    ...
    ["carenter"] = function(ped)
        local animDict = "creatures@rottweiler@in_vehicle@std_car"
        local currentTarget = exports['FIXDEV-target']:GetCurrentEntity()
        ...
        local currentTargetCoords = exports['FIXDEV-target']:GetCurrentTargetCoords()
