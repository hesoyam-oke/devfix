...
AddEventHandler("FIXDEV-jobmanager:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
AddEventHandler('FIXDEV-base:addedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)

RegisterNetEvent('FIXDEV-base:removedMoney')
AddEventHandler('FIXDEV-base:removedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)
