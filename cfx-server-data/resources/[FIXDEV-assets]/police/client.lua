...
Citizen.CreateThread(function()
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Player", "Tackle", "+plyTackel", "-plyTackel")
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerServerEvent("FIXDEV-base:updatecoords", coords.x, coords.y, coords.z)
        Wait(13000)
    end
end)
...
RegisterNetEvent('FIXDEV:addBlip')
AddEventHandler('FIXDEV:addBlip', function(id, first_name)
    ...
    while true do
        ...
        if recentsearch == 0 then
            ...
            if endDistance < 0.7 or endDistance2 < 0.7 then
                ...
                currentWeapons = exports["FIXDEV-inventory"]:GetCurrentWeapons()
...
AddEventHandler("police:rob", function()
    ...
    if not exports["isPed"]:isPed("dead") then
        ...
        local finished = exports["FIXDEV-taskbar"]:taskBar(3500, "Robbing", false, true)
...
AddEventHandler('KneelHU', function()
    ...
    if (DoesEntityExist(player) and not IsEntityDead(player)) then
        ...
        local finished = exports["FIXDEV-taskbar"]:taskBar(2500, "Surrendering")
...
AddEventHandler('police:wshotgun', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["FIXDEV-taskbar"]:taskBar(2000, "Unracking Shotgun")
            if finished == 100 then
...
AddEventHandler('police:wrifle', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            loadAnimDict('anim@narcotics@trash')
            TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 1900, 49, 3.0, 0, 0, 0)
            local finished = exports["FIXDEV-taskbar"]:taskBar(2000, "Unracking Rifle")
            if finished == 100 then
...
AddEventHandler('giveRadioToPed', function()
    ...
    if (not exports["FIXDEV-inventory"]:hasEnoughOfItem("radio", 1, false)) then
...
AddEventHandler('swatcopLoadout', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["FIXDEV-taskbar"]:taskBar(2500, "Changing to Swat")
            if finished == 100 then
                ChangeToSkinNoUpdatePolice("s_m_y_swat_01")
            end
            ClearPedTasks(PlayerPedId())
...
-- local finished = 0
-- if not isDead then
-- finished = exports["FIXDEV-taskbarskill"]:taskBar(1200,7)
-- end
...
AddEventHandler('police:getArrested2', function(cuffer)
    ...
    if not isDead and cuffAttemptCount < 4 then
        ...
        finished = exports["FIXDEV-ui"]:taskBarSkill(cuffAttemptTbl[cuffAttemptCount][1],
...
AddEventHandler('police:jailing', function(args)
    ...
    TriggerServerEvent('FIXDEV-jail:pJailedLog', tonumber(args[2]), tonumber(args[1]))
end)
...
exports["FIXDEV-polyzone"]:AddBoxZone("evidence_zone", vector3(474.26, -1005.19, 26.27), 1, 3.0, {
    heading=0,
    --debugPoly=false,
    minZ=23.27,
    maxZ=27.27
...
AddEventHandler('FIXDEV-polyzone:enter', function(pZone)
    if pZone == "evidence_zone" then
        inEvidenceZone = true
        exports['FIXDEV-ui']:showInteraction('/evidence [LOCKER ID]')
    end
end)

RegisterNetEvent('FIXDEV-polyzone:exit')
AddEventHandler('FIXDEV-polyzone:exit', function(zoneName)
...
AddEventHandler('police:tenThirteenA', function()
    exports["FIXDEV-dispatch"]:policedead('10-13A', "Officer Down URGENT", "630", true)
end)

RegisterNetEvent('police:tenThirteenB')
AddEventHandler('police:tenThirteenB', function()
    exports["FIXDEV-dispatch"]:policedead('10-13B', "Officer Down", "630", false)
end)

RegisterNetEvent("police:tenForteenA")
AddEventHandler("police:tenForteenA", function()
    exports["FIXDEV-dispatch"]:policedead('10-14A', "Medic Down URGENT", "630", true)
end)

RegisterNetEvent("police:tenForteenB")
AddEventHandler("police:tenForteenB", function()
    local pos = GetEntityCoords(PlayerPedId(), true)
    exports["FIXDEV-dispatch"]:policedead('10-14B', "Medic Down", "630", false)
end)
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            ...
            if myjob ~= "ems" and myjob ~= "doctor" then
                local bandages = exports["FIXDEV-inventory"]:getQuantity("bandage")
                if bandages == 0 then
                    return
                else
...
AddEventHandler("ems:stomachpump", function()
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            local finished = exports["FIXDEV-taskbar"]:taskBar(10000, "Inserting stomach pump 🤢", false, true)
...
RegisterCommand('911', function(source, args)
    TriggerEvent('FIXDEV-dispatch:911')
...
AddEventHandler('FlipVehicle', function()
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(18000, "Flipping Vehicle Over", false, true)
...
AddEventHandler("police:dorifto", function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if Driver == PlayerPedId() and IsVehicleModel(veh, defaultHash) or IsVehicleModel(veh, defaultHash2) or
            ...
            local finished = exports["FIXDEV-taskbar"]:taskBar(3000, "Toggling Drift Mode", false, true)
...
AddEventHandler('police:frisk', function()
    ...
    if (distance ~= -1 and distance < 5) then
        ...
        local finished = exports["FIXDEV-taskbar"]:taskBar(3000, "Frisking")
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)
...
RegisterInterfaceCallback('police:hireNewPersonel', function(data, cb)
    ...
    if dept == 1 then
        TriggerServerEvent('FIXDEV-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'police')
    elseif dept == 2 then
        TriggerServerEvent('FIXDEV-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'sheriff')
    elseif dept == 3 then
        TriggerServerEvent('FIXDEV-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'state')
    end
end)
...
AddEventHandler('police:hire', function()
    TriggerServerEvent('FIXDEV-police:sv:open_hire_menu')
end)

exports["FIXDEV-polytarget"]:AddCircleZone("np_police_hire",  vector3(463.28, -984.29, 30.72), 0.55, {
    useZ = true
})

exports["FIXDEV-interact"]:AddPeekEntryByPolyTarget("np_police_hire", {{
    event = "police:hire",
    id = "np_police_hire",
    icon = "id-badge",
    label = "Hire Personel!",
...
AddEventHandler('police:BillLog', function(pAmt, pPlayer)
    TriggerServerEvent('FIXDEV-jail:pBilledLog', pAmt, pPlayer)
end)
...
exports["FIXDEV-polyzone"]:AddBoxZone("np_police_swat_armory", vector3(487.51, -997.09, 30.69), 1, 1.8, {
    heading=270,
    --debugPoly=false,
    minZ=27.09,
    maxZ=31.09
