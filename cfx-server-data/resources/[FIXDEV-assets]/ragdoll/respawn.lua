...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        while penis < 10 do
            penis = penis + 1
            local finished = exports["FIXDEV-ui"]:taskBarSkill(math.random(2000, 6000), math.random(5, 10))
...
function releaseBody(dropItems)
    ...
    if dropItems then
        ...
        TriggerServerEvent("FIXDEV-ragdoll:removeallitems")
    end
    ...
    local doctors = RPC.execute('FIXDEV-jobmanager:jobCount', 'doctor')
    ...
    TriggerServerEvent('FIXDEV-healthcare:playerRespawn', hosp)
