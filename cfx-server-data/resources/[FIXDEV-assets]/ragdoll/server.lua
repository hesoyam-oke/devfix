...
RegisterServerEvent('FIXDEV-ragdoll:removeallitems')
AddEventHandler('FIXDEV-ragdoll:removeallitems', function()
    local src = source
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local characterId = user:getVar("character").id
    local invname = "ply-"..characterId

    exports.oxmysql:execute("DELETE FROM user_inventory2 WHERE `name` = @name", {['@name'] = invname})
end)
...
AddEventHandler('ragdoll:reviveSV', function(t)
    ...
    TriggerClientEvent('FIXDEV-hospital:client:RemoveBleed', t) 
    TriggerClientEvent('FIXDEV-hospital:client:ResetLimbs', t)
end)
