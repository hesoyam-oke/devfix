...
Citizen.CreateThread(function()
    while true do
        ...
        if statusS == true then
            TriggerServerEvent("FIXDEV-spawn:inspawn", true)
        end
    end
end)
...
AddEventHandler("FIXDEV-base:initialSpawnModelLoaded", function()
...
AddEventHandler("FIXDEV-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports['FIXDEV-ui']:showInteraction('[M] '..MenuData[zone].text..'')
...
RegisterNetEvent("raid_clothes:attempt:change", function(data)
    local pNewName = exports["FIXDEV-applications"]:KeyboardInput({
