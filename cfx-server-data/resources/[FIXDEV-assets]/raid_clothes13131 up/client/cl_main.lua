...
RegisterNetEvent("FIXDEV-clothes:notsave")
AddEventHandler("FIXDEV-clothes:notsave", function()
    TriggerEvent("FIXDEV-spawn:finishedClothing","Old")
    LoadPed(oldPed)
end)

RegisterNetEvent("FIXDEV-clothing:save")
AddEventHandler("FIXDEV-clothing:save", function()
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            ...
            TriggerEvent("FIXDEV-spawn:finishedClothing","Finished")
    ...
    TriggerEvent('FIXDEV-weapons:getAmmo')
...
RegisterInterfaceCallback("FIXDEV-ui:raid_clothes:addOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    Wait(1) --wait to fix ui bug?
    exports['FIXDEV-interface']:openApplication('textbox', {
        callbackUrl = 'FIXDEV-ui:raid_clothes:addOutfit',
        key = 1,
        items = {
            {
                icon = "pencil-alt",
...
RegisterInterfaceCallback("FIXDEV-ui:raid_clothes:addOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback("FIXDEV-ui:raid_clothes:changeOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    TriggerEvent('hotel:outfit', { true, data.key }, 3)
    exports["FIXDEV-interface"]:hideContextMenu()
end)

RegisterInterfaceCallback("FIXDEV-ui:raid_clothes:deleteOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
...
AddEventHandler('raid_clothes:ListOutfits', function(skincheck)
    ...
    for i = 1, #skincheck do
        ...
        menuData[#menuData + 1] = {
            ...
            icon = 'tshirt',
            children = {
                { title = "Change Outfit", action = "FIXDEV-ui:raid_clothes:changeOutfit", icon = 'edit', key = slot},
                { title = "Delete Outfit", action = "FIXDEV-ui:raid_clothes:deleteOutfit", icon = 'trash', key = slot},
            }
        }
    end
    if #menuData >= 0 then
        if #menuData < 20 then
            ...
            menuData[#menuData + 1] = {
                ...
                description = '',
                key = emptySlot,
                action = "FIXDEV-ui:raid_clothes:addOutfitPrompt",
                icon = 'save'
            }
        end
...
RegisterNetEvent("FIXDEV-context:closeglobal")
AddEventHandler("FIXDEV-context:closeglobal", function()
    ...
    local data = {
        {
            ...
            description = "",
            key = true,
            action = "FIXDEV-ui:raid_clothes:CustomaddOutfitPrompt"
        },
    }

    exports["FIXDEV-interface"]:showContextMenu(data)
end)

RegisterInterfaceCallback("FIXDEV-ui:raid_clothes:CustomaddOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    Wait(1) --wait to fix ui bug?
    exports['FIXDEV-ui']:openApplication('textbox', {
        callbackUrl = 'FIXDEV-ui:raid_clothes:addCustomOutfit',
        key = data.key,
        items = {
            {
                icon = "pencil-alt",
...
RegisterInterfaceCallback("FIXDEV-ui:raid_clothes:addCustomOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterCommand("h1", function(source, args, rawCommand)
    if exports['FIXDEV-inventory']:hasEnoughOfItem('hat', 1) then
...
RegisterCommand("m1", function(source, args, rawCommand)
    if exports['FIXDEV-inventory']:hasEnoughOfItem('mask', 1) then
