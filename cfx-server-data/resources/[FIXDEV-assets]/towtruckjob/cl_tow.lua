...
AddEventHandler('jobs:towVehicle', function (pParams, pEntity, pContext)
  local target = exports['FIXDEV-target']:GetEntityInFrontOfEntity(pEntity, -8, 0.6)
  ...
  if target and pEntity ~= target and GetEntityType(target) == 2 and GetEntityModel(pEntity) == `flatbed` then
    ...
    if distance <= 10 then
      ...
      local hooking = exports["FIXDEV-taskbar"]:taskBar(15000,"Hooking up vehicle.")
      if hooking == 100 then
        TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
        local towing = exports["FIXDEV-taskbar"]:taskBar(5000,"Towing Vehicle")
...
AddEventHandler('jobs:untowVehicle', function (pParams, pEntity, pContext)
  ...
  if target and target ~= pEntity and target ~= 0 then
    ...
    Citizen.Wait(1000)
    TriggerEvent("animation:tow")
    local untowing = exports["FIXDEV-taskbar"]:taskBar(7000,"Untowing Vehicle")

    if untowing == 100 then
      TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
      local unhooking = exports["FIXDEV-taskbar"]:taskBar(5000,"Unhooking Vehicle")
