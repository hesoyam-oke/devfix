...
Citizen.CreateThread(function ()
    PaintingsConfig = NPX.Utils.cache(function (id)
        local config = NPX.Procedures.execute('FIXDEV-business:uwucafe:getPaintings', id)
    ...
    exports['FIXDEV-interact']:AddPeekEntryByPolyTarget({ "uwucafe-painting" }, {
        {
            id =  'uwucafe-setpainting',
            label =  'change art pieces',
            icon =  'palette',
            event = "FIXDEV-business:uwucafe:changePainting",
            parameters =  {},
        }
    }, { distance = { radius = 5.0 } })
end)
...
local function SetCustomPainting(pPiece, pURL)
    ...
    dui = exports["FIXDEV-lib"]:getDui(pURL, 2048, 2048)
...
NPX.Events.onNet('FIXDEV-business:uwucafe:setPainting', function (pPainting, pURL)
...
AddEventHandler('FIXDEV-business:uwucafe:changePainting', function (pParams, pEntity, pCtx)
    ...
    NPX.Procedures.execute('FIXDEV-business:uwucafe:setPainting', data.id, prompt.url)
end)