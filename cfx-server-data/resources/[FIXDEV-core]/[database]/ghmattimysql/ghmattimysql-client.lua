...
function setNuiActive(booleanOrNil)
  ...
  if boolean ~= isNuiActive then
    if boolean then TriggerServerEvent('oxmysql:request-data') end
    isNuiActive = boolean
...
CreateThread(function()
  if isNuiActive then TriggerServerEvent('oxmysql:request-data') end
  Wait(300000)
end)
...
RegisterNetEvent('oxmysql:update-resource-data')
AddEventHandler('oxmysql:update-resource-data', function (resourceData)
...
RegisterNetEvent('oxmysql:update-time-data')
AddEventHandler('oxmysql:update-time-data', function (timeData)
  local timeArray = {}
...
RegisterNetEvent('oxmysql:update-slow-queries')
AddEventHandler('oxmysql:update-slow-queries', function(slowQueryData)
...
RegisterNetEvent('oxmysql:update-status')
AddEventHandler('oxmysql:update-status', function(statusData)
  SendNUIMessage({
    type = 'onStatusData',
    status = statusData,
  });
...
RegisterNetEvent('oxmysql:update-variables')
AddEventHandler('oxmysql:update-variables', function(variableData)
  SendNUIMessage({
    type = 'onVariableData',
...
RegisterNUICallback('request-server-status', function()
  TriggerServerEvent('oxmysql:request-server-status')
end)

RegisterNetEvent('oxmysql:get-table-schema')
AddEventHandler('oxmysql:get-table-schema', function(tableName, schema)
  SendNUIMessage({
    type = 'onTableSchema',
    tableName = tableName,
    schema = schema,
...
RegisterNUICallback('request-table-schema', function(tableName)
  TriggerServerEvent('oxmysql:request-table-schema', tableName)
end)
