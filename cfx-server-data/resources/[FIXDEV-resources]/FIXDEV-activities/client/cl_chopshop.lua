...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("chop_zone", vector3(1359.1, -2097.81, 52.0), 15, 15, {
    heading=40,
    --debugPoly=false,
    minZ=51.0,
    maxZ=55.0
...
AddEventHandler("FIXDEV-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == aspectChopCar then
        ...
        aspectStartChopping()
        exports['FIXDEV-ui']:showInteraction('[E] Start Chopping')
    end
end)
...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(10000,"Slicing Vehicle Tyre")
...
function ChopVehicleDoor(vehicle, boneID, doorIndex)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["FIXDEV-taskbar"]:taskBar(14000,"Messing With Door")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["FIXDEV-taskbar"]:taskBar(25000,"Seeing Whats Left ..")
...
AddEventHandler('aspect-chop:cooldown', function()
    ...
    if ClockedIntoChop then
        TriggerEvent('FIXDEV-phone:chop_shop_offer')
        CreateChopCar()
    end
end)
...
function ChopVehiclePart(vehicle)
    ...
    if success then
        TriggerServerEvent("FIXDEV-chopshop:vehicle_loot", bone.type)
...
RegisterNetEvent('FIXDEV-chopshop:clock-in')
AddEventHandler('FIXDEV-chopshop:clock-in', function()
    if CoolDownActive then
        TriggerEvent('DoLongHudText', 'You have a active cooldown!', 2)
    elseif ClockedIntoChop then
        TriggerEvent('FIXDEV-chopshop:clock-out')
    else
        ClockedIntoChop = true
        TriggerEvent('FIXDEV-phone:signInInfoChopshop')
        Citizen.Wait(math.random(60000, 120000))
        TriggerEvent('FIXDEV-phone:chop_shop_offer')
    end
end)

RegisterNetEvent('FIXDEV-chopshop:clock-out')
AddEventHandler('FIXDEV-chopshop:clock-out', function()
    if ClockedIntoChop then
