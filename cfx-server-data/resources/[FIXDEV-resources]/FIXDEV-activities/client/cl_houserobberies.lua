...
Citizen.CreateThread(function()
	local result = RPC.execute('FIXDEV-houserobbery:enter_property')
					...
					if #(GetEntityCoords(PlayerPedId()) - objCoords) <= items[i].dist and not items[i].robado then
						...
						if IsControlJustPressed(0,38) then
							...
							if items[i].anim == 'cajafuerte' then
								...
								TriggerEvent('FIXDEV-houserobber:animations',items[i].anim,items[i].delete,i)
							end
						end
					end
				end
...
RegisterNetEvent('FIXDEV-activities:acceptJob')
AddEventHandler('FIXDEV-activities:acceptJob', function()
...
RegisterNetEvent('FIXDEV-houserobbery:getJob')
AddEventHandler('FIXDEV-houserobbery:getJob', function()
	if not working then
		if exports['FIXDEV-inventory']:hasEnoughOfItem('vpnxj', 1) then
			if exports['FIXDEV-inventory']:hasEnoughOfItem('advlockpick', 1) then
				cleanup()
				working = true
				enterCoords = Houses()
                TriggerEvent('FIXDEV-phone:signInInfoHouseRobbery')
				Citizen.Wait(math.random(1,2) * 60 * 1000)
				TriggerEvent('FIXDEV-phone:house_rob_offer')
			else
...
RegisterNetEvent('FIXDEV-houserobbery:make_noise')
AddEventHandler('FIXDEV-houserobbery:make_noise', function()
	...
	while stealing do		
		if within then	
			...
			if noise > 100 then
				-- dispatch for pd
				exports["FIXDEV-dispatch"]:dispatchadd('10-31A', "House Robbery", "40")
				Citizen.Wait(300000)
			end
		end
...
RegisterCommand('raid', function()
	...
	if (job == "police" or job == "sheriff" or job == "state" or job == "sheriff" or job == "ranger") then
		TriggerEvent('FIXDEV-houserobbery:police_enter')
	end
end)


RegisterNetEvent('FIXDEV-houserobbery:police_enter')
AddEventHandler('FIXDEV-houserobbery:police_enter', function()
	...
	while working do
		local espera = 1500
			...
			if distancia < 3 and isnight() then
				espera = 5
				exports['FIXDEV-ui']:showInteraction('[H] To Enter House')
					if IsControlJustReleased(0,74) and distancia < 3 and firstentry then
						RemoveBlip(HouseRobberyLocation)
						if exports['FIXDEV-inventory']:hasEnoughOfItem('advlockpick', 1) then
							Animation()
							local finished = exports["FIXDEV-ui"]:taskBarSkill(3500,math.random(100, 200))
							if (finished == 100) then
								local finished2 = exports["FIXDEV-ui"]:taskBarSkill(1000,math.random(200,400))
								if (finished2 == 100) then
									local finished3 = exports["FIXDEV-ui"]:taskBarSkill(2000,math.random(50,100))
									if (finished3 == 100) then
										local finished4 = exports["FIXDEV-ui"]:taskBarSkill(1250,math.random(200,400))
										if (finished4 == 100) then
											local finished5 = exports["FIXDEV-ui"]:taskBarSkill(5000,math.random(200,400))
...
function EnterHouse(police)
	if not stealing then
		...
		stealing = true
		if not police then
			TriggerEvent('FIXDEV-houserobbery:make_noise')
		end
	else
...
RegisterNetEvent('FIXDEV-houserobber:animations')
AddEventHandler('FIXDEV-houserobber:animations', function(anim, delete, i)
	local ped = PlayerPedId()
	...
	if anim == 'tv' then
		...
		local StolenTV = exports['FIXDEV-taskbar']:taskBar(10000, 'Grabbing TV')
	...
	elseif anim == 'telescopio' then
		...
		FreezeEntityPosition(ped,true)
		TaskPlayAnim(ped, "mp_common_heist", 'use_terminal_loop', 2.0, 2.0, -1, 1, 0, true, true, true)
		local StolenTelescope = exports['FIXDEV-taskbar']:taskBar(2000, 'Grabbing Telescope')
	...
	elseif anim == 'pintura' then
		FreezeEntityPosition(ped,true)
		TaskPlayAnim(ped, "mp_common_heist", 'use_terminal_loop', 2.0, 2.0, -1, 1, 0, true, true, true)
		local StolenArt = exports['FIXDEV-taskbar']:taskBar(2000, 'Grabbing Art')
	...
	elseif anim == 'normal' then
		FreezeEntityPosition(ped,true)	
		AnimBolso()
		TriggerEvent('FIXDEV-houserobbery:common_items')
		if delete then
			DeleteEntity(items[i].objeto)
		end
	...
	elseif anim == 'laptop' then
		...
		FreezeEntityPosition(ped,true)
		TaskPlayAnim(ped, "mp_common_heist", 'use_terminal_loop', 2.0, 2.0, -1, 1, 0, true, true, true)
		local StolenMicrowave = exports['FIXDEV-taskbar']:taskBar(5000, 'Grabbing Microwave')
...
RegisterNetEvent("FIXDEV-houserobbery:common_items")
AddEventHandler("FIXDEV-houserobbery:common_items", function()
...
RegisterNetEvent('FIXDEV-houserobberies:sell_tv')
AddEventHandler('FIXDEV-houserobberies:sell_tv', function()
    if exports['FIXDEV-inventory']:hasEnoughOfItem('stolentv', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Selling Stolen TV.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolentv', 1)
           if exports['FIXDEV-hud']:MoneyBuff() then
                TriggerServerEvent('FIXDEV-activities:givePayout', math.random(250, 325), 'cash', '[House Robbery Saless | With Buff]')
            else
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(100, 300), 'cash', '[House Robbery Saless | Without Buff]')
            end
        end
    else
...
RegisterNetEvent('FIXDEV-houserobberies:sell_telescope')
AddEventHandler('FIXDEV-houserobberies:sell_telescope', function()
    if exports['FIXDEV-inventory']:hasEnoughOfItem('stolentelescope', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Selling Stolen Telescope.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('FIXDEV-houserobberies:sell_art')
AddEventHandler('FIXDEV-houserobberies:sell_art', function()
    if exports['FIXDEV-inventory']:hasEnoughOfItem('stolenart', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Selling Stolen Art.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolenart', 1)
           if exports['FIXDEV-hud']:MoneyBuff() then
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(225, 315), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(100, 300), 'cash', '[House Robbery Saless | Without Buff]')
        	end
   		end
...
RegisterNetEvent('FIXDEV-houserobberies:sell_laptop')
AddEventHandler('FIXDEV-houserobberies:sell_laptop', function()
    if exports['FIXDEV-inventory']:hasEnoughOfItem('stolenlaptop', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Selling Stolen Laptop.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolenlaptop', 1)
           if exports['FIXDEV-hud']:MoneyBuff() then
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(75, 126), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(50, 100), 'cash', '[House Robbery Saless | Without Buff]')
        	end
   		end
    else
...
RegisterNetEvent('FIXDEV-houserobberies:sell_microwave')
AddEventHandler('FIXDEV-houserobberies:sell_microwave', function()
    if exports['FIXDEV-inventory']:hasEnoughOfItem('stolenmicrowave', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Selling Stolen Microwave.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolenmicrowave', 1)
           if exports['FIXDEV-hud']:MoneyBuff() then
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(125, 200), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(75, 125), 'cash', '[House Robbery Saless | Without Buff]')
        	end
   		end
    else
...
RegisterNetEvent('FIXDEV-houserobberies:sell_nokia')
AddEventHandler('FIXDEV-houserobberies:sell_nokia', function()
    if exports['FIXDEV-inventory']:hasEnoughOfItem('stolennokia', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Selling Stolen Nokia.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolennokia', 1)
           if exports['FIXDEV-hud']:MoneyBuff() then
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(75, 100), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(25, 50), 'cash', '[House Robbery Saless | Without Buff]')
            end
        end
    else
...
RegisterNetEvent('FIXDEV-houserobberies:sell_iphone')
AddEventHandler('FIXDEV-houserobberies:sell_iphone', function()
    if exports['FIXDEV-inventory']:hasEnoughOfItem('stoleniphone', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Selling Stolen iPhone.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('FIXDEV-houserobberies:sell_gameboy')
AddEventHandler('FIXDEV-houserobberies:sell_gameboy', function()
    if exports['FIXDEV-inventory']:hasEnoughOfItem('stolengameboy', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Selling Stolen Game Boy.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolengameboy', 1)
           if exports['FIXDEV-hud']:MoneyBuff() then
			TriggerServerEvent('FIXDEV-activities:givePayout', math.random(75, 80), 'cash', '[House Robbery Saless | With Buff]')
		else
			TriggerServerEvent('FIXDEV-activities:givePayout', math.random(26, 50), 'cash', '[House Robbery Saless | Without Buff]')
		end
        end
    else
...
RegisterNetEvent('FIXDEV-houserobberies:sell_casio_watch')
AddEventHandler('FIXDEV-houserobberies:sell_casio_watch', function()
    if exports['FIXDEV-inventory']:hasEnoughOfItem('stolencasiowatch', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Selling Stolen Casio Watch.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolencasiowatch', 1)
			if exports['FIXDEV-hud']:MoneyBuff() then
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(126, 137), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('FIXDEV-activities:givePayout', math.random(25, 75), 'cash', '[House Robbery Saless | Without Buff]')
			end
        end
    else
...
RegisterNetEvent('FIXDEV-houserobberies:sell_goods')
AddEventHandler('FIXDEV-houserobberies:sell_goods', function()

	local HouseRobSellData = {
		{
            title = "Sell Stolen TV",
            description = "",
            action = 'FIXDEV-houserobberies:sell_television',
        },
        {
            title = "Sell Stolen Telescope",
            description = "",
            action = 'FIXDEV-houserobberies:sell_ts',
        },
        {
            title = "Sell Stolen Art",
            description = "",
            action = 'FIXDEV-houserobberies:sell_art',
        },
        {
            title = "Sell Stolen Laptop",
            description = "",
            action = 'FIXDEV-houserobberies:sell_lt',
        },
        {
            title = "Sell Stolen Microwave",
            description = "",
            action = 'FIXDEV-houserobberies:sell_mw',
        },
        {
            title = "Sell Stolen Nokia",
            description = "",
            action = 'FIXDEV-houserobberies:sell_nk',
        },
        {
            title = "Sell Stolen iPhone",
            description = "",
            action = 'FIXDEV-houserobberies:sell_phone',
        },
        {
            title = "Sell Stolen Gameboy",
            description = "",
            action = 'FIXDEV-houserobberies:sell_gb',
        },
        {
            title = "Sell Stolen Casio Watch",
            description = "",
            action = 'FIXDEV-houserobberies:sell_watch',
        },
    }
    exports["FIXDEV-interface"]:showContextMenu(HouseRobSellData)
end)


RegisterInterfaceCallback('FIXDEV-houserobberies:sell_art', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('FIXDEV-houserobberies:sell_art')
end)

RegisterInterfaceCallback('FIXDEV-houserobberies:sell_television', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('FIXDEV-houserobberies:sell_tv')
end)

RegisterInterfaceCallback('FIXDEV-houserobberies:sell_ts', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('FIXDEV-houserobberies:sell_telescope')
end)

RegisterInterfaceCallback('FIXDEV-houserobberies:sell_lt', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('FIXDEV-houserobberies:sell_laptop')
end)

RegisterInterfaceCallback('FIXDEV-houserobberies:sell_mw', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('FIXDEV-houserobberies:sell_microwave')
end)

RegisterInterfaceCallback('FIXDEV-houserobberies:sell_nk', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('FIXDEV-houserobberies:sell_nokia')
end)

RegisterInterfaceCallback('FIXDEV-houserobberies:sell_phone', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('FIXDEV-houserobberies:sell_iphone')
end)

RegisterInterfaceCallback('FIXDEV-houserobberies:sell_gb', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('FIXDEV-houserobberies:sell_gameboy')
end)

RegisterInterfaceCallback('FIXDEV-houserobberies:sell_watch', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('FIXDEV-houserobberies:sell_casio_watch')
end)
