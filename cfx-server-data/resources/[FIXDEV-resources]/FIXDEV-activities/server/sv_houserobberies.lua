...
Citizen.CreateThread(function()
	...
	RPC.register('FIXDEV-houserobbery:enter_property', function(source, ...)
		local cb = {x,y,z,num}
		return cb
	end)	
...
RegisterServerEvent('FIXDEV-activities:givePayout')
AddEventHandler('FIXDEV-activities:givePayout', function(amount, pType, text)
    local src = source
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "** NP [Activities] | Payout Log **",
      ...
      PerformHttpRequest("https://discord.com/api/webhooks/1012082748120371200/uKANz5zVohLS7k2k2e9TqIszavZMMGXZB4gssBjzjYBxZhuBh5cozpHIN9dpFlMSkgF0", function(err, text, headers) end, 'POST', json.encode({username = "NP | Job Payout Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)