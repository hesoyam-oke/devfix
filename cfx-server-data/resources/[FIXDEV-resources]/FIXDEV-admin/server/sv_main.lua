...
RegisterNetEvent("FIXDEV:admin:searchPlayerInventory")
AddEventHandler("FIXDEV:admin:searchPlayerInventory", function(pTarget)
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(tonumber(pTarget))
    local cid = user:getCurrentCharacter().id
    TriggerClientEvent("server-inventory-open", source, "1", 'ply-'..cid)
end)

RegisterServerEvent('FIXDEV-admin:insertPrio')
AddEventHandler('FIXDEV-admin:insertPrio', function()
    local src = source
