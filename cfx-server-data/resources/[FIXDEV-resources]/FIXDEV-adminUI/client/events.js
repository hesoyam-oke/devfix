...
function toggleNUI(){
    ...
    SendNuiMessage(JSON.stringify({
        action: 'setKeyBinds',
        selectionMenu: global.exports["FIXDEV-keybinds"].getKeyMapping("+enableSelectionMenu"),
        selectionKey: global.exports["FIXDEV-keybinds"].getKeyMapping("+adminSelect")
    }))
}
function exitNUI()
{
...
on('__cfx_nui:closeMenu', (data, cb) => {
    ...
    if(data.toSelect){
        global.exports["FIXDEV-admin"].enterSelection();
    }
...
on('__cfx_nui:insideCommand', (data, cb) => {
    ...
    if(data.inside){
        ...
        emit('FIXDEV-binds:should-execute',false)
        ...
        emit('FIXDEV-binds:should-execute',true)
        if(blockingActions){
...
global.exports["FIXDEV-keybinds"].registerKeyMapping("", "zzAdmin", "Selection Menu", "+enableSelectionMenu", "-enableSelectionMenu", "")
