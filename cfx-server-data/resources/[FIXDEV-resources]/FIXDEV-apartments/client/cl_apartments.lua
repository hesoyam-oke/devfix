...
RegisterNetEvent('FIXDEV-binds:keyEvent')
AddEventHandler('FIXDEV-binds:keyEvent', function(name,onDown)
...
function Apart.processBuildType(numMultiplier,roomType,isSpawn)
    ...
    local isBuiltCoords = exports["FIXDEV-build"]:getModule("func").buildRoom(name,numMultiplier,false)
    ...
    if isBuiltCoords then
        ...
        if isSpawn then
            ...
            TriggerEvent("FIXDEV-spawn:characterSpawned", cid)
        else
...
function Apart.leaveApartment()
    ...
    if Apart.exitPoint[Apart.currentRoomType] == nil then
        exports["FIXDEV-build"]:getModule("func").exitCurrentRoom(Apart.Locations[Apart.currentRoomType][Apart.currentRoomNumber])
    else
        if type(Apart.exitPoint[Apart.currentRoomType]) == typeVector then
            exports["FIXDEV-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType])
        else
            local rnd = math.random(1,#Apart.exitPoint[Apart.currentRoomType])
            exports["FIXDEV-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType][rnd])
        end
    end
...
function garageToWorld()
    exports["FIXDEV-build"]:getModule("func").exitCurrentRoom(vector3(4.67, -724.85, 32.18))
    Apart.currentRoomNumber = Apart.defaultInfo[2]
    Apart.currentRoomType = Apart.defaultInfo[1]
    Apart.currentHotelInformation = nil
    Apart.currentGarageNumber = 0
...
function Apart.WakeFromBed()
    local bedOffset = exports["FIXDEV-build"]:getModule("func").getCurrentBed()
...
function nearClothing()
    ...
    local isNear = exports["FIXDEV-build"]:getModule("func").isNearCurrentInteract(1,5.5)
    if isNear then
        return true
    end
...
AddEventHandler('apartment:removeFromBuilding', function(roomNumber,buildType)
    if Apart.currentRoomType == buildType and Apart.currentRoomNumber == roomNumber then
        if exports["FIXDEV-build"]:getModule("func").isInBuilding() then
...
RegisterUICallback("FIXDEV-apartments:handler", function(data, cb)
...
AddEventHandler('apartments:menuAction', function(action)
    ...
    elseif action == "forfeit" then
        exports["FIXDEV-ui"]:showContextMenu(MenuData["apartment_check"])
    end
end)
