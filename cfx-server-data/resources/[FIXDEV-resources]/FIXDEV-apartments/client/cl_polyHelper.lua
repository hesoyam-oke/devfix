...
function polyHelperEnter(apartmentLocNumber,apartmentNumber)
    ...
    listening = true
    exports["FIXDEV-ui"]:showInteraction(getInteractionMessage(apartmentLocNumber,apartmentNumber))
...
local function showApartmentMenu(apartmentLocNumber,apartmentNumber)
    ...
    for k,v in pairs(Apart.currentRoomLocks[apartmentLocNumber]) do
        if v == false then
            unlockedApartments[#unlockedApartments + 1] = { title = "Apartment # "..k, action = "FIXDEV-ui:apartmentsContext", key = {"EnterUnlocked",apartmentLocNumber,k} }
        end 
    end
...
RegisterUICallback("FIXDEV-ui:apartmentsContext", function(data, cb)
    ...
    if action == "locks" then
        ...
        exports["FIXDEV-ui"]:showInteraction(getInteractionMessage(globalLocNumber,globalNumber))
    end
