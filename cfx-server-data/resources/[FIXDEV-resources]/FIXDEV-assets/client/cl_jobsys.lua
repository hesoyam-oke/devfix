...
AddEventHandler('jobssystem:current', function(cb)
  LocalPlayer = exports["FIXDEV-base"]:getModule("LocalPlayer")
  cb(LocalPlayer:getVar("job"))
end)

local PaycheckSpot, called = false, false

exports["FIXDEV-polyzone"]:AddBoxZone("paycheck_collect", vector3(241.63, 226.48, 106.29), 4, 4, {
  heading=0
})
...
AddEventHandler('FIXDEV-polyzone:enter', function(name)
  if name == "paycheck_collect" then
    PaycheckSpot = true
    exports["FIXDEV-ui"]:showInteraction("[E]Collect Paycheck")
    PaycheckFunction()
  end
end)
  ...
  Citizen.CreateThread(function()
    while PaycheckSpot do
      ...
      if IsControlJustReleased(0, 38) then
        local finished = exports["FIXDEV-taskbar"]:taskBar(2000,"Collecting Your Paycheck")
