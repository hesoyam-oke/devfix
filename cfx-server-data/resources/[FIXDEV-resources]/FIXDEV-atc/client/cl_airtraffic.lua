    ...
    local entry = {
        ...
        children = {
            ...
            },
            {
                ...
                action = "FIXDEV-atc:setFlightData",
                key = netId
            },
        }
    }
...
function UpdateAirTraffic(pAirspace)
    ...
    exports['FIXDEV-fx']:PlayEntitySound(PlayerPedId(), 'IDLE_BEEP', 'EPSILONISM_04_SOUNDSET')
end
...
function ShowAirTraffic()
    ...
    exports['FIXDEV-ui']:showContextMenu(elements)
end

function SetFlightData(pNetId)
    local result = exports['FIXDEV-ui']:OpenInputMenu({
    ...
    RPC.execute('FIXDEV-atc:updateFlightData', pNetId, data)
end
...
AddEventHandler('FIXDEV-atc:setFlightData', function ()
...
RegisterUICallback('FIXDEV-atc:setFlightData', function (data, cb)
...
AddEventHandler('FIXDEV-atc:showAirTraffic', function ()
    if not HasRadarEnabled then return end
