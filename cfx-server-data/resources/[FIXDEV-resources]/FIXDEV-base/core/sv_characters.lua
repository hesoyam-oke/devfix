function NPX.Core.LoginPlayer(self, args, src, callback)
    TriggerEvent("FIXDEV-base:playerAttemptLogin", src)
    ...
    local function fetchData(_err)
        ...
        NPX.DB:FetchPlayerData(src, function(data, err)
            ...
            if not err then TriggerEvent("FIXDEV-base:playerLoggedIn", user) TriggerClientEvent("FIXDEV-base:playerLoggedIn", src) end
        end)
    end
...
NPX.Events:AddEvent(NPX.Core, NPX.Core.LoginPlayer, "FIXDEV-base:loginPlayer")
...
function NPX.Core.FetchPlayerCharacters(self, args, src, callback)
	...
	NPX.DB:FetchCharacterData(user, function(data, err)
		if err then
			...
			TriggerEvent("FIXDEV-base:charactersLoaded", user, data)
			TriggerClientEvent("FIXDEV-base:charactersLoaded", src, data)
		end

		callback(data)
	end)
end
NPX.Events:AddEvent(NPX.Core, NPX.Core.FetchPlayerCharacters, "FIXDEV-base:fetchPlayerCharacters")
...
NPX.Events:AddEvent(NPX.Core, NPX.Core.CreateCharacter, "FIXDEV-base:createCharacter")
...
NPX.Events:AddEvent(NPX.Core, NPX.Core.DeleteCharacter, "FIXDEV-base:deleteCharacter")
...
function NPX.Core.SelectCharacter(self, id, src, callback)
	...
	TriggerClientEvent('FIXDEV-base:setcontrols', src)
	TriggerClientEvent('updatepasses', src)

	TriggerEvent("FIXDEV-base:characterLoaded", user, selectedCharacter)
	TriggerClientEvent("FIXDEV-base:characterLoaded", src, selectedCharacter)

	callback({loggedin = true, chardata = selectedCharacter})
end
NPX.Events:AddEvent(NPX.Core, NPX.Core.SelectCharacter, "FIXDEV-base:selectCharacter")