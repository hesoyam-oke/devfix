        ...
        if inZone > 0 then
            ...
            placeObject("np_beehive", function(pPlaced, pCoords, pHeading)
                if pPlaced then
                    ...
                    local finished = exports["FIXDEV-taskbar"]:taskBar(15000, _L("beekeeping-installing", "Installing beehive"), false, true, false, false, nil, 5.0, PlayerPedId())
                    ClearPedTasks(PlayerPedId())
                    if finished == 100 then
                        RPC.execute("FIXDEV-beekeeping:installHive", pCoords, pHeading)
...
AddEventHandler("FIXDEV-beekeeping:checkBeehive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    showHiveMenu(hiveId)
end)

RegisterUICallback('FIXDEV-beekeeping:addQueen', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    local finished = exports["FIXDEV-taskbar"]:taskBar(15000, _L("beekeeping-adding-queen-bee", "Adding Queen bee"), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("FIXDEV-beekeeping:addQueen", data.key.id)
        TriggerEvent("inventory:removeItem", "beequeen", 1)
    end
end)

RegisterUICallback('FIXDEV-beekeeping:removeHive', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["FIXDEV-taskbar"]:taskBar(15000, _L("beekeeping-removing", "Removing"), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getReady = getHiveReadyPercent(getHiveById(data.key.id)) > 20.0
        local success = RPC.execute("FIXDEV-beekeeping:removeHive", data.key, getReady)
        if not success then
...
AddEventHandler("FIXDEV-beekeeping:harvestHive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    ...
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["FIXDEV-taskbar"]:taskBar(5000, _L("beekeeping-harvesting", "Harvesting"), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("FIXDEV-beekeeping:harvestHive", hiveId)
    end
end)

AddEventHandler("FIXDEV-polyzone:enter", function(zone, data)
    if zone == "FIXDEV-beekeeping:bee_zone" then
        inZone = inZone + 1
        if inZone == 1 then
            local result = RPC.execute("FIXDEV-beekeeping:getBeehives")
        end
    end
end)
...
RegisterNetEvent('FIXDEV-beekeeping:trigger_zone')
AddEventHandler('FIXDEV-beekeeping:trigger_zone', function (type, pData)
    --Update all hives
    if type == 1 then
...
function showHiveMenu(pHiveId)
    ...
    if getStageFromPercent(harvest) < 2 and not hive.has_queen then
        context[#context+1] = {
            ...
            key = { id = pHiveId },
            action = 'FIXDEV-beekeeping:addQueen',
            description = _L("beekeeping-add-queen-desc", 'Make the hive happy.'),
            disabled = not exports["FIXDEV-inventory"]:hasEnoughOfItem("beequeen", 1, false)
        }
    end
    ...
    if harvest >= 95 or myjob == "police" or myjob == "judge" then
        context[#context+1] = {
            ...
            key = { id = pHiveId },
            action = 'FIXDEV-beekeeping:removeHive',
        }
    end
