RPC.register("FIXDEV-beekeeping:installHive", function(pSource, pCoords, pHeading)
    ...
    if installHive.affectedRows then
        TriggerClientEvent("FIXDEV-beekeeping:trigger_zone", -1, 2, {
        ...
        exports["FIXDEV-log"]:AddLog("Bee Keeping", 
            pSource, 
            "Installed Hive", 
...
RPC.register("FIXDEV-beekeeping:addQueen", function(pSource, pID)
    ...
    if addQueen.affectedRows > 0 then
        ...
        TriggerClientEvent("FIXDEV-beekeeping:trigger_zone", -1, 3, dataQueen[1])
    end

    return true
end)

RPC.register("FIXDEV-beekeeping:removeHive", function(pSource, pData, pIsReady)
    SQL.execute("DELETE FROM _beehives WHERE id = ?", pData.id)

    TriggerClientEvent("FIXDEV-beekeeping:trigger_zone", -1, 4, pData)
    return true
end)

RPC.register("FIXDEV-beekeeping:harvestHive", function(pSource, pID)
    ...
    if harvestHive.affectedRows > 0 then
        ...
        if ((os.time() - dataHive[1].timestamp) / 60) >= HiveConfig.LifeTime then
            ...
            TriggerClientEvent("FIXDEV-beekeeping:trigger_zone", -1, 4, dataHive[1])
        else
            TriggerClientEvent("FIXDEV-beekeeping:trigger_zone", -1, 3, dataHive[1])
        end
        return true
    end
end)

RPC.register("FIXDEV-beekeeping:getBeehives", function(pSource)
    ...
    TriggerClientEvent("FIXDEV-beekeeping:trigger_zone", -1, 1, getBeehives)
    return true
end)