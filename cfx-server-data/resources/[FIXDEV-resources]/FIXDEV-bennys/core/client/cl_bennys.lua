...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("FIXDEV-bennys:attemptPurchase",cheap, type, upgradeLevel)
...
function RepairVehicle()
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(10000 + (missingBodyHealth / 50), "Repairing vehicle...", false, true)
...
function ExitBennys()
    ...
    TriggerServerEvent("FIXDEV-bennys:removeFromInUse", currentBennys)
    if currentBennys:find('^devbennys') then
        --
    else
        exports["FIXDEV-ui"]:showInteraction("Bennys")
    end
    currentBennys = nil
end
...
--#[Event Handlers]#--
RegisterNetEvent("FIXDEV-bennys:purchaseSuccessful")
AddEventHandler("FIXDEV-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("FIXDEV-bennys:purchaseFailed")
AddEventHandler("FIXDEV-bennys:purchaseFailed", function()
...
AddEventHandler("bennys:enter", function()
    ...
    --[[local isBennysInUse = RPC.execute("FIXDEV-bennys:checkIfUsed", currentBennys)
    ...
    ::hasAccess::
    TriggerServerEvent("FIXDEV-bennys:addToInUse", currentBennys)
