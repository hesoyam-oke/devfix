...
function dwStatusHud(pValues)
  if dwRefreshChipCount then
    ...
    dwCurrentChipCount = RPC.execute("FIXDEV-casino:getCurrentChipCount")
  end
...
Citizen.CreateThread(function()
  --Wait(10000)
  dealerPedSeeds = RPC.execute("FIXDEV-casino:getDealerPedSeeds")
    ...
    while true do 
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        if exports['FIXDEV-inventory']:hasEnoughOfItem("casinomember", 1, false, true) and not isHighRoller then
                            background = "success"
                        ...
                        exports["FIXDEV-ui"]:showInteraction(
            ...
            elseif uiIntShown then
                uiIntShown = false
                local background = "info"
                if exports['FIXDEV-inventory']:hasEnoughOfItem("casinomember", 1, false, true) or exports['FIXDEV-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                    background = "success"
                else
                    background = "error"
                end
                exports["FIXDEV-ui"]:hideInteraction(background)
            end
        elseif uiIntShown then
            uiIntShown = false
            local background = "info"
...
RegisterUICallback("FIXDEV-ui:blackjackHitStand", function(data, cb)
  ...
  elseif action == "double" then
    local dwUseChipsResult = RPC.execute("FIXDEV-casino:useChips", currentBetAmount)
  ...
  elseif action == "surrender" then
    ...
    -- RPC.execute("FIXDEV-casino:winChips", currentBetAmount / 2)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "FIXDEV-ui:blackjackHitStand",
...
RegisterUICallback("FIXDEV-ui:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("FIXDEV-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("FIXDEV-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
