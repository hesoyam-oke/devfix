...
RegisterServerEvent('FIXDEV-boosting:removeGNE')
AddEventHandler('FIXDEV-boosting:removeGNE', function(amount, cid)
    exports.oxmysql:execute('SELECT gne FROM boosting_users WHERE identifier = ?', {cid}, function(result)
...
RegisterServerEvent('FIXDEV-boosting:pay_gne')
AddEventHandler('FIXDEV-boosting:pay_gne', function()
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
    local gnes = GetGNE(cid)
...
RegisterServerEvent('FIXDEV-boosting:giveGNE')
AddEventHandler('FIXDEV-boosting:giveGNE', function(amount)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
