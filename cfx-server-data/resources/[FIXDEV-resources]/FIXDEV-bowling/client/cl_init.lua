...
local function canUseLane(pLaneId)
    local info = (exports["FIXDEV-inventory"]:GetInfoForFirstItemOfName("bowlingreceipt") or { information = "{}" })
    local laneId = json.decode(info.information).lane

    return (exports['FIXDEV-inventory']:hasEnoughOfItem("bowlingreceipt", 1, false, true)) and (laneId == pLaneId)
end
...
Citizen.CreateThread(function()
    for k, v in pairs(lanes) do
        ...
        exports["FIXDEV-polyzone"]:AddBoxZone("FIXDEV-bowling:lane_"..k, v.pos, 1.8, 2.0, {
            heading=0,
            minZ=23.85,
            maxZ=27.85
        })
    ...
    exports["FIXDEV-polyzone"]:AddBoxZone("bowling_alley", vector3(743.95, -774.54, 26.34), 16.8, 30.4, {
        heading=0,
        minZ=23.85,
        maxZ=28.85
    })
    ...
    local bowlingVendor = exports["FIXDEV-npcs"]:RegisterNPC(data, "FIXDEV-bowling:bowlingVendor")
    exports["FIXDEV-npcs"]:EnableNPC(bowlingVendor)

    for k, v in pairs(lanes) do
        if (not v.enabled) then goto continuePeak end

        exports['FIXDEV-interact']:AddPeekEntryByPolyTarget('FIXDEV-bowling:lane_'..k, {{
            event = "FIXDEV-bowling:setupPins",
            id = "bowling_pins_"..k,
            icon = "bowling-ball",
    ...
    exports["FIXDEV-interact"]:AddPeekEntryByFlag({'isNPC'}, {{
            ...
            icon = "bowling-ball",
            event = 'FIXDEV-bowling:handleVendorPurchase',
...
AddEventHandler('FIXDEV-bowling:handleVendorPurchase', function(pParams)
    ...
    for itemId, item in pairs(options) do
        if (itemId == 'bowlingreceipt') then
            ...
            for k, v in ipairs(lanes) do
                table.insert(lanesSorted, { title = "Lane #" .. k, action = "FIXDEV-ui:bowlingPurchase", key = k, disabled = not v.enabled })
            end
            ...
            data[#data + 1] = {
                ...
                key = itemId,
                children = {
                    { title = _L("bowling-ui-confirmpurchase", "Confirm Purchase"), action = "FIXDEV-ui:bowlingPurchase", key = itemId },
                },
            }
        end
    end
...
RegisterUICallback("FIXDEV-ui:bowlingPurchase", function(data, cb)
    local isLane = type(data.key) == "number"

    local success, message = NPX.Procedures.execute("FIXDEV-bowling:purchaseItem", data.key, isLane)
    if not success then
...
AddEventHandler('FIXDEV-bowling:setupPins', function(pParameters, pEntity, pContext)
