...
local buffConfig = {
  ["resfooditem"] = {
    ...
    restaurantBaseBuff = {
      ["burger_shot"] = function(info)
        ...
        TriggerEvent("FIXDEV-buffs:applyBuff", "burger_shot", { { buff = "stressblock", percent = 1.0, timeOverride = 180 * 60000 } })
        TriggerEvent("hadsugar")
        return processInfo(info)
      end,
      ["rooster"] = function(info)
        TriggerEvent("FIXDEV-buffs:applyBuff", "rooster", { { buff = "jobluck", percent = 1.0 } })
        return processInfo(info)
      end,
      ["uwu_cafe"] = function(info)
        TriggerEvent("FIXDEV-buffs:applyBuff", "uwu_cafe", { { buff = "strength", percent = 0.5, timeOverride = 60000 * 60 } })
        return processInfo(info)
      end,
      ["maldinis"] = function(info)
        TriggerEvent("FIXDEV-buffs:applyBuff", "maldinis", { { buff = "fullness", percent = 1.0 } })
        return processInfo(info)
      end,
    },
  },
  ...
  ["sandwich"] = {
    attachItem = "sandwich",
    onUse = function(info)
      local config = exports["FIXDEV-config"]:GetModuleConfig("misc")
...
AddEventHandler("FIXDEV-buffs:itemUsedSuccess", function(pSuccess)
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(pItem, pInfo)
  ...
  itemUseSuccess = nil
  TriggerEvent("FIXDEV-inventory:attachPropPlayAnim", config.attachItem)
  ...
  TriggerEvent("FIXDEV-buffs:applyBuff", pItem, buffs)
