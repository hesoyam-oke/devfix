...
function HasPermission(pBusiness, pPermission)
    ...
    if not PermissionCache[pBusiness][pPermission] or (GetGameTimer() - PermissionCache[pBusiness][pPermission].UpdatedAt) > 15 * 60000 then
        ...
        local success = RPC.execute("FIXDEV-business:hasPermission", pBusiness, pPermission, cid)
...
RegisterNetEvent('FIXDEV-spawn:phonerefresh')
AddEventHandler('FIXDEV-spawn:phonerefresh', function()
    RefreshEmploymentList()
end)
...
AddEventHandler('FIXDEV-business:employmentStatus', function(pBusiness, pEmployed)
