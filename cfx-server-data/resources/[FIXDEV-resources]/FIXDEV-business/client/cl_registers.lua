...
for id,register in ipairs(activeRegisters) do
        ...
        exports["FIXDEV-polytarget"]:AddBoxZone(ptId, register.polytarget[1], register.polytarget[2], register.polytarget[3], register.polytarget[4])
        exports['FIXDEV-interact']:AddPeekEntryByPolyTarget(ptId, {{
            event = "FIXDEV-business:registerPurchasePrompt",
            id = ptId .. "_purchase",
            icon = "cash-register",
            label = "make payment",
        ...
        exports['FIXDEV-interact']:AddPeekEntryByPolyTarget(ptId, {{
            event = "FIXDEV-business:registerChargePrompt",
            id = ptId .. "_charge",
            icon = "credit-card",
...
AddEventHandler('FIXDEV-business:registerPurchasePrompt', function(pParameters, pEntity, pContext)
    ...
    local acceptContext = {
        ...
        },
        {
            ...
            action = "FIXDEV-business:finishPurchasePrompt",
            icon = 'credit-card',
    ...
    exports['FIXDEV-interface']:showContextMenu(acceptContext)
end)

AddEventHandler('FIXDEV-business:registerChargePrompt', function(pParameters, pEntity, pContext)
    exports['FIXDEV-interface']:openApplication('textbox', {
        callbackUrl = 'FIXDEV-interface:business:charge',
...
RegisterNetEvent('FIXDEV-business:activePurchase')
AddEventHandler("FIXDEV-business:activePurchase", function(data)
    activePurchases[data.registerId] = data
end)

--Remove at registerId pos
RegisterNetEvent('FIXDEV-business:closePurchase')
AddEventHandler("FIXDEV-business:closePurchase", function(data)
    activePurchases[data.registerId] = nil
end)

RegisterInterfaceCallback('FIXDEV-business:finishPurchasePrompt', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local success, message = RPC.execute("FIXDEV-business:completePurchase", data.key)  
    if not success then
        TriggerEvent("DoLongHudText", message, 2)
    end 
end)

RegisterInterfaceCallback("FIXDEV-interface:business:charge", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    RPC.execute("FIXDEV-business:startPurchase", {cost = cost, comment = comment, registerId = data.key.registerId, business = data.key.business})
end)