...
RegisterCommand("creationmenu", function()
	...
	local hasPerm = RPC.execute("FIXDEV-business:hasPermission", "ace_industries", "craft_access")
	
	if employed and hasPerm then
		TriggerEvent("FIXDEV-business:stash:creationMenu")
	end
end)
...
RegisterInterfaceCallback("FIXDEV-business:stash:enteredAuthCode", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["FIXDEV-interface"]:closeApplication("textbox")
  hasPdAuthCode = true
end)
AddEventHandler("FIXDEV-interface:application-closed", function(name)
...
function getPDAuthCode()
  ...
  exports["FIXDEV-interface"]:openApplication("textbox", {
    callbackUrl = "FIXDEV-business:stash:enteredAuthCode",
    key = "",
    items = {
...
function openInventory(type, data)
  ...
  if type == "stash" then
    local success = RPC.execute("FIXDEV-business:hasPermission", biz, "stash_access")
  ...
  elseif type == "craft" then
    local success = RPC.execute("FIXDEV-business:hasPermission", biz, "craft_access")
    if not success and isPD() then
      success = getPDAuthCode()
    end
    if success then
...
function enterPoly(name, data)
  ...
  exports["FIXDEV-interface"]:showInteraction("[E] " .. name)
end
...
local creationMenu = {
  key = 1,
  show = true,
  callbackUrl = "FIXDEV-business:client:createStash",
  items = { }
}

Citizen.CreateThread(function()
  exports["FIXDEV-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      id = "business_stash",
      event = "FIXDEV-business:client:openStash",
      icon = "archive",
      label = "Open Storage",
    },
  }, 
  { 
    ...
    isEnabled = function(pEntity)
      local objInfo = exports["FIXDEV-objects"]:GetObjectByEntity(pEntity)
      return objInfo ~= nil
    end
  })
  exports["FIXDEV-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      ...
      event = "FIXDEV-business:client:removeStash",
      icon = "trash",
      label = "Remove Storage",
    },
    {
      id = "business_stash_update",
      event = "FIXDEV-business:client:changeStashModel",
      icon = "archive",
...
RegisterNetEvent("FIXDEV-business:stash:creationMenu")
AddEventHandler("FIXDEV-business:stash:creationMenu", function ()
  ...
  exports["FIXDEV-interface"]:openApplication("textbox", creationMenu)
end)

AddEventHandler("FIXDEV-business:client:removeStash", function (pData, pEntity)
  local objInfo = exports["FIXDEV-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end

	TriggerEvent("animation:PlayAnimation", "hammering")

  local finished = exports["FIXDEV-taskbar"]:taskBar(15000, "Destroying Storage")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local deleted = exports["FIXDEV-objects"]:DelObject(objInfo.id)
  if not deleted then
...
AddEventHandler("FIXDEV-business:client:openStash", function (pData, pEntity)
  local objInfo = exports["FIXDEV-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end
  ...
  local success = RPC.execute("FIXDEV-business:hasPermission", businessCode, "stash_access")
  if not success and isPD() then
    success = getPDAuthCode()
  end
...
AddEventHandler("FIXDEV-business:client:changeStashModel", function (pData, pEntity)
  local objInfo = exports["FIXDEV-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end
  ...
  local menuData = {
    key = { id = objInfo.id },
    show = true,
    callbackUrl = "FIXDEV-business:client:updateStashModel",
    items = {
      { icon = "object-ungroup", _type = "select", label = "New Unit Model", name = "stashProp", options = unitOptions },
    }
  }
  exports["FIXDEV-interface"]:openApplication("textbox", menuData)
end)

RegisterInterfaceCallback("FIXDEV-business:client:updateStashModel", function(pData, pCb) 
  ...
  local finished = exports["FIXDEV-taskbar"]:taskBar(15000, "Changing Storage")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local updated = exports["FIXDEV-objects"]:UpdateObject(pData.key.id, stashProp)
end)

RegisterInterfaceCallback("FIXDEV-business:client:createStash", function(pData, pCb) 
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  exports["FIXDEV-interface"]:closeApplication("textbox")
  ...
  TriggerServerEvent('FIXDEV-objects:requestObjects')
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(item)
  ...
  local prompt = exports["FIXDEV-interface"]:OpenInputMenu({
    {
      name = 'name',
