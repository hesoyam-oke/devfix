...
local function explodeVehicle(pEntity)
  exports['FIXDEV-sync']:SyncedExecution('NetworkExplodeVehicle', pEntity, 1, 0, 0)
  TriggerServerEvent('FIXDEV-miscscripts:carbombs:removeBomb', NetworkGetNetworkIdFromEntity(pEntity))
end
...
local function checkForCarBomb(pEntity) 
  ...
  local progress = exports['FIXDEV-taskbar']:taskBar(2500, 'Searching for car bomb...', true)
  ClearPedTasks(PlayerPedId())

  if progress ~= 100 then return end

  local metaData = RPC.execute("FIXDEV-miscscripts:carbombs:getCarBombDataFromNetID", NetworkGetNetworkIdFromEntity(pEntity))
...
--[[   local remoteBomb = RPC.execute('FIXDEV-usableprops:hasPhoneBomb', NetworkGetNetworkIdFromEntity(pEntity))

  if remoteBomb and remoteBomb.planted then
    local number = remoteBomb.bomb.number
    TriggerServerEvent('FIXDEV-usableprops:defusePhoneBomb', number)
    TriggerEvent('FIXDEV-police:client:showTextPopup', { show = true, text = 'Cell Phone Found, Number: ' .. number })
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    TriggerServerEvent('FIXDEV-miscscripts:carbombs:foundBomb', NetworkGetNetworkIdFromEntity(pEntity), carBombMeta)
...
RegisterNetEvent("FIXDEV-miscscripts:carbombs:foundBombAll", function(pNetTable)
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(name, info)
  ...
  local prompt = exports['FIXDEV-interface']:OpenInputMenu(elements)
  ...
  local progress = exports['FIXDEV-taskbar']:taskBar(2500, 'Planting car bomb...', true)
  ...
  local added = RPC.execute('FIXDEV-miscscripts:carbombs:addCarBomb', netId, minSpeed, ticksBeforeExplode, ticksForRemoval, gridSize, 0, timeToComplete)
  TriggerEvent('inventory:removeItem', 'car_bomb', 1)
  
  return TriggerEvent('DoLongHudText', 'Successfully added car bomb to vehicle', 1)
end)

AddEventHandler('FIXDEV-miscscripts:carBombs:removeBomb', function (pData, pEntity)
    local metaData = RPC.execute("FIXDEV-miscscripts:carbombs:getCarBombDataFromNetID", NetworkGetNetworkIdFromEntity(pEntity))
    local carBombMeta = metaData.carBombData or false
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    ...
    TriggerEvent('animation:PlayAnimation', 'kneel')
    TriggerServerEvent('FIXDEV-miscscripts:carbombs:removeBomb', myNetIdentifier)
...
--[[     exports['FIXDEV-interface']:openApplication('memorygame', {
      gameFinishedEndpoint = 'FIXDEV-miscscripts:carBombs:completeHacking',
