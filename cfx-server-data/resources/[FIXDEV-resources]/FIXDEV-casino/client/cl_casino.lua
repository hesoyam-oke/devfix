...
function drawCarForWins()
  ...
  if carOnShowAppearence then
    exports['FIXDEV-vehicles']:SetVehicleAppearance(spinningCar, carOnShowAppearence)
  end
  if carOnShowMods then
    exports['FIXDEV-vehicles']:SetVehicleMods(spinningCar, carOnShowMods)
  end
...
AddEventHandler("FIXDEV-casino:elevatorEnterCasino", function()
  enterCasino(true)
end)
AddEventHandler("FIXDEV-casino:elevatorExitCasino", function()
  enterCasino(false)
end)
...
--   -- TriggerServerEvent("FIXDEV-bugs:playerLogAction", {
...
function enterCasino(pIsInCasino)
  ...
  --     -- end
  --     if inCasino then
  --       local pedCoords = RPC.execute("FIXDEV-casino:getSpawnedPedCoords", true)
  ...
  if not inCasino then
    ...
    RPC.execute("FIXDEV-casino:getSpawnedPedCoords", false)
    TriggerEvent("FIXDEV-casino:casinoExitedEvent")
    TriggerServerEvent('FIXDEV-infinity:scopes:casino', false)
  ...
  -- handlePedCoordsBaby(pedCoords)
  doInitStuff()
  TriggerEvent("FIXDEV-casino:casinoEnteredEvent")
  TriggerServerEvent('FIXDEV-infinity:scopes:casino', true)
end

Citizen.CreateThread(function()
  -- enterCasino(true)
  Wait(10000)
  RPC.execute("FIXDEV-casino:getCurrentInteriorSetName")
  local data = RPC.execute("FIXDEV-heists:getVaultLowerState")
    ...
    while inCasino do
      ...
      if inCasino and postGateTriggered then
        if hasMembership() then
          --RPC.execute("FIXDEV-casino:generateRaffleTicket")
        end
      end
    end
...
function doSomeTextureReplacesBruv(apply)
  if not apply then
    ...
    if wheelDui then exports["FIXDEV-lib"]:releaseDui(wheelDui.id) end
    wheelDui = nil
    return
  end
  ...
  if wheelDui == nil then
    wheelDui = exports["FIXDEV-lib"]:getDui(wheelUrl, 1024, 1024)
    AddReplaceTexture('vw_prop_vw_luckywheel_01a', 'script_rt_casinowheel', wheelDui.dictionary, wheelDui.texture)
  else
    exports["FIXDEV-lib"]:changeDuiUrl(wheelDui.id, wheelUrl)
  end
end
...
function showThe6StrImage(apply)
  if not apply then
    ...
    exports["FIXDEV-lib"]:releaseDui(tvDui.id)
    tvDui = nil
    return
  end
  if tvDui == nil then
    tvDui = exports["FIXDEV-lib"]:getDui(tvUri, 512, 256)
    AddReplaceTexture('vw_prop_vw_cinema_tv_01', 'script_rt_tvscreen', tvDui.dictionary, tvDui.texture)
  else
    exports["FIXDEV-lib"]:changeDuiUrl(tvDui.id, tvUri)
  end
end
...
function handlePedCoordsBaby(pPedCoords)
  ...
  RPC.execute("FIXDEV-casino:handoffPedData", myPeds)
  Citizen.CreateThread(function()
    while inCasino do
...
-- chips
RegisterUICallback("FIXDEV-ui:casinoPurchaseChipsAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local amount = data.values.amount
  RPC.execute("FIXDEV-casino:purchaseChips", amount)
  exports['FIXDEV-ui']:closeApplication('textbox')
end)

RegisterUICallback("FIXDEV-ui:transferChipsAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local amount = data.values.amount
  local stateId = data.values.state_id
  RPC.execute("FIXDEV-casino:transferChips", stateId, amount)
  exports['FIXDEV-ui']:closeApplication('textbox')
end)

AddEventHandler("FIXDEV-casino:purchaseChipsAction", function(pArgs)
  ...
  if action == "purchase" then
    ...
    exports['FIXDEV-ui']:openApplication('textbox', {
      callbackUrl = 'FIXDEV-ui:casinoPurchaseChipsAmount',
      key = 1,
      items = {
        {
          icon = "dollar-sign",
  ...
  elseif action == "purchase:dirty" then
    ...
    if exports["FIXDEV-inventory"]:hasEnoughOfItem("markedbills", 20, false, true) then
      TriggerEvent("inventory:removeItem", "markedbills", 20)
      payment = payment + (250 * 20) -- $5k / $250 per
    end
    if exports["FIXDEV-inventory"]:hasEnoughOfItem("rollcash", 5, false, true) then
      TriggerEvent("inventory:removeItem", "rollcash", 5)
      payment = payment + (30 * 5) -- $150 / $30 per
    end
    if exports["FIXDEV-inventory"]:hasEnoughOfItem("band", 5, false, true) then
      TriggerEvent("inventory:removeItem", "band", 5)
      payment = payment + (300 * 5) -- $1500, / $300 per
    end
    RPC.execute("FIXDEV-casino:purchaseChipsDirty", payment)
  elseif action == "withdraw:cash" then
    RPC.execute("FIXDEV-casino:withdrawChips", "cash")
  elseif action == "withdraw:bank" then
    RPC.execute("FIXDEV-casino:withdrawChips", "bank")
  elseif action == "transfer" then
    Wait(100)
    exports['FIXDEV-ui']:openApplication('textbox', {
      callbackUrl = 'FIXDEV-ui:transferChipsAmount',
      key = 1,
      items = {
        {
          icon = "user",
...
AddEventHandler("FIXDEV-casino:purchaseMembership", function()
  ...
  exports['FIXDEV-ui']:openApplication('textbox', {
    callbackUrl = 'FIXDEV-ui:casinoGetMembership',
    key = 1,
    items = {
      {
        icon = "user",
...
RegisterUICallback("FIXDEV-ui:casinoGetMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['FIXDEV-ui']:closeApplication('textbox')
  TriggerEvent("player:receiveItem", "casinomember", 1, false, { state_id = data.values.state_id }, { state_id = data.values.state_id })
end)

AddEventHandler("FIXDEV-casino:purchaseMembershipCard", function()
  local characterId = exports["isPed"]:isPed("cid")
  RPC.execute("FIXDEV-casino:purchaseMembershipCard", characterId)
end)

AddEventHandler("FIXDEV-casino:getLoyaltyCard", function()
  local characterId = exports["isPed"]:isPed("cid")
  local jobAccess = RPC.execute("IsEmployedAtBusiness", { character = { id = characterId }, business = { id = "casino"} })
  if not jobAccess then
    TriggerEvent("DoLHudText", 2, "casino-please-talk-to-floor-staff", "Please talk to a member of floor staff")
...
AddEventHandler("FIXDEV-casino:purchaseVipMembership", function()
  local characterId = exports["isPed"]:isPed("cid")
  local jobAccess = RPC.execute("IsEmployedAtBusiness", { character = { id = characterId }, business = { id = "casino"} })
  if not jobAccess then
    TriggerEvent("DoLHudText", 2, "casino-please-talk-to-floor-staff", "Please talk to a member of floor staff")
  ...
  exports['FIXDEV-ui']:openApplication('textbox', {
    callbackUrl = 'FIXDEV-ui:casinoGetVipMembership',
    key = 1,
    items = {
      {
        icon = "user",
...
RegisterUICallback("FIXDEV-ui:casinoGetVipMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['FIXDEV-ui']:closeApplication('textbox')
  TriggerEvent("player:receiveItem", "casinovip", 1, false, { state_id = data.values.state_id }, { state_id = data.values.state_id })
end)

AddEventHandler("FIXDEV-casino:purchaseDrinks", function(data)
  TriggerEvent("server-inventory-open", "46", "Shop")
end)

function hasMembership(hrOnly)
  if exports['FIXDEV-config']:GetMiscConfig("casino.membership.bypass") then
    return true
  end
  -- if    (not hrOnly and not exports['FIXDEV-inventory']:hasEnoughOfItem("casinomember", 1, false, true))
  --   and not exports['FIXDEV-inventory']:hasEnoughOfItem("casinovip", 1, false, true)
  -- then
  --   return false
  -- end
  local characterId = exports["isPed"]:isPed("cid")
  -- local info = hrOnly and nil or exports["FIXDEV-inventory"]:GetInfoForFirstItemOfName("casinomember")
  -- if not info then
  --   info = exports["FIXDEV-inventory"]:GetInfoForFirstItemOfName("casinovip")
  -- end
  local info = exports["FIXDEV-inventory"]:GetInfoForFirstItemOfName("casinomember")
  if not info then
    return false
  end
...
AddEventHandler("FIXDEV-casino:inVRHeadset", function(pInVRHeadset)
  inVRHeadset = pInVRHeadset
end)

RegisterUICallback("FIXDEV-ui:casinoCheckSpentWheelAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local stateId = data.values.state_id
  RPC.execute("FIXDEV-casino:checkSpentAmount", stateId)
  exports['FIXDEV-ui']:closeApplication('textbox')
end)
AddEventHandler("FIXDEV-casino:wheel:checkSpentAmount", function()
  ...
  exports['FIXDEV-ui']:openApplication('textbox', {
    callbackUrl = 'FIXDEV-ui:casinoCheckSpentWheelAmount',
    key = 1,
    items = {
      {
        icon = "user-alt",
...
-- end, false)
-- Citizen.CreateThread(function()
--   exports["FIXDEV-keybinds"]:registerKeyMapping("", "Casino", "Add Coords", "+addCasinoCoords", "-addCasinoCoords")
-- end)

-- Citizen.CreateThread(function()
--   for _, v in pairs(exports["FIXDEV-casino"]:getPedCoordsC()) do
