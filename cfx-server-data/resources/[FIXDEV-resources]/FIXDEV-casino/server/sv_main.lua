...
RegisterNetEvent("FIXDEV-casino:casinoPurchaseChips",function(qty)
    local src = source
...
RegisterNetEvent("FIXDEV-casino:withdraw",function(moneytype,qty)
    local src = source
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local quantity = tonumber(qty)
...
RegisterNetEvent("FIXDEV-casino:purchageMembership",function(cardtype, cid)
    local src = source
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('FIXDEV-luckywheel:getLucky')
AddEventHandler('FIXDEV-luckywheel:getLucky', function()
    local src = source
    if not isRoll then
        isRoll = true
        ...
        SetTimeout(12000, function()
            ...
            TriggerClientEvent("FIXDEV-luckywheel:rollFinished", -1)
        end)
        TriggerClientEvent("FIXDEV-luckywheel:doRoll", -1, _priceIndex)
        --print(_priceIndex)
    end
end)
