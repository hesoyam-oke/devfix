...
--   local balance = RPC.execute("FIXDEV-casino:getCurrentChipCount")
--  --local balance = 400
--   local values = {}
    if state then 
...
--     --   values = values
--     -- })
--     exports["FIXDEV-textdisplay"]:DisplayText({
--       header = title,
--         rows = {
--           {
--               id = 0, 
...
--   })
    ...
    exports["FIXDEV-textdisplay"]:HideText()
  end
end
...
local function getUpFromSlotMachine()
  ...
  TriggerServerEvent("FIXDEV-slotmachines-sv:getUpFromSlotMachine")
  drawStatusHUD(false, {})
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "FIXDEV-ui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "FIXDEV-ui-slotmachines:leaveMachine",
            title = "Leave Machine",
            key = {},
          })
...
local function initializeSlotMachines()
  ...
  TriggerServerEvent("FIXDEV-slotmachines-sv:getMachineData")
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["FIXDEV-ui"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            if exports["FIXDEV-inventory"]:hasEnoughOfItem("casinomember", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            elseif exports["FIXDEV-inventory"]:hasEnoughOfItem("casinovip", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            else
            ...
            exports["FIXDEV-ui"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
        
          if IsControlJustReleased(1, 38) and canPlayAtSlotMachine then --Key: E
            TriggerServerEvent("FIXDEV-slotmachines-sv:sitDownAtSlotMachine", k)
            --print("E")
            break
          end
...
--#[Event Handlers]#--
RegisterNetEvent("FIXDEV-slotmachines-cl:getMachineData")
AddEventHandler("FIXDEV-slotmachines-cl:getMachineData", function(data)
...
RegisterNetEvent("FIXDEV-slotmachines-cl:updateMachineData")
AddEventHandler("FIXDEV-slotmachines-cl:updateMachineData", function(machine, ply)
...
RegisterNetEvent("FIXDEV-slotmachines-cl:sitDownAtSlotMachine")
AddEventHandler("FIXDEV-slotmachines-cl:sitDownAtSlotMachine", function(machine, coinValue)
  local plyPed = PlayerPedId()
  local plyPos = GetEntityCoords(plyPed)
...
RegisterNetEvent("FIXDEV-slotmachines-cl:playSlotMachineSound")
AddEventHandler("FIXDEV-slotmachines-cl:playSlotMachineSound", function(machinePos, machineModel, sound)
  if isSMSetup then
    local soundRef = ""
...
RegisterNetEvent("FIXDEV-slotmachines-cl:stopReels")
AddEventHandler("FIXDEV-slotmachines-cl:stopReels", function(reel, rot)
...
RegisterNetEvent("FIXDEV-slotmachines-cl:spinFinished")
AddEventHandler("FIXDEV-slotmachines-cl:spinFinished", function(amountWon)
  if amountWon > 0 then
    -- RPC.execute("FIXDEV-casino:winChips", amountWon)
...
RegisterNetEvent("FIXDEV-slotmachines-cl:toggleSlotMachines")
AddEventHandler("FIXDEV-slotmachines-cl:toggleSlotMachines", function(state)
...
AddEventHandler("FIXDEV-casino:casinoEnteredEvent", function()
  isInsideCasino = true
...
AddEventHandler("FIXDEV-casino:casinoExitedEvent", function()
...
--#[NUI Callbacks]#--
RegisterUICallback("FIXDEV-ui-slotmachines:placeBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["FIXDEV-ui"]:hideContextMenu(data)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    if not exports["FIXDEV-inventory"]:hasEnoughOfItem("redchip", currentBet, false, true) then 
    ...
    TriggerServerEvent(
      "FIXDEV-slotmachines-sv:spinMachine",
    ...
    local title = "Slot Machine - " .. currentSlotMachineLabel
    --local balance = RPC.execute("FIXDEV-casino:getCurrentChipCount")
    local balance = exports["FIXDEV-inventory"]:getQuantity("redchip")
    --local values = {}
    --table.insert(values, "Balance: $" .. balance)
      exports["FIXDEV-textdisplay"]:DisplayText({
      header = title,
...
RegisterUICallback("FIXDEV-ui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["FIXDEV-ui"]:hideContextMenu(data)
...
function spinSlot(tickRate)
  ...
  for i=1,300,1 do
    ...
    elseif i == 180 then
      ...
      TriggerEvent("FIXDEV-slotmachines-cl:stopReels", 1, ang1)
    end
    if i < 240 then
    elseif i == 240 then
      ang2 = tickRate.b*22.5-180+0.0
      print(ang2)
      TriggerEvent("FIXDEV-slotmachines-cl:stopReels", 2, ang2)
    end
    if i < 300 then
    elseif i == 300 then
      ang3 = tickRate.c*22.5-180+0.0
      TriggerEvent("FIXDEV-slotmachines-cl:stopReels", 3, ang3)
    end				
    Citizen.Wait(10)
  end
  TriggerServerEvent('FIXDEV-slotmachines-sv:slotsCheckWin', tickRate, data)
end

RegisterNetEvent('FIXDEV-slotmachines-cl:startSpin')
AddEventHandler('FIXDEV-slotmachines-cl:startSpin',function(tickRate)
  spinSlot(tickRate)
end)
