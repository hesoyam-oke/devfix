...
RegisterNetEvent("FIXDEV-slotmachines-sv:sitDownAtSlotMachine",function(slot)
    activeSlots = slot
    print(slot)
    local coinValue = 1
    TriggerClientEvent("FIXDEV-slotmachines-cl:sitDownAtSlotMachine",source, slot)
end)

RegisterNetEvent("FIXDEV-slotmachines-sv:spinMachine",function(currentSlotMachinePos,currentSlotMachineModel,currentBet,cid)
    ...
    TriggerClientEvent('FIXDEV-slotmachines-cl:startSpin', source, w)
end)

RegisterNetEvent('FIXDEV-slotmachines-sv:slotsCheckWin')
AddEventHandler('FIXDEV-slotmachines-sv:slotsCheckWin',function(win, dt)
	CheckForWin(win, dt)
end)
...
function CheckForWin(w, data)
    ...
    TriggerClientEvent("FIXDEV-slotmachines-cl:spinFinished", source, total)
end