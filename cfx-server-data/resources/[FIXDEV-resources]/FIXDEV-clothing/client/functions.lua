...
RegisterNetEvent("FIXDEV-clothing:isPedFreemodeModel")
AddEventHandler("FIXDEV-clothing:isPedFreemodeModel", function()
...
function getPedAppearance(ped)
    return {
        ...
        headBlend = exports["FIXDEV-clothing"]:getPedHeadBlend(ped),
        faceFeatures = exports["FIXDEV-clothing"]:getPedFaceFeatures(ped),
        headOverlays = exports["FIXDEV-clothing"]:getPedHeadOverlays(ped),
        components = getPedComponents(ped),
        props = exports["FIXDEV-clothing"]:getPedProps(ped),
        hair = getPedHair(ped),
...
function setPedAppearance(player, data)
    ...
    Citizen.CreateThread(function()    
        ...
        if data.faceFeatures then
            exports["FIXDEV-clothing"]:setPedFaceFeaturesJS(GetPlayerPed(-1), data.faceFeatures)
        end

        if data.headBlend then
            exports["FIXDEV-clothing"]:setPedHeadBlend(GetPlayerPed(-1), data.headBlend)
        end

        if data.props then
            exports["FIXDEV-clothing"]:setPedProps(GetPlayerPed(-1), data.props)
        end

        if data.headOverlays then
            exports["FIXDEV-clothing"]:setPedHeadOverlays(GetPlayerPed(-1), data.headOverlays)
        end
    ...
    TriggerServerEvent('FIXDEV-walkstyles:server:walkstyles', 'get')
    playerTattoos = {}
end
