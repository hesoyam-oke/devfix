...
RegisterServerEvent("FIXDEV-clothing:insert_character_current")
AddEventHandler("FIXDEV-clothing:insert_character_current",function(data)
...
RegisterServerEvent("FIXDEV-clothing:insert_character_face")
AddEventHandler("FIXDEV-clothing:insert_character_face",function(data)
    if not data then return end
    local src = source
...
RegisterServerEvent("FIXDEV-clothing:get_character_face")
AddEventHandler("FIXDEV-clothing:get_character_face",function(pSrc)
    ...
    exports.oxmysql:execute("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM character_face cf INNER JOIN character_current cc on cc.cid = cf.cid WHERE cf.cid = @cid", {['cid'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("FIXDEV-clothing:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("FIXDEV-clothing:setpedfeatures", src, false)
        end
	end)
end)

RegisterServerEvent("FIXDEV-clothing:get_character_current")
AddEventHandler("FIXDEV-clothing:get_character_current",function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local characterId = user:getCurrentCharacter().id
    ...
    exports.oxmysql:execute("SELECT * FROM character_current WHERE cid = @cid", {['cid'] = characterId}, function(result)
        ...
        TriggerClientEvent("FIXDEV-clothing:setclothes", src, temp_data,0)
	end)
end)

RegisterServerEvent("FIXDEV-clothing:retrieve_tats")
AddEventHandler("FIXDEV-clothing:retrieve_tats", function(pSrc)
    local src = (not pSrc and source or pSrc)
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
	exports.oxmysql:execute("SELECT * FROM character_tattoos WHERE cid = @identifier", {['identifier'] = char.id}, function(result)
        if(#result == 1) then
			TriggerClientEvent("FIXDEV-clothing:settattoos", src, json.decode(result[1].tattoos))
		else
			local tattooValue = "{}"
			...
			TriggerClientEvent("FIXDEV-clothing:settattoos", src, {})
		end
	end)
end)

RegisterServerEvent("FIXDEV-clothing:set_tats")
AddEventHandler("FIXDEV-clothing:set_tats", function(tattoosList)
	local src = source
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
...
RegisterServerEvent("FIXDEV-clothing:get_outfit")
AddEventHandler("FIXDEV-clothing:get_outfit",function(slot)
    ...
    exports.oxmysql:execute("SELECT * FROM character_outfits WHERE cid = @cid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("FIXDEV-clothing:setclothes", src, data,0)
...
RegisterServerEvent("FIXDEV-clothing:set_outfit")
AddEventHandler("FIXDEV-clothing:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local characterId = user:getCurrentCharacter().id
...
RegisterServerEvent("FIXDEV-clothing:remove_outfit")
AddEventHandler("FIXDEV-clothing:remove_outfit",function(slot)
...
RegisterServerEvent("FIXDEV-clothing:list_outfits")
AddEventHandler("FIXDEV-clothing:list_outfits",function()
    --print("list_outfits")
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    ...
    exports.oxmysql:execute("SELECT slot, name FROM character_outfits WHERE cid = @cid", {['cid'] = cid}, function(skincheck)
    	TriggerClientEvent("FIXDEV-clothing:ListOutfits",src, skincheck)
	end)
end)
        ...
        exports.oxmysql:scalar("SELECT count(model) FROM character_current WHERE cid = @cid LIMIT 1", {
            ...
            if result == 0 then
                exports.oxmysql:execute("select count(cid) assExist from (select cid  from character_current union select cid from characters_clothes) a where cid =  @cid", {['cid'] = cid}, function(clothingCheck)
                    ...
                    TriggerClientEvent('FIXDEV-clothing:setclothes',src,{},existsClothing)
                end)
                return
            else
                ...
                TriggerEvent("FIXDEV-clothing:get_character_current", src)
            end
            TriggerClientEvent("FIXDEV-clothing:inService",src,isService)
    	end)
    end)
end)
