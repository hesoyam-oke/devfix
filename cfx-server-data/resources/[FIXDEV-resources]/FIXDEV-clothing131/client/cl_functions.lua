...
function TargetClothing()
    ...
    for i = 1, #Config.Stores do
        ...
        exports['FIXDEV-target']:AddBoxZone(Store['Type'] .. i, Store['Coords'], Store['Length'], Store['Width'], {
            name = Store['Type'] .. i,
            debugPoly = false,
    ...
    for i = 1, #Config.ClothingRooms do
        ...
        if Room['IsGang'] then
            TargetAction = function()
                ...
                TriggerEvent('FIXDEV-clothing/client/getOutfits', Room['Job'], Grade)
            end
        else
            TargetAction = function()
        ...
        exports['FIXDEV-target']:AddBoxZone('clothing_' .. Room['Job'] .. i, Room['Coords'], Room['Length'], Room['Width'],
            {
...
function PolyClothing()
    ...
    -- Zone in/out Check
    local StoreCombo = PolyZone:Create(StoreZones, { name = "StoreCombo", debugPoly = false })
    local RoomCombo = PolyZone:Create(RoomZones, { name = "RoomCombo", debugPoly = false })
    ...
    StoreCombo:onPlayerInOut(function(isPointInside, point, zone)
        if isPointInside then
            ...
            if zoneName == 'surgeon' then
                exports["FIXDEV-ui"]:showInteraction(("[E] %s"):format("Plastic Surgery"))
            elseif zoneName == 'clothing' then
                exports["FIXDEV-ui"]:showInteraction(("[E] %s"):format("Clothing Shop"))
            elseif zoneName == 'barber' then
                exports["FIXDEV-ui"]:showInteraction(("[E] %s"):format("Barber"))
            end
        else
            inZone = false
...
AddEventHandler("wild-clothes:barber", function()
    openMenu({
        { menu = "clothing", label = "Hair", selected = true },
    })
end)
...
AddEventHandler("wild-clothes:clothing", function()
    openMenu({
        { menu = "character", label = "Clothing", selected = true },
        { menu = "accessoires", label = "Accessories", selected = false }
    })
...
function SaveSkin(IsAdmin)
    -- Mercy.Functions.TriggerCallback('FIXDEV-clothing/server/pay-for-clothes', function(HasPaid)
    local HasPaid = RPC.execute("FIXDEV-clothing/server/pay-for-clothes", IsAdmin)
    if HasPaid then
        local Model = GetEntityModel(PlayerPedId())
        local Clothing = json.encode(Config.SkinData)
        TriggerServerEvent("FIXDEV-clothing/saveSkin", Model, Clothing, IsAdmin)
    end
end
