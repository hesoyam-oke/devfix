...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 2.0 and pedCreated then
            exports["FIXDEV-ui"]:showInteraction("[E] Drop off")
...
Citizen.CreateThread(function()
	exports["FIXDEV-polyzone"]:AddBoxZone("oxy_run_start", vector3(-1487.02, -909.79, 10.02), 1.8, 1, {
		name="oxy_run_start",
		heading=230,
		minZ=6.82,
		maxZ=10.82
...
function DoDropOff(requestMoney)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
			...
			if OxyRun then
				...
				if exports["FIXDEV-inventory"]:hasEnoughOfItem("rollcash",rollcount) then     
				...
				elseif exports["FIXDEV-inventory"]:hasEnoughOfItem("band",bandcount) then     
				...
				elseif exports["FIXDEV-inventory"]:hasEnoughOfItem("markedbills",markedbillsamount) then     
