...
--[[Citizen.CreateThread(function ()
  local config = exports['FIXDEV-config']:GetModuleConfig('main')['defaultDensity']

  defaultDensity = (config == nil or type(config) ~= "number") and 0.8 or config
end)

AddEventHandler('FIXDEV-config:configLoaded', function (pId, pConfig)
...
Citizen.CreateThread(function()
    while true do
      ...
      if #toDelete > 0 then
        TriggerServerEvent('FIXDEV:peds:rogue', toDelete)
      end
...
RegisterNetEvent('FIXDEV:peds:rogue:delete')
AddEventHandler('FIXDEV:peds:rogue:delete', function(pNetId)
...
RegisterNetEvent('FIXDEV:peds:decor:set')
AddEventHandler('FIXDEV:peds:decor:set', function (pNetId, pType, pProperty, pValue)
  local entity = NetworkGetEntityFromNetworkId(pNetId)
