...
RegisterNetEvent("FIXDEV-dispatch:openFull")
AddEventHandler("FIXDEV-dispatch:openFull", function()
    print("test")
-- function openMenu()
...
RegisterNetEvent('FIXDEV-dispatch:altercallsign;spawn')
AddEventHandler('FIXDEV-dispatch:altercallsign;spawn',function(getCallsign,pStatusType)
    exports['FIXDEV-dispatch']:callsign_command(getCallsign,pStatusType)
end)

RegisterNetEvent('FIXDEV-dispatch:beepEffect')
AddEventHandler('FIXDEV-dispatch:beepEffect', function()
    if exports['isPed']:isPed('myjob') == "police" then
        TriggerServerEvent('InteractSound_SV:PlayOnSource', '10-1314', 0.2)
    end
end)

RegisterNetEvent('FIXDEV-dispatch:stolenCar')
AddEventHandler('FIXDEV-dispatch:stolenCar', function(plate)
    local playerPed = PlayerPedId()	
    local padCoords = GetEntityCoords(playerPed)
    local alertCoord = vector3(padCoords.x, padCoords.y, padCoords.z)
...
RegisterCommand('911', function(source, args, rawCommand)
    ...
    if string.len(msg) > 0 then
        ...
        local cancelled = exports['FIXDEV-taskbar']:taskBar(7000,'Dialing 911')
...
RegisterCommand('311', function(source, args, rawCommand)
    ...
    if string.len(msg) > 0 then
        ...
        local phone_number = user.phone_number
        TriggerEvent('animation:phonecall')
        local cancelled = exports['FIXDEV-taskbar']:taskBar(7000,'Dialing 311')
        if cancelled == 100 then
        ...
        if IsPedShooting(PlayerPedId()) then
            if math.random(100) <= 10 then
                if IsPedInAnyVehicle(PlayerPedId()) then
                    ...
                    exports["FIXDEV-dispatch"]:dispatchadd('10-71', "Gun shots Reported", "432")
                    canSendGunshot = false
                    Citizen.Wait(3000)
                    canSendGunshot = true
                end
