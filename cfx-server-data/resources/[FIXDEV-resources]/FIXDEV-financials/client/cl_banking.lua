...
AddEventHandler('bank:openbank', function()
  ...
  open = not open
  bankanimation()
  local finished = exports["FIXDEV-taskbar"]:taskBar(1500,"Showing bank documentation")
...
AddEventHandler('bank:openatm', function()
  ...
  open = not open
  bankanimation()
  local finished = exports["FIXDEV-taskbar"]:taskBar(3000, "Inserting card")
  if (finished == 100) then 
  SetNuiFocus(open, open)
  SendReactMessage('setVisible', open)
  banktype = "atm"
...
RegisterNUICallback('bankWithdraw', function(data, cb)
  ...
  if success then
    --print("success")
    TriggerServerEvent('FIXDEV-financials:withdraw_log', data.amount, data.comment)
    local accounts, cash, name = RPC.execute("GetAccounts")
    local transactions = RPC.execute("GetTransactionLogs", data.id)
    SendReactMessage('bankData', {
      type = banktype,
...
RegisterNUICallback('bankDeposit', function(data, cb)
  ...
  if success then
    --print("success")
    TriggerServerEvent('FIXDEV-financials:deposit_log', data.amount, data.comment)
    local accounts, cash, name = RPC.execute("GetAccounts")
    local transactions = RPC.execute("GetTransactionLogs", data.id)
    SendReactMessage('bankData', {
      type = banktype,
