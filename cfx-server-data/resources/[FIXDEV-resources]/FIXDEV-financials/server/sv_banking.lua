...
RPC.register("BankWithdraw", function(pSource, pId, pAmount, pComment, pTransId, pDate)
    ...
    --TriggerClientEvent("FIXDEV-financials:transferUpdate", pSource, ply, newbankamount)
...
RPC.register("DoTransaction", function(pSource, pId, pAmount, pComment, pTransId, pDate, pTarget)
    ...
    if id == "personal" then
        ...
        local userSender = exports["FIXDEV-base"]:getModule("Player"):GetUser(pSource)
        ...
        local userReceiver = exports["FIXDEV-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('bank:givemecash', function(reciever, amount)
  local src = source
  local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
  local player = exports["FIXDEV-base"]:getModule("Player"):GetUser(tonumber(reciever))
  if tonumber(amount) <= user:getCash() then
    ...
    TriggerEvent('FIXDEV-base:bankgivecash', src, tonumber(receiver), pName, pSteam, pDiscord, amount, tonumber(user:getCash()), tonumber(user:getBalance()), amount, tonumber(player:getCash()), tonumber(player:getBalance()))
  else
...
RegisterServerEvent('FIXDEV-financials:addMoney')
AddEventHandler('FIXDEV-financials:addMoney', function(amount)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    user:addMoney(tonumber(amount))
end)

RegisterServerEvent('FIXDEV-financials:removeMoney')
AddEventHandler('FIXDEV-financials:removeMoney', function(amount)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(tonumber(amount))
end)

RegisterServerEvent("FIXDEV-financials:cash:get")
AddEventHandler('FIXDEV-financials:cash:get', function(amount)

    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local cash = user:getCash()
    TriggerClientEvent("FIXDEV-financials:cash",cash)
end)

RegisterServerEvent("FIXDEV-financials:business_money", function(amt, account, case)
...
RegisterServerEvent('FIXDEV-financials:deposit_log')
AddEventHandler('FIXDEV-financials:deposit_log', function(pDepositAmt, pDepositComment)
    ...
    local connect = {
        {
	        ...
	        ["footer"] = {
                ["text"] = "[FIXDEV-financials]",
            },
        }
    }
    PerformHttpRequest(Deposit_Log, function(err, text, headers) end, 'POST', json.encode({username = "NoPixel",  avatar_url = "https://cdn.discordapp.com/attachments/991122949828530258/992926357413711872/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)
...
RegisterServerEvent('FIXDEV-financials:withdraw_log')
AddEventHandler('FIXDEV-financials:withdraw_log', function(pWithDrawAmt, pWithDrawComment)
    local pName = GetPlayerName(source)
    local connect = {
        {
            ["color"] = "16777215",
    ...
    PerformHttpRequest(Withdraw_Log, function(err, text, headers) end, 'POST', json.encode({username = "NoPixel",  avatar_url = "https://cdn.discordapp.com/attachments/991122949828530258/992926357413711872/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)