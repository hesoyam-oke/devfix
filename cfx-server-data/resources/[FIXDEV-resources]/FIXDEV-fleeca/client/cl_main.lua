...
RegisterNetEvent('FIXDEV-robbery:restartSoon')
AddEventHandler('FIXDEV-robbery:restartSoon', function()
    rAllowed = false
end)

RegisterNetEvent('FIXDEV-robbery:openSmallBankDoor')
AddEventHandler('FIXDEV-robbery:openSmallBankDoor', function(vType)
...
RegisterNetEvent('FIXDEV-robbery:sBankLoot')
AddEventHandler('FIXDEV-robbery:sBankLoot', function()
    ...
    if math.random(100) > 95 then
        if vaultCard then 
            ...
            TriggerEvent('FIXDEV-banned:getID', 'Gruppe6Card22', 1)
            vaultCard = true
        end
    else
    ...
    if math.random(100) > 45 then
        ...
        if pick == 1 then
            TriggerEvent("FIXDEV-banned:getID", "goldbar", math.random(10, 50))
        elseif pick == 2 then
            TriggerEvent("FIXDEV-banned:getID", "gemstonesapphire", math.random(1, 3))
        elseif pick == 3 then
            TriggerEvent("FIXDEV-banned:getID", "rolexwatch", math.random(100, 200))
        elseif pick == 4 then
            TriggerEvent("FIXDEV-banned:getID", "gemstoneaqua", math.random(3, 7))
        end
    end
end)

RegisterNetEvent('FIXDEV-fleeca:UseGreenLapTop')
AddEventHandler('FIXDEV-fleeca:UseGreenLapTop', function()
    if exports["isPed"]:isPed("countpolice") >= 0 then
        ...
        if vDoor ~= 0 then
            if exports['FIXDEV-inventory']:hasEnoughOfItem('heistlaptop3', 1) then
                ...
                exports['hacking']:OpenHackingGame(function(Success)
                    if Success then
                        ...
                        DeleteObject(laptop)
                        ClearPedTasksImmediately(ply)
                        TriggerServerEvent('FIXDEV-robbery:PoliceNotifyFleeeee')
                        local timeout = math.random(180000,300000)
                        Citizen.SetTimeout(timeout, function()
                            TriggerServerEvent('FIXDEV-robbery:smallBankAttempt', bId)
                        end)
...
RegisterNetEvent('FIXDEV-robbery:updateBankData')
AddEventHandler('FIXDEV-robbery:updateBankData', function(bData)
...
function OpenVaultDoor(vType)
    if vType then
        TriggerEvent('FIXDEV-robbery:openSmallBankDoor', 2121050683)
        TriggerEvent('FIXDEV-robbery:openSmallBankDoor', -63539571)
    else
        TriggerEvent('FIXDEV-robbery:openSmallBankDoor', -131754413)
    end
end
...
for k,v in pairs(Config.Trollys) do
    exports["FIXDEV-polytarget"]:AddBoxZone("RobFleecaasTrolly", vector3(v['Coords']['X'], v['Coords']['Y'], v['Coords']['Z']), 1.05, 1, {
        heading= 0
    })
end
...
RegisterNetEvent('FIXDEV-bankrobbery:client:trolly')
AddEventHandler('FIXDEV-bankrobbery:client:trolly', function()
...
function GetMoneyFromTrolly(TrollyNumber)
    ...
    GrabbingMoney = false
    TriggerServerEvent('FIXDEV-bankrobbery:server:set:trolly:state', TrollyNumber, true)
...
RegisterNetEvent('FIXDEV-bankrobbery:client:set:trolly:state')
AddEventHandler('FIXDEV-bankrobbery:client:set:trolly:state', function(TrollyNumber, bool)
    Config.Trollys[TrollyNumber]['Open-State'] = bool
end)


RegisterNetEvent('FIXDEV-bankrobbery:client:CreateTrollysEvent')
AddEventHandler('FIXDEV-bankrobbery:client:CreateTrollysEvent', function()
    CreateTrollys()
end)
---------------------------------------------------
