...
local function doFuel(sentVeh)
	if DoesEntityExist(sentVeh) then
		...
		if currFuel == -1.0 then
			TriggerServerEvent('FIXDEV-fuel:initFuel', VehToNet(sentVeh))
...
exports('SetFuel', SetFuel) -- exports['FIXDEV-fuel']:SetFuel(veh, fuel)
...
RegisterNetEvent('FIXDEV-fuel:SelectFuel', function()
	local menuData = {
		{
            title = "Gas Pump",
        ...
        },
		{
			...
			icon = 'gas-pump',
            key = "EVENTS.REFUEL",
			action = 'FIXDEV-fuel:FuelSelected'
        },
    }
    exports["FIXDEV-ui"]:showContextMenu(menuData)
end)

RegisterUICallback("FIXDEV-fuel:FuelSelected", function(data, cb)
...
RegisterNetEvent('FIXDEV-fuel:ReturnNozel', function()
	if hasNozle then
...
RegisterNetEvent('FIXDEV-fuel:RefillVehicle', function()
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['FIXDEV-fuel']:GetFuel(veh))
	...
	local menuData = {
		{
			...
			description = "Fuel Amount: ".. math.ceil(exports['FIXDEV-fuel']:GetFuel(veh)) .. " | Total Cost: $".. FuelCost .. ".0",
			icon = 'info-circle',
			key = "EVENTS.GAS",
		},
		{
			...
			icon = 'gas-pump',
			key = "EVENTS.REFUEL",
			action = 'FIXDEV-fuel:RefuelVehicle',
...
RegisterUICallback('FIXDEV-fuel:pay_bank', function(data, cb, pParams)
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['FIXDEV-fuel']:GetFuel(veh))
	local endFuel = (100 - vehicleCurrentFuel)
	local FuelCost = endFuel * GallonPrice
	SetUIFocus(false, false)
	local exports['FIXDEV-interface']:openApplication('textbox', {
		{
			icon = "address-card",
			label = "Paypal ID",
			name = "ppaypal_id",
	...
	if input["ppaypal_id"] then
		TriggerServerEvent("FIXDEV-phone:send_gas_bill", input["ppaypal_id"], FuelCost)
		print(json.encode(input))
	end
end)

RegisterNetEvent('FIXDEV-fuel:SendBillToBank', function(fCost)
	TriggerEvent('FIXDEV-phone:client:gasOffer', 'Fuel Cost: $' .. fCost)
end)

RegisterNetEvent('FIXDEV-fuel:OfferAccepted', function()
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['FIXDEV-fuel']:GetFuel(veh))
	local endFuel = (100 - vehicleCurrentFuel)
	local FuelCost = endFuel * GallonPrice
	if not paid then
		paid = true
		TriggerServerEvent('FIXDEV-fuel:PurchaseSuccessful', FuelCost)
	end
	canRefillVehicle = false
	canPayBill = true
end)

RegisterUICallback('FIXDEV-fuel:RefuelVehicle', function(data, cb)
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['FIXDEV-fuel']:GetFuel(veh))
	...
	SetUIFocus(false, false)
	-- exports['FIXDEV-taskbar']:Progress({
	-- 	duration = timer,
	-- 	label = "Refueling",
	-- }, function(cancelled)
	...
	if finished == 100 then
		paid = false
		exports['FIXDEV-fuel']:SetFuel(veh, 100)
