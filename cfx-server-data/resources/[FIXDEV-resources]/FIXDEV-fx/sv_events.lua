...
RegisterServerEvent("FIXDEV-fx:setSteamId")
AddEventHandler("FIXDEV-fx:setSteamId", function()
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local steamId = user:getVar("steamid")
    TriggerClientEvent("FIXDEV-fix:setSteamIdClient", src, steamId)
end)

RegisterServerEvent("FIXDEV-fx:smoke:grenade")
AddEventHandler("FIXDEV-fx:smoke:grenade", function(pCoords)
...
RegisterNetEvent("FIXDEV-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrength, pScale)
