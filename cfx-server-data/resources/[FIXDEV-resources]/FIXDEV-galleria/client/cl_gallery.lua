...
function IsGalleryEmployee()
  local jobAccess = exports['FIXDEV-business']:IsEmployedAt('gallery')
...
function IsJewelryEmployee()
  local jobAccess = exports['FIXDEV-business']:IsEmployedAt('jeweled_dragon')
  if not jobAccess then
    TriggerEvent("DoLongHudText", "They do not recognize you", 2)
    return false
  end
...
AddEventHandler("FIXDEV-gallery:sellGems", function()
  local anyoneSellGems = exports['FIXDEV-config']:GetMiscConfig("gallery.anyone.sell.gems")
  if anyoneSellGems then
    TriggerServerEvent("FIXDEV-gallery:sellGemsFromNPC")
    return
  end
  if not IsGalleryEmployee() then return end
  local craftAccess = RPC.execute("FIXDEV-business:hasPermission", "gallery", "craft_access", characterId)
...
function listenForEscapeKeypress()
  ...
  Citizen.CreateThread(function()
    exports["FIXDEV-ui"]:showInteraction("[ESC] Exit")
    while listening do
      if IsControlJustReleased(0, 177) then
          ...
          Wait(1000)
          deleteGem()
          exports["FIXDEV-ui"]:sendAppEvent("preferences", {
...
AddEventHandler('FIXDEV-gallery:gemTrade', function(pParameters, pEntity, pContext)
  TriggerEvent('server-inventory-open', '1', 'gallery_gemtrade')
end)

RegisterUICallback("FIXDEV-ui:galleryGemCraft", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local gemType = data.key
  local ringCount = 0
  local items = exports["FIXDEV-inventory"]:GetItemsByItemMetaKV("gallerygem", "type", gemType)
  if #items == 0 then
...
AddEventHandler('FIXDEV-gallery:gemCraft', function(pParameters, pEntity, pContext)
  if not IsGalleryEmployee() then return end
  local data = {
    {
      title = "Jade",
      ...
      key = "jade",
      action = "FIXDEV-ui:galleryGemCraft",
    },
    {
      title = "Citrine",
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(item)
  ...
  if item == "craftedgemaquamarine" then
    TriggerEvent("FIXDEV-buffs:applyBuff", "craftedgemaquamarine", { { buff = "swimming", percent = 0.5 } })
  end
