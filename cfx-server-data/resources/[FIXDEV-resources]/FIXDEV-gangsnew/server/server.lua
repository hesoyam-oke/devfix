...
AddEventHandler("unwind-gangs:server:setgang", function(svid, gang, rank)
    local pSrc = source
    local pUser = exports["FIXDEV-base"]:getModule("Player"):GetUser(svid)
    local pSteam = pUser:getCurrentCharacter().owner
    local pCid = pUser:getCurrentCharacter().id
	local count = 0
    exports.oxmysql:execute('SELECT * FROM characters_gangs', {}, function(players)
		for j = 1, #players do			 
			if players[j].gang == gang then
				count = count + 1
			end
		end	
        exports.oxmysql:execute('SELECT * FROM characters_gangs WHERE `owner` = @owner', { ['@owner'] = pSteam }, function(result)
            if result[1] ~= nil then
                -- UPDATE if exists
                exports.oxmysql:execute("UPDATE characters_gangs SET `owner` = @owner, `cid` = @cid, `gang` = @gang, `rank` = @rank WHERE `owner` = @zeb", {
                    ['zeb'] = pSteam, 
                    ['owner'] = pSteam, 
                    ['cid'] = pCid,
                    ['gang'] = gang,
                ...
                if tonumber(count) > tonumber(Config["Gangs"][gang].max_members) then
                    ...
                    exports.oxmysql:execute("INSERT INTO characters_gangs SET `owner` = @owner, `cid` = @cid, `gang` = @gang, `rank` = @rank", {
                        ['owner'] = pSteam, 
                        ['cid'] = pCid,
                        ['gang'] = gang,
                        ['rank'] = rank
...
AddEventHandler("unwind-gangs:server:firegang", function(svid)
    local pSrc = source
    local SUser = exports["FIXDEV-base"]:getModule("Player"):GetUser(pSrc)
    local SSteam = SUser:getCurrentCharacter().owner
    local pUser = exports["FIXDEV-base"]:getModule("Player"):GetUser(svid)
    local pCid = pUser:getCurrentCharacter().id
    exports.oxmysql:execute('SELECT * FROM characters_gangs WHERE `owner` = @owner', { ['@owner'] = SSteam }, function(result)
        if tonumber(result[1]) ~= nil then
            local gang = result[1].gang
            local SrcRank = tonumber(result[1].rank)
            if SrcRank == 5 then
                exports.oxmysql:execute("DELETE FROM characters_gangs WHERE cid = @cid", {['@cid'] = pCid})
            else
...
--     local GangData = {}
--     local pSrc = source
--     local pUser = exports["FIXDEV-base"]:getModule("Player"):GetUser(pSrc)
--     local pCid = pUser:getCurrentCharacter().id
--     exports.oxmysql:execute("SELECT * FROM characters_gangs WHERE cid= ?", {pCid}, function(data3)
--         if data3[1] ~= nil then
--             print(data3[1].gang)
