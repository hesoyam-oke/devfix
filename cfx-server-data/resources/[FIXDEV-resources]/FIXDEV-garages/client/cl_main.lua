...
RegisterNetEvent("FIXDEV-garages:open2")
AddEventHandler("FIXDEV-garages:open2", function()
    local house2 = exports["FIXDEV-menu"]:NearHouseGarage2()
    exports['FIXDEV-garages']:DeleteViewedCar()
    if house2 then
...
RegisterNetEvent("FIXDEV-garages:attempt:spawn", function(data, pRealSpawn)
    for ind, value in pairs(data) do
        ...
        local house = exports["FIXDEV-menu"]:NearHouseGarage()
            if house then
                RPC.execute("FIXDEV-garages:attempt:sv", value.id)
                SpawnVehicle(value.model, exports['FIXDEV-menu']:currentGarage(), value.fuel, value.data, value.license_plate, true)
                pPlate = value.license_plate
                SetFitment = true
                TriggerEvent('FIXDEV-garages:fitmentSet')
            end
        else
            SpawnVehicle(value.model, exports['FIXDEV-menu']:currentGarage(), value.fuel, value.data, value.license_plate, false)
        end
    end
end)

RegisterNetEvent("FIXDEV-garages:attempt:spawn2", function(data, pRealSpawn)
    for ind, value in pairs(data) do
        if pRealSpawn then
        local house2 = exports["FIXDEV-menu"]:NearHouseGarage2()
            if house2 then
                RPC.execute("FIXDEV-garages:attempt:sv2", value.id)
                TriggerServerEvent('FIXDEV-wheelfitment:GetVehicleStatus', GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)), GetVehiclePedIsIn(PlayerPedId(), false))
                SpawnVehicle(value.model, exports['FIXDEV-menu']:currentGarage(), value.fuel, value.data, value.license_plate, true)
                pPlate = value.license_plate
                SetFitment = true
                TriggerEvent('FIXDEV-garages:fitmentSet')
...
RegisterNetEvent("FIXDEV-garages:takeout", function(pData)
    local house = exports["FIXDEV-menu"]:NearHouseGarage()
    if house then
        RPC.execute("FIXDEV-garages:spawned:get", pData)
    end
end)

RegisterNetEvent("FIXDEV-garages:takeout2", function(pData)
    local house2 = exports["FIXDEV-menu"]:NearHouseGarage2()
    if house2 then
        RPC.execute("FIXDEV-garages:spawned:getShared", pData)
    end
end)
...
RegisterNetEvent("FIXDEV-garages:resetcartoimpound", function(plate)
    local found = false
    for vehicle in EnumerateVehicles() do
        ...
        if not found then
            TriggerServerEvent("FIXDEV-garages:resetcartoimpound",plate)
        end
    end
end)
...
RegisterNetEvent("FIXDEV-garages:store", function()
    ...
    if (curVeh ~= 0) then
        local Stored = RPC.execute("FIXDEV-garages:states", "In", exports["FIXDEV-garages"]:NearVehicle("plate"), exports['FIXDEV-menu']:currentGarage(), exports["FIXDEV-garages"]:NearVehicle("Fuel"))
        if Stored then
...
RegisterNetEvent("FIXDEV-garages:resetpdgarage", function()
    local myJob = exports["isPed"]:isPed("myJob")
    if myJob == "police" then
        TriggerServerEvent("FIXDEV-garages:resetpdgarage")
    else
...
AddEventHandler("FIXDEV-polyzone:enter", function(zone, data)
    ...
    if plyPed then
        if job == "police" or job == "doc" or job == "judge" or job == "ems" then
            exports["FIXDEV-ui"]:showInteraction("Parking")
...
RegisterNetEvent('FIXDEV-garages:fitmentSet')
AddEventHandler('FIXDEV-garages:fitmentSet', function()
    Citizen.CreateThread(function()
        while SetFitment do
            Citizen.Wait(1000)
            if SetFitment then
                if GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId())) == pPlate then
                    if IsPedInAnyVehicle(PlayerPedId()) then
                        SetFitment = false
                        TriggerServerEvent('FIXDEV-wheelfitment:GetVehicleStatus', GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)),  GetVehiclePedIsIn(PlayerPedId()))
                    end
                end
            end
        end
...
RegisterNetEvent('FIXDEV-garages:pd_heli')
AddEventHandler('FIXDEV-garages:pd_heli', function()
    local PDHeliGarage = {
        {
            ...
            children = {
                ...
                children = {
                    title = "Take Out",
                    action = "FIXDEV-garages:spawn_heli"
                }
            }
        },
    }
    exports["FIXDEV-interface"]:showContextMenu(PDHeliGarage)
end)

RegisterInterfaceCallback('FIXDEV-garages:spawn_heli', function(data, cb)
