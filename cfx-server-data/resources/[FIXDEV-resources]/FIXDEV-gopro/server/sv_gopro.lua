RPC.register("FIXDEV-gopros:addSecCamera", function(pSource, pName, pCoords, pRotation)
...
RPC.register("FIXDEV-gopros:getSecurityCameraById", function(pSource, pCameraID)
    local result = MySQL.query.await([[
...
RPC.register("FIXDEV-gopros:getSecurityCameraByStateID", function(pSource, pCid)
...
RPC.register("FIXDEV-gopros:getSecurityCameraAccessByCameraID", function(pSource, pCamera)
...
RPC.register("FIXDEV-gopros:addUserToCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})

    print(pCid)
    local pCid = exports['FIXDEV-phone']:getCIDFromPhoneNumber(pCid)
    print(pCid)
...
RPC.register("FIXDEV-gopros:removeUserFromCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})
