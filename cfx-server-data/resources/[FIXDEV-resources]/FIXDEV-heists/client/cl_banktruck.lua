...
RegisterNetEvent('FIXDEV-heists:start_loop')
AddEventHandler('FIXDEV-heists:start_loop', function()
    pickup = true
    TriggerEvent("FIXDEV-heists:pick_cash")
    Wait(180000)
    Wait(additionalWait)
    pickup = false
end)

RegisterNetEvent('FIXDEV-heists:pick_cash')
AddEventHandler('FIXDEV-heists:pick_cash', function()
        ...
        if aDist < 2.0 and not LootedTruck then
            ...
            if IsDisabledControlJustReleased(0, 38) then
                if not LootedTruck then
                    ...
                    TriggerEvent('FIXDEV-hud:show_money')
                    local BankTruckBarLoot = exports['FIXDEV-taskbar']:taskBar(60000, 'Looting the truck')
                    if BankTruckBarLoot == 100 then
                        FreezeEntityPosition(PlayerPedId(), false)
                        TriggerEvent('FIXDEV-heists:banktruck_loot') 
                        TriggerEvent('FIXDEV-hud:hide_money') 
                        LootedTruck = true
                        Citizen.Wait(900000) 
                        LootedTruck = false     
                    else
...
RegisterNetEvent('FIXDEV-heists:banktruck_loot')
AddEventHandler('FIXDEV-heists:banktruck_loot', function()
...
RegisterNetEvent('FIXDEV-heists:spawn_peds')
AddEventHandler('FIXDEV-heists:spawn_peds', function(veh)
...
RegisterNetEvent('FIXDEV-heists:banktruck:cl')
AddEventHandler('FIXDEV-heists:banktruck:cl', function()
