RegisterNetEvent('FIXDEV-heists:bay_city:first_door')
AddEventHandler('FIXDEV-heists:bay_city:first_door', function()
    ...
    if exports["isPed"]:isPed("countpolice") >= 0 then
        if distance < 1.5 then
            if exports["FIXDEV-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                firstDoorBC()
                TriggerServerEvent('FIXDEV-heists:bayCityLog')
                exports["FIXDEV-dispatch"]:dispatchadd('10-90', "Bay City Bank", "500")
                ...
                exports['FIXDEV-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerServerEvent('FIXDEV-doors:change-lock-state', 569, false) 
                        Citizen.Wait(3000)
...
RegisterNetEvent('FIXDEV-heists:bay_city:second_door')
AddEventHandler('FIXDEV-heists:bay_city:second_door', function()
    if exports["isPed"]:isPed("countpolice") >= 0 then
        exports["FIXDEV-dispatch"]:dispatchadd('10-90', "Bay City Bank", "500")
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['FIXDEV-taskbar']:taskBar(15000, 'Opening Door.')
        if finished == 100 then
            TriggerEvent('aspect:createTrolleys')
            TriggerServerEvent('pStopSecondDoor')
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerServerEvent('FIXDEV-doors:change-lock-state', 570, false) 
            -- Create Trolleys.
        end
    end
end)

RegisterNetEvent('FIXDEV-heists:bay_city:second_door:sv')
AddEventHandler('FIXDEV-heists:bay_city:second_door:sv', function()
    TriggerServerEvent('FIXDEV-heists:second_door:cl')
end)

exports["FIXDEV-polytarget"]:AddCircleZone("np_baycity_unlock", vector3(-1309.13, -815.79, 17.7), 0.45, {
    useZ = true
})

exports["FIXDEV-interact"]:AddPeekEntryByPolyTarget("np_baycity_unlock", {{
    event = "FIXDEV-heists:bay_city:second_door:sv",
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(item)
    if item == "thermitecharge" then
        TriggerServerEvent('FIXDEV-heists:first_door:cl')
    end
end)