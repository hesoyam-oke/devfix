...
RegisterNetEvent('FIXDEV-bobcat:firstdoor')
AddEventHandler('FIXDEV-bobcat:firstdoor', function()
    ...
    if exports["isPed"]:isPed("countpolice") >= 0 then
        if distance < 1.5 then
            if exports["FIXDEV-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                frontdoor()
                TriggerServerEvent('FIXDEV-heists:bobcatLog')
                exports["FIXDEV-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery First Door", "500")
                ...
                exports['FIXDEV-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerServerEvent("FIXDEV-doors:change-lock-state", 273, false)
...
RegisterNetEvent('FIXDEV-bobcat:seconddoor')
AddEventHandler('FIXDEV-bobcat:seconddoor', function()
local player = GetEntityCoords(PlayerPedId())
...
if exports["isPed"]:isPed("countpolice") >= 0 then
    if distance < 1.5 then
        if exports["FIXDEV-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                TriggerServerEvent('FIXDEV-heists:bobcatLog')
                exports["FIXDEV-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery Second Door", "500")
                insidedoor()
                TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                exports['FIXDEV-thermite']:OpenThermiteGame(function(success)
                    if success then
                        TriggerEvent('DoLongHudText', "zort!", 1)
                        TriggerEvent("nicx:bobcatcreateped")
                        --Door IDS
                        TriggerServerEvent("FIXDEV-doors:change-lock-state", 274, false)
                        Citizen.Wait(3000)
                    else
                        Citizen.Wait(2000)
                    end
...
RegisterNetEvent('FIXDEV-heists:bobcat_armed_door')
AddEventHandler('FIXDEV-heists:bobcat_armed_door', function()
    local PlyPed = GetEntityCoords(PlayerPedId())
    local Dist = #(vector3(883.02, -2268.0, 30.47) - PlyPed)
    if exports["isPed"]:isPed("countpolice") >= 0 then
        if Dist < 1.5 then
            TriggerServerEvent('FIXDEV-heists:bobcatLog')
            exports["FIXDEV-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery Third Door", "500")
            TriggerEvent('inventory:removeItem', 'bobcatsecuritycard', 1)
            TriggerEvent('FIXDEV-heists:bobcat_explosion_ped')
            FreezeEntityPosition(PlayerPedId(), true)
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerServerEvent("FIXDEV-doors:change-lock-state", 275, false)
            TriggerServerEvent("FIXDEV-doors:change-lock-state", 276, false)
        end
    else
        TriggerEvent('DoLongHudText', 'Not enough cops on duty', 2)
    end
...
RegisterNetEvent("bobcat:success:panel", function()
    ...
    TriggerServerEvent("FIXDEV-doors:change-lock-state", 275, false)
    TriggerServerEvent("FIXDEV-doors:change-lock-state", 276, false)
    exports["FIXDEV-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
end)
...
RegisterNetEvent("bobcat2:success:panel", function()
    TriggerEvent('DoLongHudText', "Success!", 1)
    TriggerServerEvent("FIXDEV-doors:change-lock-state", 277, false)
    exports["FIXDEV-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
end)
...
RegisterNetEvent('FIXDEV-bobcat:bubbles')
...
RegisterNetEvent('blow_door', function()
    TriggerServerEvent('FIXDEV-heists:bobcatLog')
    TriggerServerEvent('FIXDEV-bobcat:blow_door')
end)

RegisterNetEvent('FIXDEV-heists:bobcat_explosion_ped')
AddEventHandler('FIXDEV-heists:bobcat_explosion_ped', function()
...
RegisterNetEvent('FIXDEV-heists:explosion_ped_walk') -- PED WALK
AddEventHandler('FIXDEV-heists:explosion_ped_walk', function()
    exports["FIXDEV-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery Last Door", "500")
...
AddEventHandler('lumo:client:requestblast', function()
	TriggerServerEvent("FIXDEV-bobcat:request_door")
end)
...
AddEventHandler('lumo:pedbomb', function()
    ...
    TriggerServerEvent("FIXDEV-bobcat:bubbles")
    -- TriggerEvent("lumo:propcreate")
    Wait(1500)
end)


RegisterNetEvent('FIXDEV-bobcat:search_crate_1unchecked') -- PED WALK
AddEventHandler('FIXDEV-bobcat:search_crate_1unchecked', function()
    TriggerServerEvent("FIXDEV-bobcat:search_crate_1")
end)

RegisterNetEvent('FIXDEV-bobcat:search_crate_2unchecked') -- PED WALK
AddEventHandler('FIXDEV-bobcat:search_crate_2unchecked', function()
    TriggerServerEvent("FIXDEV-bobcat:search_crate_2")
end)

RegisterNetEvent('FIXDEV-bobcat:search_crate_3unchecked') -- PED WALK
AddEventHandler('FIXDEV-bobcat:search_crate_3unchecked', function()
    TriggerServerEvent("FIXDEV-bobcat:search_crate_3")
end)

RegisterNetEvent('FIXDEV-bobcat:search_crate_1') 
AddEventHandler('FIXDEV-bobcat:search_crate_1', function()
    FreezeEntityPosition(GetPlayerPed(-1),true)
    TriggerEvent("doAnim","search")
    local finished = exports["FIXDEV-taskbar"]:taskBar(30000,"Looting")
    if finished == 100 then
        TriggerEvent('FIXDEV-heists:bobcat_loot')
...
RegisterNetEvent('FIXDEV-bobcat:search_crate_2') 
...
RegisterNetEvent('FIXDEV-bobcat:search_crate_3')
...
RegisterNetEvent('FIXDEV-heists:bobcat_loot')
AddEventHandler('FIXDEV-heists:bobcat_loot', function()
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(item)
    if item == "thermitecharge" then
        TriggerEvent('FIXDEV-bobcat:firstdoor')
        TriggerEvent('FIXDEV-bobcat:seconddoor') 
    end
end)

AddEventHandler("FIXDEV-inventory:itemUsed", function(item)
    if item == "bobcatsecuritycard" then
        TriggerEvent('FIXDEV-heists:bobcat_armed_door')
    end
end)
...
exports["FIXDEV-polytarget"]:AddCircleZone("np_bobcat_loot_1",  vector3(886.61, -2287.32, 29.92), 1.0, {
    useZ = true,
})

exports["FIXDEV-polytarget"]:AddCircleZone("np_bobcat_loot_3",  vector3(881.4, -2282.62, 30.42), 1.0, {
    useZ = true,
})

exports["FIXDEV-polytarget"]:AddCircleZone("np_bobcat_loot_2",  vector3(882.49, -2286.17, 30.37), 1.0, {
    useZ = true,
})

exports["FIXDEV-interact"]:AddPeekEntryByPolyTarget('np_bobcat_loot_1', {{
    id = "np_bobcat_loot_1",
    event = "FIXDEV-bobcat:search_crate_1unchecked",
    icon = "circle",
    label = "Loot!",
    paramers = {},
}}, { distance = { radius = 3.5 }  })

exports["FIXDEV-interact"]:AddPeekEntryByPolyTarget('np_bobcat_loot_2', {{
    id = "np_bobcat_loot_2",
    event = "FIXDEV-bobcat:search_crate_2unchecked",
    icon = "circle",
    label = "Loot!",
    paramers = {},
}}, { distance = { radius = 3.5 }  })

exports["FIXDEV-interact"]:AddPeekEntryByPolyTarget('np_bobcat_loot_3', {{
    id = "np_bobcat_loot_3",
    event = "FIXDEV-bobcat:search_crate_3unchecked",
    icon = "circle",
    label = "Loot!",
    paramers = {},
}}, { distance = { radius = 3.5 }  })
