...
RegisterNetEvent('FIXDEV-paleto:box')
AddEventHandler('FIXDEV-paleto:box', function()
    ...
    if exports["isPed"]:isPed("countpolice") >= 0 then
        if distance < 1.5 then
            if exports["FIXDEV-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                ...
                PaletoStep1()
                exports["FIXDEV-dispatch"]:dispatchadd('10-90', "Paleto Bank Robbery", "108")
...
RegisterNetEvent('FIXDEV:createpaletomoney')
AddEventHandler('FIXDEV:createpaletomoney', function(toggle)
    if toggle == true then
...
function CreateMoney()
    ...
    TriggerEvent('FIXDEV:CleanClientVaultD')
    TriggerEvent("FIXDEV:createpaletomoney" ,true)
    Citizen.Trace("Crates Spawned :  Sucess")
    Citizen.Wait(240000)
    Citizen.Wait(60000)
    TriggerEvent("FIXDEV:createpaletomoney" ,false)
end


RegisterNetEvent('FIXDEV-paleto:startpaleto')
AddEventHandler('FIXDEV-paleto:startpaleto', function()
    local player = GetEntityCoords(PlayerPedId())
...
function OnHackSuccess()
    ...
    TriggerEvent("inventory:removeItem", "heistlaptop2", 1)
    TriggerServerEvent('FIXDEV-paleto:startCoolDown')
    loadDicts()
    ...
    TriggerServerEvent('FIXDEV-doors:change-lock-state', 45, false) 
    CreateMoney()
end

RegisterNetEvent('FIXDEV-paletogold')
AddEventHandler('FIXDEV-paletogold', function()
    print('called 2?')
    TriggerServerEvent('FIXDEV-paleto:tut_tut')
end)

AddEventHandler("FIXDEV-inventory:itemUsed", function(pItem)
    if pItem == "thermitecharge" then
        TriggerEvent('FIXDEV-paleto:box')
    elseif pItem == "heistlaptop2" then
        TriggerServerEvent("FIXDEV-paleto:hacklaptop")
    end
end)