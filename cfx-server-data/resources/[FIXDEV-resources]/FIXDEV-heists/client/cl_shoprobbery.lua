...
if store_safe_1 == false then
    ...
    if exports["isPed"]:isPed("countpolice") >= 1 then
        if exports["FIXDEV-inventory"]:hasEnoughOfItem("safecrackingkit", 1) then
            if store_crack_1 == true then
                TriggerEvent("FIXDEV-dispatch:storerobbery2")
                exports['FIXDEV-thermite']:OpenThermiteGame(function(success)
                    if success then
                        exports["FIXDEV-dispatch"]:dispatchadd('10-31A', "Store Robbery", "59")
...
RegisterNetEvent("FIXDEV-heists:safe:rob", function()
    ...
    if exports["isPed"]:isPed("countpolice") >= 0 then
        if GetEntityHealth(obj) < 1600 then
            local finished = exports["FIXDEV-ui"]:taskBarSkill(25000, math.random(10, 20))
            if (finished == 100) then
                TriggerEvent("FIXDEV-dispatch:storerobbery2")
                local finished = exports["FIXDEV-ui"]:taskBarSkill(30000, math.random(10, 20))
                if (finished == 100) then
                    TriggerEvent("FIXDEV-dispatch:storerobbery2")
                    ClearPedTasks(PlayerPedId())
