...
RegisterNetEvent("FIXDEV-heists:bobcatMoneyTruckSpawnSettings")
AddEventHandler("FIXDEV-heists:bobcatMoneyTruckSpawnSettings", function(pNetId)
...
RegisterNetEvent("FIXDEV-heists:controlBobcatNpc")
AddEventHandler("FIXDEV-heists:controlBobcatNpc", function(pNetId)
  Citizen.CreateThread(function()
    local ped = 0
    while ped == 0 do
...
AddEventHandler("FIXDEV-heists:bobcatPickupLootCache", function(p1, p2, pArgs)
...
RegisterNetEvent("FIXDEV-heists:bobcatMoneyTruckSpawned")
AddEventHandler("FIXDEV-heists:bobcatMoneyTruckSpawned", function(pCoords)
  if myJob == "police" then return end
  if not exports['FIXDEV-inventory']:hasEnoughOfItem("vpnxja", 1, false, true) then return end
...
local thermiteMinigameUICallbackUrl = "FIXDEV-ui:heistsThermiteMinigameResult"
local gameSettings = {
...
AddEventHandler("FIXDEV-heists:bobcatThermiteMoneyTruckDoor", function(pArgs, pEntity, pContext)
  ...
  print(success)
  if not success then return end
  RPC.execute("FIXDEV-heists:bobcatMoneyTruckBegin", netId)
end)
...
AddEventHandler("FIXDEV-heists:bobcatMoneyTruckTakeGoods", function(p1, pEntity)
  RPC.execute("FIXDEV-heists:bobcatMoneyTruckLootPlease", NetworkGetNetworkIdFromEntity(pEntity))
end)

AddEventHandler("FIXDEV-heists:interactBobcatC4Npc", function(pArgs, pEntity)
  RPC.execute("heists:activateC4Npc", NetworkGetNetworkIdFromEntity(pEntity))
end)

Citizen.CreateThread(function()
  exports["FIXDEV-polytarget"]:AddBoxZone("bc_weapon_cache", vector3(882.07, -2282.89, 32.44), 1.8, 0.4, {
    heading=335,
    minZ=29.64,
    maxZ=31.04,
    data = {
  ...
  exports["FIXDEV-polytarget"]:AddBoxZone("bc_weapon_cache", vector3(882.58, -2285.71, 30.47), 0.4, 1.0, {
    heading = 340,
    minZ = 29.87,
    maxZ = 30.67,
    data = {
  ...
  exports["FIXDEV-polytarget"]:AddBoxZone("bc_weapon_cache", vector3(886.47, -2286.88, 30.47), 0.8, 1.0, {
    heading = 0,
    minZ = 29.87,
    maxZ = 31.27,
    data = {
  ...
  exports['FIXDEV-interact']:AddPeekEntryByPolyTarget('bc_weapon_cache', {{
    event = "FIXDEV-heists:bobcatPickupLootCache",
...
AddEventHandler('baseevents:enteredVehicle', function(pVehicle, pSeat, pName, pClass)
  ...
  Citizen.CreateThread(function()
    while inStockade do
      ...
      TriggerServerEvent("FIXDEV-heists:bobcat:inStockade", netId, atDeliveryLocation)
      Citizen.Wait(15000)
    end
...
RegisterNetEvent("FIXDEV-heists:bobcat:updateDeliveryLocation")
AddEventHandler("FIXDEV-heists:bobcat:updateDeliveryLocation", function(pLocation, pEmailed)
...
RegisterUICallback("FIXDEV-ui:heists:acceptBobcatJob", function(data, cb)
  cb({ data = {}, meta = { success = true, message = "" }})
  TriggerServerEvent("FIXDEV-heists:bobcat:acceptBobcatOffer")
end)

RegisterNetEvent("FIXDEV-heists:bobcat:offerSecurityTruck")
AddEventHandler("FIXDEV-heists:bobcat:offerSecurityTruck", function()
  local hasVpn = exports["FIXDEV-inventory"]:hasEnoughOfItem("vpnxja", 1, false, true)
...
RegisterNetEvent("FIXDEV-heists:bobcat:securityTruckResult")
AddEventHandler("FIXDEV-heists:bobcat:securityTruckResult", function(isMyTruck, pLocation)
  ...
  Citizen.CreateThread(function()
    while not truckHasSpawned do
      truckHasSpawned = RPC.execute("FIXDEV-heists:bobcat:updateMyLocationForTruck", GetEntityCoords(PlayerPedId()))
      Citizen.Wait(5000)
    end
...
RegisterNetEvent("FIXDEV-heists:bobcat:updateMyLocationForTruckSpawned")
AddEventHandler("FIXDEV-heists:bobcat:updateMyLocationForTruckSpawned", function(pNetId)
