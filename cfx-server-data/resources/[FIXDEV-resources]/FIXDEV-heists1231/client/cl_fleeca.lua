local function usePanel(laptopId)
    Citizen.CreateThread(function()
        ...
        if not success then
            RPC.execute("FIXDEV-heists:fleecaPanelFail")
            return
        end
...
AddEventHandler("heists:fleecaTrolleyGrab", function(loc, type)
    local canGrab = RPC.execute("FIXDEV-heists:fleecaCanGrabTrolley", loc, type)
    if canGrab then
        Loot(type)
        TriggerEvent("DoLongHudText", "You discarded the counterfeit items", 1)
        RPC.execute("FIXDEV-heists:payoutTrolleyGrab", "fleeca", type)
    else
...
-- 		local success = RPC.execute("FIXDEV-heists:fleeca:openBank", currentFleeca.doorConf, currentFleeca.name)
