...
local function release(entity)
    grabbed = false
    exports["FIXDEV-ui"]:hideInteraction("success")
...
local function grab(entity)
    ...
    Wait(2000)
    exports["FIXDEV-ui"]:showInteraction("[F] Release!", "success")
...
local function showInteraction()
    if not interactionUp then
        ...
        exports["FIXDEV-ui"]:showInteraction("[G] Grab!", "error")
    end
end
local function hideInteraction()
    if interactionUp then
        listening = false
        interactionUp = false
        exports["FIXDEV-ui"]:hideInteraction("error")
    end
end
...
AddEventHandler("FIXDEV-casino:casinoEnteredEvent", function() inCasino = true end)
AddEventHandler("FIXDEV-casino:casinoExitedEvent", function() inCasino = false end)

AddEventHandler('FIXDEV-heist:disableHostage', function(pDisabled)
    ...
    while true do
        ...
        then
            ...
            if lastEntity ~= entity then
                local flags = exports["FIXDEV-flags"]:GetPedFlags(entity)
