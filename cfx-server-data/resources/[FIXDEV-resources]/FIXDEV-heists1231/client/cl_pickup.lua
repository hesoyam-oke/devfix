...
RegisterUICallback("FIXDEV-ui:heistsPurchaseItem", function(data, cb)
    ...
    -- if requiredLaptopItem and not exports["FIXDEV-inventory"]:hasEnoughOfItem(requiredLaptopItem, 1, true, true) then
    --   TriggerEvent(
    --     'phone:emailReceived',
    --     'Dark Market',
    --     '#A-1001',
...
RegisterNetEvent("FIXDEV-heists:itemStockAvailable")
AddEventHandler("FIXDEV-heists:itemStockAvailable", function(code, name)
...
RegisterNetEvent("FIXDEV-heists:laptopReadyForPickup")
AddEventHandler("FIXDEV-heists:laptopReadyForPickup", function(code)
  if exports['FIXDEV-inventory']:hasEnoughOfItem('vpnxj', 1, false, true) then
    TriggerEvent(
      'phone:emailReceived',
      'Dark Market',
...
AddEventHandler("FIXDEV-heists:laptopPurchase", function()
  local pracWhitelist = exports['FIXDEV-config']:GetMiscConfig("heists.prac.whitelist")
  local exp = RPC.execute("FIXDEV-heists:hacks:getExperience")
  local items = {
    {
      action = "FIXDEV-heists:context:laptopPurchase",
...
RegisterUICallback("FIXDEV-heists:context:laptopPurchase", function(data, cb)
  ...
  if dongle then
    local hasDongle = exports["FIXDEV-inventory"]:hasEnoughOfItem(dongle, 1, false, true)
...
AddEventHandler("FIXDEV-heists:banks:bankCheck", function()
  local items = RPC.execute("FIXDEV-heists:banksAvailability")
  exports["FIXDEV-ui"]:showContextMenu(items)
end)
RegisterUICallback("FIXDEV-ui:heistsBanksAvailable", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "" }})
end)
