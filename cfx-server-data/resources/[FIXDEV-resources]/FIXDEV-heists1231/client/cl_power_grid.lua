...
Citizen.CreateThread(function()
  exports['FIXDEV-interact']:AddPeekEntryByModel({ -2008643115 }, {
    {
      event = 'FIXDEV-heists:grid:checkNodeState',
      id = 'FIXDEV-heists:grid:checkNodeState',
      icon = 'info-circle',
      label = 'check state',
      parameters = {},
    },
    ...
    isEnabled = function(pEntity)
      ...
      return active and node and node.state ~= 3 and
                 exports['FIXDEV-inventory']:hasEnoughOfItem('relayprobe', 1, false, true)
    end,
  })

  exports['FIXDEV-interact']:AddPeekEntryByModel({ -2008643115 }, {
    {
      event = 'FIXDEV-heists:grid:useThermiteCharge',
      id = 'FIXDEV-heists:grid:useThermiteCharge',
      icon = 'fire-alt',
      label = 'use thermite',
      parameters = {},
    },
    ...
    isEnabled = function(pEntity)
      ...
      return active and node and node.state ~= 3 and (not node.disabled or flashState) and
                 exports['FIXDEV-inventory']:hasEnoughOfItem('thermitecharge', 1, false, true)
    end,
  })

  exports['FIXDEV-polytarget']:AddBoxZone('grid_bank_node', vector3(-355.92, -50.55, 54.42), 1.6, 1, {
    heading = 340,
    minZ = 53.42,
    maxZ = 54.82,
  ...
  exports['FIXDEV-polytarget']:AddBoxZone('grid_bank_node', vector3(1158.43, 2708.93, 37.97), 2.0, 1, {
    heading = 0,
    minZ = 36.97,
    maxZ = 38.77,
  ...
  exports['FIXDEV-polytarget']:AddBoxZone('grid_bank_node', vector3(-2948.24, 481.2, 15.26), 1.8, 1, {
    heading = 355,
    minZ = 14.26,
    maxZ = 16.86,
  ...
  exports['FIXDEV-polytarget']:AddBoxZone('grid_bank_node', vector3(-1217.42, -333.12, 42.12), 1.0, 1.6,
                                      {
    heading = 295,
    minZ = 41.12,
    maxZ = 42.92,
  ...
  exports['FIXDEV-polytarget']:AddBoxZone('grid_bank_node', vector3(319.59, -315.69, 51.21), 1.8, 1, {
    heading = 340,
    minZ = 50.21,
    maxZ = 51.61,
  ...
  exports['FIXDEV-polytarget']:AddBoxZone('grid_bank_node', vector3(138.96, -1055.9, 29.19), 1.8, 1, {
    heading = 250,
    minZ = 28.19,
    maxZ = 29.59,
  ...
  exports['FIXDEV-interact']:AddPeekEntryByPolyTarget('grid_bank_node', {
    {
      event = 'FIXDEV-heists:grid:checkBankNodeState',
      id = 'FIXDEV-heists:grid:checkBankNodeState',
      icon = 'info-circle',
      label = 'check state',
      parameters = {},
    },
...
function startPowerGrid()
  ...
  local nodeData = RPC.execute('FIXDEV-heists:grid:getNodeState')
  ...
  local genData = RPC.execute('FIXDEV-heists:grid:getGenState')
...
AddEventHandler('FIXDEV-heists:grid:checkBankNodeState', function(pArgs, pEntity, pContext)
  mechanicAnim()

  local bank = GetNodeContextId(pContext)
  local node = GetBankNode(bank)

  local finished = exports['FIXDEV-taskbar']:taskBar(1500, 'Getting Info')
  ...
  if finished == 100 then
    local context = {
      ...
      }, { title = 'Test Ping', action = 'FIXDEV-heists:grid:sendTestPing', key = node },
    }
...
AddEventHandler('FIXDEV-heists:grid:checkNodeState', function(pArgs, pEntity, pContext)
  mechanicAnim()
  local node = GetNodeForEntity(pEntity)
  if not node then
  ...
  if finished == 100 then
    local context = {
      ...
      { title = 'View Redirection', action = 'FIXDEV-heists:grid:viewConnected', key = node },
    }
    exports['FIXDEV-ui']:showContextMenu(context)
  end
end)

RegisterUICallback('FIXDEV-heists:grid:viewConnected', function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' } })
  local node = data.key
  local connected = RPC.execute('FIXDEV-heists:grid:getConnected', node.id)
...
RegisterUICallback('FIXDEV-heists:grid:sendTestPing', function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' } })
  local node = data.key

  local success = RPC.execute('FIXDEV-heists:grid:pingNode', node)
  if not success then
...
AddEventHandler('FIXDEV-heists:grid:useThermiteCharge', function(pArgs, pEntity, pContext)
  local node = GetNodeForEntity(pEntity)
  if not node then
  ...
  sendAlert()
  if success then
    RPC.execute('FIXDEV-heists:grid:useThermiteCharge', node)
  end
end)

AddEventHandler('FIXDEV-inventory:itemUsed', function(pItemId, pInfo)
  ...
  if pItemId == 'heistlaptop333' then -- disabled
    if not exports['FIXDEV-inventory']:hasEnoughOfItem('heistusb5', 1, false, true) then
    ...
    Citizen.CreateThread(function()
      ...
      if MinigameResult then
        ...
        startPowerGrid()
        TriggerServerEvent('FIXDEV-heists:hack:success', minigameHackExp)
      end
      MinigameResult = nil
    end)
    return
...
RegisterNetEvent('FIXDEV-heists:grid:setActive', function(pState)
  active = pState
  if not active then
    for _, node in ipairs(Nodes) do
  ...
  exports['FIXDEV-island']:hideBlips(active)
end)

RegisterNetEvent('FIXDEV-heists:grid:updateNodeState', function(pNode)
  for idx, node in ipairs(Nodes) do
...
RegisterNetEvent('FIXDEV-heists:grid:updateGenState', function(pGenId, pState)
  for _, gen in ipairs(Generators) do
