...
RegisterNetEvent("FIXDEV-heists:updateDoorStatus")
AddEventHandler("FIXDEV-heists:updateDoorStatus", function(name, heading, frameCount)
    prevHeadings[name] = heading
    if doorEntities[name] == nil then return end
    ChangeDoorHeading(doorEntities[name], heading, frameCount)
end)

AddEventHandler("FIXDEV-heists:doors:vaultDoor", function(pParams)
