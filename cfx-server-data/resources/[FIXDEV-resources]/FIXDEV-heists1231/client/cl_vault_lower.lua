...
function VaultLowerUsePanel(laptopId)
    ...
    if not success then
        RPC.execute("FIXDEV-heists:vaultLowerPanelFail")
        return
    end
...
RegisterNetEvent("FIXDEV-heists:swapLowerVaultIPL")
AddEventHandler("FIXDEV-heists:swapLowerVaultIPL", function(state, uState)
...
Citizen.CreateThread(function()
    local result = RPC.execute("FIXDEV-heists:getVaultLowerState")
...
AddEventHandler("FIXDEV-polyzone:enter", function(name)
    ...
    RPC.execute("FIXDEV-heists:lowerVaultEntranceEnter")
end)

AddEventHandler("heists:vaultLowerTrolleyGrab", function(loc, type)
    local canGrab = RPC.execute("FIXDEV-heists:vaultLowerCanGrabTrolley", loc, type)
    if canGrab then
        Loot(type)
        TriggerEvent("DoLongHudText", "You discarded the counterfeit items", 1)
        RPC.execute("FIXDEV-heists:payoutTrolleyGrab", loc, type)
    else
...
Citizen.CreateThread(function()
  exports["FIXDEV-polytarget"]:AddBoxZone("lower_vault_keypad", vector3(286.53, 220.17, 97.69), 0.4, 0.4, {
    heading=0,
    minZ=98.09,
    maxZ=98.49,
    data = {
  ...
  exports["FIXDEV-polytarget"]:AddBoxZone("lower_vault_keypad", vector3(284.7, 221.63, 97.69), 0.4, 0.4, {
    heading=340,
    minZ=98.09,
    maxZ=98.49,
    data = {
  ...
  exports["FIXDEV-polytarget"]:AddBoxZone("lower_vault_keypad", vector3(286.83, 227.45, 97.69), 0.4, 0.4, {
    heading=340,
    minZ=98.09,
    maxZ=98.49,
    data = {
  ...
  exports["FIXDEV-polytarget"]:AddBoxZone("lower_vault_keypad", vector3(289.21, 227.46, 97.69), 0.4, 0.4, {
    heading=325,
    minZ=98.09,
    maxZ=98.49,
    data = {
  ...
  exports['FIXDEV-interact']:AddPeekEntryByPolyTarget('lower_vault_keypad', {{
    event = "FIXDEV-heists:lowerVaultPanelPush",
...
local publicGameFinishedEndpointNumbers = 'FIXDEV-ui:heistsLVPublicMinigameNumbersEndpoint'
local activePanelId = 0
RegisterUICallback(publicGameFinishedEndpointNumbers, function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  RPC.execute("FIXDEV-heists:lowerVaultPanelPublicPush", activePanelId, data.success)
  activePanelId = 0
end)
AddEventHandler("FIXDEV-heists:lowerVaultPanelPush", function(p1, p2, pArgs)
  local id = pArgs.zones["lower_vault_keypad"].id
  exports['FIXDEV-ui']:openApplication('textbox', {
    callbackUrl = 'FIXDEV-ui:heists:lowerVaultPanel',
    key = id,
    items = {
      {
        icon = "user-secret",
...
RegisterUICallback("FIXDEV-ui:heists:lowerVaultPanel", function(data, cb)
...
--
local gameFinishedEndpointNumbers = 'FIXDEV-ui:heistsLVMinigameNumbersEndpoint'
AddEventHandler("FIXDEV-heists:vaultLowerKeyboardInteract", function()
  local hasUsb = exports['FIXDEV-inventory']:hasEnoughOfItem('vcomputerusb', 1, false, true)
  if not hasUsb then
    TriggerEvent("DoLongHudText", "Missing PC USB stick", 2)
    return
  end
  RPC.execute("FIXDEV-heists:registerLVUsbUse")
  exports["FIXDEV-ui"]:openApplication("minigame-numbers", {
