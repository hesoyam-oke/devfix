...
function VaultUpperUsePanel(laptopId)
    ...
    if not success then
        RPC.execute("FIXDEV-heists:vaultUpperPanelFail")
        return
    end
...
RegisterCommand("spawn-gold", function()
  local trolleyConfig1 = GetTrolleyConfig("mazebank_cash_1")
  SpawnTrolley(trolleyConfig1.cashCoords, "cash", trolleyConfig1.cashHeading)
end, false)

AddEventHandler("heists:vaultUpperTrolleyGrab", function(loc, type)
    local canGrab = RPC.execute("FIXDEV-heists:vaultUpperCanGrabTrolley", loc, type)
    if canGrab then
        Loot(type)
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(item)
  ...
  if varHeistDoors[foundLocationName] and success then
    RPC.execute('FIXDEV-heists-serverroom:varDoorOpen', foundLocationName)
  end
  if hasinoDoors[foundLocationName] and success then
    TriggerEvent("FIXDEV-hasino:thermiteSuccess", foundLocationName)
  end
end)
