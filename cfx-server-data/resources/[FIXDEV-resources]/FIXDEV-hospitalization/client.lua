...
RegisterNetEvent("FIXDEV-hospitalization:checkin")
AddEventHandler("FIXDEV-hospitalization:checkin", function()
	if exports["isPed"]:isPed("countems") <= 0 or exports["isPed"]:isPed("myJob") == 'ems'  then
		...
		local finished = exports["FIXDEV-taskbar"]:taskBar(1700,"Checking Credentials")
...
RegisterNetEvent("FIXDEV-hospitalization:page")
AddEventHandler("FIXDEV-hospitalization:page", function()
	loadAnimDict('anim@narcotics@trash')
	TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
	local finished = exports["FIXDEV-taskbar"]:taskBar(1700,"Paging a Doctor")
	if finished == 100 then
...
AddEventHandler("client:bed",function()
  ...
  if DoesEntityExist(objFound) then
	...
	exports['FIXDEV-textui']:showInteraction("[E] Leave Bed")
...
function findBed(fadein)
	...
	if not inbed then
		...
		inbed = false
		injurycount = 0
		TriggerEvent('FIXDEV-hospital:client:ResetLimbs')
		TriggerEvent('FIXDEV-hospital:client:RemoveBleed')
