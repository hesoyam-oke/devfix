...
AddEventHandler('house:updatespawns', function(info, house_id)
    ...
    if IsRealtor(src) == "real_estate" then
        exports.oxmysql:execute('SELECT * FROM __housedata WHERE house_id = ?', {house_id}, function(result)
            exports.oxmysql:execute("UPDATE __housedata SET `data` = @data WHERE `house_id` = @house_id", {
...
AddEventHandler('house:purchasehouse', function(cid,house_id,house_model,upfront,price,housename,garages)
    ...
    if tonumber(user:getCash()) >= tonumber(price) then
        ...
        exports.oxmysql:execute('SELECT `housename` FROM __housedata WHERE `housename`= ?', {housename}, function(data)
            if data[1].cid == nil then
                exports.oxmysql:execute("UPDATE __housedata SET `cid` = @cid, `upfront` = @upfront, `housename` = @housename, `garages` = @garages, `house_model` = @house_model, `overall` = @overall, `payments` = @payments, `due` =@due, `days` = @days WHERE `house_id` = @house_id", {
                    ['@cid'] = cid,
                    ['@upfront'] = upfront,
                    ['@housename'] = housename,
                ...
                local payout = price
                TriggerClientEvent('FIXDEV-motels:RealtorPay', src, payout)
            else
...
AddEventHandler('housing:requestSpawnTable', function(table, id, house_name, house_model)
    local src = source
    exports.oxmysql:execute('SELECT * FROM __housedata WHERE `house_id` = @house_id', { ['@house_id'] = id }, function(result)
        if result[1] ~= nil then
            TriggerClientEvent('UpdateCurrentHouseSpawns', src, id, result[1].data)
        else
            exports.oxmysql:execute("INSERT INTO __housedata (data, house_id, house_model, housename) VALUES (@data, @house_id, @house_model, @housename)", {
                ['@house_id'] = id,
...
AddEventHandler('housing:getGarage', function(house_id, house_model)
    ...
    exports.oxmysql:execute('SELECT * FROM __housekeys WHERE `house_id`= ? AND `house_model`= ?  AND `cid` = ?', {house_id, house_model, cid}, function(returnData)
    ...
    exports.oxmysql:execute('SELECT * FROM __housedata WHERE `house_id`= ? AND `house_model`= ?  AND `cid` = ?', {house_id, house_model, cid}, function(penis)
        if penis[1] ~= nil then
...
AddEventHandler('house:enterhouse', function(cid,isRealEstate,house_id,house_model,furniture)
    local src = source
    exports.oxmysql:execute('SELECT * FROM __housedata WHERE `house_id`= ? AND `house_model`= ?', {house_id, house_model}, function(house)
        if house[1].force_locked ~= "locked" then
            if house[1].cid == cid then
                ...
                exports.oxmysql:execute('SELECT * FROM __housekeys WHERE `house_id`= ? AND `house_model`= ? AND `cid` = ?', {house_id, house_model, cid}, function(house2)
                    if house2[1] ~= nil then
                        exports.oxmysql:execute('SELECT * FROM __housedata WHERE `house_id`= ? AND `house_model`= ?', {house_id, house_model}, function(penis)
            ...
            exports.oxmysql:execute('SELECT * FROM __housedata WHERE `house_id`= ? AND `house_model`= ?', {house_id, house_model}, function(guest)
                if isRealEstate == true then
                    ...
                    if guest[1].cid ~= cid then
                        if guest[1].status ~= "locked" then
                            ...
                            exports.oxmysql:execute('SELECT * FROM __housekeys WHERE `house_id`= ? AND `house_model`= ? AND `cid` = ?', {house_id, house_model, cid}, function(guest2)
...
local function checkKeyExist(cid, housename, cb)
    exports.oxmysql:execute("SELECT * FROM __housekeys WHERE cid = @cid AND housename = @housename LIMIT 1;", {["cid"] = cid, ["housename"] = housename}, function(result)
...
AddEventHandler('house:givekey', function(house_id,house_model,house_name,target)
    ...
    local zUser = exports["FIXDEV-base"]:getModule("Player"):GetUser(target)
    local zchar = zUser:getCurrentCharacter()
    exports.oxmysql:execute("SELECT * FROM __housedata WHERE house_id = @house_id AND house_model = @house_model", {['@house_id'] = house_id, ['@house_model'] = house_model}, function(result)
        if result[1].cid == char.id then  
            exports.oxmysql:execute('SELECT * FROM __housekeys WHERE `cid`= ?', {zchar.id}, function(data)
                checkKeyExist(zchar.id, house_name, function(exists)
                    if not exists then
                        local name = zchar.first_name .. " " .. zchar.last_name
                        exports.oxmysql:execute('INSERT INTO __housekeys(cid, house_id, house_model, housename, name) VALUES(?, ?, ?, ?, ?)', {zchar.id, house_id, house_model, house_name, name})
...
AddEventHandler('house:retrieveKeys', function(house_id, house_model)
    local src = source
    local shared = {}
    exports.oxmysql:execute('SELECT * FROM __housekeys WHERE `house_id`= ?', {house_id}, function(data)
    ...
    if IsRealtor(src) == "real_estate" then
        exports.oxmysql:execute('SELECT * FROM __housedata WHERE house_id = ?', {house_id}, function(result)
            ...
            if result[1].force_locked == "unlocked" then
                exports.oxmysql:execute("UPDATE __housedata SET `force_locked` = @force_locked WHERE `house_id` = @house_id ", {
                    ['@force_locked'] = "locked",
                    ['@house_id'] = house_id
                })
...
AddEventHandler("housing:unlock", function(house_id, house_model)
    ...
    exports.oxmysql:execute('SELECT * FROM __housedata WHERE house_id = ?', {house_id}, function(result)
        ...
        if result[1].status == "unlocked" then
            exports.oxmysql:execute("UPDATE __housedata SET `status` = @status WHERE `house_id` = @house_id ", {
                ['@status'] = "locked",
                ['@house_id'] = house_id
            })
...
AddEventHandler('house:removeKey', function(house_id, house_model, target)
    local src = source
    exports.oxmysql:execute('SELECT * FROM __housekeys WHERE house_id = ? AND cid = ?', {house_id, target}, function(data)
        local name = data[1].name
        TriggerClientEvent('DoLongHudText', src, "You removed " .. name .. "'s keys.")
        exports.oxmysql:execute('DELETE FROM __housekeys WHERE `house_id`= ? AND `house_model`= ? AND `cid`= ?', {house_id, house_model, target})
        TriggerClientEvent("FIXDEV-phone:keys:reload", src)
    end)
...
AddEventHandler("houses:removeSharedKey", function(house_id, cid)
    local src = source
    exports.oxmysql:execute('DELETE FROM __housekeys WHERE `house_id`= ? AND `cid`= ?', {house_id, cid})
...
AddEventHandler('CheckFurniture', function(house_id, house_model)
    local src = source
    exports.oxmysql:execute('SELECT * FROM __housedata WHERE `house_id`= ? AND `house_model`= ?', {house_id, house_model}, function(data)
...
AddEventHandler('house:evictHouse', function(hid, model, cid)
    ...
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    if IsRealtor(src) == "real_estate" then
        exports.oxmysql:execute('SELECT * FROM __housedata WHERE house_id = ? AND house_model = ?', {hid, model}, function(result)
            if result[1] ~= nil then
                TriggerClientEvent("DoLongHudText", src, result[1].housename.. " has been deleted", 2)
                exports.oxmysql:execute("DELETE FROM __housedata WHERE house_id = @house_id AND house_model = @house_model", {['house_id'] = hid, ['house_model'] = model})
            end
        end)
    end
end)
...
AddEventHandler("house:PropertyOutstanding", function(house_id, house_model)
    local src = source
    exports.oxmysql:execute('SELECT * FROM __housedata WHERE house_id = ? AND house_model = ?', {house_id, house_model}, function(result)
        if result[1] ~= nil then
...
AddEventHandler("housing:garagesSET", function(garages, house_id)
    ...
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    if IsRealtor(src) == "real_estate" then
        exports.oxmysql:execute("UPDATE __housedata SET `garages` = @garages WHERE `house_id` = @house_id", {
            ['@garages'] = json.encode(garages),
            ['@house_id'] = house_id
        })

        exports.oxmysql:execute("UPDATE __housekeys SET `garages` = @garages WHERE `house_id` = @house_id", {
            ['@garages'] = json.encode(garages),
            ['@house_id'] = house_id
        })
    end
...
AddEventHandler("house:transferHouse", function(house_id, house_model, cid)
    ...
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    if IsRealtor(src) == "real_estate" then
        exports.oxmysql:execute("UPDATE __housedata SET `cid` = @cid WHERE `house_model` = @house_model AND `house_id` = @house_id", {
            ['@cid'] = transfercid,
            ['@house_id'] = house_id,
            ['@house_model'] = house_model
        })
            ...
            if result[1].can_pay ~= "false" then
                if result[1].due ~= tonumber(0) then
                    if user:getBalance() >= amountdue then
                        ...
                        exports.oxmysql:execute("UPDATE __housedata SET `payments` = @payments, `due` = @due, `days` = @days, `can_pay` = @can_pay WHERE `house_id` = @house_id AND `house_model` = @house_model", {
...
function updateFinance()
    exports.oxmysql:execute('SELECT days, house_id FROM __housedata WHERE days > @days', {
        ["@days"] = 0
    }, function(result)
        for i=1, #result do
            ...
            if financeTimer ~= nil then
                exports.oxmysql:execute('UPDATE __housedata SET days = @days WHERE house_id = @house_id', {
                    ['@house_id'] = house_id,
                    ['@days'] = newTimer
                })
            end
            if financeTimer < 100 then
                exports.oxmysql:execute("UPDATE __housedata SET `can_pay` = @can_pay WHERE `house_id` = @house_id ", {
                    ['@can_pay'] = "true",
                    ['@house_id'] = house_id
                })
            end
...
AddEventHandler('UpdateFurniture', function(house_id, house_model, modifiedObjects)
    exports.oxmysql:execute("UPDATE __housedata SET `furniture` = @furniture WHERE `house_id` = @house_id AND `house_model` = @house_model", {
...
function IsRealtor(pSrc)
    ...
    exports.oxmysql:execute('SELECT `name` FROM character_passes WHERE cid = ?', {pCid}, function(data)
        pJob = data[1].name
    end)
	...
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "real_estate" and v.rank >= 5 then
				TriggerClientEvent('FIXDEV-jobsystem:reJobMenuHire', src)
			end
		end
	end)
end)
...
RegisterCommand("firere", function(source)
	...
	exports.oxmysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "real_estate" and v.rank >= 5 then
				TriggerClientEvent('FIXDEV-jobsystem:reJobMenuFire', src)
			end
		end
	end)
end)