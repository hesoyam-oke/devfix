...
RegisterNetEvent("FIXDEV-housing:enterBUILDINGCMON", function()
    InteractionPressed()
end)
...
RegisterCommand("FIXDEV-housing:getGarages", function()
    TriggerServerEvent("FIXDEV-housing:getGarages")
end)

function getIsPropLock()
    local isComplete, _propertyID, dist, zone = Housing.func.findClosestProperty()
    local pp =  RPC.execute("FIXDEV-housing:isHousingLockedServer", _propertyID)
...
AddEventHandler('FIXDEV-housing:phone:check', function(type)
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
    ...
    local isThisPropertyOwned = RPC.execute("FIXDEV-housing:isThisPropertyOwned", propertyId)
    ...
    if type == "edit" then
        ...
        TriggerEvent('FIXDEV-phone:editingHouse',id,street,price)
    else
        TriggerEvent('FIXDEV-phone:sentHouse',nil,id,street,price,category)
...
function enterEdit(pPropertyId)
    ...
    if playerInRangeOfProperty(propertyID) then
        ...
        TriggerEvent("FIXDEV-housing:edit")
    else
...
function setInventory()
    ...
    if Housing.currentlyInsideBuilding then
        ...
        local buildingVector = exports["FIXDEV-build"]:getModule("func").currentBuildingVector()
...
function exitingBuilding()
    ...
    if id then
        TriggerEvent("FIXDEV-editor:destroyName", id)
    end
...
function Housing.func.loadInteractions(model,overWrite)
    ...
    local isBuiltCoords = exports["FIXDEV-build"]:getModule("func").addInteractionPoints(model,interactionPoints,overWrite)
end
...
AddEventHandler("FIXDEV-housing:sell", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
AddEventHandler("FIXDEV-housing:sellTo", function(pParams)
    local input = exports["FIXDEV-ui"]:showInput({
		{
            icon = "id-card",
            label = "PayPal ID",
            name = "cid",
	...
	if input["cid"] then
		...
		TriggerServerEvent("FIXDEV-housing:sell", cid, pParams.propertyId)
	end
end)

RegisterNetEvent("FIXDEV-housing:buy")
AddEventHandler("FIXDEV-housing:buy", function(pPropertyId, pSeller)
...
RegisterCommand('rent', function()
    TriggerEvent('FIXDEV-housing:rent')
end)

RegisterNetEvent("FIXDEV-housing:rent", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
    ...
    local menuData = {
        ...
        {
            title = "Rent",
            action = "FIXDEV-housing:rent2",
            params = { propertyId = propertyId, total = tax, tax = 0 },
        },
    }
    exports["FIXDEV-interface"]:showContextMenu(menuData)
end)

RegisterNetEvent("FIXDEV-housing:rent2", function(pParams)
    local complete, info = RPC.execute("FIXDEV-housing:rent", pParams.propertyId, pParams.total, pParams.tax)
...
RegisterNetEvent("FIXDEV-housing:buyed", function(pParams)
    print('hello?')
    local complete, info = RPC.execute("FIXDEV-housing:buy", pParams.propertyId, pParams.total, pParams.tax, pParams.seller)
...
RegisterNetEvent("FIXDEV-housing:edit", function()
end)

RegisterNetEvent("FIXDEV-housing:editOptions", function(pParams)
...
AddEventHandler("housing:charLogout", function()
    ...
    if max.canHaveCharSelect then
        ...
        exports["FIXDEV-build"]:getModule("func").CleanUpArea()
        local cid = exports["isPed"]:isPed("cid")
        TriggerServerEvent("FIXDEV-jobmanager:onCharSwap", cid)
        Citizen.Wait(1000)   
...
AddEventHandler("housing:frontdoor", function()
    DoScreenFadeOut(1)
    exports["FIXDEV-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id]["pos"])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
AddEventHandler("housing:internalBackdoor", function()
    ...
    if max.canHaveBackDoor then
        ...
        DoScreenFadeOut(1)
        exports["FIXDEV-build"]:getModule("func").exitCurrentRoom(Housing.currentHousingInteractions.backdoor_coordinates.external)
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)
...
AddEventHandler("housing:kickFromHouse", function(propertyID)
    ...
    if id == propertyID then
        DoScreenFadeOut(1)
        exports["FIXDEV-build"]:getModule("func").exitCurrentRoom(Housing.info[id]["pos"])
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)

RegisterNetEvent("FIXDEV-housing:blips")
AddEventHandler("FIXDEV-housing:blips", function()
    Housing.Blips = not Housing.Blips

    local houses = RPC.execute("FIXDEV-housing:owned")
...
RegisterNetEvent('FIXDEV-housing:logShit')
AddEventHandler('FIXDEV-housing:logShit', function(hID, Price)
    TriggerServerEvent('FIXDEV-housing:buyLog', hID, Price)
end)