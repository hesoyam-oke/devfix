...
GetUserByCid = function(cid)
    for index, value in pairs(GetPlayers()) do
        local xPlayer = exports['FIXDEV-base']:getModule("Player"):GetUser(tonumber(value))
...
RPC.register("FIXDEV-housing:isThisPropertyOwned", function(src, propertyId)
    local result = MySQL.query.await([[
...
RegisterNetEvent("FIXDEV-housing:enterHouse")
AddEventHandler("FIXDEV-housing:enterHouse", function(pPropertyId)
    local src = source
    Housing.Players[src] = pPropertyId
end)

RegisterNetEvent("FIXDEV-housing:leftHouse")
AddEventHandler("FIXDEV-housing:leftHouse", function(pPropertyId)
    local src = source
...
RPC.register("FIXDEV-phone:getHouseKeys", function(src, pHouseId)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return {} end
...
RPC.register("FIXDEV-phone:giveKey", function(src, pHouseId, pPlayerId)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false, "ID não encontrado" end

    local user2 = exports['FIXDEV-base']:getModule("Player"):GetUser(pPlayerId)
    ...
    TriggerClientEvent("FIXDEV-phone:notification", pPlayerId, "fas fa-key", "Keys", "Você recebeu a chave da propriedade " .. Housing.info[pHouseId]["street"], 5000)
    TriggerClientEvent("FIXDEV-housing:refresh", pPlayerId)

    return true, "Chave recebida"
end)

RPC.register("FIXDEV-phone:removeKey", function(src, pHouseId, pPlayerId)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
...
RPC.register("FIXDEV-phone:payHouse", function(src, pHouseId)
  ...
  --  local tax = price
   ...
   -- local accountId = exports["FIXDEV-base"]:getChar(src, "bankid")
   -- local bank = exports["FIXDEV-financials"]:getBalance(accountId)
   -- if bank < priceTotal then
   --     return false, "Você não tem $" .. priceTotal .. " em sua conta"
    --end

    --local groupBank = exports["FIXDEV-groups"]:groupBank("real_estate")

    --local comment = "Pagamento de aluguel da propriedade " .. Housing.info[pHouseId]["street"]
    --local success, message = exports["FIXDEV-financials"]:transaction(accountId, groupBank, price, comment, cid, 5)
    --if not success then
      --  return false, message
    --end

    --exports["FIXDEV-financials"]:addTax("Propertys", tax.tax)
...
RPC.register("FIXDEV-phone:removeSharedKey", function(src, pHouseId)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false end
    ...
    if affectedRows and affectedRows > 0 then
        TriggerClientEvent("FIXDEV-housing:refresh", src)
        return true
    end
...
RegisterServerEvent('FIXDEV-housing:buyLog')
AddEventHandler('FIXDEV-housing:buyLog', function(pHouse, price)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/1012082840038555690/uBrYz4sosUN9pwxpHI6h13f87WB8HQzvUcVFPhXTI6xJOsvsbltkRUGccok-4ICS51PG", function(err, text, headers) end, 'POST', json.encode({username = "np | House Bought Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)