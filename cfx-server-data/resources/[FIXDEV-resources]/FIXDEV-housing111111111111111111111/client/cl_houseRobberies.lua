...
function attemptToLockPickHouse(skipPicking)
    ...
    if not skipPicking then
        if not Housing.housingBeingRobbedClient[propertyID] then
            ...
            local finished = exports["FIXDEV-taskbar"]:taskBar(math.random(5000, 10000), "Breaking in")
...
function alarm(clearAlarm)
    if Housing.alarm then
        local alarmOffset = exports["FIXDEV-build"]:getModule("func").getAlarm()
...
function displayPickup(force,itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["FIXDEV-textui"]:showInteraction("[E] Take Object")
        else
            exports["FIXDEV-textui"]:showInteraction("[E] Look for some belongings.")
        end
    end
end
...
function interactRob()
    ...
    if type(item) == "string" then
        local weight = exports["FIXDEV-inventory"]:getCurrentWeight()
    ...
    if type(item) == "string" then
        ...
        local finished = exports["FIXDEV-taskbar"]:taskBar(timer,"Securing Item")
        ClearPedTasks(PlayerPedId())
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["FIXDEV-taskbar"]:taskBar(15000,"Procurando")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
    ...
    if finished then
        if type(item) == "string" then
            ...
            if euemeusmanosodiamosoartmike >= 90 then
                TriggerServerEvent("FIXDEV-heists:complete", math.random(30, 50))
...
RegisterNetEvent("FIXDEV-weathersync:currentTime", function(pHour, pMinute)
...
RegisterNetEvent("FIXDEV-housing:alarmFail")
AddEventHandler("FIXDEV-housing:alarmFail", function()
...
RegisterNetEvent("FIXDEV-housing:alarmEnter")
AddEventHandler("FIXDEV-housing:alarmEnter", function()
    insideGame = true
    local successevrp
...
AddEventHandler("housing:exitFrontDoor", function()
    ...
    TriggerServerEvent("FIXDEV-housing:leftHouse", Housing.currentHousingInteractions.id)
...
RegisterNetEvent("FIXDEV-housing:alarm")
AddEventHandler("FIXDEV-housing:alarm", function(pHouseId)
    if pHouseId == Housing.currentlyRobInside then
        local gameTimer = GetGameTimer() + 30000


        while true do
            local alarmOffset = exports["FIXDEV-build"]:getModule("func").currentBuildingVector()
...
RegisterNetEvent("FIXDEV-housing:beingRobbed", function(pPropertyId)
