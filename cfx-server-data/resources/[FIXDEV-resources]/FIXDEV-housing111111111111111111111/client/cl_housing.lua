...
function Housing.func.enterBuilding(propertyID,enterOverride,counterPart)
    ...
    local isBuiltCoords, objects = exports["FIXDEV-build"]:getModule("func").buildRoom(model,spawnBuildingLocation,false,Housing.destroyedObjects,enterOverride,true)
    if counterPart and Housing.staticObjectRobPoints == nil then
        Housing.staticObjectRobPoints = exports["FIXDEV-build"]:getModule("func").getRobLocationsForObjects(model,spawnBuildingLocation,Housing.housingRobTargets.static)
    ...
    if isBuiltCoords then
        ...
        TriggerServerEvent("FIXDEV-housing:enterHouse", propertyID)

        if not counterPart then
            TriggerEvent("FIXDEV-editor:buildName", propertyID, objects)
        end
    else
        Housing.currentHousingInteractions = nil
        Housing.currentlyInsideBuilding = false
...
RegisterNetEvent("FIXDEV-housing:loadHousingClient")
AddEventHandler("FIXDEV-housing:loadHousingClient", function()
    gatherPlayerInfo()
end)

RegisterNetEvent("FIXDEV-housing:reset")
AddEventHandler("FIXDEV-housing:reset", function()
    -- cl_housing
    Housing.currentOwned = {}
    Housing.currentKeys = {}
    Housing.currentlyEditing = false
...
RegisterNetEvent("FIXDEV-housing:refresh")
AddEventHandler("FIXDEV-housing:refresh", function()
    gatherPlayerInfo()
end)
