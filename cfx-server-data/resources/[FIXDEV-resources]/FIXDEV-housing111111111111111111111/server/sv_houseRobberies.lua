...
RegisterNetEvent("FIXDEV-base:sessionStarted", function()
    local src = source
    TriggerClientEvent("FIXDEV-housing:beingRobbed", src, Housing.housingBeingRobbed)
end)
...
RPC.register("housing:robbery:pickedLock", function(src, pHouseId)
    ...
    TriggerClientEvent("FIXDEV-housing:beingRobbed", -1, pHouseId)
...
RPC.register("property:alarmDisabled", function(src, pHouseId, pFail)
    ...
    for k, v in pairs(Housing.Players) do
        if v == pHouseId then
            ...
            if pFail then
                TriggerClientEvent("FIXDEV-housing:alarm", src, pHouseId)
            end
        end
    end
