...
RegisterNetEvent("FIXDEV-housing:sell")
AddEventHandler("FIXDEV-housing:sell", function(pCid, pPropertyId)
    local src = source
    ...
    print('here')
    TriggerClientEvent("FIXDEV-housing:buy", pCid, pPropertyId, seller)
end)
...
AddEventHandler("CheckFurniture", function(pData, pPropertyId)
    ...
    if housingEditing[pPropertyId] ~= nil then
        ...
        TriggerClientEvent("FIXDEV-editor:loadEditor", src, pData)
    end
end)
...
RPC.register("FIXDEV-housing:buy", function(src, pPropertyId, pTotal, pTax, pSeller)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    ...
    TriggerClientEvent("FIXDEV-housing:loadHousingClient",src)
    TriggerClientEvent('FIXDEV-housing:logShit', src, pPropertyId, pTotal)
    return true, "Purchase Successful" 
end)

RPC.register("FIXDEV-housing:rent", function(src, pPropertyId, pTotal, pTax)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    ...
    --local success, message = exports["FIXDEV-financials"]:transaction(accountId, groupBank, pTotal, comment, cid, 5)
    --if not success then
    --    return false, message
    --end

    --exports["FIXDEV-financials"]:addTax("Propertys", pTax)
...
RPC.register("updateCurrentSelected", function(src, pPropertyId, pInformation, pOrigin)
    ...
    --exports["FIXDEV-vehicles"]:setGarage(name, data, nil, false)
...
RPC.register("FIXDEV-housing:isHousingLockedServer", function(pSrc, pHid)
    print(pHid)
...
RPC.register("FIXDEV-housing:owned", function(src)
...
RegisterNetEvent("FIXDEV-housing:getGarages", function()
    local src = source
    ...
    for i, v in ipairs(houses) do
        ...
        TriggerClientEvent("menu:send:FIXDEV-housing:garages", src,json.decode(garagedata),v.hid)
    end
end)