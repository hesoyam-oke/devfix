...
AddEventHandler('baseevents:leftVehicle', function ()
    exports['FIXDEV-vehicles']:nitroLevelLeave()
    pursuitmode = 0
end)

AddEventHandler('baseevents:enteredVehicle', function (currentVehicle, currentSeat)
    exports['FIXDEV-vehicles']:nitroLevelEnter()
    
    pursuitmode = exports['FIXDEV-hud']:pursuitmode() 
  end)
...
RegisterNetEvent("FIXDEV-hud:EnableHud", function(status)
    isLoggedIn = status
    if status == true then
        exports['FIXDEV-interface']:showInteraction('[F] To pay respects')
        HudActive = false
    end
end)

RegisterNetEvent("FIXDEV-hud:changeRange", function(pRange)
    voice = pRange or 2
end)

RegisterNetEvent('FIXDEV:admin:currentDevmode', function(status)
    developer = status
end)

RegisterNetEvent('FIXDEV:admin:currentDebug', function(status)
    debug = status
end)

RegisterNetEvent('FIXDEV-weapons:client:setWeaponFireRate', function (pAmount)
--    weaponFireRate = pAmount
end)

RegisterNetEvent("FIXDEV-weapons:client:showWeaponFireRate", function (pBool)
    weaponFireRate = pBool
end) 

RegisterNetEvent("FIXDEV-hud:harness_values", function(cinturon)
	harness = cinturon
end)

RegisterNetEvent("FIXDEV-hud:pursuit_values", function(pursuit)
	pursuitmode = pursuit
    print(pursuit)
end)
...
Citizen.CreateThread(function ()
	...
	while true do
        ...
        local pRadioActive = exports['FIXDEV-voice']:pRadioActive()
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            local pRadio = exports['FIXDEV-voice']:pRadioActive()
            ...
            SendNUIMessage({
                ...
                gps = gps,
                gpu = gpu,
                nos = exports['FIXDEV-vehicles']:nitroLevel(),
                dev = developer,
                debug = debug,
...
RegisterNetEvent("FIXDEV-admin:currentDevmode")
AddEventHandler("FIXDEV-admin:currentDevmode", function(devmode)
    isBlocked = devmode
end)
--- CARHUD ---
...
RegisterNetEvent('FIXDEV-hud:ChangeThirst', function(amount)
...
RegisterNetEvent('FIXDEV-hud:ChangeHunger', function(amount)
	if isLoggedIn == true then
...
RegisterNetEvent("client:newStress", function(status, amount)
    ...
    if status == true then
        ...
        Wait(1000)
        TriggerServerEvent('FIXDEV-hud:UpdateStress_SV', pStress)
...
RegisterNetEvent('FIXDEV:admin:maxstats', function()
    currentValues["thirst"] = 100
	currentValues["hunger"] = 100 
    TriggerEvent("heal", PlayerPedId())
    TriggerEvent("Hospital:HealInjuries", PlayerPedId(),true) 
    TriggerServerEvent("FIXDEV-death:reviveSV", source)
...
RegisterNUICallback("time", function(data)
    exports['FIXDEV-carhud']:SetTimeEnabled(data.time)
end)

RegisterNUICallback("SaveSettings", function(data)
    local pInfo = exports["FIXDEV-base"]:getModule("LocalPlayer")
    ...
    exports['FIXDEV-carhud']:SetCompassFPS(tonumber(data.compassFPSValue))
    exports['FIXDEV-carhud']:SetSpeedFPS(tonumber(data.speedFPSValue))
    TriggerEvent('setModel', model)
    TriggerEvent('FIXDEV-phone:grabBackground', bg)
    TriggerEvent('changeBG', bg)
    TriggerEvent("FIXDEV-phone:setSomeShit",bg,model,twatNotify,callNotify)
    local circleTaskbar = GetResourceKvpInt(cid..'-circleTaskbar')
    if circleTaskbar == 1 then
        TriggerEvent("FIXDEV-preferences:setPreferences", true)
    elseif circleTaskbar == 0 then
        TriggerEvent("FIXDEV-preferences:setPreferences", false)
    end
...
AddEventHandler('FIXDEV-preferences:setPreferences', function(bool)
    local data = {
        ["hud.misc.circle.taskbar.enabled"] = bool
    }
    exports["FIXDEV-interface"]:sendAppEvent('preferences', data)
end)

RegisterNetEvent("FIXDEV-hud:SetValues", function()
    local pInfo = exports["FIXDEV-base"]:getModule("LocalPlayer")
    local cid = exports['isPed']:isPed('cid')
    ...
    local model = exports['FIXDEV-phone']:getModelForHud()
    local background = exports['FIXDEV-phone']:getBackgroundForHud()
    local twat = exports['FIXDEV-phone']:getTwatForHud()
    local call = exports['FIXDEV-phone']:getCallForHud()
    ...
    exports['FIXDEV-carhud']:SetCompassFPS(SetCompassFPSValue)
    exports['FIXDEV-carhud']:SetSpeedFPS(SetSpeedFPSValue)
    ...
    if SetTimeToggled == 1 then
        exports['FIXDEV-carhud']:SetTimeEnabled(true)
    elseif SetTimeToggled == 0 then
        exports['FIXDEV-carhud']:SetTimeEnabled(false)
    end
...
RegisterCommand('ui-r', function()
    ...
    TriggerEvent('FIXDEV-hud:EnableHud', false)
    TriggerEvent('closeInventoryGui')
    TriggerEvent('FIXDEV-banking:ResetUI')
...
RegisterNetEvent('FIXDEV-hud:boosting_value')
AddEventHandler('FIXDEV-hud:boosting_value', function(pValue)
