...
AddEventHandler('carfill:checkmoney', function(costs)
	...
	if exports['FIXDEV-base']:getModule('GetCash')(src) >= costs then
		TriggerClientEvent("RefuelCarServerReturn", src)
		exports['FIXDEV-base']:getModule('RemoveCash')(src, costs)
	else
		moneyleft = costs - exports['FIXDEV-base']:getModule('GetCash')(src)
		TriggerClientEvent('DoLongHudText', src, "Requires $" .. costs)
	end
end)

RegisterServerEvent('FIXDEV-hud:UpdateStress_SV')
AddEventHandler('FIXDEV-hud:UpdateStress_SV',function(amount)
	local src = source
