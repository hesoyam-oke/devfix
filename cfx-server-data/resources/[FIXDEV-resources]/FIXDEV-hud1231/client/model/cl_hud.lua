...
RegisterNUICallback('FIXDEV-hud:hudSetPreferences', function(data, cb)
...
AddEventHandler('FIXDEV-preferences:setPreferences', function(pData)
    exports["FIXDEV-interface"]:sendAppEvent('preferences', pData)
end)
...
AddEventHandler("FIXDEV-hud:watch", function()
...
AddEventHandler("FIXDEV-hud:watch:reset", function()
...
RegisterNetEvent("FIXDEV-hud:changeRange")
AddEventHandler("FIXDEV-hud:changeRange", function(proximity)
    sendAppEvent("hud", {
        voiceRange = tonumber(proximity)
    })
end)
...
function getFuel()
    fuel = exports["FIXDEV-vehicles"]:CurrentFuel() or 50
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                harnessDurability = 100 --exports["FIXDEV-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end
...
-- RegisterNetEvent('FIXDEV-hud:show_weapon_out')
-- AddEventHandler('FIXDEV-hud:show_weapon_out', function()
--     sendAppEvent("hud", {
--         isArmed = true
--     })
-- end)

-- RegisterNetEvent('FIXDEV-hud:hide_weapon_out')
-- AddEventHandler('FIXDEV-hud:hide_weapon_out', function()
--     sendAppEvent("hud", {
--         isArmed = false
--     })
-- end)
            ...
            if GetPedInVehicleSeat(veh, -1) == player then
                ...
                if not DivingStatus then
                    ...
                    if updateLatestVehicle ~= veh then
                        ...
                        --end
                        Fuel = exports["FIXDEV-vehicles"]:CurrentFuel()
                    else
                        --Fuel = DecorGetInt(veh, "GetVehicleCurrentFuel")
                        Fuel = exports["FIXDEV-vehicles"]:CurrentFuel()
                    end
                ...
                if ( setLastUpdate > 300) then
                    --DecorSetInt(veh, "GetVehicleCurrentFuel", fRound(Fuel))
                    exports["FIXDEV-vehicles"]:SetVehicleFuel(veh, fRound(Fuel))
                    lasteupdate = 0
                end
...
function getVehicleInDirection(coordFrom, coordTo)
  ...
  RegisterNetEvent("FIXDEV-hud:jerry_can:refuel")
  AddEventHandler("FIXDEV-hud:jerry_can:refuel",function()
      vehicleCurrentFuel = exports["FIXDEV-vehicles"]:CurrentFuel(veh) --DecorGetInt(veh, "GetVehicleCurrentFuel")
      if exports['FIXDEV-inventory']:hasEnoughOfItem('883325847', 1) then
        local timer = (100 - vehicleCurrentFuel) * 400
        refillVehicle()
        local finished = exports["FIXDEV-taskbar"]:taskBar(timer,"Refueling")
        local veh = getVehicleClosestToMe()
    
        if finished == 100 then
            if exports['FIXDEV-inventory']:hasEnoughOfItem('883325847', 1) then
                --DecorSetInt(veh, "GetVehicleCurrentFuel", 100)
                exports["FIXDEV-vehicles"]:SetVehicleFuel(veh, 100)
                TriggerEvent('inventory:removeItem', '883325847', 1)
            else
                TriggerEvent('DoLongHudText', 'Try again!', 2)
            end
    
            local vehicleCurrentFuel = exports["FIXDEV-vehicles"]:CurrentFuel(veh) --DecorGetInt(veh, "GetVehicleCurrentFuel")
            ...
            exports["FIXDEV-vehicles"]:SetVehicleFuel(veh, endFuel)
