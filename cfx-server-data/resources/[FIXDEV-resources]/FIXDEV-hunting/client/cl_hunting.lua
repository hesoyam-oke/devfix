...
Citizen.CreateThread(function()
    local result = RPC.execute("FIXDEV-hunting:getSettings")
...
AddEventHandler("FIXDEV:target:changed", function(pEntity)
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(item)
    if item == "huntingbait" then
        ...
        local finished = exports["FIXDEV-taskbar"]:taskBar(15000, "Placing Bait", false, true, false, false, nil, 5.0, PlayerPedId())
    ...
    if item == "huntingknife" then
        ...
        TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
        local finished = exports["FIXDEV-taskbar"]:taskBar(30000, "Preparing Animal", false, true, false, false, nil, 5.0, targetedEntity)
        if finished ~= 100 then
            ClearPedTasksImmediately(PlayerPedId())
        ...
        TriggerServerEvent('FIXDEV-hunting:skinReward',  NetworkGetNetworkIdFromEntity(myAnimal), mySpawn, illegalSpawn, animalName)
        DeleteEntity(myAnimal)
     --   RPC.execute("FIXDEV-hunting:getSkinnedItem", NetworkGetNetworkIdFromEntity(myAnimal), mySpawn, illegalSpawn, animalName)
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(item)
    if item == "huntingknife" then
        ...
        ClearPedTasksImmediately(PlayerPedId())
        TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
        local finished = exports["FIXDEV-taskbar"]:taskBar(30000, "Preparing", false, true, false, false, nil, 5.0, targetedEntity)
        if finished ~= 100 then
            ClearPedTasksImmediately(PlayerPedId())
            TriggerEvent("DoLongHudText", "Preparing Cancelled", 2)
            return
