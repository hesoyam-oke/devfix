...
function RefreshPeekList()
    ...
    Citizen.SetTimeout(250, function()
        ...
        exports["FIXDEV-ui"]:sendAppEvent('eye', {
            action = "refresh",
            payload = entries or {}
        })
    end)
...
function StartPeekin()
    ...
    NearZones = {}
    nearbyZones = exports["FIXDEV-polytarget"]:GetZones(GetEntityCoords(PlayerPedId())) or {}
    ...
    Citizen.CreateThread(function()
        ...
        while IsPeeking do
            ...
            if IsPeakActive and (IsControlJustReleased(0, 24) or IsDisabledControlJustReleased(0, 24)) then
                ...
                exports["FIXDEV-ui"]:openApplication("eye", {
                    action = "interact",
    ...
    exports["FIXDEV-ui"]:sendAppEvent('eye', { action = "peek", payload = { display = true, active = false } })
end
...
AddEventHandler("FIXDEV:target:changed", function(pTarget, pEntityType, pEntityOffset)
...
AddEventHandler('FIXDEV-polyzone:enter', function(zoneName, zoneData, zoneCenter)
...
RegisterUICallback("FIXDEV-ui:targetSelectOption", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })

    IsPeeking = false

    exports["FIXDEV-ui"]:closeApplication("eye")
...
AddEventHandler('FIXDEV-ui:phoneReady', function()
    RefreshPeekList()
end)
...
Citizen.CreateThread(function()
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Player", "Peek at Target", "+targetInteract", "-targetInteract", "LMENU")
end)
