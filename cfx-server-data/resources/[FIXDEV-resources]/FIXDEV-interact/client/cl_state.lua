...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags['isJobEmployer'] then
        pContext.job = exports['FIXDEV-jobs']:GetNPCJobData(npcId)
    end
end
...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports['FIXDEV-flags']:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        context.meta = exports['FIXDEV-vehicles']:GetVehicleMetadata(pEntity)
        context.flags = exports['FIXDEV-flags']:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.meta = exports['FIXDEV-objects']:GetObjectByEntity(pEntity)
        context.flags = exports['FIXDEV-flags']:GetObjectFlags(pEntity)
    end
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports['FIXDEV-vehicles']:HasVehicleKey(pEntity)
...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('FIXDEV-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("FIXDEV-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vehicleOwner = NPX.Procedures.execute("FIXDEV-ottosauto:getVehicleOwner", vin)
...
RegisterNetEvent('FIXDEV-jobs:jobChanged')
AddEventHandler('FIXDEV-jobs:jobChanged', function(pJobId)
    CurrentJob = pJobId
...
RegisterNetEvent('FIXDEV-police:drag:escort')
AddEventHandler('FIXDEV-police:drag:escort', function()
    isEscorting = true
end)

RegisterNetEvent('FIXDEV-police:drag:releaseEscort')
AddEventHandler('FIXDEV-police:drag:releaseEscort', function()
    isEscorting = false
end)

AddEventHandler("FIXDEV-polyzone:enter", function(zone, data)
    if zone == "FIXDEV-jobs:impound:dropOff" then IsImpoundDropOff = true end
...
AddEventHandler("FIXDEV-fishing:currentlyRentedBoat", function(handle)
    currentlyRentedBoat = handle
end)

AddEventHandler("FIXDEV-interact:openFridge", function()
  TriggerEvent("server-inventory-open", "31", "Craft");
end)

AddEventHandler("FIXDEV-interact:grabDrink", function(args, entity, context)
  TriggerEvent("server-inventory-open", "1", context.zones["bar:grabDrink"].name)
end)

RegisterNetEvent('FIXDEV-interact:setNetVehicleBypassed', function(netId, bypass)
