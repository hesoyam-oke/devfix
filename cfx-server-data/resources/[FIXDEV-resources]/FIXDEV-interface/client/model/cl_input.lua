...
RegisterNUICallback('FIXDEV-interface:applicationClosed', function(data, cb)
    if (data.name ~= 'textbox' or data.callbackUrl ~= 'FIXDEV-interface:inputResponse') then return end
...
RegisterNUICallback('FIXDEV-interface:inputResponse', function(data, cb)
...
function OpenInputMenu(pEntries, pValidation)
    ...
    exports['FIXDEV-interface']:openApplication('textbox', {
        callbackUrl = 'FIXDEV-interface:inputResponse',
        key = inputId,
        items = pEntries,
        show = true,
    })
