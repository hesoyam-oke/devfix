...
local function processStressBlock()
    TriggerEvent("FIXDEV-buffs:applyBuff", "joints", { { buff = "stressblock", percent = 1.0 } })
end
...
AddEventHandler("FIXDEV-katana:removeEquippedKatana", function()
...
RegisterCommand('+useKeyFob', function()
    if Throttled("FIXDEV-doors:doorKeyFob", 1000) then return end
    TriggerEvent("FIXDEV-doors:doorKeyFob")
end, false)
RegisterCommand('-useKeyFob', function() end, false)

Citizen.CreateThread(function()
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Vehicle", "Door Keyfob", "+useKeyFob", "-useKeyFob", "Y")
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    local serverCode = exports['FIXDEV-config']:GetServerCode()
    ...
    if itemid == "weedbong" then
        ...
        if currentHits == 0 then
            ...
            local finished = exports["FIXDEV-taskbar"]:taskBar(12000, "Hitting Bong", false, false, playerVeh)
    ...
    if (itemid == "gunpaintbucket") then
        if not (hasEnoughOfItem("refinedsteels", 3, false) and hasEnoughOfItem("refinedplastics", 3, false)) then
            ...
            local finished = exports["FIXDEV-taskbar"]:taskBar(5000, "Filling Paint Gun", true, false)
    ...
    if (itemid == "pepperonipizza") then  
        ...
        TriggerEvent("FIXDEV-buffs:applyBuff", "maldini3", { { buff = "fullness", percent = 0.01, timeOverride = 60000 * 60 } })
    end

    if (itemid == "margheritapizza") then  
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Eating","inv:wellfed",true,itemid,playerVeh)
        TriggerEvent("FIXDEV-buffs:applyBuff", "maldini1", { { buff = "jobluck", percent = 0.01, timeOverride = 60000 * 60 } })
        TriggerEvent("FIXDEV-buffs:applyBuff", "maldini4", { { buff = "fullness", percent = 0.01, timeOverride = 60000 * 60 } })
    end

    if (itemid == "bbqpizza") then  
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Eating","inv:wellfed",true,itemid,playerVeh)
        TriggerEvent("FIXDEV-buffs:applyBuff", "maldini2", { { buff = "swimming", percent = 0.01, timeOverride = 60000 * 60 } })
        TriggerEvent("FIXDEV-buffs:applyBuff", "maldini5", { { buff = "fullness", percent = 0.01, timeOverride = 60000 * 60 } })
    end
...
-- uwu start
    if (itemid == "shortcake") then  
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Eating","inv:wellfed",true,itemid,playerVeh)
        TriggerEvent("FIXDEV-buffs:applyBuff", "UwUFull", { { buff = "fullness", percent = 0.01, timeOverride = 60000 * 60 } })
    end

    if (itemid == "chocolate_cake") then  
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Eating","inv:wellfed",true,itemid,playerVeh)
        TriggerEvent("FIXDEV-buffs:applyBuff", "UwUFull2", { { buff = "fullness", percent = 0.01, timeOverride = 60000 * 60 } })
        TriggerEvent("FIXDEV-buffs:applyBuff", "UwULuck", { { buff = "jobluck", percent = 0.01, timeOverride = 60000 * 60 } })
    end
    ...
    if (itemid == "nitrous") then
        ...
        local isElectric = exports['FIXDEV-vehicles']:IsVehicleElectric(currentVehicle)
        print(isElectric)
    ...
    if (itemid == "pixellaptop")  then
        TriggerEvent("FIXDEV-boosting:open:boostingApp")
    end 
    ...
    if (itemid == "weightlosspill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["FIXDEV-taskbar"]:taskBar(3000,"Losing weight! 💊",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerEvent("animation:PlayAnimation","cancel")
            Citizen.Wait(100)
    ...
    if (itemid == "adderall_pill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["FIXDEV-taskbar"]:taskBar(3000,"Gotta focus!",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            local drugEffect = true
    ...
    if (itemid == "test_pill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["FIXDEV-taskbar"]:taskBar(3000,"Taking testosterone.",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
        end
    end
        ...
        if itemInfo and itemInfo.quality > 0 then
            ...
            if currentUses == 0 then
                ...
                if (finished == 100 and hasEnoughOfItem(itemid, 1, false)) then
                    ...
                    if isCocainium then
                        TriggerServerEvent("FIXDEV-drugeffects:drugsTaken", cid, "trace radiation")
                    end
                end
            end
        end
    ...
    if itemid == "bleederburger" then
        ...
        if success then
            ...
            TriggerEvent("FIXDEV-buffs:applyBuff", "burger_shotjobluck", { { buff = "swimming", percent = 1.0, timeOverride = 60000 * 60 } })
    ...
    if itemid == "moneyshot" then
        ...
        if success then
            print("success")
            TriggerEvent('inv:wellfed')
            TriggerEvent("FIXDEV-buffs:applyBuff", "burger_shot", { { buff = "jobluck", percent = 0.01, timeOverride = 60000 * 60 } })
    ...
    if (itemid == "craftedgemjade") then
        TriggerEvent("FIXDEV-buffs:applyBuff", "gemfdsalfkj", { { buff = "strength", percent = 1.0, timeOverride = 120000 * 60 } })
        remove = true
    end

    if (itemid == "craftedgemcitrine") then
        TriggerEvent("FIXDEV-buffs:applyBuff", "Ring", { { buff = "jobluck", percent = 0.01, timeOverride = 120000 * 60 } })
        remove = true
    end
    ...
    if (itemid == "craftedgemaquamarine") then
        ...
        TriggerEvent("FIXDEV-buffs:applyBuff", "craftedgemaquamarine", { { buff = "swimming", percent = 1.0 } })
    ...
    if itemid == "cbbucket" then
        ...
        if success then
            TriggerEvent("FIXDEV-buffs:applyBuff", "cbbucket", { { buff = "int", percent = 0.5, timeOverride = 3000000000000 } })
            TriggerEvent('DoLongHudText', 'You feel revitalized', 1)
            remove = true
        end
    end

    if itemid == "eggsbacon" then
        local success = AttachPropAndPlayAnimation(
            "mp_player_inteat@burger",
            "mp_player_int_eat_burger",
            49,
        ...
        if success then
            TriggerEvent("FIXDEV-buffs:applyBuff", "eggsbacon", { { buff = "stamina", percent = 1.0 } })
            TriggerEvent('DoLongHudText', 'You feel revitalized', 1)
            remove = true
        end
    end

    if itemid == "enchiladas" then
        local success = AttachPropAndPlayAnimation(
            "mp_player_inteat@burger",
            "mp_player_int_eat_burger",
            49,
        ...
        if success then
            TriggerEvent("FIXDEV-buffs:applyBuff", "enchiladas", { { buff = "fullness", percent = 1.0 } })
            TriggerEvent('DoLongHudText', 'You feel revitalized', 1)
            remove = true
        end
    end
    ...
    if (itemid == "cbrownie" or itemid == "cgummies") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["FIXDEV-taskbar"]:taskBar(3000,"Consuming Edible",false,false,playerVeh)
        if (finished == 100) then
            if exports['FIXDEV-inventory']:hasEnoughOfItem('cbrownie', 1) or exports['FIXDEV-inventory']:hasEnoughOfItem('cgummies', 1) then
                ...
                TriggerEvent("FIXDEV-buffs:applyBuff", "cosmic4202", { { buff = "alert", percent = 1.0, timeOverride = 60000 * 60 } })
                remove = true
            end
        end
    end
    ...
    if (itemid == "420bar") then
        ...
        local finished = exports["FIXDEV-taskbar"]:taskBar(15000,"Consuming Chocolate Bar",false,false,playerVeh)
        if (finished == 100) then
            if exports['FIXDEV-inventory']:hasEnoughOfItem('420bar', 1) then
                TriggerEvent("client:newStress",false, 5000)
                SetPlayerMaxArmour(PlayerId(), 100) --60
                SetPedArmour( player, 60) --60
                ...
                TriggerScreenblurFadeOut(100)
                TriggerEvent("DoLongHudText", "Stress Relieved")
                TriggerEvent("FIXDEV-buffs:applyBuff", "cosmic420", { { buff = "jobluck", percent = 1.0, timeOverride = 60000 * 60 } })
                TriggerEvent("FIXDEV-buffs:applyBuff", "cosmic4201", { { buff = "alert", percent = 1.0, timeOverride = 60000 * 60 } })
                remove = true
            end
        end
    end
    ...
    if (itemid == "yellowjoint") then
        local finished = exports["FIXDEV-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)  -- normal joints
        if (finished == 100) then
            Wait(200)
            TriggerEvent("animation:PlayAnimation","weed")
            ...
            TriggerEvent('FIXDEV-hud:show_alert')
            if math.random(100) == 69 then
                TriggerEvent("player:receiveItem","femaleseed",1)
            end
            if math.random(600) == 69 then
            ...
            TriggerEvent("DoLongHudText", "Stress Relieved")
            TriggerEvent("FIXDEV-buffs:applyBuff", "cookiespecial", { { buff = "jobluck", percent = 0.6, timeOverride = 60000 * 60 } })
            remove = true
            TriggerScreenblurFadeOut(100.0)
        end
    end
    ...
    if itemid == "heartstopper" or itemid == "torpedo" then
        ...
        if success then
            print("success")
            TriggerEvent('inv:wellfed')
            TriggerEvent("FIXDEV-buffs:applyBuff", "burger_shot", { { buff = "strength", percent = 0.6, timeOverride = 60000 * 60 } })
            TriggerEvent("FIXDEV-buffs:applyBuff", "burger_shotfullness", { { buff = "fullness", percent = 1.0, timeOverride = 60000 * 60 } })
            TriggerEvent('DoLongHudText', 'You feel revitalized', 1)
            remove = true
        end
    end
...
AddEventHandler("FIXDEV-inventory:attachPropPlayAnim", function(pType)
    ...
    TriggerEvent("FIXDEV-buffs:itemUsedSuccess", success)
end)

RegisterNetEvent('randPickupAnim')
AddEventHandler('randPickupAnim', function()
    local ped = PlayerPedId()
    local _, isAnimalModel = pcall(function () return exports["FIXDEV-misc-code"]:isAnimalModel(GetEntityModel(ped)) end)
...
RegisterNetEvent('FIXDEV-business:repairItem')
AddEventHandler('FIXDEV-business:repairItem', function()

    local info = GetItemInfo(1)
    local Quality = info.quality
    local QualityCheck = (Quality < 80 and Quality or false)
    local PercentRepair = 80 - Quality
    local itemListInfo = json.decode(exports["FIXDEV-inventory"]:itemListInfo(info.item_id))
    local allowed = true
...
RegisterNetEvent('FIXDEV-business:examineItem')
AddEventHandler('FIXDEV-business:examineItem', function()
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh,itemreturn,itemreturnid,quality)
    ...
    if timer > 0 then
        local finished = exports["FIXDEV-taskbar"]:taskBar(timer,message,true,false,playerVeh)
...
function checkForItems()
    ...
    for _, item in ipairs(items) do
        ...
        if hasItem and not PreviousItemCheck[item] then
            ...
            TriggerEvent('FIXDEV-inventory:itemCheck', item, true, quantity)
        elseif not hasItem and PreviousItemCheck[item] then
            PreviousItemCheck[item] = false
            TriggerEvent('FIXDEV-inventory:itemCheck', item, false, quantity)
        end
    end
end
...
AddEventHandler('inv:lockPick', function(isForced, inventoryName, slot, itemType)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 and (IsPedAPlayer(driverPed) or not IsPedDeadOrDying(driverPed)) then
            ...
            exports['FIXDEV-vehicles']:LockpickVehicleDoor(targetVehicle, itemType, isForced, function(result)
                p:resolve(result)
            end)
            ...
            Citizen.SetTimeout(math.random(120, 7200) * 1000, function ()
                TriggerServerEvent('FIXDEV:vehicles:hotPlate', plate, '10-60')
            end)

            if result.success and not isForced then
                exports['FIXDEV-flags']:SetVehicleFlag(targetVehicle, 'isStolenVehicle', true)
            end

        lockpicking = false
    else
        if targetVehicle ~= 0 then
            if exports["FIXDEV-vehicles"]:HasVehicleKey(targetVehicle) then lockpicking = false return end
            TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, 'lockpick', 0.4)
            local triggerAlarm = GetVehicleDoorLockStatus(targetVehicle) > 1
            if triggerAlarm then
                SetVehicleAlarm(targetVehicle, true)
            ...
            exports['FIXDEV-vehicles']:HotwireVehicle(targetVehicle, itemType, isForced, function(result)
                p:resolve(result)
            end)
...
function getInformationFromFirstItem(itemToFind)
  local foundItem = exports["FIXDEV-inventory"]:GetInfoForFirstItemOfName(itemToFind)
...
RegisterUICallback("FIXDEV-ui:inv:moneycaseAction", function(data, cb)
	cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['FIXDEV-ui']:closeApplication('textbox')
  RPC.execute("FIXDEV-inventory:doMoneycaseAction", data)
end)

AddEventHandler("FIXDEV-inventory:openGolfStore", function(name, info)
...
function openBurner(info)
    ...
    exports["FIXDEV-ui"]:openApplication("burner", info)
end
...
AddEventHandler("ammonium_bicarbonate:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(5000 * quantity, "Breaking down into baggies...", false, false)
...
AddEventHandler("crackpipe:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(20000, "Packing crack pipe", false, false)
    if finished ~= 100 then return end
...
AddEventHandler("cokeline:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(15000, "Filling with " .. cConfig.fillString, false, false)
    if finished ~= 100 then return end
...
AddEventHandler("methpipe:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(10000, "Packing meth pipe", false, false)
    if finished ~= 100 then return end
...
AddEventHandler("weedbong:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(20000, "Packing Bong", false, false)
    if finished ~= 100 then return end
...
function notepadUsed()
    exports["FIXDEV-ui"]:openApplication("notepad", {
        canSave = true
    })
    TriggerEvent("animation:PlayAnimation", "notepad")
end

RegisterUICallback('FIXDEV-ui:cancelNotepadEmote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    TriggerEvent("animation:PlayAnimation", "cancel")
end)

RegisterUICallback('FIXDEV-ui:createNotepadNote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })

    exports["FIXDEV-ui"]:closeApplication("notepad")
...
function applyPaint(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo, minRng, maxRng)
    ...
    local prompt = exports['FIXDEV-ui']:OpenInputMenu(elements, function(values)
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(15000, "Painting gun", false, false)
    if finished ~= 100 then return end
