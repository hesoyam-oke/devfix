const baseScriptSrc = 'FIXDEV-inventory:scripts:';
...
setTimeout(async () => {
  while (translationsObject === null) {
    try {
      ...
      const result = await (await fetch('https://FIXDEV-ui/FIXDEV-ui:i18n:getTranslations')).json();
      if (result.meta.ok) {
  ...
  setInterval(async () => {
    try {
      const result = await (await fetch('https://FIXDEV-ui/FIXDEV-ui:i18n:getCurrentLangCode')).json();
      if (result.meta.ok) {
        langCode = result.data.langCode;
      }
    } catch (err) {
  ...
  setInterval(async () => {
    try {
      ...
      await fetch('https://FIXDEV-ui/FIXDEV-ui:i18n:addStringsForTranslation', {
        method: 'POST',
