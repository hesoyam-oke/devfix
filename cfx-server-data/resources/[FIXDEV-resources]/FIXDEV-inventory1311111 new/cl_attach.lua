...
AddEventHandler("AttachWeapons", function()
	...
	DeleteAttached()
	for i = 1, #w do
		if exports["FIXDEV-inventory"]:getQuantity(w[i]["id"]) > 0 then
			...
			elseif w[i]["type"] == 4 and not sheathed then
				TriggerEvent("FIXDEV-inventory:attachmentsToggle", true, w[i]["id"])
				sheathed = true
				local bone = GetPedBoneIndex(ped, 24817)
				am[#am+1] = CreateObject(mdl, 1.0 ,1.0 ,1.0, 1, 1, 0)
...
function DeleteAttached()
	TriggerEvent("FIXDEV-inventory:attachmentsToggle", false)
	for i = 1, #ag do
		DeleteEntity(ag[i])
	end
	for i = 1, #ad do
