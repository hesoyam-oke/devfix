...
on('FIXDEV-base:playerSpawned', async (broughtData) => {
...
//  $.post("http://FIXDEV-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('inventory:open_hidden', (penis, vehicleFound) => {
    let vehId = exports['FIXDEV-vehicles'].GetVehicleIdentifier(vehicleFound)
...
on('inventory-open-request', async () => {
  ...
  //  const apartmentFloor = exports["FIXDEV-apartments"].getModule("func").getApartment()
    let targetid = 0;
    cid = exports.isPed.isPed("cid");
    ...
    const currentTarget = exports['FIXDEV-target'].GetCurrentEntity()
    ...
    if (isInVehicle) {
       ...
       // let serverCode = exports["FIXDEV-config"].GetServerCode();
                ...
                if (licensePlate != null) {
                    if (vehModel === GetHashKey('npwheelchair')) {
                        ...
                        if (!IsThisModelABicycle(vehModel) && vehModel !== GetHashKey('trash2')) {
                            const vehId = exports['FIXDEV-vehicles'].GetVehicleIdentifier(vehicleFound)
                            if (!vehId) {
                                CloseGui();
...
on('FIXDEV-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('FIXDEV-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("FIXDEV-inventory:craftProgression", data);
    emitNet("FIXDEV-inventory:craftProgression", data);
});
...
RegisterNuiCallbackType('openProtectable');
on('__cfx_nui:openProtectable', async (data, cb) => {
    await CloseGui(false);
    OpenJewelryInventory();
    cb({});
});

function OpenProtectableInventory() {
    const cid = exports.isPed.isPed("cid");
    const inventory = `container-Protectable:${cid}-Protectable-Protectable`;
    emit("inventory-open-container", inventory, 5, 30.0);
}
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("FIXDEV-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("FIXDEV-voice:focus:set", false, false, false);}, 1000)
    //   }
}
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('FIXDEV-inventory:server:weightChange', cid, weight);
