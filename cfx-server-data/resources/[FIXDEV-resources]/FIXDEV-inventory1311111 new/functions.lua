...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if (itemid == "thermitecharge") then
      ...
      if (itemid == "heistlaptop3") then
          TriggerEvent('FIXDEV-robbery:usb')
      end 
  
      if (itemid == "heistlaptop2") then
          TriggerEvent('FIXDEV-paleto:UseBlueLapTop')
      end
    ...
    if Police >= 4 then
        if targetVehicle ~= 0 and GetHashKey("stockade") == GetEntityModel(targetVehicle) then
            ...
            if aDist < 2.0 then
                ...
                TriggerServerEvent('FIXDEV-heists:bankTruckLog')
                TriggerEvent('FIXDEV-dispatch:bank_truck_robbery')
                local finished = exports["FIXDEV-taskbar"]:taskBar(timeToOpen,"Unlocking Vehicle",false,false,playerVeh)
                if finished == 100 then
                    TriggerEvent('FIXDEV-ui:show_hackerman')
                    exports['FIXDEV-thermite']:OpenThermiteGame(function(success)
                        if success then
                            TriggerEvent('FIXDEV-ui:hide_hackerman')
                            TriggerEvent('inventory:removeItem', 'Gruppe6Card', 1)
                            TriggerEvent("FIXDEV-heists:start_hitting_truck", targetVehicle)
                        else
                            TriggerEvent('FIXDEV-ui:hide_hackerman')
                            TriggerEvent('inventory:removeItem', 'Gruppe6Card', 1)
                        end
    ...
    if itemid == "rice_balls" then
        ...
        local finished = exports['FIXDEV-taskbar']:taskBar(2500, 'Eating...')
        if finished == 100 then
            TriggerEvent('FIXDEV-hud:ChangeHunger', 50)
    ...
    if itemid == "chicken_noodle_soup" then
        TriggerEvent('animation:PlayAnimation', 'eat')
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Slurping Soup...')
        if finished == 100 then
            TriggerEvent('FIXDEV-hud:ChangeHunger', 80)
    ...
    if itemid == "doki_doki_pancakes" then
        TriggerEvent('animation:PlayAnimation', 'eat')
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Eating Pancakes...')
        if finished == 100 then
            TriggerEvent('FIXDEV-hud:ChangeHunger', 50)
    ...
    if itemid == "chocolate_cake" then
        TriggerEvent('animation:PlayAnimation', 'eat')
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Eating Cake...')
        if finished == 100 then
            TriggerEvent('FIXDEV-hud:ChangeHunger', 25)
    ...
    if itemid == "strawberry_shortcake" then
        ...
        local finished = exports['FIXDEV-taskbar']:taskBar(5000, 'Eating Cake...')
        if finished == 100 then
            TriggerEvent('FIXDEV-hud:ChangeHunger', 45)
    ...
    if (itemid == "bubbletea") then
        ...
        TriggerEvent('FIXDEV-hud:ChangeThirst', 30)
    ...
    if (itemid == "softdrink") then
        ...
        TriggerEvent('FIXDEV-hud:ChangeThirst', 69)
        TriggerEvent('thirstbuff')
    end
    ...
    if itemid == "fries" then
        ...
        TriggerEvent('FIXDEV-hud:ChangeHunger', 30)
    ...
    if itemid == "torpedo" then
        ...
        --attachPropsToAnimation(itemid, 6000)
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
        TriggerEvent('FIXDEV-hud:ChangeHunger', 15)
        TriggerEvent('aspect-buffs:bulb_buff')
    end
    ...
    if itemid == "moneyshot" then
        ...
        --attachPropsToAnimation(itemid, 6000)
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
        TriggerEvent('FIXDEV-hud:ChangeHunger', 69)
        TriggerEvent('aspect-buffs:money_boost_buff')
    end
    ...
    if (itemid == "pepperonipizza") then
        ...
        --attachPropsToAnimation(itemid, 6000)
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
        TriggerEvent('FIXDEV-hud:ChangeHunger', 60)
        TriggerEvent('aspect-buffs:money_boost_buff')
    end
    ...
    if (itemid == "shitlockpick") then
        ...
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("police:cuffs:uncuff")
        end
        lockpicking = false
        remove = true
    end
    ...
    if (itemid == "lockpick") then
        lockpicking = true
        TriggerEvent("animation:lockpickinvtestoutside")
        local finished = exports["FIXDEV-ui"]:taskBarSkill(3000, math.random(7, 12))
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("police:cuffs:uncuff")
        end
        lockpicking = false
    ...
    if itemid == "racingusb1" then
        if (passedItemInfo == "{}") then
            local valid = exports["FIXDEV-keyboard"]:KeyboardInput({
                header = "Racing USB",
                rows = {
                    {
                        id = 0,
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["FIXDEV-ui"]:taskBarSkill(15000,math.random(10,20))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["FIXDEV-ui"]:taskBarSkill(timer,math.random(4,7))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
...
AddEventHandler("cokeline:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(15000, "Filling with coke", false, false)
    if finished ~= 100 then return end
