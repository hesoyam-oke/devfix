...
RegisterNetEvent('FIXDEV-jobs:dodoLogisticsGetJob')
AddEventHandler('FIXDEV-jobs:dodoLogisticsGetJob', function(loc)
    ...
    TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Drive to the delivery location.')
        ...
        while isToDropLocation do
            ...
            if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 2.5 then
                if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                    exports['FIXDEV-ui']:showInteraction('[E] Drop Off Package')
                    if IsControlJustPressed(1,38) then
                        RPC.execute('FIXDEV-jobs:returnBackDodoDock')
                    end
                end
            else
...
RegisterNetEvent('FIXDEV-jobs:dodoLogisticsPayout')
AddEventHandler('FIXDEV-jobs:dodoLogisticsPayout', function()
    if pOnWayDodo == true then
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder"))  then
            if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                TriggerServerEvent('FIXDEV-jobs:dodoLogisticsPayout')
                exports['FIXDEV-ui']:hideInteraction()
...
RegisterNetEvent('FIXDEV-jobs:returningDodo')
AddEventHandler('FIXDEV-jobs:returningDodo', function()
    TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Return back to the Dodo dock.')
    exports['FIXDEV-ui']:hideInteraction()
    isToDropLocation = false
    pOnWayDodo = true
        ...
        local aDist = GetDistanceBetweenCoords(setCoords, coordA)
            if aDist < 15.0 then
                ...
                if exports['FIXDEV-hud']:MoneyBuff() then
                    RPC.execute('FIXDEV-jobs:givePayout', math.random(878, 927), "bank", '[Dodo Logistics | With Buff]')
                else
                    RPC.execute('FIXDEV-jobs:givePayout', math.random(650, 778), "bank", '[Dodo Logistics | Without Buff]')
                end
                Citizen.Wait(2500)
...
RegisterNetEvent('FIXDEV-jobs:sendDodoNoti')
AddEventHandler('FIXDEV-jobs:sendDodoNoti', function()
    TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Get into the delivery truck')
end)

RegisterNetEvent("FIXDEV-jobs:spawn-truck")
AddEventHandler("FIXDEV-jobs:spawn-truck", function()
    ...
    RPC.execute('FIXDEV-jobs:dodoStarted')
    ...
    Citizen.CreateThread(function()
        while waitingGetInVeh do
            ...
            if GetVehiclePedIsIn(PlayerPedId()) == vehicle then
                ...
                RPC.execute('FIXDEV-jobs:getADeliveryLocationDodo', math.random(1, 7))
            end
        end
    end)
end)
...
exports["FIXDEV-polyzone"]:AddBoxZone("sign_in_dodo", vector3(-438.02, -2795.2, 7.29), 1.6, 2.0, {
    name="sign_in_dodo",
    heading=45,
    --debugPoly=false,
    minZ=4.69,
            ...
            Citizen.CreateThread(function()
                while dodoSignInLoc do
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        exports['FIXDEV-phone']:phoneNotification('jobactivity', 'Signed in as Dodo Logistics.', 'Job Center')
                        exports['FIXDEV-ui']:hideInteraction()
                        dodoSignInLoc = false
                    end
                end
