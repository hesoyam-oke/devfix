...
RegisterNetEvent("FIXDEV-fishing:updateLocation")
AddEventHandler("FIXDEV-fishing:updateLocation", function(location)
    fishingLocation = location
end)
...
local function loopSkill(count)
    ...
    while loopCount < count do
        ...
        local finished = exports["FIXDEV-ui"]:taskBarSkill(math.random(2000, 2500), math.random(7, 12))
...
local function reelInFish()
    ...
    for _, fish in pairs(fishes) do
        if not found and fish.chance > fishChance then
            ...
            if success then
                TriggerEvent('FIXDEV-fishing:caught_fish')
                RPC.execute('FIXDEV-jobs:fishingCaughtAFish')
            else
...
RegisterNetEvent('FIXDEV-fishing:caught_fish')
AddEventHandler('FIXDEV-fishing:caught_fish', function()
...
local function sellFish()
    ...
    function processFish(fish, bMarket)
        local qty = exports["FIXDEV-inventory"]:getQuantity(fish, true)
    ...
    if totalFish > 0 then
        if exports['FIXDEV-hud']:MoneyBuff() then
            RPC.execute('FIXDEV-jobs:givePayout', totalFish * pricePerFish * 3, "cash", '[Fishing Job | With Buff]')
        else
            RPC.execute('FIXDEV-jobs:givePayout', totalFish * pricePerFish, "cash", '[Fishing Job | Without Buff]')
        end
    ...
    if totalBMarketFish > 0 then
        if nightTime then
            local payoutFactor = exports["FIXDEV-config"]:GetModuleConfig("main").payoutFactor
...
AddEventHandler("FIXDEV-npcs:ped:sellFish", function()
    sellFish()
end)
...
exports["FIXDEV-polyzone"]:AddBoxZone("fishing",  vector3(-335.89, 6106.1, 31.45), 2, 2, {
    name = "fishing",
    heading=50,
    --debugPoly=false,
    minZ=28.45,
        ...
        if exports['isPed']:isPed('myjob') == 'fisherman' then
            ...
            Citizen.CreateThread(function()
                while sign_in_fishing do
                    ...
                    if IsControlJustReleased(0, 38) then
                        fishCount = 0
                        exports['FIXDEV-phone']:phoneNotification("jobactivity", "Signed into fishing.", "Job Center")
...
RegisterNetEvent('FIXDEV-jobs:setFirstNotiFishing')
AddEventHandler('FIXDEV-jobs:setFirstNotiFishing', function()
    TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Use [ F1 > Blips ] to find the fishing location.')
end)

RegisterNetEvent('FIXDEV-jobs:fishingCatch')
AddEventHandler('FIXDEV-jobs:fishingCatch', function()
    fishCount = fishCount + 1

    if fishCount >= 30 then
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Return to the hut and let them know how the spot was.')
        returningHut = true
        ...
        Citizen.CreateThread(function()
            while returningHut do
                ...
                if aDist < 50.0 then
                    returningHut = false
                    TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Job Complete.')
                    fishCount = 0
                    if exports['FIXDEV-hud']:MoneyBuff() then
        ...
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', '('..fishCount..'/ 30) Fish caught.')
    end
end)