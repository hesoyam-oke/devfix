...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("return_garbagetruck", vector3(-350.18, -1560.91, 25.22), 12.4, 7.8, {
		name="return_garbagetruck",
		heading=270,
    }) 

    exports["FIXDEV-polyzone"]:AddBoxZone("garbage_zone1", vector3(973.61, -1512.86, 53.5), 120, 116.0, {
		name="garbage_zone1",
        heading=0
    }) 

    exports["FIXDEV-polyzone"]:AddBoxZone("garbage_zone2", vector3(-1303.67, -790.96, 17.59), 39.59, 185.2, {
		name="garbage_zone2",
        heading=311
    }) 

    exports["FIXDEV-polyzone"]:AddCircleZone("garbage_zone3", vector3(1144.52, -558.76, 57.64), 264.40000000001, {
		name="garbage_zone3",
        useZ=false    
    }) 
    exports["FIXDEV-polyzone"]:AddBoxZone("garbage_zone4", vector3(314.77, -186.08, 61.58), 102.2 , 67.2,  {
		name="garbage_zone4",
        heading=70    
    })
    exports["FIXDEV-polyzone"]:AddBoxZone("garbage_zone5", vector3(-1792.7, -393.05, 45.01), 100, 100, {
		name="garbage_zone5",
        useZ=false  
    })
    exports["FIXDEV-polyzone"]:AddBoxZone("garbage_zone6", vector3(254.95, -3087.25, 5.81), 100, 100, {
		name="garbage_zone6",
        useZ=false  
    })
    exports["FIXDEV-polyzone"]:AddBoxZone("garbage_zone7", vector3(1183.09, -1315.99, 34.8), 196.40000000001, 100.0, {
		name="garbage_zone7",
        useZ=false  
    })
    exports["FIXDEV-polyzone"]:AddBoxZone("garbage_zone8", vector3(854.96, -1048.24, 28.69), 80.600000000001, 110.0, {
		name="garbage_zone8",
        useZ=false  
    }) 
end)
...
RegisterNetEvent('FIXDEV-garbage:ToggleTrunkStatus', function(jStatus)
    isTrunkBusy = jStatus
end)

RegisterNetEvent('FIXDEV-jobs:startGarbageRun', function()
    ...
    Wait(1)
    TriggerServerEvent('FIXDEV-garbage:StartJob_SV', math.random(1, 8), CurrentlyStarting)
    vehPlate = truckplate
end)

RegisterNetEvent('FIXDEV-garbage:StartJob_CL', function(ZoneSelect)
    ...
    if ZoneSelected == 1 then 
        ...
        GarbageRuns = 0
        AmountOfGarbages = 10
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Go to assigned zone (Le Mesa)')
    elseif ZoneSelected == 2 then 
        ...
        GarbageRuns = 0
        AmountOfGarbages = 18
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Go to assigned zone (Del Perro)')
    elseif ZoneSelected == 3 then 
        ...
        GarbageRuns = 0
        AmountOfGarbages = 14
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Go to assigned zone (Mirror Park)')
    elseif ZoneSelected == 4 then 
        ...
        GarbageRuns = 0
        AmountOfGarbages = 14
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Go to assigned zone (Pink Cage)')
    elseif ZoneSelected == 5 then 
        ...
        GarbageRuns = 0
        AmountOfGarbages = 14
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Go to assigned zone (Von Crastenburg)')
    elseif ZoneSelected == 6 then 
        ...
        GarbageRuns = 0
        AmountOfGarbages = 10
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Go to assigned zone (Tuner Docks)')
    elseif ZoneSelected == 7 then 
        ...
        GarbageRuns = 0
        AmountOfGarbages = 20
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Go to assigned zone (El Rancho Weld)')
    elseif ZoneSelected == 8 then  
        ...
        GarbageRuns = 0
        AmountOfGarbages = 13
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Go to assigned zone (Popular St.)')
    end
...
RegisterNetEvent('FIXDEV-garbage:PickUpTrash', function()
...
RegisterNetEvent('FIXDEV-garbage:RemovePickedUpTrash', function()
    if GarbageRuns ~= AmountOfGarbages then
    ...
    Wait(1)
    if GarbageRuns == AmountOfGarbages then
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', 'Head back to depo and return the truck.')
        returningTruck = true
        RemoveBlip(ZoneBlip)
        Wait(200)
        ...
        TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', '('..GarbageRuns ..'/' .. AmountOfGarbages..') Collect Trash.')
    end
end)

RegisterNetEvent('FIXDEV-garbage:ThrowTrashInTruck', function()
    TriggerServerEvent('FIXDEV-garbage:SetState', true)
    Wait(5)
        ...
        if GarbageRuns ~= AmountOfGarbages then
            if pickedUp then
                if not isBusy then  
                    ...
                    TriggerServerEvent('FIXDEV-garbage:RemoveTrash')
                    
                    Wait(1000)
                    pickedUp = false
                    isBusy = false
                    TriggerServerEvent('FIXDEV-garbage:ResetState')
                end
            else
...
RegisterNetEvent('FIXDEV-polyzone:enter', function(name)
    if name == "return_garbagetruck" then
        if exports['isPed']:isPed('myjob') == 'sanitation_worker' and returningTruck then
            nearDropOff = true
            AtDropOffLocation()
            exports['FIXDEV-ui']:showInteraction(("[E] %s"):format("Drop Off Trash"))
        end
    end
...
RegisterNetEvent('FIXDEV-polyzone:exit', function(name)
    if name == "return_garbagetruck" then
        nearDropOff = false
...
RegisterNetEvent('FIXDEV-garbage:payClients', function()
        ...
        while nearDropOff do
            ...
            if IsControlJustReleased(0, 38) then
                if vehPlate == plate then
                    if GarbageRuns == AmountOfGarbages then
                        jobCompleted = true
                        local finished = exports['FIXDEV-taskbar']:taskBar(5000, "Dropping off trash", false)
                        if finished == 100 then
                            ...
                            GarbageRuns = 0
                            AmountOfGarbages = 0
                            TriggerServerEvent('FIXDEV-garbage:payGroupAndMembers')
                            jobCompleted = false
                            ZoneOneEntered = false
                            ZoneTwoEntered = false
                            ZoneThreeEntered = false
...
RegisterNetEvent('FIXDEV-garbage:JobCancel', function(status)
    TriggerServerEvent('FIXDEV-garbage:RequestCancel', status)
end)

RegisterNetEvent('FIXDEV-garbage:RestSomeStatus', function()
    alreadyInZone = false
    isJobStarted = false
    RemoveBlip(ZoneBlip)
end)

RegisterNetEvent('FIXDEV-garbage:FullyCanceled', function()
    vehPlate = ""
    pickedUp = false
    GarbageRuns = 0
    AmountOfGarbages = 0
...
exports["FIXDEV-polyzone"]:AddBoxZone("sign_in_garbage", vector3(-353.88, -1545.97, 27.72), 2.0, 1.4, {
                ...
                while inGarbageLocationSignIn do
                    ...
                    if IsControlJustReleased(0, 38) then
                        if not inJob then
                            ...
                            exports['FIXDEV-phone']:phoneNotification('jobactivity', 'Signed into Sanitation.', 'Job Center')
                            inJob = true
                            exports['FIXDEV-ui']:hideInteraction()
                        end
                    end
