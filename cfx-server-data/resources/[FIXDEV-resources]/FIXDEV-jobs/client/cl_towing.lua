...
RegisterNetEvent('FIXDEV-jobs:getFlatbed')
AddEventHandler('FIXDEV-jobs:getFlatbed', function()
...
RegisterNetEvent('FIXDEV-jobs_tow_trucker_off_duty')
AddEventHandler('FIXDEV-jobs_tow_trucker_off_duty', function()
...
--###################################################
RegisterNetEvent('FIXDEV-jobs:clockIntoTowing')
AddEventHandler('FIXDEV-jobs:clockIntoTowing', function(towingLocation)
	RPC.execute('FIXDEV-jobs:towtruckingGetJob', towingLocation)
			...
			if EndlessRP.Looking then
				if #(vector3(towingLocations[towingLocation]["x"], towingLocations[towingLocation]["y"], towingLocations[towingLocation]["z"]) - GetEntityCoords(PlayerPedId())) <= 120 then
					if not SentNotiBack then
						SentNotiBack = true
						TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', "Get the vehicle on the back of the flatbed.")
					end
					local vehicle = exports['FIXDEV-vehicles']:GetVehicleTable()
					...
					RPC.execute('FIXDEV-jobs:syncVehiclePlateImpound', plate)
...
RegisterNetEvent('FIXDEV-jobs:towTruckPlateSync')
AddEventHandler('FIXDEV-jobs:towTruckPlateSync', function(vehiclePlate)
	setimpoundveh = vehiclePlate
end)

RegisterNetEvent('FIXDEV-jobs:towingGetJob')
AddEventHandler('FIXDEV-jobs:towingGetJob', function(pLoc)
	TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', "Make your way over to the vehicle.")
...
RegisterNetEvent('FIXDEV-jobs:sendClientReturnImpound')
AddEventHandler('FIXDEV-jobs:sendClientReturnImpound', function()
	SentWaypoint = true
	RemoveBlip(pGoToCar)
	...
	UpdatedNoti = false
	TriggerEvent('FIXDEV-phone:JobNotify', 'CURRENT', "Go back to the impound and drop off the vehicle.")
end)

-- Imp Veh --
RegisterNetEvent('FIXDEV-jobs:towing_imp')
AddEventHandler('FIXDEV-jobs:towing_imp', function()
	...
	if DoesEntityExist(veh) and NetworkHasControlOfEntity(veh) then
		...
		if licensePlate == setimpoundveh then
			if CanImp then
				TriggerEvent('FIXDEV-jobs_tow_menu')
			end
		else
...
RegisterNetEvent('FIXDEV-jobs_tow_menu')
AddEventHandler('FIXDEV-jobs_tow_menu', function()
	...
	local TowMenu = {
		...
		{
			title = "Accept",
			action = "FIXDEV-jobs:accept_impound_req",
		},
		{
			title = "Decline",
			action = "FIXDEV-jobs:decline_impound_req",
		},
	}
	exports["FIXDEV-interface"]:showContextMenu(TowMenu)
end)

RegisterUICallback('FIXDEV-jobs:accept_impound_req', function(data, cb)
	...
	if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
		...
		if licensePlate == setimpoundveh then
			...
			RPC.execute('FIXDEV-jobs:towTruckingAcceptImpoundOffer')
			RemoveBlip(pGoToCar)
		else
...
RegisterNetEvent('FIXDEV-jobs:acceptImpoundOffer')
AddEventHandler('FIXDEV-jobs:acceptImpoundOffer', function()
	exports['FIXDEV-phone']:phoneNotification('jobactivity', 'Job Complete.', 'Job Center')
	SentWaypoint = false
	SentNotiBack = false
	if exports['FIXDEV-hud']:MoneyBuff() then
		RPC.execute('FIXDEV-jobs:givePayout', math.random(1076, 1554), "cash", '[24/7 Delivery Job | With Buff]')
	else
		RPC.execute('FIXDEV-jobs:givePayout', math.random(652, 886), "cash", '[24/7 Delivery Job | Without Buff]')
	end
...
RegisterUICallback('FIXDEV-jobs:decline_impound_req', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	RPC.execute('FIXDEV-jobs:towTruckingDeclineImpoundOffer')
end)

RegisterNetEvent('FIXDEV-jobs:rejectImpoundRequest')
AddEventHandler('FIXDEV-jobs:rejectImpoundRequest', function()
...
exports["FIXDEV-polyzone"]:AddBoxZone("towing_clock_in_zone", vector3(-191.63, -1166.46, 23.67), 2, 2, {
            ...
            Citizen.CreateThread(function()
                while towing_duty do
                    ...
                    if IsControlJustReleased(0, 38) then
						...
						exports['FIXDEV-phone']:phoneNotification('jobactivity', 'Signed in as Tow Truck driver.', 'Job Center')
                        exports['FIXDEV-ui']:hideInteraction()
                        towing_duty = false
						local needATruck = {
							{
								title = "Yes I need a towtruck",
								action = 'FIXDEV-jobs:getTowtruck',
							},
							{
								title = "Nah I have my own or im in another group.",
								action = 'FIXDEV-jobs:declineTowtruck',
							},
						}
						exports["FIXDEV-interface"]:showContextMenu(needATruck)
                    end
                end
            end)
        end
...
RegisterUICallback('FIXDEV-jobs:getTowtruck', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('FIXDEV-jobs:getFlatbed')
end)

RegisterUICallback('FIXDEV-jobs:declineTowtruck', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
end)
...
AddEventHandler('pv:tow', function()
	...
	if isVehicleTow then
		...
		if not currentlyTowedVehicle then
		    ...
		    TriggerEvent("animation:tow")
		    TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck', 0.5)
		    local finished = exports["FIXDEV-taskbar"]:taskBar(7000,"Untowing Vehicle")
        	if finished == 100 and aDist < 2.0 then
                hasVehicleHookedUp = false
        		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
        		local part2 = exports["FIXDEV-taskbar"]:taskBar(7000,"Unhooking Vehicle")
				CleanDetachedVehicles()
