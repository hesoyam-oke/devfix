...
Citizen.CreateThread(function()
    if tsunami then
        ...
        TriggerClientEvent("FIXDEV-fishing:updateLocation", -1, activeLocation)
...
RPC.register('FIXDEV-jobs:fishingStart', function(source)
        ...
        exports.oxmysql:execute("SELECT members FROM job_group WHERE id = ?", {groupID}, function(result)
            if result[1] ~= nil then
                ...
                for k,v in ipairs(jobMembers) do
                    print('Start fishing.')
                    TriggerClientEvent('FIXDEV-jobs:setFirstNotiFishing', v)
                end
            end
        end)
        return
...
RPC.register('FIXDEV-jobs:fishingCaughtAFish', function(source)
    for k, v in pairs(GetPlayers()) do
        local user = exports['FIXDEV-base']:getModule('Player'):GetUser(tonumber(v))
        local cid = user:getCurrentCharacter().id
        local groupID = exports['FIXDEV-phone']:GetMyGroupID(v)
        ...
        exports.oxmysql:execute("SELECT members FROM job_group WHERE id = ?", {groupID}, function(result)
            ...
            if result[1] ~= nil then
                local jobMembers = json.decode(result[1].members)
                for k,v in ipairs(jobMembers) do
                    TriggerClientEvent('FIXDEV-jobs:fishingCatch', v)
                end
            end
        end)
        return
