RegisterServerEvent('FIXDEV-impound:select_plate')
AddEventHandler('FIXDEV-impound:select_plate', function(pPlate)
    local src = source
    ...
    {['@license_plate'] = pPlate}, function(pValues)
        if pValues[1].current_garage == "fullimpound" or pValues[1].current_garage == "normalimpound" then
            if char.id == pValues[1].cid then
                TriggerClientEvent('FIXDEV-impound:show_car', src, pValues[1].name, pValues[1].license_plate, false)
            else
...
RegisterServerEvent('FIXDEV-impound:release_car')
AddEventHandler('FIXDEV-impound:release_car', function(pPlate)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
