RegisterServerEvent('FIXDEV-civjobs:errorlog')
AddEventHandler('FIXDEV-civjobs:errorlog', function(pErrorCode)
    local timeout = os.time()
    if (os.time() - timeout) < 5 then
        print(("[TIMEOUT] %s is spamming the event (%s)"):format(source, "FIXDEV-civjobs:errorlog"))
        timeout = os.time()
        return
    end
...
RPC.register('FIXDEV-jobs:givePayout', function(source, amount, pType, text)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local charInfo = user:getCurrentCharacter()

    TriggerClientEvent('FIXDEV-phone:paymentNoti', src, amount, pType)
    if pType == "cash" then
