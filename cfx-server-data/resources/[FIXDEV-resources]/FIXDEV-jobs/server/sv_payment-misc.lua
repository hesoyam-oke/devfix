...
RegisterServerEvent('FIXDEV-weedbox:payout')
AddEventHandler('FIXDEV-weedbox:payout', function(money)
    local source = source
        ...
        for i = 1, #Registers[regID] do
            if Registers[regID][i].regID == regID then
                ...
                if (tonumber(user:getCash()) >= tonumber(amount)) then
                    ...
                    local owner = exports["FIXDEV-base"]:getModule("Player"):GetUser(Registers[regID][i].owner)
...
RegisterServerEvent("FIXDEV-jobs:white_widow:update:pay")
AddEventHandler("FIXDEV-jobs:white_widow:update:pay", function(cid)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
