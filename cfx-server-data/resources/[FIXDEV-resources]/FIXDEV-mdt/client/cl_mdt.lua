...
RegisterUICallback("FIXDEV-ui:mdtAction", function(data, cb)
  local result = RPC.execute("FIXDEV-ui:mdtApiRequest", data)
  cb({ data = result.message, meta = { ok = result.success, message = result.message } })
end)

RegisterUICallback("FIXDEV-mdt:getVehiclesByCharacterId", function(data, cb)
  local data = RPC.execute("qpixel:vehicles:getPlayerVehicles", data.character.id, true)
  cb({ data = data, meta = { ok = true, message = 'done' } })
end)

RegisterUICallback("FIXDEV-mdt:getPlayerOnlineStatus", function(data, cb)
  local data = RPC.execute("FIXDEV-mdt:isPlayerOnline", data.character.id, true)
  cb({ data = data, meta = { ok = true, message = 'done' } })
end)

AddEventHandler("FIXDEV-ui:openMDW", function(data)
  if not hasMdwAccess() and not data.fromCmd and not data.publicApp then return end
  playAnimation()
  exports["FIXDEV-ui"]:openApplication("mdt", { publicApp = data.publicApp or false })
end)
...
RegisterUICallback("FIXDEV-ui:getHousingInformation", function(data, cb)
  local result = housingData.get(data.profile.id)
  cb({ data = result or {}, meta = { ok = true, message = 'done' } })
end)

RegisterUICallback("FIXDEV-ui:getApartmentInformation", function (data, cb)
  local result = apartmentData.get(data.profile.id)
  cb({ data = result or {}, meta = { ok = true, message = 'done' } })
end)

RegisterUICallback("FIXDEV-mdt:getUnitInformation", function (data, cb)
  local result = RPC.execute("FIXDEV-dispatch:getDispatchUnits")
  cb({ data = result, meta = { ok = true, message = 'done' } })
end)

RegisterUICallback("FIXDEV-ui:getHousingData", function (data, cb)
  local houses = exports["FIXDEV-housing"]:retrieveHousingTableMapped()
  cb({ data = houses, meta = { ok = true, message = 'done' } })
end)

RegisterUICallback("FIXDEV-ui:getPropertyOwner", function (data, cb)
  local owner = RPC.execute("property:getOwnerRaw", data.property_id)
  cb({ data = owner, meta = { ok = true, message = 'done' } })
end)

RegisterUICallback("FIXDEV-mdt:setPropertyGps", function (data, cb)
...
--   TriggerEvent("FIXDEV-props:attachProp", "cg_chain", 10706, -0.01, 0.01, -0.06, 184.0, -197.0, 4.0, true)
--   Wait(5000)
...
--   -- for i = 1, 360 do
--   --   Wait(500)
--   --   TriggerEvent("FIXDEV-props:attachProp", "cg_chain", 10706, 0.0, 0.04, -0.07, 180.0, 204.0, 0.0, true)
--   --   Wait(500)
--   --   TriggerEvent("FIXDEV-props:removeProp")
--   -- end
-- end)
...
-- Citizen.CreateThread(function()
--   while true do
--     local entity = exports["FIXDEV-target"]:GetEntityPlayerIsLookingAt(80, 5, 12)
--     print(entity)
