...
local function playAnimation()
  ...
  function openTheShitNow()
    TriggerServerEvent('FIXDEV-mdw:open')
  end

  Citizen.CreateThread(function()
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "MDW", "Open", "+mdw", "-mdw", "")
    RegisterCommand('+mdw', openTheShitNow, false)
    RegisterCommand('-mdw', function() end, false)
  end)
  
  RegisterNetEvent('FIXDEV-mdw:openMDW')
  AddEventHandler("FIXDEV-mdw:openMDW", function(pDept)
  ...
  RegisterNetEvent("FIXDEV-ui:restartUI")
  AddEventHandler("FIXDEV-ui:restartUI", function()
    StopAnimTask(PlayerPedId(), "amb@code_human_in_bus_passenger_idles@female@tablet@base", "base", 1.0)
    TriggerEvent("destroyPropPhone")
    SetPlayerControl(PlayerId(), 1, 0)
    SetNuiFocus(false, false)
  ...
  RegisterNUICallback('FIXDEV-mdw:searchProfiles', function(data, cb)
    local value = data.value
    local data = RPC.execute("searchProfiles", value)
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('FIXDEV-mdw:searchVehicles', function(data, cb)
    local value = data.value
    local data = RPC.execute("searchVehicles", value)
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:loadProfile', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('FIXDEV-mdw:loadVehicle', function(data, cb)
    local value = data.value
    local vehicle = RPC.execute("loadVehicle", value)
    cb({ data = {vehicle = vehicle} or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:editProfile', function(data, cb)
  ...
  RegisterNUICallback('FIXDEV-mdw:editVehicle', function(data, cb)
    local success, message = RPC.execute("editVehicle", data.id, data.image, data.info)
    local vehicle = RPC.execute("loadVehicle", data.id)
    cb({ data = {vehicle = vehicle} or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:loadIncident', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('FIXDEV-mdw:loadIncidentEms', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('FIXDEV-mdw:loadEvidence', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('FIXDEV-mdw:updateIncident', function(data, cb)
    local data = RPC.execute("updateIncident", data.id, data.type, data.value, data.secondValue, data.add)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('FIXDEV-mdw:updateIncidentEms', function(data, cb)
    local data = RPC.execute("updateIncidentEms", data.id, data.type, data.value, data.secondValue, data.add)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('FIXDEV-mdw:saveCriminal', function(data, cb)
    local data = RPC.execute("saveCriminal", data.id, data.cid, data.value)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:fetchBusinesses', function(data, cb)
    local data = RPC.execute("fetchBusinesses")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:loadBusiness', function(data, cb)
    local value = data.value
    local employees, count = RPC.execute("loadBusiness", value)
    cb({ data = {employees = employees, count = count} or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:fetchProperties', function(data, cb)
    local data = RPC.execute("fetchProperties")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:fetchCharges', function(data, cb)
    local data = RPC.execute("fetchCharges")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:fetchOfficers', function(data, cb)
    local data = RPC.execute("fetchOfficers")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('FIXDEV-mdw:fetchStaff', function(data, cb)
    local data = RPC.execute("fetchStaff")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('FIXDEV-mdw:fetchEms', function(data, cb)
    local data = RPC.execute("fetchEms")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:fetchIncidents', function(data, cb)
    local data = RPC.execute("fetchIncidents")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('FIXDEV-mdw:fetchIncidentsEms', function(data, cb)
    local data = RPC.execute("fetchIncidentsEms")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:fetchEvidence', function(data, cb)
    local data = RPC.execute("fetchEvidence")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('FIXDEV-mdw:fetchWarrants', function(data, cb)
    local data = RPC.execute("fetchWarrants")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('FIXDEV-mdw:saveIncident', function(data, cb)
  ...
  RegisterNUICallback('FIXDEV-mdw:saveIncidentEms', function(data, cb)
  ...
  RegisterNUICallback('FIXDEV-mdw:assignLicense', function(data, cb)
    local result = RPC.execute("assignLicense", data.cid, data.type)
    if not result then cb(false) return end
    cb(true)
  end)

  RegisterNUICallback('FIXDEV-mdw:removeLicense', function(data, cb)
    local result = RPC.execute("removeLicense", data.cid, data.type)
    if not result then cb(false) return end
    cb(true)
  end)

  RegisterNUICallback('FIXDEV-mdw:createWarrant', function(data, cb)
  ...
  RegisterNUICallback('FIXDEV-mdw:deleteWarrant', function(data, cb)
  ...
  RegisterNUICallback('FIXDEV-mdw:hireStaff', function(data, cb)
  ...
  RegisterNUICallback('FIXDEV-mdw:fireStaff', function(data, cb)
