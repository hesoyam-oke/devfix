...
AddEventHandler('FIXDEV-polyzone:entered:garages', polyZoneEnter)
AddEventHandler('FIXDEV-polyzone:exited:garages', polyZoneExit)
...
AddEventHandler('FIXDEV-polyzone:enter', function(name)
    ...
    for k, v in pairs (sharedGarages) do
        if name == v then
            local garageData = exports["FIXDEV-garages"]:getGarageData(name)
...
RegisterNetEvent("menu:send:FIXDEV-housing:garages", function(pTable, pHid, pHeading)
        exports["FIXDEV-polyzone"]:AddBoxZone("housingGarage-"..pHid, pTable, 10, 10, {
            heading = pHeading,
            minZ = -25,
            maxZ = 100,
          })
