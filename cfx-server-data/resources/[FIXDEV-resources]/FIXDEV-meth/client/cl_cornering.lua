...
AddEventHandler("FIXDEV-meth:cornerSellProduct", function(pArgs, pEntity)
    local gameTimer = GetGameTimer()

    if lastCornerSale and (gameTimer - lastCornerSale) <= 4000 then
        return TriggerEvent('FIXDEV-meth:cleanAttempt', lastCornerSale - gameTimer)
    end

    lastCornerSale = gameTimer

    if not exports["FIXDEV-inventory"]:hasEnoughOfItem("methlabproduct", 1, false, true) then
    ...
    local success = RPC.execute("FIXDEV-meth:attemptCornerSale", NetworkGetNetworkIdFromEntity(pEntity), inCornerId)
    if success then
        TriggerEvent("money:clean")
        TriggerServerEvent("FIXDEV-heists:generateSafeCrackingTool")
    end
...
AddEventHandler("FIXDEV-meth:cornerStartSelling", function()
    local result = RPC.execute("FIXDEV-meth:startCornering", inCornerId)
    if not result then return end
    TriggerEvent("FIXDEV-meth:showSellDrugsMenuItem", "cancorner", false)
    playerCornering = true
end)

AddEventHandler("FIXDEV-polyzone:enter", function(name, data)
    if name ~= "meth_corner" then return end
    if not exports["FIXDEV-inventory"]:hasEnoughOfItem("methlabproduct", 1, false) then return end
    inCorner = true
    inCornerId = 1
    -- inCornerId = data.id
    TriggerEvent("DoLongHudText", "Looks like a good spot to sell...")
    TriggerEvent("FIXDEV-meth:showSellDrugsMenuItem", "cancorner", true)
end)
AddEventHandler("FIXDEV-polyzone:exit", function(name, data)
    if name ~= "meth_corner" then return end
    if not inCorner then return end
    if playerCornering then
        RPC.execute("FIXDEV-meth:stopCornering")
...
RegisterNetEvent("FIXDEV-meth:findCandidateVehicle")
AddEventHandler("FIXDEV-meth:findCandidateVehicle", function(pCornerId, pSrc)
  PopulateNow()
  Wait(0)
  ...
  TriggerServerEvent("FIXDEV-meth:corneringFoundVehicle", pCornerId, pSrc, { vehicle = cornerVehicle, driver = cornerPed })
end)

RegisterNetEvent('FIXDEV-meth:findCandidate', function()
  PopulateNow()
  Wait(0)
  local vehicles = GetGamePool('CVehicle')
