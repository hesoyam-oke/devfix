...
local function processAction(action, pEntity, fnCb)
    ...
    if animSettings then
        ...
        result:next(function (data)
            if data == 100 then
                RPC.execute("FIXDEV-meth:doAction", insideLab, action)
                if fnCb then
                  fnCb()
                end
            else
...
--[[ RegisterUICallback("FIXDEV-ui:submitRangeValues", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ClearPedTasksImmediately(PlayerPedId())
    Wait(100)
    exports["FIXDEV-ui"]:closeApplication("range-picker")
    
    local result = RPC.execute("FIXDEV-meth:doAction", insideLab, "START_COOKING", data)
...
AddEventHandler("FIXDEV-meth:startCooking", function()
    if not skipKeyChecks[insideLab] and not exports["FIXDEV-inventory"]:hasEnoughOfItem("methlabkey", 1, false, true) then
...
AddEventHandler("FIXDEV-meth:pickupIngredient", function(pArgs, pEntity)
...
AddEventHandler("FIXDEV-meth:adjustFridgeTemp", function(pArgs, pEntity)
    processAction("FRIDGE_TEMPERATURE", pEntity)
end)

AddEventHandler("FIXDEV-meth:adjustSteamLevel", function(pArgs, pEntity)
    processAction("DISTIL_STEAM", pEntity)
end)

AddEventHandler("FIXDEV-meth:adjustDistilSettings", function(pArgs, pEntity)
    processAction("DISTIL_SETTINGS", pEntity)
end)

AddEventHandler("FIXDEV-meth:adjustMixerTemp", function(pArgs, pEntity)
    processAction("MIXER_TEMPERATURE", pEntity)
end)

AddEventHandler("FIXDEV-meth:addIngredient", function(pArgs, pEntity)
...
AddEventHandler("FIXDEV-meth:adjustMixerSettings", function(pArgs, pEntity)
...
--     RPC.execute("FIXDEV-meth:useDoorKey", NetworkGetNetworkIdFromEntity(PlayerPedId()), info)
-- end)

AddEventHandler("FIXDEV-meth:purchaseMethLabKey", function()
    local wallet, message = RPC.execute("phone:adjustCryptoBalance", 1, getConfig())
    if not wallet then
        TriggerEvent("DoLongHudText", message, 2)
        return
    end
    local result = RPC.execute("FIXDEV-meth:purchaseLabKey")
end)

-- local foundObject = nil
-- AddEventHandler("FIXDEV:target:changed", function(pEntity, pEntityType)
--   if not pEntity or pEntityType ~= 3 then
--     if foundObject then
--         foundObject = nil
--         TriggerEvent("FIXDEV-menu:updateMethObject", foundObject)
--     end
--     return
--   end
...
RegisterNetEvent("FIXDEV-meth:keyPurchasedEmail")
AddEventHandler("FIXDEV-meth:keyPurchasedEmail", function()
  TriggerEvent(
    "phone:emailReceived",
    "Dark Market",
    "Lab Information",
