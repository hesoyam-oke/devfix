...
AddEventHandler("FIXDEV-inventory:itemUsed", function(item, info)
    ...
    if not exports["FIXDEV-inventory"]:hasEnoughOfItem("qualityscales", 1, false) then
        TriggerEvent("DoLongHudText", "Need something to weigh with...")
        return
    end
    if not exports["FIXDEV-inventory"]:hasEnoughOfItem("emptybaggies", 1, false) then
        TriggerEvent("DoLongHudText", "Need something to pack with...")
        return
    end
    local finished = exports["FIXDEV-taskbar"]:taskBar(1000, "Packing")
    if finished ~= 100 then return end
    RPC.execute("FIXDEV-meth:packCuredMeth", json.decode(info))
...
Citizen.CreateThread(function()
    ...
    local rv = exports["FIXDEV-npcs"]:RegisterNPC(data, 'FIXDEV-meth')
end)
