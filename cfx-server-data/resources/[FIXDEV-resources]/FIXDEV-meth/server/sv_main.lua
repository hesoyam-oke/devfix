...
local function get_info_from_db ()
  local src = source
  local user = exports['FIXDEV-base']GetModule('Player')(src)
  local cid = user.id
...
RPC.register('FIXDEV-meth:getConfig',function(pData, pInfo)
  print(pData, pInfo)
  local src = source
  table = {
    ACTIVE_CORNERS = {
...
RegisterServerEvent('FIXDEV-meth:corneringFoundVehicle',function (pCorner , pSrc , pTable)
  print('Hmmm',json.encode(pTable))
  
end)

RPC.register('FIXDEV-meth:startCornering', function(pID)
  local src = source
  TriggerClientEvent('FIXDEV-meth:findCandidateVehicle', 1 , src)
  return true
end)

RPC.register('FIXDEV-meth:corneringFoundVehicle',function(pData, pInfo , pTable)
  local src = source
  return false , 'No vehicle found bro'
end)

RPC.register('FIXDEV-meth:attemptCornerSale', function (pCar , pInfo)
...
RPC.register('FIXDEV-meth:stopCornering', function()
  local src = source 
  TriggerClientEvent('FIXDEV-meth:showSellDrugsMenuItem',src,"cancorner" ,false)
end)

RPC.register('FIXDEV-meth:purchaseLabKey', function()
  local src = source
