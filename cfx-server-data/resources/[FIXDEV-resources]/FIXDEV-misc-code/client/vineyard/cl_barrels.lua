...
AddEventHandler("FIXDEV-inventory:itemUsed", function(pItem, pData)
...
AddEventHandler("FIXDEV-vineyard:pickupBarrel", function(p1, p2, p3)
  ...
  exports['FIXDEV-objects']:DeleteObject(p3.meta.id)
  TriggerEvent("player:receiveItem", "vineyardbarrel", 1, false, data, data)
end)


RegisterUICallback("FIXDEV-vineyard:changeBarrelName", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["FIXDEV-ui"]:closeApplication("textbox")
  RPC.execute("FIXDEV-vineyard:changeBarrelName", data.key, data.values.name)
end)

RegisterUICallback("FIXDEV-vineyard:addGrapesToBarrel", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["FIXDEV-ui"]:closeApplication("textbox")
  local newData = data.key.newData
  local objectId = data.key.objectId
  local items = exports["FIXDEV-inventory"]:getItemsOfType("vineyardgrapes", false, true)
  ...
  RPC.execute("FIXDEV-vineyard:changeBarrelData", objectId, newData)
end)

local function getTimestamp(data)
  local remaining = math.floor((exports["FIXDEV-misc-code"]:getCurrentTime() - data.created_at) / 1000)
...
AddEventHandler("FIXDEV-vineyard:barrelAction", function(p1, p2, p3)
  if not isGrapeBarrel(p3) then return end
  local data = p3.meta.data.metadata
  ...
  if p1.action == "change_name" then
    exports["FIXDEV-ui"]:openApplication("textbox", {
      callbackUrl = "FIXDEV-vineyard:changeBarrelName",
      key = objectId,
      items = {
  ...
  if p1.action == "seal" then
    ...
    local newData = { sealed = true, created_at = exports["FIXDEV-misc-code"]:getCurrentTime() }
    RPC.execute("FIXDEV-vineyard:changeBarrelData", objectId, newData)
    return
  end
  ...
  if p1.action == "add_grapes" then
    ...
    if #items == 0 then return end
    exports["FIXDEV-ui"]:openApplication("textbox", {
      callbackUrl = "FIXDEV-vineyard:addGrapesToBarrel",
  ...
  if p1.action == "pour_bottles" then
    local hasEnough = exports["FIXDEV-inventory"]:hasEnoughOfItem("vineyardwinebottleempty", 1, false, true)
    if not hasEnough then
      TriggerEvent("DoLongHudText", "You need empty wine bottles!", 2)
      return
    end
    TriggerEvent("doAnim","cokecut")
    local finished = exports["FIXDEV-taskbar"]:taskBar(10000, "Bottling Wine")
    ClearPedTasks(PlayerPedId())
    if finished ~= 100 then return end
    exports['FIXDEV-objects']:DeleteObject(objectId)
...
AddEventHandler("FIXDEV-inventory:attachmentsToggle", function(pEnabled, pId)
...
Citizen.CreateThread(function()
  exports["FIXDEV-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
    {
      id = "peek_prop_vineyard_barrel",
      event = "FIXDEV-vineyard:pickupBarrel",
      icon = "shopping-basket",
      label = "Pick Up",
    },
    {
      id = "peek_prop_vineyard_barrel_details",
      event = "FIXDEV-vineyard:barrelAction",
      icon = "info-circle",
      label = "View Label",
  ...
  exports["FIXDEV-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
    ...
    isEnabled = function(p1, p2, p3)
      ...
      local hasEnough = exports["FIXDEV-inventory"]:hasEnoughOfItem("vineyardbarrelheadopener", 1, false, true)
      return hasEnough
    end,
  })
end)
