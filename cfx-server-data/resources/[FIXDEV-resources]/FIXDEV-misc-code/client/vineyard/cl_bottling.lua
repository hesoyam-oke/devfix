RegisterUICallback("FIXDEV-vineyard:setupWineboxContainer", function(data, cb)
  ...
  if data.key.item == "vineyardwinebox" then
    RPC.execute("FIXDEV-restaurants:getTakeoutBox", {
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(pItem, pData)
  ...
  exports["FIXDEV-ui"]:openApplication("textbox", {
    callbackUrl = "FIXDEV-vineyard:setupWineboxContainer",
...
AddEventHandler("FIXDEV-inventory:itemUsed", function(pItem, pData)
  ...
  local amount = exports["FIXDEV-inventory"]:getQuantity("wineglass", true, false)
  if amount == 0 then
