...
local function getVehicleSpeed(pEntity)
  return GetEntitySpeed(pEntity) * 2.6 -- 2.236936
end
...
local function checkForCarBomb(pEntity) 
  ...
  local progress = exports['FIXDEV-taskbar']:taskBar(25000, 'Searching for car bomb...', true)
  ClearPedTasks(PlayerPedId())

  if progress ~= 100 then return end

  local metaData = exports['FIXDEV-vehicles']:GetVehicleMetadata(pEntity)
  local carBombMeta = metaData.carBombData or false
  local remoteBomb = NPX.Procedures.execute('FIXDEV-usableprops:hasPhoneBomb', NetworkGetNetworkIdFromEntity(pEntity))
...
AddEventHandler('FIXDEV-inventory:itemUsed', function (name, info)
  ...
  local progress = exports['FIXDEV-taskbar']:taskBar(25000, 'Planting car bomb...', true)
  ...
  local added = NPX.Procedures.execute('FIXDEV-miscscripts:carbombs:addCarBomb', netId, minSpeed, ticksBeforeExplode, ticksForRemoval, gridSize, coloredSquares, timeToComplete)
...
RegisterUICallback('FIXDEV-miscscripts:carBombs:completeHacking', function(data, cb)
  ...
  TriggerServerEvent('FIXDEV-miscscripts:carbombs:removeBomb', netId)
