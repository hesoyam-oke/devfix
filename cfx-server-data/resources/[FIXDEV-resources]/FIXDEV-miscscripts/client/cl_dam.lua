...
CreateThread(function ()
    exports["FIXDEV-polytarget"]:AddBoxZone("dam_controls", vector3(1664.16, -25.04, 162.43), 0.3, 1.5, {
        heading=12,
        minZ=159.43,
        maxZ=164.03
    })
    exports["FIXDEV-interact"]:AddPeekEntryByPolyTarget("dam_controls", {{
        id =  'dam_controls',
        label =  'Controls',
        icon =  'smile',
        NPXEvent = "FIXDEV-miscscripts:dam:control",
        parameters =  {},
    }}, { distance = { radius = 1.5 } })
end)

NPX.Events.on("FIXDEV-miscscripts:dam:control", function ()
    if not damdata then
        local sliderSettings = NPX.Procedures.execute("FIXDEV-miscscripts:dam:getSliders")
        damdata = sliderSettings.data
    end

    exports['FIXDEV-ui']:openApplication('range-picker', {
        ["sliders"] = 4,
        ["submitUrl"] = "FIXDEV-miscscripts:dam:submitSliders"
    })
end)

RegisterUICallback("FIXDEV-miscscripts:dam:submitSliders", function (result, cb)
