...
CreateThread(function ()
    exports["FIXDEV-polyzone"]:AddBoxZone("reed_pickup", vector3(1001.51, -2116.59, 30.48), 3.5, 3.5, {
        heading=357,
        minZ=29.48,
        maxZ=35.00,
        data = {
    ...
    exports['FIXDEV-interact']:AddPeekEntryByEntityType(
        { 1 },
        {
          {
            id = 'reed_pickup',
            label = "Attach",
            icon = 'dizzy',
            event = 'FIXDEV-miscscripts:reedHook',
            parameters = {},
          },
        },
        {
...
AddEventHandler("FIXDEV-polyzone:enter", function(pName, pData)
    if pName ~= "reed_pickup" then return end
    isInsideHookZone = true
end)

AddEventHandler("FIXDEV-polyzone:exit", function(pName, pData)
    if pName ~= "reed_pickup" then return end
    isInsideHookZone = false
end)

AddEventHandler("FIXDEV-miscscripts:reedHook", function(pParameters, pEntity, pContext)
    local hookedTargetServerId = GetPlayerServerId(NetworkGetPlayerIndexFromPed(pEntity))
    if hookedTargetServerId ~= 0 then
        NPX.Events.emitNet('FIXDEV-miscscripts:reedSetHook', hookedTargetServerId)
    end
end)
...
NPX.Events.onNet('FIXDEV-miscscripts:reedSetHooked', function(pTarget, pState)
...
RegisterCommand("-unhook", function()
    if isHooked or isInsideHookZone then
        NPX.Events.emitNet('FIXDEV-miscscripts:reedSetHook')
    end
end, false)