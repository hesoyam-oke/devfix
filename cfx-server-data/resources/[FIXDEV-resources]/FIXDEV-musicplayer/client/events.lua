RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("FIXDEV-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("FIXDEV-musicplayer:stateSound")
AddEventHandler("FIXDEV-musicplayer:stateSound", function(state, data)
