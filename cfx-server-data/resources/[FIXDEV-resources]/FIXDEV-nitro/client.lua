...
RegisterCommand('nitro', function()
	TriggerEvent("FIXDEV-nitro:client:placeNitro")
end)
		...
		if nitroactive and nitroseviye > 0 and IsPedInVehicle(PlayerPedId(), GetVehiclePedIsIn(PlayerPedId(), false), false) then
			...
			if fisfismode then
				...
				elseif IsControlPressed(0, 121) and nitroflowrate <= 0.5 then -- home
					...
					exports['pNotify']:SendAlert('inform', 'Purge Spray Flowrate: ' .. nitroflowrate)
					Wait(500)
			...
			if IsControlJustPressed(0, 21) and nitroveh == GetVehiclePedIsIn(PlayerPedId(), false) then
				if not fisfismode and nitromode then
					fisfismode = true
					nitromode = false
					exports['pNotify']:SendAlert('inform', 'Nitro Modu: Duman Çıkartma')
					exports['pNotify']:SendAlert('inform', 'INSERT tuşuna basarak dumanı arttırabilir, DELETE tuşuna basarak dumanı azaltabilirsiniz', 5000)
				elseif not nitromode and fisfismode then
					nitromode = true
					fisfismode = false
					exports['pNotify']:SendAlert('inform', 'Nitro Modu: Nitro')
				end
			end
