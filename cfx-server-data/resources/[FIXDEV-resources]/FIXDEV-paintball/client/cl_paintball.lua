...
local function removeGuns()
  local qty = exports["FIXDEV-inventory"]:getQuantity("-2009644972")
  ...
  local megaphoneQty = exports["FIXDEV-inventory"]:getQuantity("megaphone", true, { type = "paintball" })
  if megaphoneQty and megaphoneQty > 0 then
    TriggerEvent("inventory:removeItemByMetaKV", "megaphone", megaphoneQty, "type", "paintball")
  end
  local smokeQty = exports["FIXDEV-inventory"]:getQuantity("smokegrenadenpa")
  if smokeQty and smokeQty > 0 then
    TriggerEvent("inventory:removeItem", "smokegrenadenpa", smokeQty)
  end
end

AddEventHandler("FIXDEV-paintball:getPaintballGun", function()
  TriggerEvent("player:receiveItem", "-2009644972", 1)
end)

AddEventHandler("FIXDEV-paintball:getCaddy", function()
  if not isNpa() then return end
  RPC.execute("FIXDEV:vehicles:basicSpawn", "caddy", { x = 5513.32, y = 5992.66, z = 590.01 }, 359.1, "paintball")
end)

AddEventHandler("FIXDEV-paintball:getGoKart", function()
  if not isNpa() then return end
  RPC.execute("FIXDEV:vehicles:basicSpawn", "kart", { x = 5513.32, y = 5992.66, z = 590.01 }, 359.1, "paintball")
end)

AddEventHandler("FIXDEV-paintball:getPaintballSmoke", function()
  if not isNpa() then return end
  TriggerEvent("player:receiveItem", "smokegrenadenpa", 1)
end)

AddEventHandler("FIXDEV-paintball:getPaintballAmmo", function()
  TriggerEvent("player:receiveItem", "paintballs", 20)
end)

AddEventHandler("FIXDEV-paintball:getMegaphone", function()
  if not isNpa() then return end
...
RegisterNetEvent("FIXDEV-paintball:reviveAfterDown", function()
...
AddEventHandler("FIXDEV-paintball:setArenaType", function(pArgs)
  if not isNpa() then return end
  RPC.execute("FIXDEV-paintball:setArenaType", pArgs[1])
end)

RegisterNetEvent("FIXDEV-paintball:changeArenaType", function(pType)
...
Citizen.CreateThread(function()
  Citizen.Wait(30000)
  TriggerServerEvent("FIXDEV-paintball:getArenaType")
end)
...
AddEventHandler("FIXDEV-paintball:swapLocations", function()
  DoScreenFadeOut(400)
...
AddEventHandler("FIXDEV-paintball:enterArena", function()
  inArena = true
  TriggerEvent("FIXDEV-cleanup:enableCleanup", false)
  Citizen.CreateThread(function()
    while inArena do
      if currentLocation == "arena" then
...
AddEventHandler("FIXDEV-paintball:exitArena", function()
  inArena = false
  TriggerEvent("FIXDEV-cleanup:enableCleanup", true)
  removeGuns()
end)
