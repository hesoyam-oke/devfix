...
AddEventHandler("FIXDEV-paleto:freezeDoors", function()
    Citizen.CreateThread(function()
...
-- RegisterNetEvent("FIXDEV-paleto:toggleVault")
...
--             TriggerServerEvent("FIXDEV-paleto:updateVaultState", key, Doors[key][1].state)
--         elseif state then
--             local obj = GetClosestObjectOfType(Paleto.Banks[key].doors.startloc.x, Paleto.Banks[key].doors.startloc.y, Paleto.Banks[key].doors.startloc.z, 2.0, GetHashKey(Paleto.vaultdoor), false, false, false)
--             local count = 0
...
AddEventHandler("FIXDEV-paleto:reset", function(name, data)
    ...
    -- TriggerServerEvent("FIXDEV-paleto:toggleVault", name, true)
...
AddEventHandler("FIXDEV-paleto:startheist", function(data, name)
  ...
  --  currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    ...
    --TriggerServerEvent("FIXDEV-paleto:toggleVault", name, false)
    startdstcheck = true
    currentname = name
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("FIXDEV-paleto:startLoot", data, name, missionplayers)
    done = false
end
...
RegisterNetEvent('FIXDEV-paleto:darkmail')
AddEventHandler('FIXDEV-paleto:darkmail', function()
    Citizen.Wait(1000)
...
RegisterNetEvent('FIXDEV-paleto:UseBlueLapTop')
AddEventHandler('FIXDEV-paleto:UseBlueLapTop', function(item)
    local ply = PlayerPedId()
    local plyCoords = GetEntityCoords(ply)
    local p = promise:new()

    Paleto.Banks, doors = RPC.execute("FIXDEV-paleto:getBanks")
	TriggerEvent("FIXDEV-paleto:freezeDoors")
    ...
    for k, v in pairs(Paleto.Banks) do
        if not v.onaction then
            ...
            if dist < 1.0 then
                ...
                if exports["FIXDEV-inventory"]:hasEnoughOfItem("heistlaptop2", 1) then
                    -- if HasItem then
...
AddEventHandler('paletogate:doorburn', function()
	TriggerServerEvent('FIXDEV-doors:change-lock-state', 498, false)
end)

RegisterNetEvent('paletogate2:doorburn') -- Doorlock
AddEventHandler('paletogate2:doorburn', function()
	TriggerServerEvent('FIXDEV-doors:change-lock-state', 517, false)
end)
