...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("FIXDEV-pdm:CarTablePDM",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("FIXDEV-pdm:testdrive")
AddEventHandler("FIXDEV-pdm:testdrive", function()
	if exports["FIXDEV-business"]:IsEmployedAt("pdm") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("FIXDEV-pdm:EnableBuy")
AddEventHandler("FIXDEV-pdm:EnableBuy", function()
	if #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["FIXDEV-business"]:IsEmployedAt("pdm") then
		return
	end
	...
	TriggerServerEvent("FIXDEV-pdm:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("FIXDEV-pdm:BuyEnabledCL")
AddEventHandler("FIXDEV-pdm:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("FIXDEV-pdm:PDMCommission")
AddEventHandler("FIXDEV-pdm:PDMCommission", function(newAmount)
	if exports["FIXDEV-business"]:IsEmployedAt("pdm") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("FIXDEV-pdm:ReturnPDMTTable")
AddEventHandler("FIXDEV-pdm:ReturnPDMTTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('FIXDEV-pdm:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #carspawns do
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["FIXDEV-business"]:IsEmployedAt("pdm") then
				if buyPlate[addplate] ~= nil then
					exports['FIXDEV-ui']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change | [G] to buy.")
				else
					exports['FIXDEV-ui']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change")
				end
			else
				if buyPlate[addplate] ~= nil then
					exports['FIXDEV-ui']:showInteraction("$" .. math.ceil(price) .. " [G] to buy.")
				else
					exports['FIXDEV-ui']:showInteraction("Buy Price: $" .. math.ceil(price) .. " ")
				end
			end
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("FIXDEV-pdm:RequestPDMTTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			local VehicleProps = exports['FIXDEV-bennys']:FetchVehProps(personalvehicle)
			...
			TriggerServerEvent('FIXDEV-pdm:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('FIXDEV-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'PDM')
			TriggerServerEvent('FIXDEV-financials:business_money', price / 10, 'pdm', 'add')
			exports['FIXDEV-ui']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("FIXDEV-pdm:FailedPurchase")
AddEventHandler("FIXDEV-pdm:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "jobvehicles" or this == "a-class" or this == "b-class" or this == "s-class" or this == "pdm-tuners" or this == "c-class" or this == "casino" or this == "d-class" or this == "offroad" or this == "suvs" or this == "vans" or this == "industrial" or this == "cycles" or this == "motorcycles" then
		if ownerMenu then
			...
			TriggerServerEvent('FIXDEV-pdm:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["FIXDEV-business"]:IsEmployedAt("pdm") and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commissionpdm', function(source, args, raw)
	if exports["FIXDEV-business"]:IsEmployedAt("pdm") then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('FIXDEV-pdm:PDMCommission', amount)
		else
...
RegisterCommand('testdrivepdm', function(source, args, raw)
	if exports["FIXDEV-business"]:IsEmployedAt("pdm") then
		TriggerEvent('FIXDEV-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuypdm', function(source, args, raw)
	if exports["FIXDEV-business"]:IsEmployedAt("pdm") then
		TriggerEvent('FIXDEV-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)

RegisterNetEvent('FIXDEV-fuel:pdmmenushit', function()
	...
	local menuData = {
		{
			...
			icon = 'credit-card',
			key = "EVENTS.CARD",
			action = 'FIXDEV-pdm:pdmcom',
			disabled = canPayBill
		},
	}
	exports["FIXDEV-ui"]:showContextMenu(menuData)
end)

RegisterInterfaceCallback('FIXDEV-pdm:pdmcom', function(data, cb, pParams)
	cb({ data = {}, meta = { ok = true, message = '' } })
	exports['FIXDEV-ui']:openApplication('textbox', {
		callbackUrl = 'FIXDEV-pdm:send_pdm',
		key = 1,
		items = {
		  {
			icon = "address-card", 
...
RegisterInterfaceCallback('FIXDEV-pdm:send_pdm', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerServerEvent("FIXDEV-pdm:wtf", data.values.pPDMPayCom)
end) 



RegisterNetEvent("FIXDEV-pdm:commissionpdm") 
AddEventHandler("FIXDEV-pdm:commissionpdm", function(args)
	print("YO")
	if exports["FIXDEV-business"]:IsEmployedAt("pdm") then
		print("IM PDM WORKER")
		local amount = args
...
RegisterNetEvent("FIXDEV-pdm:testdrivepdm")
AddEventHandler("FIXDEV-pdm:testdrivepdm", function()
	if exports["FIXDEV-business"]:IsEmployedAt("pdm") then
		TriggerEvent('FIXDEV-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
...
RegisterNetEvent("FIXDEV-pdm:enableBuypdm")
AddEventHandler("FIXDEV-pdm:enableBuypdm", function()
	if exports["FIXDEV-business"]:IsEmployedAt("pdm") then
		TriggerEvent('FIXDEV-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
