...
RegisterNetEvent("FIXDEV-phone:addToCurrentBanks", function(data)
    local addedBank = {
    ...
    RPC.execute("FIXDEV-phone:addToCurrentBanks", addedBank)
end)

RegisterNUICallback('tryClaimBank', function(data, cb)
    if not claimedBank then
        claimedBank = true
        RPC.execute('FIXDEV-phone:claimBank:BankBusters', data.bbid, exports['isPed']:isPed('cid'))
        Citizen.Wait(5000)
        claimedBank = false
    end
end)
...
RegisterNUICallback('getCurrentBanks', function(data, cb)
    local banks = RPC.execute('FIXDEV-phone:getCurrentBanks')
...
RegisterNUICallback('onPlayerJoinBankBusters', function(data, cb)
    RPC.execute("FIXDEV-phone:addPlayerToGroup", tonumber(data.targetCID), data.groupID, tonumber(data.senderID))
end)
...
RegisterNUICallback('addBankbusterGroup', function(data, cb)
    RPC.execute("FIXDEV-phone:requestPlayerToGroup", tonumber(data.cid), data.groupID)
end)
...
RegisterNUICallback('leaveBankbusterGroup', function(data, cb)
    for k,v in pairs(currentGroups) do
        for j, s in pairs(v.members) do
            if s.leader then
                RPC.execute("FIXDEV-phone:deleteGroup", data.groupID)
            else
                RPC.execute("FIXDEV-phone:removePlayerFromGroup", tonumber(data.cid), data.groupID)
            end
        end
    end
end)