...
RegisterNUICallback('getCallHistory', function()
  local recentCallHistory = RPC.execute("FIXDEV-phone:getRecentCallsByCid", exports['isPed']:isPed("cid"))
  SendNUIMessage({
...
Citizen.CreateThread(function()
  while true do
    Wait(10000)
    TriggerServerEvent("FIXDEV-phone:sendRecentCalls", recentcalls, exports['isPed']:isPed("cid"))
  end
end)
...
AddEventHandler('phone:otherClientEndCall', function()
  ...
  Citizen.CreateThread(function()
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Player", "Hangup Call", "+hangup", "-hangup", "5")
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Player", "Answer Call", "+answer", "-answer", "6")
  end)
  ...
  AddEventHandler('phone:addToCall', function(voipchannel)
    exports['FIXDEV-voice']:addPlayerToCall(tonumber(voipchannel))
  end)
...
RegisterNetEvent('FIXDEV-phone:RemoveCall')
AddEventHandler('FIXDEV-phone:RemoveCall', function()
    SendNUIMessage({
      openSection = 'callnotifyEnd'
    })
    SendNUIMessage({
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports['FIXDEV-voice']:removePlayerFromCall()
end)
...
function endCall()
  ...
  mySourceID = 0
  TriggerEvent('phone:setCallState', isNotInCall)
  TriggerEvent('FIXDEV-phone:RemoveCall')
  onhold = false
  mySourceHoldStatus = false
  callid = 0
end
...
function checkForPayPhone()
  ...
  local ph = exports["FIXDEV-applications"]:KeyboardInput({
    header = "Payphone",
    rows = {
      {
        id = 1,
