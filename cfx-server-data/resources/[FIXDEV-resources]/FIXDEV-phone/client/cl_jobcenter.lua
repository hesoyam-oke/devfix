...
RegisterNUICallback('job-center', function(data, cb)
  local activeGroups = RPC.execute("FIXDEV-phone:getGroupsCount")
  local idle = RPC.execute("FIXDEV-phone:getIdleGroup", exports['isPed']:isPed('myjob'))
  local busy = RPC.execute("FIXDEV-phone:getBusyGroup", exports['isPed']:isPed('myjob'))
  local myG = RPC.execute("FIXDEV-phone:getMyGroup")
  local members = RPC.execute("FIXDEV-phone:get_groupsMem")
  local job = exports['isPed']:isPed('myjob')
  local leader = RPC.execute('FIXDEV-phone:getGroupLeader', exports['isPed']:isPed('cid'))
...
RegisterNetEvent('refreshJobCenter', function()
  ...
  local busy = RPC.execute("FIXDEV-phone:getBusyGroup", exports['isPed']:isPed('myjob'))
  local myG = RPC.execute("FIXDEV-phone:getMyGroup")
  local members = RPC.execute("FIXDEV-phone:get_groupsMem", idle)
  local leader = RPC.execute('FIXDEV-phone:getGroupLeader', exports['isPed']:isPed('cid'))
...
RegisterNUICallback('FIXDEV-phone:joinGroup', function(data)
    local gId = data.gId
    RPC.execute("FIXDEV-phone:j_group", gId, exports['isPed']:isPed('cid'))
end)
...
RegisterNUICallback('JobReady', function(data)
    ...
    if data.changeStatus then
      RPC.execute('FIXDEV-phone:setGroupStatus', grpStatus, exports['isPed']:isPed('fullname'))
    end
end)
...
RegisterNUICallback('jobInviteAccepted', function()
    ...
    RPC.execute("FIXDEV-phone:joinGroup_Accepted", currentSource, pendingGroupID, jCount)
    Wait(100)
    pendingGroupID = 0
    currentSource = 0
end)
...
RegisterNUICallback('jobInviteDecline', function()
  RPC.execute("FIXDEV-phone:j_group_decline")
  Wait(100)
  pendingGroupID = 0
  currentSource = 0
end)

RegisterNetEvent('FIXDEV-phone:client:joboffer', function(jInfo)
    SendNUIMessage({
...
RegisterNetEvent('FIXDEV-phone:CreateGroupInvite', function(jInfo, jMessage, pInfo, gID)
  pendingGroupID = gID
  currentSource = pInfo
  SendNUIMessage({
...
RegisterNetEvent('FIXDEV-phone:JobNotify', function(jInfo, jMessage)
  SendNUIMessage({
...
RegisterNetEvent('FIXDEV-phone:JobNotifyStable', function(jInfo, jMessage)
  SendNUIMessage({
...
RegisterNetEvent('FIXDEV-phone:JobNotifyClose', function()
  SendNUIMessage({
    openSection = "jobNotifyClose"
  })
  RPC.execute('FIXDEV-phone:setGroupStatus', "0", exports['isPed']:isPed('fullname'))
end)

RegisterNetEvent('FIXDEV-phone:JobNotifyCloseNotFully', function()
  SendNUIMessage({
...
local currentTask = RPC.execute('FIXDEV-phone:getGroupTask')
  if currentTask == 0 then 
    updateTasks()
  end
end

RegisterNetEvent('FIXDEV-phone:CancelJob', function(status)
  local job = exports['isPed']:isPed('myjob')

    if status == 1 then
      if job == 'sanitation_worker' then
        TriggerEvent('FIXDEV-garbage:JobCancel', status)
      end
      if job == 'fisherman' then
        TriggerEvent('FIXDEV-fishing:JobCancel', status)
      end
      if job == 'delivery_driver' then
        TriggerEvent('FIXDEV-delivery:JobCancel', status)
      end
    else
      if job == 'sanitation_worker' then
        TriggerEvent('FIXDEV-garbage:JobCancel', status)
...
Citizen.CreateThread(function()
  while true do
    ...
    if status then
      ...
      elseif job == 'criminal' then
        if exports['FIXDEV-inventory']:hasEnoughOfItem('vpnxj', 1) and exports['FIXDEV-inventory']:hasEnoughOfItem('darkmarketdeliveries', 1) then
...
RegisterNUICallback('accept_fishing_job', function()
  RPC.execute('FIXDEV-jobs:fishingStart')
  SendNUIMessage({openSection = "resetJobStatus"}) 
end)

RegisterNUICallback('accept_garbage_job', function()
  TriggerEvent('FIXDEV-jobs:startGarbageRun')
  SendNUIMessage({openSection = "resetJobStatus"}) 
end)

RegisterNUICallback('accept_oxy_run', function()
  RPC.execute('FIXDEV-activities:syncOxyStart', math.random(1, 3))
  SendNUIMessage({openSection = "resetJobStatus"}) 
end)

RegisterNUICallback('accept_tow_job', function()
	TriggerEvent('FIXDEV-jobs:clockIntoTowing', math.random(1, 8))
  SendNUIMessage({openSection = "resetJobStatus"}) 
end)
...
RegisterNUICallback('accept_chop_shop', function(data)
    TriggerEvent('FIXDEV-activities:startJobChop', math.random(1, 7))
    SendNUIMessage({openSection = "resetJobStatus"}) 
end)
