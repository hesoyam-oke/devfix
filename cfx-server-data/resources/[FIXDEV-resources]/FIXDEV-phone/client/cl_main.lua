...
RegisterNetEvent('FIXDEV-base:playerSpawned')
AddEventHandler('FIXDEV-base:playerSpawned', function()
  Wait(1000)
...
RegisterCommand('renten', function()
  TriggerEvent('FIXDEV-housing:rent_activate_cl')
end)

RegisterNetEvent('FIXDEV-housing:rent_activate_cl')
AddEventHandler('FIXDEV-housing:rent_activate_cl', function()
...
AddEventHandler('FIXDEV-phone:sanitationIn', function()
  -- print("SANITATION SIGN IN")
  sanitation = true
end)

RegisterNetEvent("FIXDEV-phone:grabBackground")
AddEventHandler("FIXDEV-phone:grabBackground", function(link)
    wallPaper = link
end)
...
RegisterNUICallback('wallpaper', function(data, cb)
  ...
  TriggerEvent('FIXDEV-phone:grabBackground', wallPaperSelecionado)
...
RegisterNUICallback('sendThePM', function(data, cb)
  ...
  RPC.execute("FIXDEV-phone:sendThePM", thePM)
end)

RegisterCommand("fuckmeintheasdasvf", function()
  RPC.execute("FIXDEV-phone:sendThePM", 'f')
end)
...
RegisterNUICallback('submitSettings', function(data, cb)
  ...
  Wait(5)
  if bg ~= "" then
    TriggerEvent('FIXDEV-phone:grabBackground', bg)
...
RegisterNetEvent("FIXDEV-phone:setSomeShit", function(bg, model, twatNotify, callNotify)
  SetResourceKvp(exports['isPed']:isPed('cid').."-bg", bg)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-pmodel", model)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ntwat", twatNotify)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ncall", callNotify)
...
RegisterNUICallback('btnHousing', function()
  ...
  allNewHouses = {}
  myAccessHouse = {}
  --local apt = RPC.execute("FIXDEV-phone:apt")
  local housing = exports['FIXDEV-housing']:housingForPhone()
  local apartment = exports['FIXDEV-apartments']:getMyCurrentApartment()
  myHousess = RPC.execute("FIXDEV-phone:getCurrentOwned")
  accessHouse = RPC.execute('FIXDEV-phone:getAccessHouse_2')
  ...
  for i,v in pairs(myHousess) do
    ...
    table.insert(newHouses, {
      ...
      cid = v.cid,
      status = v.status,
      cat = exports['FIXDEV-housing']:getHousingCatFromPropertID(v.hid),
    })
  end
  for l,s in pairs(accessHouse) do
      table.insert(myAccessHouse, {
        ...
        cid = s.cid,
        status = s.status,
        cat = exports['FIXDEV-housing']:getHousingCatFromPropertID(s.house),
      })
  end
  SendNUIMessage({
    openSection = "housing",
...
RegisterNUICallback('btnKeyHolders', function(data, cb)
  local myKittens = {}
  print(data.hid)
  local keys = RPC.execute('FIXDEV-housing:getKeyHolderz', data.hid)
  SendNUIMessage({
...
RegisterNUICallback('whatthefuckdoesthisshitwork', function(data,cb)
  local cid = data.cid
  local hid = data.hid
  local success,msg = RPC.execute('FIXDEV-housing:removeKey:phone',cid, hid)
  if success then
...
RegisterNUICallback('OpenCamera', function(data,cb)
  RPC.execute('FIXDEV-phone:openCamera', data.cameraId)
end)
...
--   while true do
--     Wait(3000)
--     local myHouse = RPC.execute("FIXDEV-phone:getCurrentOwned")
...
RegisterInterfaceCallback("FIXDEV-phone:signInHandler", function(data, cb)
  if data.key.sign_in then
    RPC.execute("FIXDEV-phone:addDOJ", data.key.job)
  else
    RPC.execute("FIXDEV-phone:removeDOJ", data.key.job)
  end
...
RegisterNUICallback('getActiveDOJ', function(data,cb)
      SendNUIMessage({
        ...
        penisinmybuttholefuckshit = RPC.execute("FIXDEV-phone:getDOJ")
      })
end)
...
RegisterNUICallback('abdulSignIn', function(data,cb)
    ...
    if taxiStatus then
      RPC.execute("FIXDEV-phone:addAbdulTaxi")
      addAbdulTaxi()
    ...
    elseif not taxiStatus then
      RPC.execute("FIXDEV-phone:removeAbdulTaxi")
      removeAbdulTaxi()
      local availableDrivers = getAbdulTaxi()
      SendNUIMessage({
        openSection = "addTaxi",
...
RegisterNetEvent("FIXDEV-phone:manualUpdateAbdulTaxies", function(availableDrivers)
  addAbdulTaxi()
  local availableDrivers = getAbdulTaxi()
  SendNUIMessage({
    openSection = "addTaxi",
...
function addAbdulTaxi () 
  currenttable = {}
  local table = RPC.execute("FIXDEV-phone:getAbdulTaxies")
--  local currenttable = {}
...
RegisterNUICallback('sellVeh', function(data)
  ...
  TriggerServerEvent('FIXDEV-phone:sendVehBill', data.paypal, data.plate, data.amount)
end)

RegisterNetEvent('FIXDEV-phone:sellCar')
AddEventHandler('FIXDEV-phone:sellCar', function(plate,amount,sId)
  sellCarPlate = plate
...
RegisterNUICallback('purchaseCar', function(data)
  TriggerServerEvent('FIXDEV-phone:purchasedCar', sellCarPlate, data.amount,data.senderID)
end)

RegisterNUICallback('purchase_gas', function(data)
  TriggerEvent('FIXDEV-fuel:OfferAccepted')
end)

RegisterNetEvent('FIXDEV-hud:send_fuel_bill', function(pCost)
...
function SpawnVehicle(vehicle, x,y,z, Fuel, customized, plate, IsViewing, enginehealth, bodyhealth)
  if DoesEntityExist(spawningVeh) then
	...
	Citizen.CreateThread(function()			
        ...
        exports['FIXDEV-vehicles']:SetVehicleIdentifier(spawningVeh, vid)
        ...
        if not IsViewing then    
            ...
            RPC.execute("FIXDEV-garages:states", "Out", plate, exports['FIXDEV-menu']:currentGarage(), pUpdatedFuel)
        end
    end)
end
...
RegisterNUICallback('sethGps', function(data)
 local house = exports["FIXDEV-housing"]:retrieveHousingTableMapped()
...
RegisterNUICallback('unlockHouses', function(data)
  ...
  if statuus == "Unlock" then
    --TriggerEvent('FIXDEV-phone:h-Lock', hId,"Lock")
  else
    --TriggerEvent('FIXDEV-phone:h-Lock', hId,"Unlock")
  end
  TriggerServerEvent('FIXDEV-phone:house:status',hId,statuus)
  Wait(200)
  TriggerEvent('updateUIHousing')
end)

RegisterNetEvent("FIXDEV-phone:h-Lock", function(hId, status)
  TriggerServerEvent("FIXDEV-phone:h-Lock:sv", hId, status)
...
RegisterNUICallback('editHousingOwn', function()
  -- if exports['FIXDEV-housing']:isNearProperty() then
    if exports["FIXDEV-housing"]:canEdit() then
      print("HELLO!")
      local isComplete, _propertyID, dist, zone = exports['FIXDEV-housing']:findClosetedPerson()
      TriggerEvent("FIXDEV-housing:editOptions",{action = "enter", propertyId = _propertyID})
    else
      SendNUIMessage({
...
RegisterNUICallback('sellOwnHouse', function(data)
  local price = RPC.execute('FIXDEV-phone:getHousePrice', data.hid)
end)

RegisterNUICallback('giveAccess', function(data)
  local complete = RPC.execute('FIXDEV-phone:GiveAccess', data.hid,data.cid)
...
RegisterNUICallback('houseAccess', function(data)
  ...
  local access = RPC.execute('FIXDEV-phone:getAccessHouse', data.hid)
  local data = {
      {
        title = "Close",
        action = "FIXDEV-housing:close_access",
        key = {},
    },
  }
  local player = {}
  for i,k in pairs(access) do
    data[#data + 1] = {
          ...
          key = {"lockdown"},
          -- action = "FIXDEV-ui:apartmentsContext"
          children = {
            {
                title = "Remove Key",
  ...
  exports["FIXDEV-contexts"]:showContext(data)
end)

AddEventHandler("FIXDEV-housing:close_access", function()
  exports["FIXDEV-contexts"]:hideContext()
end)

AddEventHandler('FIXDEV-ui:apartmentsContext', function(data)
  RPC.execute("FIXDEV-phone:removeKeys",data[2],data[1])
  exports["FIXDEV-ui"]:hideContext(data)
end)
...
function hasDecrypt2()
    if exports["FIXDEV-inventory"]:hasEnoughOfItem("vpnxj",1,false,true) then
      return true
    else
      return false
    end
...
function hasTrucker()
    if exports["FIXDEV-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      return true
    else
      return false
    end
...
function hasDecrypt()
    if exports["FIXDEV-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) or exports["FIXDEV-inventory"]:hasEnoughOfItem("decryptersess",1,false) or exports["FIXDEV-inventory"]:hasEnoughOfItem("decrypterfv2",1,false) and not exports["isPed"]:isPed("disabled") or exports["FIXDEV-inventory"]:hasEnoughOfItem(80,1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDevice()
    if exports["FIXDEV-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      (
      (exports["FIXDEV-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
      exports["FIXDEV-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
      exports["FIXDEV-inventory"]:hasEnoughOfItem("stolens8",1,false) or
      exports["FIXDEV-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
      exports["FIXDEV-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
      exports["FIXDEV-inventory"]:hasEnoughOfItem("assphone",1,false) or
      exports["FIXDEV-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
function hasRadio()
    if (exports["FIXDEV-inventory"]:hasEnoughOfItem("radio",1,false) or exports["FIXDEV-inventory"]:hasEnoughOfItem("pdradio",1,false)) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function openGui(isBurner, cid)
   ...
   TriggerEvent("FIXDEV-spawn:phonerefresh") --Refreshes jobs
  ...
  if hasPhone() then
    ...
    local near = exports['FIXDEV-housing']:isNearProperty()
    local house = RPC.execute("FIXDEV-phone:getHouses")
    ...
    SendNUIMessage({
      ...
      hasDevice = device, 
      hasDecrypt = decrypt, 
      pNum = RPC.execute("FIXDEV-phone:getPhoneNumber"),
...
function canCreateRace()
  local racingCreateUsbItem = exports["FIXDEV-inventory"]:GetInfoForFirstItemOfName("racingusb0")
...
function canOpenRaceApp()
  local racingUsbItem = exports["FIXDEV-inventory"]:GetInfoForFirstItemOfName("racingusb2")
...
function canJoinRaces()
  local hasRaceUsbAndAlias = exports["FIXDEV-racing"]:getHasRaceUsbAndAlias()
...
function closeGui()
  ...
  -- TriggerEvent('FIXDEV-phone:UpdateStatePhone')
  pPhoneOpen = false
  insideDelivers = false
end

-- RegisterNetEvent('FIXDEV-phone:UpdateStatePhone')
-- AddEventHandler('FIXDEV-phone:UpdateStatePhone', function()
--     Wait(5)
...
RegisterNUICallback('updateMyWallpaper', function(data, cb)
  wallPaper = ""
  Wait(5)
  TriggerEvent('FIXDEV-phone:grabBackground', data.name .."?auto=compress&cs=tinysrgb&h=350")
...
AddEventHandler('phone:casino_paid', function(message)
  if exports['FIXDEV-phone']:pOpen() == false then 
    SendNUIMessage({openSection = "emailnotify", pEHandle = 'Payment Received.', pEMessages = message})
    SendNUIMessage({openSection = "phonemedio", timeout = "5200", pOpen = exports['FIXDEV-phone']:pOpen()}) 
  end
...
RegisterNetEvent("phone:sendThePM", function(name, msg)
  if exports["FIXDEV-inventory"]:hasEnoughOfItem("thepmdongle",1,false,true) then
...
RegisterNetEvent('FIXDEV-spawn:confirm_spawned')
AddEventHandler('FIXDEV-spawn:confirm_spawned', function()
  serverSpawn = true
end)
...
RegisterNUICallback('jobcenter-setgps', function(data)	
  ...
  if data.item == "oxy" then	
    TriggerServerEvent("FIXDEV-deliveries:JobCenter")	
...
RegisterCommand('house', function()
  local isComplete, propertyID, dist, zone = exports['FIXDEV-housing']:findClosetedPerson()
  print("HOUSING",isComplete, propertyID, dist, zone)
end)
RegisterNUICallback('checkHouse', function()
  
 -- if exports["FIXDEV-housing"]:canEdit() then
    TriggerEvent('FIXDEV-housing:phone:check')
 -- else
 --   SendNUIMessage({
 --     openSection = "noProperty"
 --   })
...
RegisterNUICallback('leaveEditMode', function()
  print("LEAVING EDIT")
  TriggerEvent('FIXDEV-housing:editOptions', { action = "exit" })
end)
...
RegisterNUICallback('openFurnitureUI', function()
  ...
  TriggerEvent('FIXDEV-housing:editOptions', { action = "furniture" })
end)

RegisterNUICallback('setBackdoorUI', function()
  print("SET BACKDOOR")
  TriggerEvent('FIXDEV-housing:editOptions', { action = "backdoor" })
end)

RegisterNUICallback('setWardrobeUI', function()
  print("SET WARDROBE")
  TriggerEvent('FIXDEV-housing:editOptions', { action = "wardrobe" })
end)

RegisterNUICallback('setCraftingUI', function()
  print("SET CRAFTING")
  TriggerEvent('FIXDEV-housing:editOptions', { action = "crafting" })
end)

RegisterNUICallback('setGarageUI', function()
  print("SET GARAGE")
  TriggerEvent('FIXDEV-housing:editOptions', { action = "garage" })
end)

RegisterNUICallback('setStashUI', function()
  print("SET STASH")
  TriggerEvent('FIXDEV-housing:editOptions', { action = "stash" })
end)

RegisterNUICallback('sellsHouse', function()
  print("EDITING")
  TriggerEvent('FIXDEV-housing:phone:check',"edit")
end)

AddEventHandler('FIXDEV-phone:sentHouse', function(hasPrice,id,street,price,category)
...
AddEventHandler('FIXDEV-phone:editingHouse', function(id,street,price)
  print("EDITING HERE")
  SendNUIMessage({
...
RegisterNUICallback('purchaseHouse', function(data)
  ...
  if rentEnabled then
      print("can purchase")
   --   TriggerEvent('FIXDEV-phone:sentHouse')
      local success, msg = RPC.execute("FIXDEV-housing:buy", data.hid, data.price, 0, exports['isPed']:isPed("fullname"))
      TriggerEvent("DoLongHudText",msg,1)
     return
  end
...
-- AddEventHandler('FIXDEV-phone:noProperty', function()
...
RegisterNUICallback('editHousePrice', function(data)
  TriggerServerEvent('FIXDEV-housing:editPrice', data.hid,data.price)
end)

AddEventHandler('FIXDEV-phone:EnableSellRent', function()
    exports['FIXDEV-ui']:openApplication('textbox', {
    callbackUrl = 'FIXDEV-phone:SellRentEnable',
...
RegisterInterfaceCallback("FIXDEV-phone:SellRentEnable", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  print('THIS IS ENABLED RENT/SALE FOR PLAYER', data[1].value,json.encode(data))
  TriggerServerEvent('FIXDEV-housing:rent_activate_sv', data[1].value)
  exports["FIXDEV-ui"]:hideContextMenu()
  cb({ data = {}, meta = { ok = true, message = '' } })
end)
...
RegisterNetEvent('FIXDEV-phone:signInInfoTowing')
AddEventHandler('FIXDEV-phone:signInInfoTowing', function()
  phoneNotification("email", "Grab the towtruck parked in the bay.", "Job Center")
end)

RegisterNetEvent('FIXDEV-phone:towing_offer')
AddEventHandler('FIXDEV-phone:towing_offer', function()
...
RegisterNUICallback('accept_tow_job', function(data)
  TriggerEvent('FIXDEV-jobs:towing_accept')
end)

-- 24/7 Delivery --

RegisterNetEvent('FIXDEV-phone:signInInfoDelivery')
AddEventHandler('FIXDEV-phone:signInInfoDelivery', function()
  phoneNotification("email", "Signed in as 24/7 Delivery Driver.", "Job Center")
end)

RegisterNetEvent('FIXDEV-phone:delivery_offer')
AddEventHandler('FIXDEV-phone:delivery_offer', function()
...
RegisterNUICallback('accept_delivery_job', function(data)
    TriggerEvent('FIXDEV-jobs:deliveries:getCar')
end)

-- Dodo Logistics --

RegisterNetEvent('FIXDEV-phone:signInDodo')
AddEventHandler('FIXDEV-phone:signInDodo', function()
  phoneNotification("email", "Signed in as a Dodo Logistics worker.", "Job Center")
end)

RegisterNetEvent('FIXDEV-phone:dodoJobOffer')
AddEventHandler('FIXDEV-phone:dodoJobOffer', function()
...
RegisterNUICallback('accept_dodo_job', function(data)
    TriggerEvent('FIXDEV-jobs:spawn-truck')
end)

RegisterNetEvent('FIXDEV-phone:goToLocation')
AddEventHandler('FIXDEV-phone:goToLocation', function()
  phoneNotification("email", "Go to the delivery location on the GPS.", "Job Center")
end)

RegisterNetEvent('FIXDEV-phone:goBackDepo')
AddEventHandler('FIXDEV-phone:goBackDepo', function()
  phoneNotification("email", "Return back to the depo.", "Job Center")
end)

-- House Robberies --

RegisterNetEvent('FIXDEV-phone:signInInfoHouseRobbery')
AddEventHandler('FIXDEV-phone:signInInfoHouseRobbery', function()
  phoneNotification("email", "Signed in as Criminal.", "Job Center")
end)

RegisterNetEvent('FIXDEV-phone:house_rob_offer')
AddEventHandler('FIXDEV-phone:house_rob_offer', function()
...
RegisterNUICallback('accept_house_robbery', function(data)
    TriggerEvent('FIXDEV-activities:acceptJob')
end)

-- Chopshop --

RegisterNetEvent('FIXDEV-phone:signInInfoChopshop')
AddEventHandler('FIXDEV-phone:signInInfoChopshop', function()
  phoneNotification("email", "Signed in as Chopshop.", "Job Center")
end)

RegisterNetEvent('FIXDEV-phone:chop_shop_offer')
AddEventHandler('FIXDEV-phone:chop_shop_offer', function()
...
-- Oxy Runs --
RegisterNetEvent('FIXDEV-phone:oxyRunNoti')
AddEventHandler('FIXDEV-phone:oxyRunNoti', function(pText)
  phoneNotification("email", pText, "Job Center")
end)

RegisterNetEvent('FIXDEV-phone:oxyrun:offer')
AddEventHandler('FIXDEV-phone:oxyrun:offer', function()
...
RegisterNUICallback('accept_oxy_run', function(data)
    TriggerEvent('FIXDEV-oxy:Accept:Offer')
end)

-- Paycheck --

RegisterNetEvent('FIXDEV-phone:paymentNoti')
AddEventHandler('FIXDEV-phone:paymentNoti', function(pAmt, pType)
  phoneNotification("email", "$"..pAmt..".00 has been added to your "..pType.. " Balance", "Job Center")
end)

RegisterNetEvent('FIXDEV-phone:businessRegisterNoti')
AddEventHandler('FIXDEV-phone:businessRegisterNoti', function(pBusinessName, pText)
  phoneNotification("charge", pText, pBusinessName)
end)

-- Fishing --

RegisterNetEvent('FIXDEV-phone:signInFishing')
AddEventHandler('FIXDEV-phone:signInFishing', function()
