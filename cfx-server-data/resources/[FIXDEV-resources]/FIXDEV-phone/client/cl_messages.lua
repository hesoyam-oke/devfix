...
function loadSMS(cid)
  print("LOAD SMS")
  local messages, phone = RPC.execute("FIXDEV-phone:getSMS",cid)
  local _messages = {}
...
function loadSMSBurner(cid)
  print("LOAD SMS")
  local messages, phone = RPC.execute("FIXDEV-phone:getTextsFromNumber",cid)
  local _messages = {}
  for i, v in ipairs(messages) do
      local name = ""
...
local data = message
local app = "sms"
if exports["FIXDEV-phone"]:phasPhone() then
  if exports['FIXDEV-phone']:pNotify() == true then
    if inReadSMS then
