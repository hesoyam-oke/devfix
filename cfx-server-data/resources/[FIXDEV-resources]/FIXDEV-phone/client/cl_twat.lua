...
RegisterNUICallback('btnTwatter', function()
    ...
    local twattermessages = RPC.execute("FIXDEV-phone:getTwatters")
    -- if not getTwat then
...
RegisterNUICallback('newTwatSubmit', function(data, cb)
    ...
    TriggerServerEvent('FIXDEV-phone:twatSendNotification', handle, data, data.time)
...
RegisterNUICallback("FIXDEV-phone:getTwatter", function(data, cb)
  cb({ data = currentTwats, meta = { ok = success, message = (not success and currentTwats or 'done') } })
end)



RegisterNetEvent('FIXDEV-phone:insertTwat')
AddEventHandler('FIXDEV-phone:insertTwat', function(name,msg,time,attach)
  table.insert(currentTwats, {
    handle = name,
    message = msg,
    attachment = attach,
