...
$(document).ready(function () {
    ...
    $.post('http://FIXDEV-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            ...
            $.post('http://FIXDEV-phone/close', JSON.stringify({}));
        }
    }
    ...
    $(".phone-screen").on('click', '#wifi', function (e) {
        if (wifizone){
            if (enablePassword) {
                ...
                $.post('https://FIXDEV-phone/connectWifi', JSON.stringify({}));
            }
        }
    });
    setInterval(function () {
        ...
        if (actionButton !== undefined) {
            switch (actionButton) {
                case "home":
                    ...
                    $.post('https://FIXDEV-phone/refreshMySMS', JSON.stringify({status: false}));
                    $.post('https://FIXDEV-phone/closeTwat', JSON.stringify({}));
                    $.post('https://FIXDEV-phone/checkNotify', JSON.stringify({}));
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://FIXDEV-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://FIXDEV-phone/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://FIXDEV-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://FIXDEV-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        if (!isBurnah) {
                            $.post('http://FIXDEV-phone/' + action, JSON.stringify({}));
                        } else if (isBurnah) {
                            $.post('http://FIXDEV-phone/getRecentCallsByCid', JSON.stringify({
                                cid: burnahCid
                            }));
                        }
                    break;
                ...
                case "doj-app":
                    ...
                    $.post('https://FIXDEV-phone/getActiveDOJ', JSON.stringify({}))
                    break;
                case "camera-app":
                    $(".camera-inside").empty();
                    openContainer('camera')
                    $.post('https://FIXDEV-phone/getCurrentCameras', JSON.stringify({}))
                    break;
                case "lsbn":
                    openContainer('lsbn');
                    break;
                ...
                case "btnBankBusters":
                    ...
                    $.post('https://FIXDEV-phone/getCurrentBanks', JSON.stringify({}))
                    setTimeout(() => {
                        $.post('https://FIXDEV-phone/inBankBusterGroup', JSON.stringify({}))
                    }, 1500)
//                    $.post('https://FIXDEV-phone/getCurrentBanks', JSON.stringify({}))
                 //   $.post('https://FIXDEV-phone/sendNotifAlert', JSON.stringify({
                 //       type: "alert"
                 //   }))
                    break;
                case "documents":
                    ...
                    $.post('https://FIXDEV-phone/getDocuments', JSON.stringify({
                        pType: txt
                    }))
                    break;
                case "diamond":
                    ...
                    $.post('https://FIXDEV-phone/getDiamondSportsEvents', JSON.stringify({}))
                    break;
                default:
                    $.post('http://FIXDEV-phone/' + action, JSON.stringify({}));
                    break;
    ...
    $(".document-submit-cid").click(function (e) {
        ...
        setTimeout(() => {
            ...
            $.post('https://FIXDEV-phone/shareDocumentPermanent', JSON.stringify({
                documentID: pDocID,
                targetCID: ptCID
            }))        
        })
    ...
    $('.btn-share-local').click(function() {
        ...
        setTimeout(() => {
            let pDocID = document.getElementById("document-id").value;
            $.post('https://FIXDEV-phone/shareDocumentLocal', JSON.stringify({
                documentID: pDocID,
            }))        
        })
    })
    ...
    $('.btn-drop-qr-code').click(function() {
        ...
        setTimeout(() => {
            let pDocID = document.getElementById("document-id").value;
            $.post('https://FIXDEV-phone/dropQRCode', JSON.stringify({
                documentID: pDocID,
            }))        
        })
    })
    ...
    $('.btn-delete-document').click(function(){
        ...
        setTimeout(() => {
            ...
            $.post('https://FIXDEV-phone/deleteDocument', JSON.stringify({
...
function createDiamond(events, isEmployed) {
    $.each(events, function (__, event) {
        ...
        $('#delete-diamondEvent').click(function(){
            ...
            $.post('http://FIXDEV-phone/deleteDiamondEvent', JSON.stringify({
...
function createDOJDepartments(doj) {
    $.each(doj, function (__, dojs) {
        ...
        $('.dojAction-div').click(function () {
            ...
            $.post('http://FIXDEV-phone/callContact', JSON.stringify({
                name: '',
                number: number
            }));
        })
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.openPhone === true) {
            ...
            $.post('http://FIXDEV-phone/checkModel', JSON.stringify({}));

            $.post('http://FIXDEV-phone/retreivePhoneContacts', JSON.stringify({}));
...
//                $(".send-message").show();
        ...
        switch (item.openSection) {
            ...
            case "messageRead":
                ...
                $.post('https://FIXDEV-phone/refreshMySMS', JSON.stringify({status: true}));
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://FIXDEV-phone/race:completed', JSON.stringify({
...
function burnahMessages() {
    $.post('https://FIXDEV-phone/burnahMessages', JSON.stringify({
        cid: burnahCid
    }))
}
...
$('.btn-racing-clear').click(function() {
    $.post('http://FIXDEV-phone/racing:map:removeBlips', JSON.stringify({}));
});
...
function createCamera(cameras) {
    ...
    $(".eye-camera").click(function () {
        ...
        $.post('https://FIXDEV-phone/OpenCamera', JSON.stringify({
            cameraId: cameraId
        }));
    })
    ...
    $(".remove-camera").click(function() {
        ...
        $(".removeCamera").empty();
        $.post("https://FIXDEV-phone/getCameraAccessPlayers", JSON.stringify({
            cameraId: cameraId
        }))
    })
};
...
$("#add-camera-submit").click(function() {
    ...
    complateInputQuick()
    setTimeout(() => {
        $.post("https://FIXDEV-phone/addUserToCamera", JSON.stringify({
            cameraId: camid,
...
$("#remove-camera-submit").click(function() {
    ...
    setTimeout(() => {
        $("#cameraRemoveID").val("");
        $.post("https://FIXDEV-phone/removeUserFromCamera", JSON.stringify({
            cameraId: camid,
            targetPhoneNumber: cid
        }))
    }, 1500)
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://FIXDEV-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});
...
function gpsJobCenter(data) {
    $.post(
      'https://FIXDEV-phone/jobcenter-setgps',
...
">${moment.utc(emails[email].time).local().fromNow()}
    ...
    $.post('http://FIXDEV-phone/emailReads', JSON.stringify({}));
}
...
$('.diamondsports-createevent').on('click', '#submit-input', function(){ 
    ...
    $.post('http://FIXDEV-phone/submitNewEventDiamondSports', JSON.stringify({
...
function openBrowser(url) {
    ...
    $.post('http://FIXDEV-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function purchaseDarkMarket(data) {
    itemamount = 1
    $.post("https://FIXDEV-phone/purchase-darkMarket", JSON.stringify({item:data,itemamount}))
}
function trackvehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://FIXDEV-phone/vehtrack", JSON.stringify({vehplate}))
}

function spawnVehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://FIXDEV-phone/vehspawn", JSON.stringify({vehplate}))
}
...
$('.sellingvehicle').on('click', '#submit-input', function(){
    ...
    complateInput();
    setTimeout(() => {
        $.post("https://FIXDEV-phone/sellVeh", JSON.stringify({
            plate: vplate,
            paypal: paypal,
            amount: amount
        }))
...
$("#selfieclose").click(function () {
    $('.selfieBox').hide();
    $.post('http://FIXDEV-phone/closeSelfi', JSON.stringify({}));
})

$("#settings").click(function () {

  $.post('http://FIXDEV-phone/openSettings', JSON.stringify({}));
})
...
$('#submit-input-the-pm').on('click', function() {
    ...
    setTimeout(() => {
        $('#the-pm-input').val("")
        $.post("https://FIXDEV-phone/sendThePM", JSON.stringify({
            thePM: misinput
        }))
    }, 1900)
})
...
$("#contact-submit-input").click(function (e) {
    ...
    setTimeout(() => {
        ...
        $.post('http://FIXDEV-phone/newContactSubmit', JSON.stringify({
            name: clean,
...
$(document).ready(function () {
    ...
    $('.send-ping').click(function () {
        ...
        // $.post('http://FIXDEV-phone/btnCamera', JSON.stringify({}));
        $.post('http://FIXDEV-phone/FIXDEV-ui:pingSend', JSON.stringify({
            number: targetId
        }));
        $('.send-value').val('');
    });
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://FIXDEV-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate", controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://FIXDEV-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://FIXDEV-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://FIXDEV-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://FIXDEV-phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://FIXDEV-phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://FIXDEV-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://FIXDEV-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://FIXDEV-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://FIXDEV-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-edit-submit").click(function (e) {
    ...
    $.post('http://FIXDEV-phone/editContactSubmit', JSON.stringify({
        name: clean,
    ...
    setTimeout(() => {
        $.post('http://FIXDEV-phone/refreshContacts', JSON.stringify({}));
    }, 2000);
...
$("#wallpaper-form").submit(function (e) {
    ...
    $.post('http://FIXDEV-phone/updateMyWallpaper', JSON.stringify({
        name: escapedName,
    }));
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('https://FIXDEV-phone/stocksTradeToPlayer', JSON.stringify({
...
$('#stock-transfer').click(function(){
    ...
    complateInput()
    setTimeout(() => {
        $.post('https://FIXDEV-phone/exchangeCrypto', JSON.stringify({
...
$('#stock-submit').click(function(){
    ...
    complateInput()
    setTimeout(() => {
        $.post('https://FIXDEV-phone/buyCrypto', JSON.stringify({
            stockID: escapeHtml(stockID),
            amount: escapeHtml(amount),
        }));
    }, 1000);
...
//     event.preventDefault();
//     $.post('http://FIXDEV-phone/newTwatSubmit', JSON.stringify({
...
$(".submit-bg").click(function (event){
    if ($('#bg-set-body').val() == '') {
        ...
        $.post('http://FIXDEV-phone/submitBg', JSON.stringify({
            bg: bg
        }));
...
$('#message-submit').click(function (event){
    ...
    $.post('http://FIXDEV-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://FIXDEV-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            console.log('5543')
            ...
            setTimeout(function () {
                if (!isBurnah) {
                    $.post('http://FIXDEV-phone/messages', JSON.stringify({}));     
                } else if (isBurnah) {
                    $.post('http://FIXDEV-phone/getTextsByCid', JSON.stringify({
                        cid: burnahCid
                    }))
                }
            }, 300);
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://FIXDEV-phone/btnEvictHouse', JSON.stringify({}));
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://FIXDEV-phone/racing:map:cancel', JSON.stringify({}));
});
...
$('#racing-start-tracks').on('change', function (e) {
    ...
    if(maps[selectedMap] !== undefined) {
        $.post('http://FIXDEV-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://FIXDEV-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
//     $.post('http://FIXDEV-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://FIXDEV-phone/racing:map:save', JSON.stringify({
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://FIXDEV-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$("#phone-submit-number").click(function () {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            $.post("https://FIXDEV-phone/getCallHistory", JSON.stringify({}))
        }, 300)
    }, 900)
})
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://FIXDEV-phone/btnNotifyToggle', JSON.stringify({}));
});



$('.account-information-toggle-pager').click(function () {
    $.post('http://FIXDEV-phone/btnPagerToggle', JSON.stringify({}));
...
function clickImage(img) {
    $('.selfieBox').click(function(){
        copyToClipboard(img);
        $.post('http://FIXDEV-phone/closeSelfieBox', JSON.stringify({}));
    });
}

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://FIXDEV-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://FIXDEV-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://FIXDEV-phone/removeHouseKey', JSON.stringify({
...
$('.group-manage-entries').on('click', '.group-manage-fire', function () {
    ...
    $.post('http://FIXDEV-phone/FIXDEV-phone:fireEmp', JSON.stringify({ id: $(this).data('id'), group: $(this).data('group'), name: $(this).data('name') }));
});

$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://FIXDEV-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://FIXDEV-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://FIXDEV-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://FIXDEV-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://FIXDEV-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://FIXDEV-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://FIXDEV-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://FIXDEV-phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.jss1481, .calls-container').on('click', '.btn-contacts-call', function () {
    $.post('http://FIXDEV-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
...
$('#contacts-delete-submit').click(function (event) {
    ...
    complateInputQuick();
    setTimeout(() => {
        $.post('http://FIXDEV-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
    }, 1500)
});
...
$('.delete-yp').click(function (event) {
    ...
    setTimeout(() => {
        ...
        $('#youradhere').hide();
        hasPostedYP = false;
        $.post('http://FIXDEV-phone/deleteYP', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
    }, 1500)
});
...
$("#h-check").click(function () {
    ...
    $.post('http://FIXDEV-phone/checkHouse', JSON.stringify({}))
})

$("#edit-h").click(function () {
    // console.log("CHECK PROPERTY")
    // $.post('http://FIXDEV-phone/editHouse', JSON.stringify({}))
    //noPropty()
})

$("#sell-h").click(function () {
    // console.log("CHECK PROPERTY")
    $.post('http://FIXDEV-phone/sellsHouse', JSON.stringify({}))
})
...
$('.h-found').on('click', '.h-found-purchase', function (event) {
    ...
    complateInput();
    $('#twat-modal').modal('close');
    $.post('http://FIXDEV-phone/purchaseHouse', JSON.stringify({
...
$('.sell-house-own').on('click', '.confirm-sellHouse', function (event) {
    ...
    $.post('http://FIXDEV-phone/sellOwnHouse', JSON.stringify({
        hid: $('#h-p-id').val()
    }))
})
...
$('#submit-h-price').click(function(){
    ...
    $.post('http://FIXDEV-phone/editHousePrice', JSON.stringify({
        hid: hid,
        price: price
    }))
    $('.housing-edit-price').css('display', 'none');
...
function closePhoneShell() {
    ...
    $.post('https://FIXDEV-phone/refreshMySMS', JSON.stringify({ status: false }))
    $.post('http://FIXDEV-phone/closeTwat')
    $('#twat-modal').modal('close')
    $('#contacts-container').css('display', 'none')
    $('#contacts-container').css('display', 'none')
    $('#calculator-container').css('display', 'none')
...
$('.phone-selfie').click(function(){
    $.post('http://FIXDEV-phone/phone:selfie', JSON.stringify({}));
    closePhoneShell()
})
...
$(document).on('click', '#twat-sendimg', function (e) {
    ...
    $.post('http://FIXDEV-phone/phone:selfie', JSON.stringify({}));
    e.preventDefault();
    $.post('http://FIXDEV-phone/PostNewImage', JSON.stringify({}),
        function (url) {
...
//                 $.post('http://FIXDEV-phone/btnAnswer', JSON.stringify({}));
//             }
//         });
...
//                             setTimeout(() => {
//                                 Notif.addClass('notification-container-fade-out')
//                                 $.post('http://FIXDEV-phone/btnHangup', JSON.stringify({}));
//                             }, 1000)
//                         }, 200)
//                     }
//                     // Notif.addClass('notification-container-fade-out')
...
//             }
//             Notif.addClass('notification-container-fade-out')
//             $.post('http://FIXDEV-phone/pingEnded', JSON.stringify({}));
//             setTimeout(() => {
//                 Notif.remove()
//                 var count = 0
...
//             $.post('http://FIXDEV-phone/FIXDEV-ui:pingAccept', JSON.stringify({}));
//             const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
//                 if (Notif) {
//                     var count = 0
...
//             activePing = false
//             $.post('http://FIXDEV-phone/FIXDEV-ui:pingReject', JSON.stringify({}));
//             const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
//                 if (Notif) {
//                     var count = 0
...
$(".house-entries, .access-house-entries").on('click', '.h-setgps', function (e) {
    ...
    $.post('http://FIXDEV-phone/sethGps', JSON.stringify({hid: overid}));
})
...
$(".house-key-entries").on('click', '.h-removeKey', function (e) {
    ...
    $.post('http://FIXDEV-phone/whatthefuckdoesthisshitwork', JSON.stringify({cid: overid, hid: overid2}));
})
//a-signin
...
$('.a-signin').click(function(){
    if (cuunt <= 0) {
     ...
     $.post('http://FIXDEV-phone/abdulSignIn', JSON.stringify({status: taxiOn}));
 })
...
$(".house-entries, .access-house-entries").on('click', '.h-unlock', function (e) {
    ...
    $.post('http://FIXDEV-phone/unlockHouses', JSON.stringify({hid: overid, penisfuck: hstatus}));
 })
 ...
 $(".house-entries").on('click', '.h-edit-s', function (e) {
    ...
    $("#house-keys").hide();
    $("#house-2").hide();
    $.post('http://FIXDEV-phone/editHousingOwn', JSON.stringify({}));
 })
 ...
 $(".house-entries").on('click', '.h-access-keys', function (e) {
    ...
    $("#house-2").hide();
    $('.access-house-entries').css('display', 'none');
    $.post('http://FIXDEV-phone/btnKeyHolders', JSON.stringify({
        hid: overid
    }));
 ...
 $('.h-leaveedit').click(function(){
    editingHouse = false;
    $.post('http://FIXDEV-phone/leaveEditMode', JSON.stringify({}))
    $("#house-2").show();
    $("#house-3").hide();
    $("#house-keys").hide();
    $(".btn-2").show();
 ...
 $('.h-openfurniture').click(function(){
    $.post('http://FIXDEV-phone/openFurnitureUI', JSON.stringify({}))
 })

 $('.h-setbackdoor').click(function(){
    $.post('http://FIXDEV-phone/setBackdoorUI', JSON.stringify({}))
 })

 $('.h-setwardrobe').click(function(){
    $.post('http://FIXDEV-phone/setWardrobeUI', JSON.stringify({}))
 })

 $('.h-setcrafting').click(function(){
    $.post('http://FIXDEV-phone/setCraftingUI', JSON.stringify({}))
 })

 $('.h-setgarage').click(function(){
    $.post('http://FIXDEV-phone/setGarageUI', JSON.stringify({}))
 })

 $('.h-setstash').click(function(){
    $.post('http://FIXDEV-phone/setStashUI', JSON.stringify({}))
 })
 ...
 $('.submit-h-acess').click(function(){
    ...
    $('.phone-acces-house').hide()
    $.post('http://FIXDEV-phone/giveAccess', JSON.stringify({
        hid: hid,
        cid: cid
    }));
})
    ...
    $(document).ready(function () {
        $('.submit-settings').click(function(){
            ...
            $('.settings').hide()
                ...
                complateInput();
                $.post('http://FIXDEV-phone/submitSettings', JSON.stringify({
                    bg: bg,
                    model: model,
                    twat: twat,
                    call: call
...
function addDebtCars(vehData){ 
    ...
    $('.c-pawynow-button').click(function(){
        ...
        $.post("https://FIXDEV-phone/car_paid", JSON.stringify({
            plate:plate,
            payment:payment
        }))
        complateInputJustGreen();
        setTimeout(() => {
            $.post("https://FIXDEV-phone/btnDebt", JSON.stringify({}))
        }, 2000);
   })
