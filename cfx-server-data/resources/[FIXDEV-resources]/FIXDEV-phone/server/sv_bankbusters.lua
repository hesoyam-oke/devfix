...
RPC.register("FIXDEV-phone:addToCurrentBanks", function(pSource, pBank)
...
RPC.register("FIXDEV-phone:getCurrentBanks", function(pSource)
    return currentBanks
end)

RPC.register("FIXDEV-phone:requestPlayerToGroup", function(pSource, pCid, pGroupID)
...
RPC.register("FIXDEV-phone:addPlayerToGroup", function(pSource, pCid, pGroupID, pSenderID)
...
RPC.register('FIXDEV-phone:removePlayerFromGroup', function(pSource, pCid, pGroupID)
...
RPC.register('FIXDEV-phone:deleteGroup', function(source, pGroupID)
    table.remove(groupTable, groupTable.groupID)
end)

RPC.register('FIXDEV-phone:getMyGroupBankBusters', function(pSource, pCid)
    for k, v in pairs(groupTable) do
        for j, s in pairs(v.members) do
...
RPC.register('FIXDEV-phone:claimBank:BankBusters', function(source, bankID, StateID)
                        ...
                        exports.oxmysql:execute('SELECT * FROM _bankbusters_queue WHERE bankName = @bnkName', {
                            ...
                            if bbResult[1] ~= nil then
                                ...
                                for y,n in pairs(GetPlayers()) do
                                    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(tonumber(n))
...
RPC.register('FIXDEV-phone:getBankBustersCodesSlot', function(source, cid)
    local data = MySQL.query.await([[
