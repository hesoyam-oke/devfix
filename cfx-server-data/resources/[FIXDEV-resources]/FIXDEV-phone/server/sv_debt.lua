...
RPC.register('FIXDEV-phone:debt', function(source)
    local src = source
...
function buildInfo(house)
    ...
    for k,v in pairs(house) do
        ...
        local street = exports['FIXDEV-housing']:HouseStreet(v.hid)
...
RPC.register('FIXDEV-phone:debtPrice', function(source, id)
    local src = source
    local hId = id
    return getPrice(hId)
end)
...
RPC.register('FIXDEV-phone:car_payment', function(source, pl)
    local src = source
    local plate = pl
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local status
...
RPC.register('FIXDEV-phone:h_payment', function(source,id,pay)
    local src = source
    local hId = id
    local payment = pay
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
...
RPC.register('FIXDEV-phone:p_payment', function(source,id,p,did)
    local src = source
    local pId = id
    local dId = did
    local payment = p
...
RPC.register('FIXDEV-phone:addBill', function(source,id,a,t,c)
    local src = source
    local cid = id
    local amount = a
    local type = t
