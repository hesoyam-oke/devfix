...
RPC.register('FIXDEV-phone:getDocuments', function(pSource, pType)
...
RPC.register("FIXDEV-phone:sendDocumentEvent", function(pSource, pTarget, pDocumentID)
    TriggerClientEvent("FIXDEV-phone:sendDocumentEvent", pTarget, pDocumentID)
end)

RegisterNetEvent("FIXDEV-phone:checkForNonDocumentedLicenses", function(pCid)
    local sex = "sex"
...
RPC.register("FIXDEV-phone:shareDocumentPermanent", function(pSource, dID, tCID)
...
RPC.register("FIXDEV-phone:acceptDocumentReceiveRequest", function(pSource, pDocumentID)
    local p = promise.new()
    local doc = MySQL.query.await([[
        SELECT * FROM __documents WHERE id = ?
    ]], {pDocumentID})
...
RPC.register('FIXDEV-phone:deleteDocument', function(pSource, pId)
    exports.oxmysql:execute("DELETE FROM __documents WHERE id = ?",{pId})
end)

RPC.register('FIXDEV-phone:updateDocument', function(pSource, pId, pTitle, pText)
...
RPC.register("FIXDEV-phone:createDocument", function(pSource, pOwner, pTitle, pText, pType)
    MySQL.query.await([[
