...
RPC.register("FIXDEV-phone:getIdleGroup", function(source, cJob)
    local src = source
    local idle = {}
...
RPC.register("FIXDEV-phone:getBusyGroup", function(source, cJob)
    local src = source
    local busy = {}
    local g = MySQL.query.await([[
...
RPC.register('FIXDEV-phone:getGroupLeader', function(source)
...
RPC.register("FIXDEV-phone:getMyGroup", function(source)
    local src = source
    local myGroup = {}
    local g = MySQL.query.await([[
        SELECT * FROM job_group
...
RPC.register("FIXDEV-phone:c_group", function(source)
    local src = source    
...
RPC.register("FIXDEV-phone:getGroupsCount", function(source)
...
RPC.register("FIXDEV-phone:joinGroup_Accepted", function(source, pSrc, id, jCount)
    local src = source
    ...
    exports.oxmysql:execute("SELECT COUNT(group_id) FROM group_members WHERE group_id = ?", {gId}, function(result)
        if result ~= jCount then
            ...
            for k, v in pairs(GetPlayers()) do
                ...
                if exports['FIXDEV-phone']:GetGroupID(pSrc) ~= 0 then
                    currentGroupID = exports['FIXDEV-phone']:GetGroupID(pSrc)
                else
...
RPC.register("FIXDEV-phone:j_group_decline", function(source)
    local src = source
    for k,v in pairs(GroupInvite) do
        if(tonumber(v.StateID) == src) then
            GroupInvite[k] = nil
...
RPC.register("FIXDEV-phone:j_group", function(source, id, cid)
    local pSrc = source
    local user = exports['FIXDEV-base']:getModule('Player'):GetUser(source)
    local name = user:getCurrentCharacter().first_name .. ' '.. user:getCurrentCharacter().last_name
    ...
    exports.oxmysql:execute("SELECT src FROM job_group WHERE id = ?", {id}, function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent('FIXDEV-phone:CreateGroupInvite', tonumber(result[1].src), 'Group Request', name .. ' Accept?', pSrc, id)
...
RPC.register('FIXDEV-phone:setGroupStatus', function(source, status, charName)
...
RPC.register("FIXDEV-phone:get_groupsMem", function(source)
    local src = source
    local user = exports['FIXDEV-base']:getModule('Player'):GetUser(source)
    local cid = user:getCurrentCharacter().id
...
RPC.register('FIXDEV-phone:getGroupTask', function(source)
    local src = source 
    local user = exports['FIXDEV-base']:getModule('Player'):GetUser(source)
    local cid = user:getCurrentCharacter().id
...
RPC.register('FIXDEV-phone:updateGroupTask', function(source, tsk)
    local src = source 
    local user = exports['FIXDEV-base']:getModule('Player'):GetUser(source)
    local cid = user:getCurrentCharacter().id
    local groupinfo = groupInfo(cid) 
...
RPC.register('FIXDEV-phone:leave_group', function(source, id)
    local src = source
    local gId = id
    ...
    if getLeader[1] ~= nil then
        ...
        for k, v in pairs(GetPlayers()) do
            ...
            if exports['FIXDEV-phone']:GetGroupID(src) ~= 0 then
                currentGroupID = exports['FIXDEV-phone']:GetGroupID(src)
            else
                currentGroupID = exports['FIXDEV-phone']:GetMyGroupID(src)
            end
            ...
            exports.oxmysql:execute("SELECT members FROM job_group WHERE id = ?", {currentGroupID}, function(result)
                if result[1] ~= nil then
                    ...
                    for k,v in ipairs(jobMembers) do
                        TriggerClientEvent('refreshJobCenter', v)
                        TriggerClientEvent('FIXDEV-phone:CancelJob', v, 1)
                    end
        ...
        for k, v in pairs(GetPlayers()) do
            ...
            exports.oxmysql:execute("SELECT members FROM job_group WHERE id = ?", {currentGroupID}, function(result)
                if result[1] ~= nil then
                    ...
                    TriggerClientEvent('FIXDEV-phone:CancelJob', src, 0)
                    Wait(1000)
                    local affectedRows = MySQL.query.await([[
                        DELETE FROM group_members
                        WHERE src = ?
...
RPC.register('FIXDEV-phone:hasJobOffered', function(source) 
    -- We have an job ?  
    return true
end)
...
RPC.register('FIXDEV-phone:group_CheckOut', function(source)
    local pSrc = source
    local user = exports['FIXDEV-base']:getModule('Player'):GetUser(source)
    local job = user:getVar("job")
