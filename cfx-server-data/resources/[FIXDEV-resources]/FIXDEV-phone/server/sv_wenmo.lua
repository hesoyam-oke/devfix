RegisterNetEvent('FIXDEV-phone:transferWenmo', function(num,amount,comm)
    local src = source
    local zcid = getIdentifierByPhoneNumber(num)
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(tonumber(src))
...
RPC.register('FIXDEV-phone:getWenmo', function(source)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(tonumber(src))
