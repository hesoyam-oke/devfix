RegisterNUICallback('addEmployee', function(data)
  RPC.execute("FIXDEV-business:business:hireEmployee",data.state_id,data.rank,data.business)
end)

RegisterNUICallback('chargeCustomer', function(data)
  local success = RPC.execute('FIXDEV-phone:sendChargeCustomerNotif',data.GroupName, data.GroupID, data.stateID, data.amount, data.reason)
end)

RegisterNUICallback('editEmployee', function(data)
  RPC.execute("FIXDEV-business:phone:updateEmployee", data.state_id, data.business, data.rank)
end)

RegisterNetEvent("FIXDEV-phone:chargeCustomerNotif", function(data)
...
RegisterNUICallback('acceptBusinessCharge', function(data)
    RPC.execute("FIXDEV-phone:chargeCustomerBus", data.pAmount, data.pBID, data.pTargetID, data.pSenderID)
end)

RegisterNUICallback('fireEmployee', function(data)
    RPC.execute("FIXDEV-business:business:fireEmployee", data.state_id, data.business)
end)
...
RegisterNUICallback('payCheckEmployee', function(data)
    ...
    local complete, paid = RPC.execute('FIXDEV-phone:business_paycheck', business,cid,amount)
    if complete then
...
RegisterNUICallback('btnTaskGroups', function()
  ...
  local mypasses = RPC.execute("FIXDEV-business:phone:GetEmploymentInformation")
  local groupObject = {}

  for k,v in pairs(mypasses) do 
    local myRoles = RPC.execute("FIXDEV-business:phone:GetRolePermission", v.businessid, v.businessrole)
  ...
  AddEventHandler("group:fullList", function(bank,groupid)
    ...
    local fullGroupList = {}
    local peniashiut = RPC.execute("FIXDEV-business:hasPermission", groupid)
    local groupseemployyees = RPC.execute("FIXDEV-businesses:getRoleAndEmployeesList", groupid)
  ...
  RegisterNUICallback('openBusinessLogs', function(data)
    local businessLogs = RPC.execute("FIXDEV:getBusinessLogs", data.business)
  ...
  RegisterNUICallback('payExternal', function(data)
    local typee
    local success, msg = RPC.execute('FIXDEV-business:payExternal', data.pStateId, data.pAmount, data.pBusinessID)
    if success then
      typee = 1
    else
      typee = 2
  ...
  RegisterNUICallback('deleteRole', function(data)
    RPC.execute("FIXDEV-business:business:deleteRole", data.pBusinessID, data.pRole)
  end)

  RegisterNUICallback('createBusinessRole', function(data)
    RPC.execute("FIXDEV-business:business:createRole", data.pBusinessId, data.pRoleName, data.pPermHire, data.pPermFire, data.pPermChangeRoles, data.pPermKeys, data.pPermStash, data.pPermCraft, data.pPermCharge, data.pPermBank, data.pPermLogs)  
  end)

  RegisterNUICallback("editBusinessRole", function(data)
    RPC.execute("FIXDEV-business:business:editRole", data.pBusinessId, data.pCurRole, data.pRoleName, data.pPermFire, data.pPermHire, data.pPermChangeRoles, data.pPermKeys, data.pPermStash, data.pPermCraft, data.pPermCharge, data.pPermBank, data.pPermLogs)
  end)
  ...
  RegisterNUICallback('retreiveGroupRanks', function(data)
    ...
    local roles = RPC.execute("FIXDEV-business:business:GetBusinessRoles", data.bID)
  ...
  RegisterNUICallback('FIXDEV-phone:fireEmp', function(data,cb)
    TriggerServerEvent('FIXDEV-phone:fireEmp', data.id,data.group,data.name)
  end)

  RegisterNUICallback('addCashBank', function(data)
    local bid = data.bid
    local amount = data.amount
    local bank = RPC.execute("FIXDEV-phone:bus_addBank",bid,amount)
    if bank then
