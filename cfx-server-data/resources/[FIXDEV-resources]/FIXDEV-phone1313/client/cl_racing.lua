...
RegisterNUICallback('racing:event:start', function(data)
  local canJoinOrErr = exports["FIXDEV-racing"]:canJoinOrStartRace()
  if canJoinOrErr ~= true then
    exports["FIXDEV-phone"]:phoneNotification("racen",canJoinOrErr,"RACING", 3000)
    return
  end
  print(data.id, json.encode(data))
    exports["FIXDEV-racing"]:createPendingRace(data.id, data)
    Wait(100)
    SendNUIMessage({
...
RegisterNUICallback("racing:event:join", function(data)
  ...
  exports["FIXDEV-racing"]:joinRace(id, alias, cid)
  Wait(100)
  SendNUIMessage({
    openSection = "racing:event:update",
    canJoin = false,
...
RegisterNetEvent("FIXDEV-racing:removedActiveRaced")
AddEventHandler("FIXDEV-racing:removedActiveRaced", function(id)
  -- print("PUTANG INA")
    openRacingApp()
...
function openRacingApp()
  ...
  local alias = RPC.execute("GetAlias")
  print(alias)
  local _races = exports["FIXDEV-racing"]:getAllRaces()
...
AddEventHandler("FIXDEV-racing:api:updatedState", function()
...
RegisterNUICallback("racing:map:save", function(data)
  if creating then
      ...
      if veh ~= 0 and GetPedInVehicleSeat(veh, -1) == ped then
          ...
          TriggerEvent("FIXDEV-racing:cmd:racecreate", options)
          closeGui()
...
RegisterNUICallback('racing:map:cancel', function()
  creating = false
  TriggerEvent('FIXDEV-racing:cmd:racecreatecancel')
  openRacingApp()
end)
...
RegisterNUICallback('FIXDEV-phone:racing:setGPS', function(data)
  local mapID = data.mID
  exports["FIXDEV-racing"]:locateRace(mapID)
  phoneNotification("racen","GPS Set to your map.","RACING")
end)

RegisterNUICallback('FIXDEV-phone:racing:seePreview', function(data)
  local mapID = data.mID
  if not mapID then return end
  id = tostring(mapID)
  exports["FIXDEV-racing"]:previewRace(mapID)
...
RegisterNUICallback('FIXDEV-phone:r-users', function(data, cb)
  ...
  if race ~= nil then
    ...
    local racer = RPC.execute('FIXDEV-phone:racing:getRacer', raceID)
    cb(racer)
  end
  print(json.encode(newPendingRace[raceID]))
end)

AddEventHandler("FIXDEV-racing:api:startingRace", function(startTime)
  TriggerEvent("DoLongHudText", "Starting race in " .. tostring(startTime / 1000) .. " seconds")
end)

AddEventHandler("FIXDEV-racing:api:playerJoinedYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " joined your race")
end)

AddEventHandler("FIXDEV-racing:api:playerLeftYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " left your race")
end)

RegisterNetEvent('FIXDEV-racing:pendingRacenotif')
AddEventHandler("FIXDEV-racing:pendingRacenotif", function(race)
  ...
  exports["FIXDEV-phone"]:phoneNotification("racen","Pending race available...","From the PM", 12000)
end)
-- Citizen.CreateThread(function()
--   while true do
--     Wait(5000)
