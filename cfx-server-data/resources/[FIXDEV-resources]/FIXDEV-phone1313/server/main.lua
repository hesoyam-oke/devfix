...
RPC.register("FIXDEV-phone:getPhoneNumber", function(pSource)
...
RPC.register("FIXDEV-phone:sendThePM", function(src, msg)
...
RPC.register("acceptBusinessCharge", function(pSource, pBID, pAmt, pCID)
    ...
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(SID)
    local char = user:getCurrentCharacter()
...
RegisterNetEvent("FIXDEV-phone:h-Lock:sv", function(hid, status)
    print("HELLO? " .. " " .. hid .. " " ..status)
    exports.oxmysql:execute("UPDATE housing SET status = ? WHERE hid = ?",{status,hid})
end)

RPC.register("FIXDEV-financials:getCurrentBank", function(pSource)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("FIXDEV-apartments:getCurrentApartment", function(src)
...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
  ...
  local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(sourcePlayer)
...
--     local src = source
--     local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(tonumber(id))
--     local char = user:getCurrentCharacter()
  ...
  exports.oxmysql:execute("SELECT * FROM character_passes WHERE cid = @cid", {['cid'] = ply.id}, function(result)
      if result[1] ~= nil then
          ...
          if result[1].pass_type == "police" or result[1].pass_type == "ems" then
              ...
              TriggerClientEvent("FIXDEV-jobmanager:playerBecameJob", src, result[1].pass_type, result[1].pass_type, false)
          end
      end
  end)
end)
...
RegisterServerEvent("FIXDEV-phone:grabWallpaper")
AddEventHandler("FIXDEV-phone:grabWallpaper", function()
  ...
  local cid = char.id
    exports.oxmysql:execute("SELECT `wallpaper` FROM characters WHERE id = @id",{['id'] = cid}, function(data)
      if data[1].wallpaper ~= nil then
          TriggerClientEvent('FIXDEV-phone:grabBackground', src, data[1].wallpaper)
        --   print('Grabbbing',data[1].wallpaper)
      else
          TriggerClientEvent('FIXDEV-phone:grabBackground', src, "https://i.imgur.com/JVPavDg.jpg")
        --   print('Not Grabbbing')
      end
    end)
end)

RegisterServerEvent("FIXDEV-phone:GiveCrypto")
AddEventHandler("FIXDEV-phone:GiveCrypto", function(type,amount)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("FIXDEV-phone:getChips", function(pSource)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("FIXDEV-phone:getGroupSanitation", function(pSource)
    local src = source
    -- local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    -- local char = user:getVar("character")
    -- local id = char.id
...
RPC.register("FIXDEV-phone:incomingCrypto", function(pSource)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
...
RPC.register("FIXDEV-phone:blueprintid", function(pSource)
    return BPid
end)

RPC.register("FIXDEV-phone:exchangeCrypto", function(pSource,type,amount)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("FIXDEV-phone:transferCrypto", function(pSource,type,amount,target)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("FIXDEV-phone:GetShungite", function (pSource)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("FIXDEV-phone:GetTgb", function (pSource)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("FIXDEV-phone:GetDvd", function (pSource)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("FIXDEV-phone:buyCrypto", function (pSource,ctype,amount)
    local src = source
    local amount = amount
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
...
RPC.register("FIXDEV-phone:purchaseDarkMarket", function (pSource,type,price)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local id = char.id
...
RPC.register('FIXDEV-phone:getMyGroup', function(cid)
...
RPC.register('FIXDEV-phone:getHouses', function(src)
...
RPC.register('FIXDEV-phone:apt', function(source)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
...
RPC.register('FIXDEV-phone:getCurrentOwned', function(source)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local house
...
RegisterNetEvent('FIXDEV-phone:house:status')
AddEventHandler('FIXDEV-phone:house:status', function(hid,status)
     local src = source
...
RPC.register("FIXDEV-housing:getKeyHolderz", function(src,hid)
    print(src,hid)
    local myKeyz = {}
...
RPC.register("FIXDEV-housing:removeKey:phone", function(src,cid,hid)
    local keys = MySQL.query.await([[
...
RPC.register('FIXDEV-phone:getHousePrice', function(source,hid)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local hIds = hid
...
RPC.register('FIXDEV-phone:GiveAccess', function(source,hid,cid)
    local src = source
    local hIds = hid
    local cid = cid
    local access
...
-- RPC.register('FIXDEV-phone:getAccessHouse', function(source)
--     local src = source
--     local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
--     local cid = user:getCurrentCharacter().id
--     local house
...
RPC.register('FIXDEV-phone:getAccessHouse', function(source,house)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local hid = house
...
RPC.register('FIXDEV-phone:getAccessHouse_2', function(house)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local hid = house
...
RPC.register("FIXDEV-phone:addDOJ", function(pSource, pJob)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("FIXDEV-phone:removeDOJ", function(pSource, pJob)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("FIXDEV-phone:getDOJ", function(src)
    return availableDOJ
end)

local availableDrivers = {}

RPC.register("FIXDEV-phone:addAbdulTaxi", function(src)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    local phonenumber = char.phone_number
...
RPC.register("FIXDEV-phone:getAbdulTaxies", function(src)
    return availableDrivers
end)

RegisterNetEvent("FIXDEV-phone:getAbdulTaxies", function()
    TriggerClientEvent("FIXDEV-phone:manualUpdateAbdulTaxies", source)
end)

RPC.register("FIXDEV-phone:removeAbdulTaxi", function(src)
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("FIXDEV-phone:removeKeys", function(src, pHouseId, pPlayerId)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
...
-- RPC.register("FIXDEV-garages:states",function(pSource,state,plate,curGarage,fuel)
--     local src = source
--     local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RPC.register("FIXDEV-phone:Groups", function(source,gIp)
--     local src = source
...
RegisterServerEvent('FIXDEV-phone:sendVehBill')
AddEventHandler('FIXDEV-phone:sendVehBill', function(paypal,plate,amount)
    src = source
    TriggerClientEvent('FIXDEV-phone:sellCar', paypal, plate, amount,src)
end)


RPC.register('FIXDEV-phone:sellVehicle', function(pSource,pl,py,am)
    local src = pSource
    local plate = pl
    local paypal = py
    local amount = am
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local player = exports["FIXDEV-base"]:getModule("Player"):GetUser(paypal)
    local ply = user:getCurrentCharacter()
...
RegisterServerEvent('FIXDEV-phone:purchasedCar')
AddEventHandler('FIXDEV-phone:purchasedCar', function(plate,amount,sellerID)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local player = exports["FIXDEV-base"]:getModule("Player"):GetUser(sellerID)
...
RegisterNetEvent('FIXDEV-phone:send_gas_bill')
AddEventHandler('FIXDEV-phone:send_gas_bill', function(targetId, pFuelCost)
    local pSrc = source
    
	if targetId ~= nil then
        TriggerClientEvent('FIXDEV-hud:send_fuel_bill', targetId, pFuelCost, pSrc)
    end
end)
