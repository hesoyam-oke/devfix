...
RegisterNetEvent('FIXDEV-phone:fireEmp')
AddEventHandler('FIXDEV-phone:fireEmp', function(id, job, name)
  local src = source
...
RPC.register("FIXDEV-phone:sendChargeCustomerNotif", function(pSource, BusinessName, GroupID, stateID, amount, reason)
    local tSrc = getPlayerFromCidForCharge(tonumber(stateID))
    local tax = exports['FIXDEV-business']:taxTotal(amount)
    ...
    TriggerClientEvent("FIXDEV-phone:chargeCustomerNotif", tSrc, { 
...
RPC.register("FIXDEV-phone:chargeCustomerBus", function(pSource, amount, GroupID, stateID, sID)
    local amtTax = amount
    local tSrc = getPlayerFromCidForCharge(tonumber(stateID))
    local user = exports['FIXDEV-base']:getModule("Player"):GetUser(tSrc)
    local char = user:getCurrentCharacter()
    ...
    if bank >= tonumber(amtTax) then
        ...
        TriggerClientEvent("FIXDEV-phone:businessRegisterNoti", sID, "SERVICE CHARGE", "They couldn't accept the payment")
        TriggerClientEvent("FIXDEV-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "Not enough money to complete the payment.")
        return
    end
    TriggerClientEvent("FIXDEV-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "Payment Successful.")
    TriggerClientEvent("FIXDEV-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "You have been deducted " .. amtTax .. " from you're bank account")
    TriggerClientEvent("FIXDEV-phone:businessRegisterNoti", sID, "SERVICE CHARGE", "They accepted the payment")
    return
end)
...
RegisterServerEvent('FIXDEV-phone:business_hired', function(StateID, Rank, pBusiness)
...
RegisterServerEvent('FIXDEV-phone:business_edit', function(pRank, pStateID, pPassType)
...
RPC.register("FIXDEV:getBusinessLogs", function(pSource, pBusiness)
...
RPC.register('FIXDEV-phone:business_paycheck', function(source,bus,p,a)
    local src = source
    local bId = bus
    local cid = p
    local amount = a
...
function getPlayerFromCid(zcid, amountAdded)
    for _, player in ipairs(GetPlayers()) do
        local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(tonumber(player))
...
RPC.register("FIXDEV-business:payExternal", function(pSource, pCid, pAmount, pBusinessID)
...
RPC.register('FIXDEV-phone:bus_addBank', function(source,id,a)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local ply = user:getCurrentCharacter()
    local bId = id
...
RegisterServerEvent('FIXDEV-phone:business_fire', function(pBusinessName, pStateID)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
