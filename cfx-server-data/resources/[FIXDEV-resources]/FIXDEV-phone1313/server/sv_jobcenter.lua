...
RPC.register("FIXDEV-phone:getIdleGroup", function(source)
    local src = source
    local idle = {}
...
RPC.register("FIXDEV-phone:j_group", function(source, id)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
...
RPC.register("FIXDEV-phone:get_groupsMem", function(source, id)
...
RPC.register('FIXDEV-phone:g_ready', function(source,stat,id)
    local src = source
    local status = stat
    local gid = id
...
--         end
--       end
--     --   TriggerLatentClientEvent('mhFIXDEV-housing:collectHouses', -1, 30000, Houses)
--     end)
-- end)