...
AddEventHandler('Tweet', function(handle, data, time)
  ...
  TriggerClientEvent('FIXDEV-phone:insertTwat', -1, handle,data.text,time,data.attachment)
  -- TriggerClientEvent('Client:UpdateTweet1', src, newtwat,allTweet)
 
end)

RegisterNetEvent('FIXDEV-phone:twatSendNotification')
AddEventHandler('FIXDEV-phone:twatSendNotification', function(handle, data, time)

  local newtwat = { ['handle'] = handle, ['message'] = data, ['time'] = time}
  TriggerClientEvent('FIXDEV-phone:TwatNotify', -1, newtwat)
end)

RegisterNetEvent('FIXDEV-phone:openGetTweets')
AddEventHandler('FIXDEV-phone:openGetTweets', function()
  local src = source
  exports.oxmysql:execute('SELECT * FROM (SELECT * FROM tweets ORDER BY `time` DESC LIMIT 50) sub ORDER BY time ASC', {}, function(tweets) -- Get most recent 100 tweets
...
RPC.register('FIXDEV-phone:getTwatters', function(source)
...
RPC.register("FIXDEV-phone:twitterhandle", function(pSource)
  local src = source
  local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
