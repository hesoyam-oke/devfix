    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerEvent('FIXDEV-hud:hide_ping')
                TriggerServerEvent('FIXDEV-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('FIXDEV-ping:client:SendPing')
AddEventHandler('FIXDEV-ping:client:SendPing', function(sender, senderId, pCoords)
    if pendingPing == nil then
        ...
        TriggerServerEvent('FIXDEV-ping:server:SendPingResult', pendingPing.id, 'received')
        TriggerEvent('DoLongHudText', pendingPing.name .. ' Sent You a Ping, Use /ping accept To Accept', 1)
        TriggerEvent('FIXDEV-hud:show_ping')
        isPending = true
        ...
        TriggerServerEvent('FIXDEV-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('FIXDEV-ping:client:AcceptPing')
AddEventHandler('FIXDEV-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerEvent('FIXDEV-hud:hide_ping')
        TriggerServerEvent('FIXDEV-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('FIXDEV-ping:client:RejectPing')
AddEventHandler('FIXDEV-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('FIXDEV-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('FIXDEV-ping:client:RemovePing')
AddEventHandler('FIXDEV-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
...
RegisterCommand('ping', function(src, args, raw)
    TriggerServerEvent("FIXDEV-ping:attempt",  GetEntityCoords(PlayerPedId()), args[1])
end)
