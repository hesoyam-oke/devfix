...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("void_extra_spot", vector3(443.78, -988.09, 25.7), 30, 34.6, {
...
RegisterNetEvent('FIXDEV-police:bill_ply')
AddEventHandler('FIXDEV-police:bill_ply', function(pAmt, pJob)
	TriggerServerEvent('FIXDEV-banking:business_money', pAmt, pJob, 'add')
end)
...
AddEventHandler('evidence:dnaSwab', function()
    local finished = exports["FIXDEV-taskbar"]:taskBar(8000,"DNA Testing")
...
AddEventHandler('revive', function(t)
	...
	if(distance ~= -1 and distance < 10) then
		TriggerServerEvent("FIXDEV-death:reviveSV", GetPlayerServerId(t))
		KneelMedic()
...
-- 		local finished = exports["FIXDEV-taskbar"]:taskBar(4000,"Grabbing Scuba Gear")
-- 	  	if finished == 100 then
-- 	  		loadAnimDict('anim@narcotics@trash')
...
AddEventHandler('police:gsr', function(t)
	...
	local finished = exports["FIXDEV-taskbar"]:taskBar(15000,"GSR Testing")
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
...
AddEventHandler('police:giveweaponlicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("FIXDEV-base:giveLicenses", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:removeweaponlicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("FIXDEV-base:removeLicenses", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:givehuntinglicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("FIXDEV-base:giveLicenseshunt", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:removehuntinglicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("FIXDEV-base:removeLicenseshunt", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:givefishinglicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("FIXDEV-base:giveLicensesfish", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:removeLicensesfish', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("FIXDEV-base:removeLicensesfish", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:uncuffMenu', function()
	...
	if not IsPedInAnyVehicle(PlayerPedId(), false) then
		if(distance ~= -1 and distance < 2) then
			...
			local finished = exports['FIXDEV-taskbar']:taskBar(3000, 'Uncuffing')
			if finished == 100 then
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	local finished = 0
	if not exports['FIXDEV-death']:GetDeathStatus() then
		local taskbar = exports["FIXDEV-ui"]:taskBarSkill(math.random(950, 1100), math.random(13, 15))
		if taskbar == 100 then
...
AddEventHandler('police:jailing2', function(pPlayer, time)
	...
	TriggerEvent('FIXDEV-base:setEverything')
...
AddEventHandler('police:forceEnter', function(id)
	...
	if(distance ~= -1 and distance < 3) then
		...
		if not isInVeh then
			...
			local SeatPly = exports['FIXDEV-taskbar']:taskBar(2000, 'Seating In Vehicle')
			if SeatPly == 100 then
...
Citizen.CreateThread(function() 
  ...
  while true do
    ...
    if handCuffedWalking or handCuffed then
		...
		DisableControlAction(1, 37, true) --Disables INPUT_SELECT_WEAPON (tab) Actions
		DisablePlayerFiring(PlayerPedId(), true) -- Disable weapon firing
		if (not IsEntityPlayingAnim(PlayerPedId(), "mp_arresting", "idle", 3) and not exports['FIXDEV-death']:GetDeathStatus()) or (IsPedRagdoll(PlayerPedId()) and not exports['FIXDEV-death']:GetDeathStatus()) then
		...
		if exports['FIXDEV-death']:GetDeathStatus() then
			Citizen.Wait(1000)
		end
    end
...
AddEventHandler('unseatPlayer', function()
	...
	if(distance ~= -1 and distance < 10) then
		...
		local finished = exports['FIXDEV-taskbar']:taskBar(2000, 'Unseating From Vehicle')
          if finished == 100 then
...
Citizen.CreateThread(function()
	while true do
		...
		if dragging then
			...
			if exports['FIXDEV-death']:GetDeathStatus() or IsControlJustPressed(0, 38) or (`WEAPON_UNARMED` ~= GetSelectedPedWeapon(PlayerPedId())) then
				dragging = false
				ClearPedTasks(PlayerPedId())
...
AddEventHandler('FlipVehicle', function()
	...
	local finished = exports["FIXDEV-taskbar"]:taskBar(12000,"Flipping Vehicle Over",false,true)	
	if finished == 100 then
...
AddEventHandler('impoundsc', function()
    ...
    targetVehicle = getVehicleInDirection(coordA, coordB)
	licensePlate = GetVehicleNumberPlateText(targetVehicle)
	TriggerServerEvent("FIXDEV-imp:ImpoundCar",licensePlate)
	deleteVeh(targetVehicle)
end)
...
AddEventHandler('police:stealrob', function()
	if not exports['FIXDEV-death']:GetDeathStatus() then
		...
		local finished = exports["FIXDEV-taskbar"]:taskBar(15000,"Robbing",false,true)	
...
RegisterCommand('livery', function(source, args, raw)
  ...
  if isPolice or isState or isSheriff or isRanger or isEMS and GetVehicleLiveryCount(vehicle) - 1 >= tonumber(args[1]) then
...
RegisterCommand("serial", function(source, args)
	...
	if dist <= 2.0 then 
		if isPolice or isState or isSheriff or isRanger then
			...
			exports["FIXDEV-taskbar"]:taskBar(20000,"Checking Serial")
			end
		else
...
AddEventHandler('police:tenThirteenA', function()
	...
	if myJob == 'police' or myJob == 'ems' or myJob == 'state' or myJob == 'sheriff' or myJob == 'ranger' then	
		...
		exports["FIXDEV-dispatch"]:policedead("10-13A", "Officer Down URGENT!", icon)
	end
end)
...
AddEventHandler('police:tenThirteenB', function()
	...
	if myJob == 'police' or myJob == 'ems' or myJob == 'state' or myJob == 'sheriff' or myJob == 'ranger' then	
		...
		local icon = 189 -- https://docs.fivem.net/docs/game-references/blips/
		exports["FIXDEV-dispatch"]:policedead("10-13B", "Officer Down", icon)
	end
end)
...
AddEventHandler('police:policePanic', function()
	...
	if myJob == 'police' or myJob == 'ems' or myJob == 'state' or myJob == 'sheriff' or myJob == 'ranger' then	
		...
		local icon = 189 -- https://docs.fivem.net/docs/game-references/blips/
		exports["FIXDEV-dispatch"]:policedead("10-78", "Panic Button", icon)
	end
end)
...
AddEventHandler("police:tenForteenA", function()
	...
	local pos = GetEntityCoords(PlayerPedId(),  true)
		...
		local icon = 189 -- https://docs.fivem.net/docs/game-references/blips/
		exports["FIXDEV-dispatch"]:policedead("10-14A", "Medic Down URGENT!", icon)
	end
end)
...
AddEventHandler("police:tenForteenB", function()
	...
	if myJob == 'police' or myJob == 'ems' or myJob == 'state' or myJob == 'sheriff' or myJob == 'ranger' then	
		...
		local icon = 189 -- https://docs.fivem.net/docs/game-references/blips/
		exports["FIXDEV-dispatch"]:policedead("10-14B", "Medic Down", icon)
	end
end)
...
AddEventHandler('ems:panicbutton', function()
	...
	if myJob == 'ems' then
		...
		local icon = 189 -- https://docs.fivem.net/docs/game-references/blips/
		exports["FIXDEV-dispatch"]:policedead("10-78", "EMS Panic Button", icon)
	end
end)
...
Citizen.CreateThread(function()
	exports["FIXDEV-binds"]:registerKeyMapping("", "Vehicle", "Cruise Control", "+vehCruise", "-vehCruise", "X")
	RegisterCommand('+vehCruise', vehCruise, false)
	RegisterCommand('-vehCruise', function() end, false)

	exports["FIXDEV-binds"]:registerKeyMapping("", "Player", "Tackle", "+plyTackel", "-plyTackel")
	RegisterCommand('+plyTackel', plyTackel, false)
	RegisterCommand('-plyTackel', function() end, false)

	exports["FIXDEV-polytarget"]:AddBoxZone("pd_badge", vector3(484.85, -999.71, 30.69), 0.5, 0.8, {
        heading=0,
        minZ=27.54,
        maxZ=31.09
    })
        exports["FIXDEV-interact"]:AddPeekEntryByPolyTarget("pd_badge", {{
...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("void_evidence_spot", vector3(473.95, -994.79, 26.27), 6, 4.5, {
...
AddEventHandler('FIXDEV-polyzone:enter', function(name)
    if name == "void_evidence_spot" then
        ...
        if job == "police" or job == "sheriff" or job == "state" or job == "ranger" then
			...
			exports['FIXDEV-ui']:showInteraction('/evidence [number]')
        end
    end
end)
...
RegisterCommand("evidence", function(source, args)
	...
	if job == 'police' or job == 'state' or job == 'sheriff' or job == 'ranger' then
		if isNearvoidPDEvidence then
			...
			TriggerServerEvent('FIXDEV-police:log_evidence_lockers', args[1])
		else
	...
	local Police = {
		{
			...
			key = "PD",
			action = 'FIXDEV-police:lock',
		},
		{
			title = "Clothing Shop",
			description = "Get the PD Drip",
			key = "PD",
			action = 'FIXDEV-police:cloth',
		}
	}
	exports["FIXDEV-ui"]:showContextMenu(Police)
end)

RegisterUICallback('FIXDEV-police:lock', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('void-personal-pd-locker')
end)

RegisterUICallback('FIXDEV-police:cloth', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('FIXDEV-police:clothing_menu')
end)

RegisterNetEvent('FIXDEV-police:clothing_menu')
AddEventHandler('FIXDEV-police:clothing_menu', function()
...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("void_mrpd_clothing_or_locker", vector3(461.5, -997.7, 30.69), 4.9, 5.0, {
...
AddEventHandler('FIXDEV-polyzone:enter', function(name)
    if name == "void_mrpd_clothing_or_locker" then
        ...
        if job == "police" or job == "sheriff" or job == "state" or job == 'ranger' then
			...
			exports['FIXDEV-ui']:showInteraction('[E] Police Changing Room')
        end
    end
end)
...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("police_armory_void_rp", vector3(482.66, -995.69, 31.67), 2, 1.6, {
...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("void_state_hq_locker", vector3(361.75, -1593.02, 25.45), 3, 2.6, {
...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("police_armory_void_rp_2", vector3(364.16, -1603.62, 25.45), 2, 2, {
...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("police_armory_sandy", vector3(1862.46, 3690.73, 34.22), 3, 1.1, {
        name="police_armory_void_rp_2",
		heading=30,
		--debugPoly=true,
		minZ=32.02,
...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("metaldetect_court:1", vector3(241.3, -1079.17, 29.29), 1.0, 1.2, {
	...
	exports["FIXDEV-polyzone"]:AddBoxZone("metaldetect_court:2", vector3(245.65, -1079.15, 29.29), 1.0, 1.0, {
...
Citizen.CreateThread(function()
    exports["FIXDEV-polyzone"]:AddBoxZone("void_courthouse_guy", vector3(244.88, -1075.6, 29.29), 1, 1, {
...
AddEventHandler('FIXDEV-polyzone:enter', function(name)
    if name == "void_courthouse_guy" then
		...
		VoidCourthouse()
		exports['FIXDEV-ui']:showInteraction('[E] Collect Items')
    end
end)
...
RegisterNetEvent('FIXDEV-police:fake_id')
AddEventHandler('FIXDEV-police:fake_id', function()
	exports['FIXDEV-ui']:openApplication('textbox', {
		callbackUrl = 'FIXDEV-police:create_id',
		key = 1,
		items = {
		{
			icon = "id-card-alt",
...
RegisterUICallback('FIXDEV-police:create_id', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
	local information = {
...
RegisterNetEvent('FIXDEV-police:id_make_yes')
AddEventHandler('FIXDEV-police:id_make_yes', function()
	TriggerServerEvent('FIXDEV-police:make_id')
end)

-- RegisterNetEvent('FIXDEV-police:requestimpound')
-- AddEventHandler('FIXDEV-police:requestimpound', function()
-- 		TriggerEvent('DoShortHudText', 'You have requested an impound')
-- 		TriggerServerEvent('FIXDEV-police:requestimpound')
-- end)

-- RegistrerNetEvent('FIXDEV-police:requestimpound_yes')
-- AddEventHandler('FIXDEV-police:requestimpound_yes', function()
-- 		TriggerServerEvent('FIXDEV-police:requestimpound_yes')
-- 		local pd_tow = exports["FIXDEV-duty"]:LawAmount()
--         if pd_tow >= 1 then
...
AddEventHandler('police:RifleRack', function()
	...
	if exports["isPed"]:isPed("myJob") == 'police' or exports["isPed"]:isPed("myJob") == 'sheriff' or exports["isPed"]:isPed("myJob") == 'ranger' or exports["isPed"]:isPed("myJob") == 'state' then
		local finished = exports["FIXDEV-taskbar"]:taskBar(1000,"Unlocking rifle rack..",false,true)
		if finished == 100 then
...
RegisterNetEvent('FIXDEV-police:get_percent')
AddEventHandler('FIXDEV-police:get_percent', function(pNum)
