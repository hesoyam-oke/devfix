...
AddEventHandler('police:targetCheckBank', function(target)
	local src = source
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler("police:targetCheckInventory", function(target, status)
	local src = source
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('police:jailGranted', function(args)
	...
	local target = exports["FIXDEV-base"]:getModule("Player"):GetUser(player)
	local character = target:getCurrentCharacter()
	local playerName = character.first_name .. ' ' .. character.last_name

	local pdunit = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
	...
	--TriggerEvent("FIXDEV:news:newConviction", {name = playerName, duration = time, charges = reason})

	exports["FIXDEV-base"]:getModule("JobManager"):SetJob(target, "unemployed", true)
end)
...
AddEventHandler('police:dnaAsk', function(t)
	local src = source
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(t)
...
AddEventHandler("911a", function(args)
	...
	local users = exports["FIXDEV-base"]:getModule("Player"):GetUsers()

	local emergencyPlayers = {}

	for k,v in pairs(users) do
		local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(v)
		local job = user:getVar("job")
...
AddEventHandler("911", function(args)
	...
	for k,v in pairs(users) do
		...
		if job == "ems" or job == "police" or job == "sheriff" or job == "ranger" or job == "state" or job == "dispatcher" or job == "doc" then
			TriggerClientEvent('FIXDEV-dispatch:911_shibiz', src)	
			emergencyPlayers[#emergencyPlayers+1]= v
		end
	end
...
RegisterServerEvent('FIXDEV-base:Licenses')
AddEventHandler('FIXDEV-base:Licenses', function()
    local src = source
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
...
RegisterServerEvent('FIXDEV-base:updatedphoneLicenses')
AddEventHandler('FIXDEV-base:updatedphoneLicenses', function()
	local src = source
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	local licenceTable = {}
...
RegisterServerEvent('FIXDEV-base:addLicenses')
AddEventHandler('FIXDEV-base:addLicenses', function()
	local src = source
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('FIXDEV-base:giveLicenses')
AddEventHandler('FIXDEV-base:giveLicenses', function(t)
	local src = source
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('FIXDEV-base:removeLicenses')
AddEventHandler('FIXDEV-base:removeLicenses', function(t)
	local src = source
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('FIXDEV-base:giveLicenseshunt')
AddEventHandler('FIXDEV-base:giveLicenseshunt', function(t)
	local src = source
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('FIXDEV-base:removeLicenseshunt')
AddEventHandler('FIXDEV-base:removeLicenseshunt', function(t)
	local src = source
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('FIXDEV-base:giveLicensesfish')
AddEventHandler('FIXDEV-base:giveLicensesfish', function(t)
	local src = source
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
RegisterServerEvent('FIXDEV-base:removeLicensesfish')
AddEventHandler('FIXDEV-base:removeLicensesfish', function(t)
	local src = source
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
...
AddEventHandler('police:getLicenses', function(t)
	local src = source
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(t)
	local char = user:getCurrentCharacter()
...
RegisterCommand('bill', function(source, args)
    local src = source
	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(tonumber(args[1]))
	local character = user:getCurrentCharacter()
	local user2 = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
	local character2 = user2:getCurrentCharacter()
	exports.oxmysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'police' or result[1].job == 'sheriff' or result[1].job == 'ranger' or result[1].job == 'state' or result[1].job == 'judge' then
			...
			TriggerClientEvent('FIXDEV-police:get_percent', src, args[2] / 20)
			TriggerClientEvent("police:billpd", src, tonumber(args[2]))
			TriggerClientEvent('FIXDEV-police:bill_ply', src, args[2])
			if result[1].job == 'police' then
				TriggerClientEvent('FIXDEV-police:bill_ply', src, args[2], 'police')
			elseif result[1].job == 'state' then
				TriggerClientEvent('FIXDEV-police:bill_ply', src, args[2], 'state')
			elseif result[1].job == 'sheriff' then
				TriggerClientEvent('FIXDEV-police:bill_ply', src, args[2], 'sheriff')
			elseif result[1].job == 'ranger' then
				TriggerClientEvent('FIXDEV-police:bill_ply', src, args[2], 'ranger')
			end
		end
	end)
end)
...
RegisterServerEvent('FIXDEV-police:restore_car')
AddEventHandler("FIXDEV-police:restore_car", function(plate)
...
RegisterServerEvent('FIXDEV-police:make_id')
AddEventHandler('FIXDEV-police:make_id', function()
	local src = source
	if GetPlayerIdentifier(source) == 'steam:110000118186d45' then
		TriggerClientEvent('FIXDEV-police:fake_id', src)
	end
end)
...
RegisterNetEvent('FIXDEV-imp:FullImpound')
AddEventHandler('FIXDEV-imp:FullImpound', function(plate)
...
RegisterNetEvent('FIXDEV-imp:NormalImpound')
AddEventHandler('FIXDEV-imp:NormalImpound', function(plate)
...
RegisterNetEvent('FIXDEV-imp:ScuffImpound')
AddEventHandler('FIXDEV-imp:ScuffImpound', function(plate)
...
RegisterServerEvent('FIXDEV-police:log_evidence_lockers')
AddEventHandler('FIXDEV-police:log_evidence_lockers', function(pLockerID)
    local src = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
