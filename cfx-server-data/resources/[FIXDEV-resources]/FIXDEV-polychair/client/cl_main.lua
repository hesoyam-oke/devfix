Citizen.CreateThread(function()
    ...
    for idx,polyTarget in ipairs(ChairZones) do
        exports['FIXDEV-interact']:AddPeekEntryByPolyTarget('FIXDEV-polychair:chair_' .. tostring(idx) .. "-".. tostring(polyTarget[4].name), {{
            event = "FIXDEV-polychair:chairSit",
...
function chairSit(chairPosition, cancel)
    ...
    if isSitting then
        ...
        isSitting = false
        TriggerEvent('FIXDEV-polychair:stopSitting')
    else
        ...
        TriggerEvent('FIXDEV-polychair:startSitting')
    end
    exports["FIXDEV-flags"]:SetPedFlag(ped, 'isSittingOnChair', isSitting)
end

AddEventHandler('FIXDEV-polychair:chairSit', function(pParameters, pEntity, pContext)
    chairSit(pParameters.chairPosition, true)
end)

RegisterNetEvent("FIXDEV-emotes:sitOnChair")
AddEventHandler("FIXDEV-emotes:sitOnChair", function(pArgs, pEntity, pContext)
    chairSit(nil, true)
end)
