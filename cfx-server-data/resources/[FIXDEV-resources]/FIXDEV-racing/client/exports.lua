...
function createPendingRace(id, options)
    ...
    RPC.execute("FIXDEV-racing:createPendingRace", id, options)
end
...
function startRace(countdown)
    ...
    for k, v in pairs(pendingRaces) do
        ...
        if v.owner == characterId then
            ...
            RPC.execute("FIXDEV-racing:startRace", v.id, countdown or v.countdown, v.owner)
            return
        end
    end
end
...
function endRace()
    ...
    if curRace then
        RPC.execute("FIXDEV-racing:endRace")
        print("ENDING RACE 1")
    else
        RPC.execute("FIXDEV-racing:leaveRace")
    end
end

function joinRace(id, alias, characterId)
    RPC.execute("FIXDEV-racing:joinRace", id, alias, characterId)
end
...
function leaveRace()
    ...
    if curRace then
        RPC.execute("FIXDEV-racing:dnfRace", curRace.id)
        cleanupRace()
    else
        RPC.execute("FIXDEV-racing:leaveRace")
    end
...
function getAllRaces()
    ...
    local res = RPC.execute("FIXDEV-racing:getAllRaces")
    races = res.races
...
function getHasRaceUsbAndAlias()
    ...
    local pdRacingUsbItem = exports["FIXDEV-inventory"]:GetInfoForFirstItemOfName("racingusb3")
