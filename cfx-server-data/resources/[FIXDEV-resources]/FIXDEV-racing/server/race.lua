...
function joinRace(src, id, alias, cid, dontpay)
    ...
    TriggerClientEvent("FIXDEV-racing:joinedRace", src, Races[id])
    TriggerClientEvent("FIXDEV-racing:addedPendingRace", -1, PendingRaces[id])

    for k, v in pairs(PendingRaces[id]["players"]) do
        TriggerClientEvent("FIXDEV-racing:playerJoinedYourRace", v.serverId, cid, alias)
    end
end
...
RegisterNetEvent("FIXDEV-racing:updatePositionInfo")
AddEventHandler("FIXDEV-racing:updatePositionInfo", function(curRaceid, totalDist)
    local src = source
    local srcString = tostring(src)
    ...
    TriggerClientEvent("FIXDEV-racing:updatePosition", src, position)
end)
...
RPC.register("FIXDEV-racing:createPendingRace", function(src, ids, optionss)
    ...
    TriggerClientEvent("FIXDEV-racing:addedPendingRace", -1, options)
    TriggerClientEvent('FIXDEV-racing:pendingRacenotif', -1)
    print("GETTING TO JOIN AFTER CREATING RACE",options)
    joinRace(src, id, alias, cid, true)
end)

RPC.register("FIXDEV-racing:joinRace", function(src, ids, aliass, cids)
    local id = ids
    local alias = aliass
    local cid = cids
...
RPC.register("FIXDEV-racing:leaveRace", function(src)
    local srcString = tostring(src)
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    if cid == PendingRaces[raceId]["owner"] then
        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("FIXDEV-racing:endRace", v.serverId)
        end

        PendingRaces[raceId] = nil
        TriggerClientEvent("FIXDEV-racing:removedPendingRace", -1, raceId)
    else
        PendingRaces[raceId]["players"][srcString] = nil
        TriggerClientEvent("FIXDEV-racing:leftRace", src)
        TriggerClientEvent("FIXDEV-racing:addedPendingRace", -1, PendingRaces[raceId])

        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("FIXDEV-racing:playerLeftYourRace", v.serverId, cid, alias)
        end
    end
end)

RPC.register("FIXDEV-racing:dnfRace", function(src, ids)
    local id = ids
    local srcString = tostring(src)
    ...
    TriggerClientEvent("FIXDEV-racing:leftRace", src)
    TriggerClientEvent("FIXDEV-racing:updatedActiveRace", -1, ActiveRaces[id])
    ...
    if ActiveRaces[id]["positions"] > countPlayers then
        ...
        TriggerClientEvent("FIXDEV-racing:raceHistory", -1, ActiveRaces[id])
        ...
        ActiveRaces[id] = nil
        TriggerClientEvent("FIXDEV-racing:removedActiveRace", -1, id)
    end
end)

RPC.register("FIXDEV-racing:startRace", function(src, ids, countdowns, ownerID)
    ...
    TriggerClientEvent("FIXDEV-racing:addedActiveRace", -1, ActiveRaces[id])

    PendingRaces[id] = nil
    TriggerClientEvent("FIXDEV-racing:removedPendingRace", -1, id)
    ...
    for k, v in pairs(ActiveRaces[id]["players"]) do
        ...
        if isStartEnabled then
            TriggerClientEvent("FIXDEV-racing:startRace", v.serverId, race, countdown * 1000)
            TriggerClientEvent("FIXDEV-racing:api:currentRace", v.serverId, race)
        else
            local user = exports['FIXDEV-base']:getModule("Player"):GetUser(v.serverId)
            local char = user:getCurrentCharacter()
...
RPC.register("FIXDEV-racing:endRace", function(src)
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    ...
    TriggerClientEvent("FIXDEV-racing:removedActiveRace", -1, raceId)
end)

RPC.register("FIXDEV-racing:finishedRace", function(src, curRaceids, bestLapTimes, totalTimes)
    ...
    TriggerClientEvent("FIXDEV-racing:finishedRace", src, ActiveRaces[curRaceid]["players"][srcString]["position"], totalTime, ActiveRaces[curRaceid]["buyIn"])
    TriggerClientEvent("FIXDEV-racing:updatedActiveRace", -1, ActiveRaces[curRaceid])
    ...
    if ActiveRaces[curRaceid]["players"][srcString]["position"] == countPlayers then
        ...
        TriggerClientEvent("FIXDEV-racing:raceHistory", -1, ActiveRaces[curRaceid])
        ...
        TriggerClientEvent("FIXDEV-racing:removedActiveRace", -1, curRaceid)
    elseif ActiveRaces[curRaceid]["dnfPosition"] > 0 and ActiveRaces[curRaceid]["dnfPosition"] == ActiveRaces[curRaceid]["players"][srcString]["position"] then
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            TriggerClientEvent("FIXDEV-racing:startDNFCountdown", v.serverId, countdown)
        end
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            if v.position == 0 then
                TriggerClientEvent("FIXDEV-racing:dnfRace", v.serverId)
            end
        end
