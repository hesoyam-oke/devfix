...
RegisterNetEvent('FIXDEV-rcvehicles:car:setup', function(pCarSettings)
...
RegisterNetEvent('FIXDEV-rcvehicles:car:spawn', function(pCarSettings)
...
RegisterNetEvent('FIXDEV-rcvehicles:car:pilot', function(pCarId)
  ...
  if not DoesEntityExist(CURRENT_CAR.handle) then
    DoScreenFadeIn(400)
    RPC.execute('FIXDEV-rcvehicles:leaveCar', CURRENT_CAR.id)
  ...
  TriggerEvent('FIXDEV-rc:carEntered', CURRENT_CAR)
  ...
  TriggerEvent('FIXDEV-ui:setVehicleBypassed', CURRENT_CAR.handle, true)
  DoScreenFadeIn(1000)
  ...
  Citizen.CreateThread(function()
    ...
    TriggerEvent('FIXDEV-ui:setVehicleBypassed', CURRENT_CAR.handle)

    SetNetworkIdCanMigrate(CURRENT_CAR.netId, true)

    TriggerEvent('FIXDEV-rc:carExited', CURRENT_CAR)
    RPC.execute('FIXDEV-rcvehicles:leaveCar', CURRENT_CAR.id)
    if CURRENT_CAR.crashed then
      RPC.execute('FIXDEV-rcvehicles:crashCar', CURRENT_CAR.id)
    end
    CURRENT_CAR = nil
  end)
...
RegisterNetEvent('FIXDEV-rcvehicles:car:destroy', function(pCarId)
