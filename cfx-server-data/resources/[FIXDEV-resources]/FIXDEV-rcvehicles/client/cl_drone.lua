...
RegisterNetEvent('FIXDEV-rcvehicles:drone:spawn', function(pDroneSettings)
...
RegisterNetEvent('FIXDEV-rcvehicles:drone:pilot', function(pDroneId)
  ...
  exports['FIXDEV-actionbar']:disableActionBar(true)
  ...
  TriggerEvent('FIXDEV-rc:droneEntered', CURRENT_DRONE)
  ...
  Citizen.CreateThread(function()
    ...
    while isPiloting do
      ...
      if IsDisabledControlJustPressed(0, Keys['SPACE']) then
        TriggerEvent('FIXDEV-rc:droneAbility', CURRENT_DRONE)
      end
      ...
      if (math.abs(rightAxisX) > 0) and (math.abs(rightAxisY) > 0) then
        ...
        rotz = rotation.z + rightAxisX * -5.0

        local yValue = rightAxisY * -2.5
      ...
      forceZ = forceZ + (9.81 * delta * (DRONE_MOVE_SPEED / 4.0) * (50.0 + (math.random(1,100))))
    ...
    exports['FIXDEV-actionbar']:disableActionBar(false)
    ...
    TriggerEvent('FIXDEV-rc:droneExited', CURRENT_DRONE)
    RPC.execute('FIXDEV-rcvehicles:leaveDrone', CURRENT_DRONE.id)
    if CURRENT_DRONE.crashed then
        RPC.execute('FIXDEV-rcvehicles:crashDrone', CURRENT_DRONE.id)
    end
    CURRENT_DRONE = nil
  end)
...
RegisterNetEvent('FIXDEV-rcvehicles:drone:destroy', function(pDroneId)
