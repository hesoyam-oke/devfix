...
Citizen.CreateThread(function()
    ...
    while(true) do
		...
		if addedPolyZone == false then
			    exports["FIXDEV-polyzone"]:AddBoxZone("carrent", vector3(110.4619140625, -1090.6267089844, 29.302474975586), 0.8, 0.9, {
				name="carrent",
				heading=331.47,
				debugPoly=false,
				minZ=22.37,
...
--RegisterNetEvent('FIXDEV-carrent:menu')
--AddEventHandler('FIXDEV-carrent:menu', function()   
...
AddEventHandler('FIXDEV-carrent:menu', function()                           
  TriggerEvent('FIXDEV-context:sendMenu', {
        ...
        },
        {
			...
			params = {
                event = "FIXDEV-carrent:checkmoney",
                args = {
                    car = Config.Car1,
...
RegisterNetEvent('FIXDEV-carrent:checkmoney')
AddEventHandler('FIXDEV-carrent:checkmoney', function(data)                            
        local vehicle = data.car
        local price = data.price
        TriggerServerEvent("FIXDEV-carrent:hasmoney", price, vehicle)

end)

RegisterNetEvent('FIXDEV-carrent:spawncar')
AddEventHandler('FIXDEV-carrent:spawncar', function(vehicleName)                            
