...
function manageFood(restaurant, foodType)
  ...
  exports['FIXDEV-ui']:openApplication('textbox', {
    callbackUrl = 'FIXDEV-ui:restaurants:createFood',
...
RegisterUICallback("FIXDEV-ui:restaurants:createFood", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  exports['FIXDEV-ui']:closeApplication('textbox')
  RPC.execute('FIXDEV-restaurants:createFoodItem', data)
end)

RegisterUICallback("FIXDEV-ui:restaurants:createMenuItem", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  Wait(100)
  manageFood(data.key.restaurant, data.key.foodType)
end)

RegisterUICallback("FIXDEV-ui:restaurants:manageFoodItem", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  Wait(100)
  if data.key.action == "delete" then
    RPC.execute("FIXDEV-restaurants:deleteFoodItem", data.key.id, data.key.restaurant)
    return
  end
end)

AddEventHandler("FIXDEV-restaurants:manageFoodMenu", function(p1, p2, pContext)
  ...
  local hasCraftAccess = exports["FIXDEV-business"]:HasPermission(restaurant, "craft_access")
      ...
      children = {
        {
          ...
          action = "FIXDEV-ui:restaurants:createMenuItem",
        },
        {
          title = "Side Dish",
          icon = "bacon",
      ...
      children = {
        {
          ...
          action = "FIXDEV-ui:restaurants:manageFoodItem",
        },
      },
    }
  end
...
AddEventHandler("FIXDEV-restaurants:manageDailyMenu", function(p1, p2, pContext)
  local restaurant = pContext.zones["restaurant_manage_food_menu"].id
  ...
  RPC.execute("FIXDEV-restaurants:setMenu", restaurant, dishes)
end)
