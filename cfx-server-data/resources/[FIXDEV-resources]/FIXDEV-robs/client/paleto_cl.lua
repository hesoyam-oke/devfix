...
RegisterNetEvent("FIXDEV-paleto:getBanks")
AddEventHandler("FIXDEV-paleto:getBanks", function(result)
    paletoBanks = result
end)

RegisterNetEvent("FIXDEV-paleto:getHit")
AddEventHandler("FIXDEV-paleto:getHit", function(result)
    paletoBanksTimes = result
end)

RegisterNetEvent("FIXDEV-paleto:getDoorCheckCL")
AddEventHandler("FIXDEV-paleto:getDoorCheckCL", function(result)
    paletoDoorState = result
end)

RegisterNetEvent("FIXDEV-paleto:GetTimeCL")
AddEventHandler("FIXDEV-paleto:GetTimeCL", function(result)
    paletoTimesHit = result
end)

RegisterNetEvent("FIXDEV-paleto:GetTime2CL")
AddEventHandler("FIXDEV-paleto:GetTime2CL", function(result)
    paletoTimesHit2 = result
end)


RegisterNetEvent("FIXDEV-paleto:GetDoorAccessCL")
AddEventHandler("FIXDEV-paleto:GetDoorAccessCL", function(result)
...
RegisterNetEvent("FIXDEV-paleto:resetDoorState")
AddEventHandler("FIXDEV-paleto:resetDoorState", function(name)
    Freeze2[name] = 0
end)

RegisterNetEvent("FIXDEV-paleto:lootup_c")
AddEventHandler("FIXDEV-paleto:lootup_c", function(var, var2)
    LootCheck2[var][var2] = true
end)

RegisterNetEvent("FIXDEV-paleto:outcome")
AddEventHandler("FIXDEV-paleto:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check2[arg] = true
        TriggerEvent("FIXDEV-paleto:startheist", paleto.Banks[arg], arg)
    elseif not oc then
        exports['FIXDEV-notification']:Alert({style = 'error', duration = 3000, message = arg})
    end
end)

RegisterNetEvent("FIXDEV-paleto:OpenPaletoDoor")
AddEventHandler("FIXDEV-paleto:OpenPaletoDoor", function()
    TriggerServerEvent('FIXDEV-paleto:getGetDoorStateSVSV', true)
    local count = 0
...
RegisterNetEvent("FIXDEV-paleto:ClosePaletoDoor")
AddEventHandler("FIXDEV-paleto:ClosePaletoDoor", function()
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418,6472.1274414062,31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
        local heading = GetEntityHeading(VaultDoor) - 0.10
        ...
        Citizen.Wait(10)
    until count == 900
    TriggerServerEvent('FIXDEV-paleto:getGetDoorStateSVSV', false)
end)
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isNear then
            TriggerServerEvent('FIXDEV-paleto:getGetDoorStateSV')
            local VaultDoor = GetClosestObjectOfType(-104.8747253418,6472.1274414062,31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
...
AddEventHandler('plootCash1',function()
    TriggerServerEvent("FIXDEV-paleto:lootup", "P1", "Loot1")
    StartGrab2(name)
end)

RegisterNetEvent('plootCash2')
AddEventHandler('plootCash2',function()
    TriggerServerEvent("FIXDEV-paleto:lootup", "P1", "Loot2")
    StartGrab2(name)
end)

RegisterNetEvent('plootCash3')
AddEventHandler('plootCash3',function()
    TriggerServerEvent("FIXDEV-paleto:lootup", "P1", "Loot3")
    StartGrab2(name)
end)

RegisterNetEvent("FIXDEV-paleto:startLoot_c")
AddEventHandler("FIXDEV-paleto:startLoot_c", function(data, name)
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    if not LootCheck2[name].Stop then
        Citizen.CreateThread(function()

            exports["FIXDEV-target"]:AddBoxZone("pCash1", vector3(-107.39, 6473.98, 31.63), 0.6, 1, {
            name="pCash1",
            heading=312,
            minZ=27.78,
            maxZ=31.78
            ...
            exports["FIXDEV-target"]:AddBoxZone("pCash2", vector3(-102.78, 6476.79, 31.65), 0.6, 1, {
            name="pCash2",
            heading=318,
            minZ=27.75,
            maxZ=31.75
            ...
            exports["FIXDEV-target"]:AddBoxZone("pCash3", vector3(-104.7, 6478.54, 31.63), 0.6, 1, {
            name="pCash3",
            heading=316,
            minZ=27.73,
            maxZ=31.73
            ...
            while true do
                ...
                if dst < 40 then
                    if LootCheck2[name].Stop or (LootCheck2[name].Loot1 and LootCheck2[name].Loot2 and LootCheck2[name].Loot3) then
                        ...
                        if initiator then
                            TriggerEvent("FIXDEV-paleto:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("FIXDEV-paleto:stopHeist_c")
AddEventHandler("FIXDEV-paleto:stopHeist_c", function(name)
...
RegisterNetEvent("FIXDEV-paleto:reset")
AddEventHandler("FIXDEV-paleto:reset", function(name, data)
    for i = 1, #LootCheck2[name], 1 do
        LootCheck2[name][i] = false
    end
    Check2[name] = false
    exports['FIXDEV-notification']:Alert({style = 'error', duration = 3000, message = 'VAULT DOOR WILL CLOSE IN 2 MINUTES!'})
    Citizen.Wait(120000)
    exports['FIXDEV-notification']:Alert({style = 'error', duration = 3000, message = 'VAULT DOOR CLOSING'})
    TriggerEvent("FIXDEV-paleto:cleanUp", data, name)
    TriggerServerEvent("FIXDEV-paleto:closeDoor")
end)

RegisterNetEvent("FIXDEV-paleto:startheist")
AddEventHandler("FIXDEV-paleto:startheist", function(data, name)
    currentname = name
    initiator = true

    exports['FIXDEV-notification']:Alert({style = 'info', duration = 5000, message = 'Nice! You bypassed the captcha. Give me a few minutes to open the door!'})

    Citizen.Wait(300000)

    TriggerServerEvent('FIXDEV-paleto:openDoor')
    startdstcheck = true
    currentname = name
    SpawnTrolleys2(data, name)
end)

AddEventHandler("FIXDEV-paleto:cleanUp", function(data, name)
    TriggerServerEvent("FIXDEV-paleto:setCooldown", name)
    Citizen.Wait(30000)
...
function SpawnTrolleys2(data, name)
    ...
    TriggerServerEvent("FIXDEV-paleto:startLoot", data, name)
    done = false
end
		    ...
		    while GetGameTimer() - startedGrabbing < 37000 do
			    ...
			    if HasAnimEventFired(ped, GetHashKey("RELEASE_CASH_DESTROY")) then
                    if IsEntityVisible(grabobj) then
                        SetEntityVisible(grabobj, false, false)
                        TriggerServerEvent("FIXDEV-paleto:rewardCash")
				    end
			    end
		    end
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("FIXDEV-paleto:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
...
Citizen.CreateThread(function()
    while true do
        if paletoBanksTimes < 1 then
            Citizen.Wait(600000)
            TriggerServerEvent('FIXDEV-paleto:getHitSVSV', 3)
        end
        Citizen.Wait(5)
    end
end)
...
RegisterNetEvent("FIXDEV-paleto:hackinganim")
AddEventHandler("FIXDEV-paleto:hackinganim", function(toggle)
...
RegisterNetEvent("FIXDEV-paleto:removePlease")
AddEventHandler("FIXDEV-paleto:removePlease", function()
    if not paletoStateAlready == true then
        TriggerServerEvent('FIXDEV-paleto:getHitSVSV', paletoBanksTimes - 1)
        TriggerEvent('FIXDEV-robberies:hackinganim', false)
...
RegisterNetEvent("FIXDEV-paleto:addPlease")
AddEventHandler("FIXDEV-paleto:addPlease", function()
    for k, v in pairs(paletoBanks) do
        ...
        if not v.onaction then
            if dst <= 5 and not Check[k] then
                ...
                ClearPedTasksImmediately(PlayerPedId())
                TriggerEvent('FIXDEV-robberies:hackinganim', false)
                TriggerServerEvent("FIXDEV-paleto:startcheck", k)
                return
            end
        end
    end
...
RegisterNetEvent("FIXDEV-paleto:policenotify")
AddEventHandler("FIXDEV-paleto:policenotify", function(name)
...
RegisterNetEvent("FIXDEV-paleto:startPaletoHeist")
AddEventHandler("FIXDEV-paleto:startPaletoHeist", function()
    TriggerServerEvent('FIXDEV-paleto:getBanksSV')
    TriggerServerEvent('FIXDEV-paleto:getHitSV')
    TriggerServerEvent('FIXDEV-paleto:getTimeSV')
    TriggerServerEvent('FIXDEV-paleto:getTime2SV')
    TriggerServerEvent('FIXDEV-paleto:getDoorAccessSV')
    Citizen.Wait(1000)
    ...
    if countpolice >= 0 then
        if not paletoStateAlready == true then
            if paletoTimesHit2 > paletoTimesHit then
                if paletoBanksTimes >= 1 then
                    if exports['FIXDEV-inventory']:hasEnoughOfItem('electronickit', 1, false) then
                        TriggerEvent('FIXDEV-paleto:policenotify')
                        TriggerEvent('FIXDEV-robberies:hackinganim', true)
                        Citizen.Wait(7000)
                ...
                TriggerServerEvent('FIXDEV-paleto:TimePoggers')
            end
        else
            TriggerServerEvent('FIXDEV-paleto:DoorAccessPoggers')
        end
    else
