...
RegisterNetEvent("FIXDEV-scenes:getScenes")
AddEventHandler("FIXDEV-scenes:getScenes", function()
    local src = source
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))

    if not data then
        TriggerClientEvent("FIXDEV-scenes:allScenes", src, {})
        return
    end
    ...
    TriggerClientEvent("FIXDEV-scenes:allScenes", src, scenes)
end)

RegisterNetEvent("FIXDEV-scenes:addScene")
AddEventHandler("FIXDEV-scenes:addScene", function(pData)
    local src = source
    ...
    TriggerClientEvent("FIXDEV-scenes:refreshScenes", -1, data[1].id, data[1])
end)

RegisterNetEvent("FIXDEV-scenes:deleteScene")
AddEventHandler("FIXDEV-scenes:deleteScene", function(pCoords)
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))
    for k,v in pairs(data) do
        ...
        if #(acCoord - pCoords) < 2.0 then
            ...
            if delete then
                TriggerClientEvent("FIXDEV-scenes:deleteScene", -1, v.id)
                return
            end
        end
    end
