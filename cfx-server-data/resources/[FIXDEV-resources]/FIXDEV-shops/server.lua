...
AddEventHandler("itemMoneyCheck", function(itemType,askingPrice,location)
	...
	if (user:getCash() >= askingPrice) then
		...
		if askingPrice > 0 then
			TriggerClientEvent('FIXDEV-notification:client:Alert', src, {style = 'info', duration = 3000, message = "Purchased"})
		end
		TriggerClientEvent("player:receiveItem",src,itemType,1)

	else
		TriggerClientEvent('FIXDEV-notification:client:Alert', src, {style = 'error', duration = 3000, message = "Not enough money!"})
	end
end)
...
AddEventHandler("shop:useVM:server", function(locatoion)
	...
	if (tonumber(pCash) >= 20) then
		...
		TriggerClientEvent('FIXDEV-notification:client:Alert', src, {style = 'error', duration = 3000, message = "You need 20$"})
	end
end)
...
AddEventHandler("take100", function(tgtsent)
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(tgtsent)
    local char = user:getCurrentCharacter()
...
AddEventHandler("weapon:general:check", function()
    ...
    exports.oxmysql:execute("SELECT licenses FROM characters WHERE id = @id",{['id'] = characterId}, function(result)
