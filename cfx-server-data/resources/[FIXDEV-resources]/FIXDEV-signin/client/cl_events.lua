...
RegisterNetEvent("FIXDEV-signin:signOffDuty")
AddEventHandler("FIXDEV-signin:signOffDuty", function()
  signOff()
end)

RegisterUICallback("FIXDEV-signin:handler", function(data, cb)
  local eventData = data.key
  if eventData.sign_in then
    TriggerServerEvent("FIXDEV-signin:duty", eventData.job)
...
RegisterUICallback("FIXDEV-signin:spawnVehicle", function(data, cb)
  local eventData = data.key
...
AddEventHandler("FIXDEV-signin:peekAction", function(pArgs, pEntity, pContext)
  ...
  exports["FIXDEV-ui"]:showContextMenu(MenuData[name])
end)

AddEventHandler("FIXDEV-signin:legalAid", function(pArgs, pEntity, pContext)
  TriggerEvent("DoLongHudText", "You now appear as a legal aid.")
  RPC.execute('FIXDEV-gov:dojApp:setStatus', "legal_aid", "Available")
end)
