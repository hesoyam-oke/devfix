...
local function drawStatusHUD(state, pValues)
  ...
  local values = {}
  isSlotMachinesEnabled, currentJackPot = RPC.execute("FIXDEV-slotmachines-sv:checkIfSlotMachinesState")
  ...
  -- exports["FIXDEV-textui"]:sendAppEvent("status-hud", {
  --   show = state,
  --   title = title,
  --   values = values
  -- })
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "FIXDEV-textui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "FIXDEV-textui-slotmachines:leaveMachine",
            title = "Leave Machine",
            description = "",
            key = {},
          })
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["FIXDEV-textui"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            exports["FIXDEV-textui"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
...
AddEventHandler("FIXDEV-casino:casinoEnteredEvent", function()
  isInsideCasino = true
  isSlotMachinesEnabled, currentAccJackpot = RPC.execute("FIXDEV-slotmachines-sv:checkIfSlotMachinesState")
...
AddEventHandler("FIXDEV-casino:casinoExitedEvent", function()
  ...
  isSlotMachinesEnabled = RPC.execute("FIXDEV-slotmachines-sv:checkIfSlotMachinesState")
  if not isSlotMachinesEnabled then
    return
  end
...
--#[NUI Callbacks]#--
RegisterUICallback("FIXDEV-textui-slotmachines:placeBet", function(data, cb)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    local useChipsResult = RPC.execute("FIXDEV-casino:useChips", currentBet)
    ...
    local user = exports["FIXDEV-base"]:GetModule("LocalPlayer")
    local cid = user.id
...
RegisterUICallback("FIXDEV-textui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  getUpFromSlotMachine()
  SendNUIMessage({
    app = "status-hud",
