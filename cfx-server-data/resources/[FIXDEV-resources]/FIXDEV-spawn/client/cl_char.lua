...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    events:Trigger("FIXDEV-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with FIXDEV-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("FIXDEV-base:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["FIXDEV-base"]:getModule("Events")
	events:Trigger("FIXDEV-base:selectCharacter", data.actionData, function(returnData)
        ...
        if Login.CurrentClothing[data.actionData] == nil then
	        ...
	        TriggerEvent("FIXDEV-base:firstSpawn")
	    end
    end)
end
...
function Login.setClothingForChar()
    ...
    TriggerEvent("FIXDEV-clothes:client:CreateFirstCharacter")
...
RegisterNetEvent("FIXDEV-spawn:finishedClothing")
AddEventHandler("FIXDEV-spawn:finishedClothing", function(endType)
