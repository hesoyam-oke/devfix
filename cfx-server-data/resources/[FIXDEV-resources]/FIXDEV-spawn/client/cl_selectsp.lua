...
AddEventHandler('spawn:clientSpawnData', function(spawnData, heheData)
		...
		if spawnData.overwrites ~= nil then
			...
			elseif spawnData.overwrites == "new" then
				...
				-- TriggerEvent("FIXDEV-spawn:firstspawnyo")
			end
			return
		end
		...
		if Spawn.housingCoords == nil then
			Spawn.housingCoords = exports["FIXDEV-housing"]:retriveHousingTable()
		end
...
RegisterNetEvent("FIXDEV-spawn:firstspawnyo")
AddEventHandler("FIXDEV-spawn:firstspawnyo", function()
    if GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01") then
        TriggerEvent("FIXDEV-spawn:finishedClothingstuffbish")
    else
        TriggerEvent("FIXDEV-spawn:finishedClothingstuff")
    end
end)

RegisterNetEvent('FIXDEV-spawn:finishedClothingstuff')
AddEventHandler('FIXDEV-spawn:finishedClothingstuff', function()
	...
	Citizen.CreateThread(function()
        while true do
			...
			if IsCutsceneActive() then 
				...
				Wait(3000)
				FreezeEntityPosition(GetPlayerPed(-1), false)
				if not exports["FIXDEV-inventory"]:hasEnoughOfItem("idcard",1,false) then
					TriggerEvent("player:receiveItem","idcard",1,true)
				end
				if not exports["FIXDEV-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
...
RegisterNetEvent('FIXDEV-spawn:finishedClothingstuffbish')
AddEventHandler('FIXDEV-spawn:finishedClothingstuffbish', function()
...
function Spawn.getBusinessSpawn(pBusinessCode, pBusinessName)
	...
	local hasPermission = RPC.execute("FIXDEV-business:hasPermission", pBusinessCode, "key_access")
...
function Spawn.selectedSpawn(spawnInfo)
	...
	if apartment then
		...
		if pos then
			...
			DoScreenFadeIn(2500)
			TriggerEvent("FIXDEV-spawn:characterSpawned")
			TriggerEvent("FIXDEV-spawn:characterSpawned")
		else 
