...
function Login.ShowMenu(toggle)
	...
	local lifersDisabled = exports["FIXDEV-config"]:GetMiscConfig("characters.lifers.disabled")
	local hardcoreDisabled = exports["FIXDEV-config"]:GetMiscConfig("characters.hardcore.disabled")
...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = exports["FIXDEV-base"]:getModule("Util"):ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		local events = exports["FIXDEV-base"]:getModule("Events")
		events:Trigger("FIXDEV-base:deleteCharacter", data.actionData, function(deleted)
            Login.getCharacters(true)
        end)
	end
	...
	if data.action == "newCharacter" then
		...
		if cData.type and cData.type == 'hardcore' then
			local canCreate = RPC.execute('FIXDEV-spawn:canCreateHardcoreCharacter')
			if not canCreate then callback({}) return end
		end

		events:Trigger("FIXDEV-base:createCharacter", chardata, function(created)
            if not created then
...
RegisterNetEvent("FIXDEV-base:spawnInitialized")
