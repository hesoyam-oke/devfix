...
function pinFailure() {
  $.post('http://FIXDEV-stash/failure', JSON.stringify({pinResult: false}));
  gameOver = true;
  $('#modal').fadeIn();
  ++failedAttempts;
  if (failedAttempts >= 3)
...
function pinSuccess(pElevated) {
  $.post('http://FIXDEV-stash/complete', JSON.stringify({pinResult: true, owner: pElevated}));
...
document.onkeyup = function (data) {
  if (data.which == 27 ) {
    $.post('http://FIXDEV-stash/close', JSON.stringify({pinResult: false}));
  }
};
