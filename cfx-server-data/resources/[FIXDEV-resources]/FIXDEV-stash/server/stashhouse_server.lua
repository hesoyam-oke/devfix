RegisterNetEvent('FIXDEV-stash:fetchInitialState')
AddEventHandler('FIXDEV-stash:fetchInitialState', function()
	local source = source
    if Config.Stash ~= nil then
	    TriggerClientEvent('FIXDEV-stash:setInitialState', source, Config.Stash)
    end
end)
...
AddEventHandler('stashesaddtoconfig', function(coords, pin, id, distance)
    local source = source
    if GetPlayerIdentifier(source) == 'steam:1100001479a223c' or 'steam:1100001479a223c' then
