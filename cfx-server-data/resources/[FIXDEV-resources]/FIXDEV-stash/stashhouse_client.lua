local stashes = {}
local instash = 0
RegisterNetEvent('FIXDEV-stash:setInitialState')
AddEventHandler('FIXDEV-stash:setInitialState', function(stashesp)
    stashes = stashesp
    
end)

CreateThread(function()
    TriggerServerEvent("FIXDEV-stash:fetchInitialState")
end)
...
RegisterNetEvent('FIXDEV-stash:getnearstash')
AddEventHandler('FIXDEV-stash:getnearstash', function()
    local closestDoorDistance, closestDoorId = 9999.9, -1
    local currentPos = GetEntityCoords(PlayerPedId())
    for id, handle in pairs(stashes) do
        local currentDoorDistance = #(stashes[id].StashEntry - currentPos)
...
RegisterNUICallback('complete', function(data, cb)
    CloseGui();
    cb('ok');
    TriggerEvent('FIXDEV-stash:getnearstash')
end)
