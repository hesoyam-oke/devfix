...
CreateThread(function ()
    for crate, data in pairs(Config.crates) do
        exports["FIXDEV-interact"]:AddPeekEntryByModel({ GetHashKey(data.model) }, {{
            id = "storage_lifetime",
            event = "FIXDEV-storage:checkLifetime",
            icon = "heart",
        ...
        exports["FIXDEV-interact"]:AddPeekEntryByModel({ GetHashKey(data.model) }, {{
            id = "storage_repair",
            event = "FIXDEV-storage:repairStorage",
            icon = "screwdriver",
            ...
            isEnabled = function(entity, context)
                local stash, dist = getClosestStash(data.distance)
                return stash ~= nil and exports["FIXDEV-inventory"]:hasEnoughOfItem('craterepairkit', 1, false, true)
            end
        })
        exports["FIXDEV-interact"]:AddPeekEntryByModel({ GetHashKey(data.model) }, {{
            id = "storage_destroy",
            event = "FIXDEV-storage:destroyStash",
            icon = "trash-alt",
        ...
        if not data.vendor then
            exports["FIXDEV-interact"]:AddPeekEntryByModel({ GetHashKey(data.model) }, {{
                id = "storage_open",
                event = "FIXDEV-storage:openStash",
                icon = "box-open",
            ...
            exports["FIXDEV-interact"]:AddPeekEntryByModel({ GetHashKey(data.model) }, {{
                id = "storage_breakin",
                event = "FIXDEV-storage:breakinStash",
                ...
                isEnabled = function(entity, context)
                    ...
                    local hasDetCord = exports["FIXDEV-inventory"]:hasEnoughOfItem("detcord", 1, false, true)
                    local hasThermite = exports["FIXDEV-inventory"]:hasEnoughOfItem("thermitecharge", 1, false, true)
                    local hasHacking = exports["FIXDEV-inventory"]:hasEnoughOfItem("hackingdevice", 1, false, true)
                    local hasPdHacking = exports["FIXDEV-inventory"]:hasEnoughOfItem("elevatorhackingdevice", 1, false, true)
    ...
    Wait(5000)
    if #Storages == 0 then
        TriggerServerEvent("FIXDEV-storage:requestStorages")
    end
end)

RegisterNetEvent("FIXDEV-storage:loadStorages")
AddEventHandler("FIXDEV-storage:loadStorages", function (_storages)
    for id, storage in pairs(_storages) do
        addStorage(storage)
    end
end)

RegisterNetEvent("FIXDEV:storage:prepareNewStorage")
AddEventHandler("FIXDEV:storage:prepareNewStorage", function (storage)
    addStorage(storage)
end)

AddEventHandler("FIXDEV-polyzone:enter", function (name, data)
...
AddEventHandler("FIXDEV-polyzone:exit", function (name, data)
    if name ~= "mobile_stash" then return end
    if not Storages[data.id] then return end
...
AddEventHandler("FIXDEV-storage:openStash", function ()
    ...
    if stash.is_locked then
        ...
        local hasKey = #exports["FIXDEV-inventory"]:getItemsOfType("mobilecratekey", 1, true, {
            crateId = stash.id
        }) > 0
...
function openKeyPad (stash)
    exports['FIXDEV-ui']:openApplication('textbox', {
        callbackUrl = 'FIXDEV-storage:enterKeyPad',
...
RegisterUICallback("FIXDEV-storage:enterKeyPad", function (data, cb)
...
AddEventHandler("FIXDEV-storage:breakinStash", function ()
    local closestStash, closestStashDist = getClosestStash(10)
    if closestStash == nil then return end
    local stash = Storages[closestStash]
    if closestStashDist > Config.crates[stash.size].distance then return end
    ...
    local itemUsed = nil
    local gameSettings = {
        gameFinishedEndpoint = "FIXDEV-storage:crackStorage",
    ...
    if stash.key_code then
        if not itemUsed and exports["FIXDEV-inventory"]:hasEnoughOfItem("hackingdevice", 1, false, true) then
            itemUsed = "hackingdevice"
        end
        if not itemUsed and exports["FIXDEV-inventory"]:hasEnoughOfItem("elevatorhackingdevice", 1, false, true) then
        ...
        if exports["FIXDEV-inventory"]:hasEnoughOfItem("detcord", 1, false, true) then
            itemUsed = "detcord"
        end
        if not itemUsed and exports["FIXDEV-inventory"]:hasEnoughOfItem("thermitecharge", 1, false, true) then
    ...
    if not success then
        return TriggerServerEvent("FIXDEV-storage:breakInStorageFailed", closestStash, itemUsed)
    end
    ...
    local progress = exports["FIXDEV-taskbar"]:taskBar(Config.crates[stash.size].openLength, _L("storage-breaking-open", "Breaking open..."))
    ClearPedTasks(PlayerPedId())
    if progress ~= 100 then return end

    TriggerServerEvent("FIXDEV-storage:breakInStorage", closestStash, itemUsed)
    TriggerEvent("DoLongHudText", _L("storage-broken-open", "You have opened the container"))
end)

RegisterNetEvent("FIXDEV-storage:clearStorages")
AddEventHandler("FIXDEV-storage:clearStorages", function (deletedStorages)
...
AddEventHandler("FIXDEV-storage:destroyStash", function ()
    local closestStash, closestStashDist = getClosestStash(10)
    if closestStash == nil then return end
    local stash = Storages[closestStash]
    if closestStashDist > Config.crates[stash.size].distance then return end

    local employmentState = exports["FIXDEV-business"]:IsEmployedAt("statecontracting")
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(Config.crates[stash.size].destroyLength, _L("storage-destroying", "Destroying..."))

    ClearPedTasksImmediately(PlayerPedId())

    if finished == 100 then
        TriggerServerEvent("FIXDEV-storage:destroyStash", closestStash)
        if not Config.crates[stash.size].vendor then return end
        local isCrateYoung = RPC.execute("FIXDEV-storage:canBeDeplaced", stash.id)
...
AddEventHandler("FIXDEV-storage:checkLifetime", function ()
    local closestStash, closestStashDist = getClosestStash(10)
    if closestStash == nil then return end
    local stash = Storages[closestStash]
    if closestStashDist > Config.crates[stash.size].distance then return end

    TriggerServerEvent("FIXDEV-storage:getRemainingLife", closestStash)
end)

AddEventHandler("FIXDEV-storage:repairStorage", function ()
    local closestStash, closestStashDist = getClosestStash(10)
    if closestStash == nil then return end
    local stash = Storages[closestStash]
    ...
    local hasRepairKit = exports["FIXDEV-inventory"]:hasEnoughOfItem("craterepairkit", 1, false, true)
    if not hasRepairKit then
        return TriggerEvent("DoLongHudText", _L("storage-repairing-no-kit", "You do not have a repair kit"))
    end

    local info = exports["FIXDEV-inventory"]:GetInfoForFirstItemOfName("craterepairkit")
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(definition.repairLength, _L("storage-repairing", "Repairing..."))

    ClearPedTasksImmediately(PlayerPedId())

    if finished ~= 100 then return end

    TriggerServerEvent("FIXDEV-storage:repairStorage", closestStash, chargesLeft, info.slot)
end)
...
AddEventHandler("FIXDEV-inventory:itemUsed", function (name, info)
    ...
    local progress = exports["FIXDEV-taskbar"]:taskBar(Config.crates[stash.size].lockLength, _L("storage-locking", "Locking..."))
    ClearPedTasks(PlayerPedId())
    if progress ~= 100 then return end
    ...
    exports['FIXDEV-ui']:openApplication('textbox', {
        callbackUrl = 'FIXDEV-storage:lockStorage',
        key = { crateId = stash.id, item = name },
        items = {element},
        show = true,
    })
...
RegisterUICallback("FIXDEV-storage:lockStorage", function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    exports['FIXDEV-ui']:closeApplication('textbox')
    ...
    if data.key.item == "mobilecratekeylock" then
        ...
        return TriggerServerEvent("FIXDEV-storage:lockStorage", data.values.keycode, data.key.crateId, true)
    end
    TriggerServerEvent("FIXDEV-storage:lockStorage", data.values.name, data.key.crateId)
end)

RegisterNetEvent("FIXDEV-storage:updateLockState")
AddEventHandler("FIXDEV-storage:updateLockState", function (id, state, keyCode)
...
function addStorage(storage)
    ...
    exports["FIXDEV-polyzone"]:AddCircleZone(
        "mobile_stash",
        vector,
...
function explodeCrate(stash)
    ...
    TriggerServerEvent("FIXDEV-storage:destroyStash", stash.id)
end
...
AddEventHandler("onResourceStop", function (resource)
    if resource ~= "FIXDEV-storage" then return end
...
RegisterUICallback("FIXDEV-storage:crackStorage", function(data, cb)
