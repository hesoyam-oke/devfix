RPC.register("FIXDEV-storageunits:get", function()
    return storage_units
end)


RegisterServerEvent("FIXDEV-storageunits:purchase", function(args)
    local pSrc = source
...
RPC.register("FIXDEV-storageunits:occupied", function(storage_unit_id)
    local pSrc = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(pSrc)
        ...
        if result[1] ~= nil then
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "FIXDEV-storageunits:attempt:inventory",
                        args = {
                            id = storage_unit_id,
                        }
                    }
            ...
            
                {
                    ...
                    params = {
                        event = "FIXDEV-storageunits:attempt:access",
                        args = {
                            id = storage_unit_id,
            ...
            if tonumber(storage_owner) == char_id then
                TriggerClientEvent('FIXDEV-context:sendMenu', pSrc, menu_data)  
            else
                for i=1, #keys do
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "FIXDEV-storageunits:attempt:purchase",
                        args = {
                            id = storage_unit_id,
                            price = 70000
                        }
...
RPC.register("FIXDEV-storageunits:give:access", function(storage_unit_id, given_cid)
    local pSrc = source
...
RPC.register("FIXDEV-storageunits:remove:access", function(storage_unit_id, given_cid, given_name)
    local pSrc = source
...
RegisterServerEvent("FIXDEV-storageunit:view", function(storage_unit_id)
    local pSrc = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(pSrc)
    exports["oxmysql"]:execute("SELECT `given_access` FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
                            ...
                            TriggerClientEvent('FIXDEV-context:sendMenu', pSrc, {
                                {
                                    ...
                                    txt = "CID: " ..result[i].id,
                                    params = {
                                        event = "FIXDEV-storageunit:attempt:remove",
                                        args = {
                                            id = storage_unit_id,
...
RPC.register("FIXDEV-storageunits:attenpt:transfer", function(storage_unit_id, transfer_cid)
    local pSrc = source
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(pSrc)
    local char_id = user:getCurrentCharacter().id
