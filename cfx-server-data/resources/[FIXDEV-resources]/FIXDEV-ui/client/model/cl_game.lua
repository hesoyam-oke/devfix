...
AddEventHandler('FIXDEV-config:configReady', function()
    loadPublicData()
end)

function loadPublicData()
    local showWatermark = exports['FIXDEV-config']:GetMiscConfig('hud.watermark')
    local showroomPurchase = exports['FIXDEV-config']:GetMiscConfig('showrooms.catalog.purchase')
    local skipMdwProfileCheck = exports['FIXDEV-config']:GetMiscConfig('mdw.profiles.skip')
...
function sendCharacterData()
    Citizen.CreateThread(function()
        ...
        SendUIMessage({ source = "FIXDEV-nui", app = "character", data = character });
        
        local endpointData = exports["FIXDEV-fiber"]:GetGameDetails()
        SendUIMessage({ source = "FIXDEV-nui", app = "game", data = endpointData });

        Citizen.Wait(5000)

        TriggerEvent('FIXDEV-ui:phoneReady')
        TriggerServerEvent('FIXDEV-ui:phoneReady')
    end)
end
...
-- AddEventHandler("FIXDEV-ui:hud:values", function(values)
--   -- sendAppEvent("hud", {
--   --     food = values["hunger"],
