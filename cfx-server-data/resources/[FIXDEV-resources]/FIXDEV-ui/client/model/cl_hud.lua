...
local cmdsSet = {}
local vehBypass = {}
RegisterNUICallback("FIXDEV-ui:hudSetPreferences", function(data, cb)
    ...
    if data["hud.presets"] then
      ...
      for _, p in pairs(data["hud.presets"]) do
        ...
        if not cmdsSet[idx] then
          ...
          exports["FIXDEV-keybinds"]:registerKeyMapping("", "HUD", "Enable " .. s, "+hud_enable_" .. s, "-hud_enable_" .. s)
          RegisterCommand('+hud_enable_' .. s, function()
            exports["FIXDEV-ui"]:sendAppEvent("preferences", eventOptions)
          end, false)
    ...
    TriggerEvent("FIXDEV-preferences:setPreferences", data)
end)
...
AddEventHandler("FIXDEV-ui:watch", function()
...
AddEventHandler('FIXDEV-inventory:itemCheck', function (item, hasItem, quantity)
...
AddEventHandler("FIXDEV:voice:proximity", function(proximity)
    sendAppEvent("hud", {
        voiceRange = proximity,
    })
end)

AddEventHandler("FIXDEV:voice:transmissionStarted", function(data)
    sendAppEvent("hud", {
...
AddEventHandler("FIXDEV:voice:transmissionFinished", function()
    sendAppEvent("hud", {
        voiceActive = false,
...
RegisterNetEvent("FIXDEV-admin:currentDebug")
AddEventHandler("FIXDEV-admin:currentDebug", function(debugToggle)
...
AddEventHandler('FIXDEV-vehicles:pursuitMode', function (pEnabled, pMode, pModes)
...
AddEventHandler('FIXDEV-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('FIXDEV-boosting:client:setBoostCompletions')
AddEventHandler('FIXDEV-boosting:client:setBoostCompletions', function (pAmount)
...
RegisterNetEvent('FIXDEV-weapons:client:setWeaponFireRate')
AddEventHandler('FIXDEV-weapons:client:setWeaponFireRate', function (pAmount)
    weaponFireRate = pAmount
    setHudValue("weaponFireRate", weaponFireRate)
end)

RegisterNetEvent('FIXDEV-weapons:client:showWeaponFireRate')
AddEventHandler('FIXDEV-weapons:client:showWeaponFireRate', function (pToggle)
    showWeaponFireRate = pToggle
    setHudValue("showWeaponFireRate", pToggle)
end)

RegisterNetEvent('FIXDEV-ui:hud:forceShowMinimap', function(pForce)
...
AddEventHandler('FIXDEV-drifting:showDriftMode', function (pToggle)
    showDriftMode = pToggle
    setHudValue("showDriftMode", pToggle)
end)

AddEventHandler('FIXDEV-drifting:toggleDriftMode', function (pToggle)
    driftMode = pToggle
...
AddEventHandler("FIXDEV-drifting:remainingTireThread", function (pThread)
...
function roundedRadar()
    ...
    Citizen.CreateThread(function()
        ...
        -- for k,v in pairs(exports["FIXDEV-base"]:getModule("Blips")) do
...
function getFuel()
    fuel = exports["FIXDEV-vehicles"]:CurrentFuel() or 0
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == GetPed() then
                harnessDurability = exports["FIXDEV-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end

            local degradation = exports["FIXDEV-vehicles"]:GetVehicleDegradation(veh)
...
AddEventHandler('FIXDEV-ui:setVehicleBypassed', function(veh, bypass)
...
AddEventHandler('FIXDEV-ui:setGaze', function (pEntity, pDelay)
    GazeEntity = pEntity
    GazeDelay = pDelay
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if toggleParachuteOff and pState == -1 then
      ...
      exports["FIXDEV-ui"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["FIXDEV-ui"]:sendAppEvent("hud", { hasParachute = true })
end)

AddEventHandler("FIXDEV-island:onIsland", function(pState)
    onIsland = pState
end)
