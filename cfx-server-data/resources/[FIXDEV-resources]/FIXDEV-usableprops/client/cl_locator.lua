...
AddEventHandler('FIXDEV-inventory:itemUsed', function(item, info, inventoryName, slot)
  ...
  if item == 'gps_locator' then
    ...
    local locations = RPC.execute('FIXDEV-usableprops:getLocatorPositions', trackerID)
...
function checkForTrackerId(item, meta, slot)
  ...
  local prompt = exports['FIXDEV-ui']:OpenInputMenu({ { label = 'Enter ID', name = 'id' } }, function(values)
