...
local function benchObjectHandler(pBenchType, pItem)
    ...
    local id = exports['FIXDEV-objects']:PlaceAndSaveObject(
...
local itemHandlers = {
    ...
    ['pailshovel'] = function(info)
        ...
        local finished = exports["FIXDEV-taskbar"]:taskBar(15000, "Building Sandcastle", false, true, false, false, nil, 5.0, ped)
        ClearPedTasks(ped)
    ...
    ['letterbox'] = function(info)
        ...
        local properties = exports['FIXDEV-housing']:getHousingData()
        ...
        local name
        if elements then
            local prompt2 = exports['FIXDEV-ui']:OpenInputMenu(elements)
            if not prompt2 then
                return false
            end
            ...
            if mailType == 'property' then
                local housingTable = exports['FIXDEV-housing']:retriveHousingTable()
            ...
            elseif mailType == 'business' then
                local zones = exports['FIXDEV-polyzone']:GetZones(plyCoords)
                if not zones then
        ...
        local results = exports['FIXDEV-objects']:PlaceObject(
            model,
            {
                forceGroundSnap = true,
                zOffset = -0.05,
        ...
        local objId = RPC.execute('FIXDEV-usableprops:placeLetterbox', model, results[2].coords, results[2].rotation, mailType, foundInfo, name)
        if objId then
    ...
    ['storesign'] = function(info)
        ...
        local objId = RPC.execute('FIXDEV-usableprops:placeStoreSign', model, results[2].coords, results[2].rotation, foundInfo, name)
        if objId then
    ...
    ['phonebomb'] = function(info)
        ...
        if not target or target == 0 then
            ...
            -- Place as an object
            local result = exports['FIXDEV-objects']:PlaceObject(prop, {
                forceGroundSnap = true,
                distance = 3.0,
            })
            ...
            RPC.execute('FIXDEV-usableprops:plantPhoneBombProp', meta, prop, result[2].coords, result[2].rotation)
            return
        end
        ...
        local finished = exports['FIXDEV-taskbar']:taskBar(20000, 'Placing Device', false, true, false, false, nil, 3.0, PlayerPedId())
        ClearPedTasks(ped)
        if finished == 100 and exports['FIXDEV-inventory']:hasEnoughOfItem('phonebomb', 1, false, true) then
            local meta = json.decode(info)
            RPC.execute('FIXDEV-usableprops:plantPhoneBomb', NetworkGetNetworkIdFromEntity(target), meta)
        end
    end,
    ['movementsensor'] = function(info)
        ...
        Wait(1000)
        TriggerEvent('animation:PlayAnimation', 'mechanic4')
        local finished = exports['FIXDEV-taskbar']:taskBar(7500, 'Placing Device', false, true, false, false, nil, 3.0, PlayerPedId())
        ClearPedTasks(ped)
        if finished == 100 and exports['FIXDEV-inventory']:hasEnoughOfItem('movementsensor', 1, false, true) then
            RPC.execute('FIXDEV-usableprops:placeMovementSensor', prop, result[2].coords, result[2].rotation, prompt.name, length)
        end
    end,
    ['npolaroid_camera'] = function(info)
        ...
        if IsEntityPlayingAnim(ped, animDict, animation, 3) then
            ...
            NPX.Events.emit('FIXDEV-polaroid-capture:camera:start')
            usingAnimProp = true
        end
        return true, animDict, animation
    end
...
function handleStopUsingProp()
    if currentAnimProp ~= nil and currentAnimProp == "npolaroid_camera" then
        NPX.Events.emit('FIXDEV-polaroid-capture:camera:stop')
        currentAnimProp = nil
    end
end
...
local function isPD()
    local job = exports["FIXDEV-base"]:getModule("LocalPlayer"):getVar("job")
...
Citizen.CreateThread(function()
    exports["FIXDEV-interact"]:AddPeekEntryByModel({ 2108567945, -1940238623 }, {{
        event = "FIXDEV-usableprops:payParkingMeter",
        id = "FIXDEV-usableprops:parking_pay",
        icon = "dollar-sign",
    ...
    exports["FIXDEV-interact"]:AddPeekEntryByModel({ `prop_skid_tent_01`, `prop_skid_tent_03` }, {{
        event = "FIXDEV-usableprops:pickupTent",
        id = "FIXDEV-usableprops:pickup_tent",
        icon = "arrow-up",
        label = "pickup tent",
        parameters = {},
    }}, {
        distance = { radius = 2.0 },
        isEnabled = function(pEntity)
          return exports['FIXDEV-objects']:GetObjectByEntity(pEntity) ~= nil
        end,
    })

    exports["FIXDEV-interact"]:AddPeekEntryByModel({ `prop_bench_04`, `prop_ld_bench01`, `prop_snow_bench_01` }, {{
        event = "FIXDEV-usableprops:pickupBench",
        id = "FIXDEV-usableprops:pickup_bench",
        icon = "arrow-up",
        label = "pickup bench",
        parameters = {},
    }}, {
    ...
    exports["FIXDEV-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_letterbox_04`, `prop_postbox_01a` }, {{
        event = "FIXDEV-usableprops:openMail",
        id = "FIXDEV-usableprops:open_mailbox",
        icon = "envelope",
        label = "deposit mail",
        parameters = {},
    }}, {
    ...
    exports["FIXDEV-interact"]:AddPeekEntryByModel({`np_sfsign_open`, `np_sfsign_closed` }, {{
        event = "FIXDEV-usableprops:switchSign",
        id = "FIXDEV-usableprops:switch_sign",
        icon = "hammer",
        label = "Switch Sign",
        parameters = {},
    }}, {
        ...
        isEnabled = function(pEntity)
            ...
            return exports['FIXDEV-housing']:hasPropertyAccess(object.data.metadata.id, true)
        end,
    })

    exports["FIXDEV-interact"]:AddPeekEntryByModel({`np_sfsign_open`, `np_sfsign_closed` }, {{
        event = "FIXDEV-usableprops:removeStoreSign",
        id = "FIXDEV-usableprops:remove_sign",
        icon = "arrow-up",
        label = "Remove Sign",
        parameters = {},
    }}, {
    ...
    exports["FIXDEV-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_postbox_01a` }, {{
        event = "FIXDEV-usableprops:checkMail",
        id = "FIXDEV-usableprops:check_mailbox",
        icon = "envelope-open",
        label = "check mail",
        parameters = {},
    }}, {
        ...
        isEnabled = function(pEntity)
            ...
            return (mailType == 'business' and (exports['FIXDEV-business']:HasPermission(info, 'stash_access') or isPD())) or
            (mailType == 'property' and exports['FIXDEV-housing']:hasPropertyAccess(info, true)) or
            (mailType == 'mrpd' and isPD()) or
            (mailType == 'pillbox' and (job == 'ems' or job == 'doctor'))
        end,
    })

    exports["FIXDEV-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03` }, {{
        event = "FIXDEV-usableprops:removeMailbox",
        id = "FIXDEV-usableprops:remove_mailbox",
        icon = "trash-alt",
        ...
        isEnabled = function(pEntity)
            local info, mailType = getMailboxInfo(pEntity)
            return mailType == 'business' and (exports['FIXDEV-business']:HasPermission(info, 'stash_access')) or
            (mailType == 'property' and exports['FIXDEV-housing']:hasPropertyAccess(info, false))
        end,
    })

    exports["FIXDEV-interact"]:AddPeekEntryByModel({ `prop_prototype_minibomb` }, {{
        event = "FIXDEV-usableprops:hackSensor",
        id = "FIXDEV-usableprops:hackSensor",
        icon = "laptop",
        label = "hack",
        parameters = {},
    }}, {
    ...
    NPX.Procedures.execute('FIXDEV-datagrid:subscribe', 'storesigns')
    NPX.Procedures.execute('FIXDEV-datagrid:subscribe', 'mailboxes')
    NPX.Procedures.execute('FIXDEV-datagrid:subscribe', 'tech')
end)

AddEventHandler("FIXDEV-usableprops:stealStopSign", function(pArgs, pEntity, pContext)
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(15000, "Stealing", false, true, false, false, nil, 5.0, ped)
    ClearPedTasks(ped)
    if finished == 100 then
        TriggerServerEvent("FIXDEV-usableprops:deleteLocalEntity", pContext.model, GetEntityCoords(pEntity))
...
RegisterNetEvent("FIXDEV-usableprops:deleteEntity", DeleteEntityAtCoords)
...
AddEventHandler("FIXDEV-usableprops:payParkingMeter", function(args, entity, context)
    PaidMeters[entity] = true
    local success = RPC.execute('FIXDEV-usableprops:payParkingMeter', args.price)
    if success then
        local pos = GetEntityCoords(entity)
        TriggerServerEvent("FIXDEV-scenes:addScene", {
...
AddEventHandler("FIXDEV-usableprops:pickupTent", function(args, entity, context)
    local tent = exports['FIXDEV-objects']:GetObjectByEntity(entity)
    if tent then
        local success = exports['FIXDEV-objects']:DeleteObject(tent.id)
        if success then
...
AddEventHandler("FIXDEV-usableprops:pickupBench", function(args, entity, context)
    local bench = exports['FIXDEV-objects']:GetObjectByEntity(entity)
    if bench then
        local success = exports['FIXDEV-objects']:DeleteObject(bench.id)
        if success then
...
AddEventHandler("FIXDEV-usableprops:switchSign", function(args, entity, context)
    local model = 'np_sfsign_open'
    ...
    RPC.execute('FIXDEV-usableprops:swapStoreSign', model, objId)
    
end)

AddEventHandler('FIXDEV-usableprops:removeStoreSign', function(args, entity, context)
    TriggerEvent("animation:PlayAnimation","mechanic3")
    local finished = exports['FIXDEV-taskbar']:taskBar(15000, "Removing Store Sign", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local obj = exports['FIXDEV-objects']:GetObjectByEntity(entity)
        if not obj then
            return
        end
        exports["FIXDEV-objects"]:DeleteObject(obj.id)
    end
end)

local waitForClose = false
local closeData = {}
AddEventHandler("FIXDEV-usableprops:openMail", function(args, entity, context)
    local info, mailType, object = getMailboxInfo(entity)
    waitForClose = RPC.execute('FIXDEV-usableprops:openMailbox', info, mailType, object.id)
...
AddEventHandler("FIXDEV-usableprops:checkMail", function(args, entity, context)
    ...
    local info, mailType, object = getMailboxInfo(entity)
    RPC.execute('FIXDEV-usableprops:checkMailbox', info, mailType, object.id)
end)

AddEventHandler('FIXDEV-inventory:closed', function(pName)
    if waitForClose and pName == 'mailtemp:' .. closeData.mailType .. ':' .. closeData.objId then
        RPC.execute('FIXDEV-usableprops:closeMailbox', closeData.info, closeData.mailType, closeData.objId)
        waitForClose = false
        closeData = {}
    end
end)

AddEventHandler('FIXDEV-usableprops:removeMailbox', function(args, entity, context)
    TriggerEvent("animation:PlayAnimation","mechanic3")
    local finished = exports['FIXDEV-taskbar']:taskBar(15000, "Removing mailbox", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local obj = exports['FIXDEV-objects']:GetObjectByEntity(entity)
        if not obj then
...
AddEventHandler('FIXDEV-objects:objectCreated', function(pObject, handle)
...
AddEventHandler('FIXDEV-objects:objectDeleted', function(pObject, handle)
    if pObject.data.metadata.sensor then
        SensorRemoved(pObject, handle)
    end
end)

AddEventHandler('FIXDEV-spawn:characterSpawned', function(pCharacterId)
    RPC.execute('FIXDEV-usableprops:checkMailAlerts', pCharacterId)
end)
