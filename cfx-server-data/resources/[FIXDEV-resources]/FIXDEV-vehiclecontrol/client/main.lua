...
RegisterNetEvent("FIXDEV:vehicle:transfer:control")
AddEventHandler("FIXDEV:vehicle:transfer:control", function(pServerID, vNetID)
...
RegisterNetEvent("FIXDEV:vehicle:recover:control")
AddEventHandler("FIXDEV:vehicle:recover:control", function(pServerID, vNetID)
...
function RequestVehicleControl(playerPed, vehicle)
	if DoesEntityExist(playerPed) and IsPedAPlayer(playerPed) then
		...
		TriggerServerEvent("FIXDEV:vehicle:request:control", serverID, netID)
	end
end
...
function RestoreVehicleControl(playerPed, vehicle)
	if DoesEntityExist(playerPed) and IsPedAPlayer(playerPed) then
		local serverID = GetPlayerServerId(NetworkGetPlayerIndexFromPed(playerPed))
		local netID = VehToNet(vehicle)
		TriggerServerEvent("FIXDEV:vehicle:restore:control", serverID, netID)
	end
end

RegisterNetEvent("FIXDEV:vehicle:wheelchair:control")
AddEventHandler("FIXDEV:vehicle:wheelchair:control", function(pArgs, pEntity)
