...
RegisterNetEvent("FIXDEV-nitro:client:placeNitro")
AddEventHandler("FIXDEV-nitro:client:placeNitro", function() 
...
RegisterNetEvent('FIXDEV-nitro:client:particlefx')
AddEventHandler('FIXDEV-nitro:client:particlefx', function (veh)	
	local exhaustNames = {
...
RegisterNetEvent('FIXDEV-nitro:client:particlefisfis')
AddEventHandler('FIXDEV-nitro:client:particlefisfis', function (type, veh)
		...
		if nitroactive and pNitrousLevel > 0 and IsPedInVehicle(PlayerPedId(), GetVehiclePedIsIn(PlayerPedId(), false), false) then
			...
			if fisfismode then
				if IsControlPressed(0, 21) and nitroveh == GetVehiclePedIsIn(PlayerPedId(), false) then -- fışfış
					TriggerServerEvent('FIXDEV-nitro:server:particlefisfis', 'fisfisacc', nitroveh)
					SetVehicleBoostActive(nitroveh, 4) --Boost Sound
					pNitrousInUse = true
					pNitrousLevel = pNitrousLevel - 0.1
				else
					pNitrousInUse = false
					TriggerServerEvent('FIXDEV-nitro:server:particlefisfis', 'fisfiskapatt', nitroveh)
				end
			elseif nitromode then
				if IsControlPressed(0, 21) and nitroveh == GetVehiclePedIsIn(PlayerPedId(), false) then -- nitro
					TriggerServerEvent('FIXDEV-nitro:server:particlefx', nitroveh)
					SetVehicleBoostActive(nitroveh, 4) --Boost Sound
					pNitrousInUse = true
					pNitrousLevel = pNitrousLevel - 0.1
...
AddEventHandler('onResourceStart', function (resource)
	pNitrousLevel = 0
	nitroactive = false
	TriggerServerEvent('FIXDEV-nitro:server:particlefisfis', 'fisfiskapatt')
	nitroveh = nil
	pNitrousInUse = false 
end)
...
Citizen.CreateThread(function()
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Nitrous", "Change Mode", "+nosMode", "-nosMode", "")

	exports["FIXDEV-keybinds"]:registerKeyMapping("", "Nitrous", "Flow Rate Higher", "+changeFlowPlus", "-changeFlowPlus", "")

	exports["FIXDEV-keybinds"]:registerKeyMapping("", "Nitrous", "Flow Rate Lower", "+changeFlowMinus", "-changeFlowMinus", "")
end)