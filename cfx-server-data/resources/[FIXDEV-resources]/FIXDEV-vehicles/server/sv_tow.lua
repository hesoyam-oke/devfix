...
RegisterServerEvent("FIXDEV-vehicles:checkrepo")
AddEventHandler("FIXDEV-vehicles:checkrepo", function(plate)
    local src = source
...
-- RegisterServerEvent("FIXDEV-vehicles:repo")
-- AddEventHandler("FIXDEV-vehicles:repo", function(plate)
--     local src = source
-- 	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                         ['@repoed'] = "1"
--                     })
--                     TriggerClientEvent("FIXDEV-vehicles:repo:success", src)
...
-- RegisterServerEvent("FIXDEV-vehicles:judgerepo")
-- AddEventHandler("FIXDEV-vehicles:judgerepo", function(plate)
--     local src = source
-- 	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RegisterServerEvent("FIXDEV-vehicles:judgerepodelete")
-- AddEventHandler("FIXDEV-vehicles:judgerepodelete", function(plate)
--     local src = source
-- 	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
--     exports.oxmysql:execute("SELECT `license_plate`, `repoed` FROM `characters_cars` WHERE license_plate = ?", {char.id}, function(data)
...
-- RegisterServerEvent("FIXDEV-vehicles:repo2")
-- AddEventHandler("FIXDEV-vehicles:repo2", function(plate)
--     local src = source
-- 	local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                     ['@vehicle_state'] = "In"
--                 })
--                 TriggerClientEvent("FIXDEV-vehicles:repo:success2", src)
--             else
--                 TriggerClientEvent("FIXDEV-vehicles:repo:success2", src)
--             end
--         end
...
-- RegisterServerEvent("FIXDEV-vehicles:release:vehicle")
-- AddEventHandler("FIXDEV-vehicles:release:vehicle", function(ServerID, plate)
...
--         ['repoed'] = "0"
--     })
--     exports['FIXDEV-exports']:removeCash(SrcID, 500)
...
RegisterCommand('towbill', function(source, args)
	...
	exports.oxmysql:execute('SELECT * FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'towunion' then
			...
			TriggerClientEvent("FIXDEV-vehicles:towBill", src, tonumber(args[2]))
		else
		end
	end)
end)