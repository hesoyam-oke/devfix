...
function SetEngineSound(pVehicle, pIsTemporary)
    ...
    if not sound then
        sound = exports['FIXDEV-vehicles']:GetVehicleAfterMarket(pVehicle, 'engineSound')
    end
...
function RequestSyncStateUpdate(pVehicle)
    ...
    Citizen.SetTimeout(50, function()
        ...
        TriggerServerEvent('FIXDEV-vehicleSync:updateSyncState', netId, syncState)
    end)
end
...
function UpdateSyncState(pVehicle, pSyncState)
    ...
    if flags['wheelFitment'] ~= previous['wheelFitment'] then
        local wheelFitment = exports['FIXDEV-vehicles']:GetVehicleWheelFitment(pVehicle)
        if wheelFitment and wheelFitment.width then
            if not DecorExistOn(pVehicle, "FIXDEV-wheelfitment_applied") then
                DecorSetBool(pVehicle, "FIXDEV-wheelfitment_applied", true)
            end

            DecorSetFloat(pVehicle, "FIXDEV-wheelfitment_w_width", wheelFitment.width)
            DecorSetFloat(pVehicle, "FIXDEV-wheelfitment_w_fl", wheelFitment.fl)
            DecorSetFloat(pVehicle, "FIXDEV-wheelfitment_w_fr", wheelFitment.fr)
            DecorSetFloat(pVehicle, "FIXDEV-wheelfitment_w_rl", wheelFitment.rl)
            DecorSetFloat(pVehicle, "FIXDEV-wheelfitment_w_rr", wheelFitment.rr)
        end
    end
...
RegisterNetEvent('FIXDEV-vehicleSync:updateSyncState')
AddEventHandler('FIXDEV-vehicleSync:updateSyncState', function(pNetId, pSyncState)
...
RegisterNetEvent('FIXDEV-vehicles:spawnedVehicle', function(pVehicle)
    Citizen.Wait(1000)
    ...
    local afterMarkets = exports['FIXDEV-vehicles']:GetVehicleAfterMarket(vehicle)
    ...
    local wheelFitment = exports['FIXDEV-vehicles']:GetVehicleWheelFitment(vehicle)
...
RegisterNetEvent('FIXDEV-vehicles:updateAfterMarkets', function(pNetId)
    Citizen.Wait(1000)
    local vehicle = NetworkGetEntityFromNetworkId(pNetId)
...
Citizen.CreateThread(function()
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Vehicle", "Toggle Neon Lights", "+toggleNeonLights", "-toggleNeonLights")
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Vehicle", "Toggle Light Indicator (Right)", "+toggleRightIndicator", "-toggleRightIndicator")
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Vehicle", "Toggle Light Indicator (Left)", "+toggleLeftIndicator", "-toggleLeftIndicator")
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Vehicle", "Toggle Light Indicator (Hazard)", "+toggleHazardLights", "-toggleHazardLights")
end)
