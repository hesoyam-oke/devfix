...
AddEventHandler("mumbleConnected", function()
    ...
    TriggerEvent('FIXDEV:voice:state', true)
end)

AddEventHandler("mumbleDisconnected", function()
    print("Mumble: Disconnected")
    TriggerEvent('FIXDEV:voice:state', false)
end)

AddEventHandler('FIXDEV:voice:state', function (state)
    VoiceEnabled = state

    TriggerServerEvent("FIXDEV:voice:connection:state", state)
...
function AddPlayerToTargetList(serverID, context, transmit)
    if not Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("FIXDEV:voice:transmission:state", serverID, context, true)
        end
...
function RemovePlayerFromTargetList(serverID, context, transmit, refresh)
    if Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("FIXDEV:voice:transmission:state", serverID, context, false)
        end
...
function AddGroupToTargetList(group, context, pIsInVehicle)
    ...
    TriggerServerEvent("FIXDEV:voice:transmission:state", group, context, true, nil, pIsInVehicle)
end
...
function RemoveGroupFromTargetList(group, context, transmit)
    ...
    if transmit then
        TriggerServerEvent("FIXDEV:voice:transmission:state", group, context, false)
    end
end
...
function SetVoiceProximity(proximity)
    ...
    TriggerEvent("FIXDEV:voice:proximity", proximity)
...
RegisterNetEvent("FIXDEV:voice:transmission:state")
AddEventHandler("FIXDEV:voice:transmission:state", function(serverID, context, transmitting, effect, isSourceInVehicle)
    ...
    local effect = exports['FIXDEV-galleria']:GetEffect('RADIO_STRENGTH')
...
RegisterNetEvent('FIXDEV:voice:targets:player:add')
AddEventHandler('FIXDEV:voice:targets:player:add', AddPlayerToTargetList)

RegisterNetEvent('FIXDEV:voice:targets:player:remove')
AddEventHandler('FIXDEV:voice:targets:player:remove', RemovePlayerFromTargetList)

RegisterNetEvent('FIXDEV:voice:refresh')
AddEventHandler('FIXDEV:voice:refresh', RefreshConnection)

RegisterNetEvent('FIXDEV:voice:endpoint:override')
AddEventHandler('FIXDEV:voice:endpoint:override', OverrideEndpoint)

RegisterNetEvent('FIXDEV:voice:proximity:override')
AddEventHandler('FIXDEV:voice:proximity:override', function(pId, pMode, pRange, pPriority)
...
AddEventHandler('FIXDEV-spawn:characterSpawned', RefreshConnection)
...
RegisterNetEvent('FIXDEV-voice:setTransmissionDisabled', function (pType, pDisabled)
...
Citizen.CreateThread(function()
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Voice", "Proximity / Range Toggle", "+cycleProximity", "-cycleProximity", "Z")
    RegisterCommand('+cycleProximity', CycleVoiceProximity, false)
    RegisterCommand('-cycleProximity', function() end, false)

    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Voice", "Reset Mumble VOIP", "+mumble", "-mumble", "")
    ...
    TriggerEvent("FIXDEV:voice:ready")

    SetSettings(exports["FIXDEV-base"]:getModule("SettingsData"):getSettingsTable())
end)

RegisterNetEvent('event:settings:update')
AddEventHandler('event:settings:update', SetSettings)

RegisterNetEvent('FIXDEV-voice:setVoiceProximity')
AddEventHandler('FIXDEV-voice:setVoiceProximity', function(pProximity)
...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        if (isTalking and not isTalkingToggle) or IsTalkingOnRadio and not isTalkingOnRadio then
            ...
            TriggerEvent("FIXDEV:voice:transmissionStarted", { phone = true, radio = IsTalkingOnRadio })
        elseif (not isTalking and isTalkingToggle) or  isTalkingOnRadio and not IsTalkingOnRadio then
            ...
            TriggerEvent("FIXDEV:voice:transmissionFinished", { phone = false, radio = IsTalkingOnRadio })
        end
