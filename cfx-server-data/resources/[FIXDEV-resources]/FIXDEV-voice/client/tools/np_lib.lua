...
RegisterNetEvent('FIXDEV-hud:settings', function(key, value)
    if key ~= 'FIXDEV-preferences' then
        return
    end
    ...
    exports['FIXDEV-voice']:SetSubmixBalance('radio', radioBalance)
    exports['FIXDEV-voice']:SetSubmixBalance('radio_medium_distance', radioBalance)
    exports['FIXDEV-voice']:SetSubmixBalance('radio_far_distance', radioBalance)
    exports['FIXDEV-voice']:SetSubmixBalance('atc', radioBalance)
    exports['FIXDEV-voice']:SetSubmixBalance('drivethru', radioBalance)

    exports['FIXDEV-voice']:SetSubmixBalance('phone', value['phone.balance'])
end)
...
RegisterNetEvent('FIXDEV-admin:cloakStatus', function (pCloaked)
    isCloaked = pCloaked
end)
...
function CanUseFrequency(pFrequency, pNotify, jobOverWrite)
    ...
    local hasPDRadio = exports["FIXDEV-inventory"]:hasEnoughOfItem("radio", 1, false, true)
    local hasCivRadio = exports["FIXDEV-inventory"]:hasEnoughOfItem("civradio", 1, false, true)
...
AddEventHandler('playerSpawned', function()
    ...
    local initialHudSettings = exports["FIXDEV-ui"]:GetPreferences()
    ...
    if initialHudSettings['phone.balance'] ~= nil then
        exports['FIXDEV-voice']:SetSubmixBalance('phone', initialHudSettings['phone.balance'])
    end
