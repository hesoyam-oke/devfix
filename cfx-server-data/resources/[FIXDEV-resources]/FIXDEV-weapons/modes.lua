...
Citizen.CreateThread(function()
    ...
    -- Register Keybinds
    exports["FIXDEV-keybinds"]:registerKeyMapping("", "Weapons", "Switch Weapon Mode", "+switchWeaponMode", "-switchWeaponMode")
...
function SwitchWeaponMode()
	...
	if ForceWeaponMode[pWeaponHash] ~= nil then
		if SELECTED_FIRE_RATE == ForceWeaponMode[pWeaponHash] then
			...
			TriggerEvent("FIXDEV-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		return
	end
...
RegisterNetEvent("FIXDEV-weapons:client:weaponEquiped")
AddEventHandler("FIXDEV-weapons:client:weaponEquiped", function(pWeaponHash, newInformation, sqlID, itemToRemove, pArmed)
	if ForceWeaponMode[pWeaponHash] ~= nil then
		SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
		TriggerEvent("FIXDEV-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
	end
	...
	TriggerEvent("FIXDEV-weapons:client:showWeaponFireRate", pArmed)
	TriggerEvent("FIXDEV-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
end)
