    ...
    data = {
        {
            ...
            icon = "handshake",
            event = "FIXDEV-weed:startCornering",
            parameters = {}
        }
    },
    options = {
        ...
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["FIXDEV-inventory"]:hasEnoughOfItem("weedq", 1, false, true)
    ...
    data = {
        {
            ...
            event = "FIXDEV-weed:stopCornering",
            parameters = {}
        }
    },
    options = {
    ...
    data = {
        {
            ...
            event = 'FIXDEV-weed:prepareBaggies',
            parameters = {},
        }
    },
    options = {
        distance = { radius = 5.0 },
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["FIXDEV-inventory"]:hasEnoughOfItem("weedpackage", 1, false, true)
            return hasWeed
        end
    }
}
...
Citizen.CreateThread(function()
    exports["FIXDEV-interact"]:AddPeekEntryByEntityType(InteractCorner.group, InteractCorner.data, InteractCorner.options)
    exports["FIXDEV-interact"]:AddPeekEntryByEntityType(InteractCornerVehicle.group, InteractCornerVehicle.data, InteractCornerVehicle.options)
    exports["FIXDEV-interact"]:AddPeekEntryByEntityType(InteractStartCorner.group, InteractStartCorner.data, InteractStartCorner.options)
    exports["FIXDEV-interact"]:AddPeekEntryByEntityType(InteractStopCorner.group, InteractStopCorner.data, InteractStopCorner.options)
end)

AddEventHandler("FIXDEV-weed:startCornering", function(pContext, pEntity)
    ...
    local canCorner, message, engMessage = RPC.execute("FIXDEV-weed:startCorner", CurrentCornerCoords)
    ...
    Citizen.CreateThread(function()
        ...
        while CurrentlyCornering do
            ...
            if not foundPed then
                ...
                RPC.execute("FIXDEV-weed:cornerPed", coords, NetworkGetNetworkIdFromEntity(foundPed), NetworkGetNetworkIdFromEntity(CurrentCornerVehicle))
            end
...
AddEventHandler('FIXDEV-weed:cornerSale', function(pContext, pEntity)
    ...
    local hasBaggies = exports['FIXDEV-inventory']:hasEnoughOfItem('weedq', 1, true)
    ...
    RPC.execute('FIXDEV-weed:cornerSyncHandoff', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity))
    ...
    local didSale = RPC.execute('FIXDEV-weed:cornerSale', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity), CurrentCornerZone)
    if didSale then
      PlayAmbientSpeech1(pEntity, 'Chat_State', 'Speech_Params_Force')
      local rndAmt = math.random(1, 3)
      if rndAmt == 1 and exports['FIXDEV-inventory']:hasEnoughOfItem('weedq', 1) then
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
        elseif rndAmt == 2 and exports['FIXDEV-inventory']:hasEnoughOfItem('weedq', 3) then
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
        elseif rndAmt == 3 and exports['FIXDEV-inventory']:hasEnoughOfItem('weedq', 2) then
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
    ...
    if didSale and math.random() < 0.05 then
      TriggerEvent('FIXDEV-mdt:drugsale')
    end
  end)

AddEventHandler("FIXDEV-weed:prepareBaggies", function(pContext, pEntity)
    if CornerConfig.RequireScales then
        local hasScale = exports["FIXDEV-inventory"]:hasEnoughOfItem("qualityscales", 1, false) or exports["FIXDEV-inventory"]:hasEnoughOfItem("smallscales", 1, false)
    ...
    local hasBaggies = exports["FIXDEV-inventory"]:hasEnoughOfItem("emptybaggies", CornerConfig.BaggiesPerBrick / 4, false)
    if not hasBaggies then
    ...
    local finished = exports["FIXDEV-taskbar"]:taskBar(30000, _L('weed-corner-prepare', 'Preparing'), false, true, false, false, nil, 5.0, pEntity)
    if finished == 100 then
        local weedpackages = exports["FIXDEV-inventory"]:getItemsOfType("weedpackage", 1, true)
        if #weedpackages == 0 then return end
        local package = weedpackages[1]
        local info = json.decode(package.information)
        RPC.execute("FIXDEV-weed:prepareBaggies", info)
    end
end)

AddEventHandler("FIXDEV-weed:stopCornering", function(pContext, pEntity)
    TriggerEvent("DoLongHudText", _L('weed-corner-not-selling', 'No longer selling...'))
    stopCornering()
end)

RegisterNetEvent("FIXDEV-weed:addCorneredPed")
AddEventHandler("FIXDEV-weed:addCorneredPed", function(pPed)
    local ped = NetworkGetEntityFromNetworkId(pPed)
    ActivePeds[ped] = false
end)

RegisterNetEvent("FIXDEV-weed:cornerSyncHandoff")
AddEventHandler("FIXDEV-weed:cornerSyncHandoff", function(pPed)
    local ped = NetworkGetEntityFromNetworkId(pPed)
...
RegisterNetEvent("FIXDEV-weed:cornerPed")
AddEventHandler("FIXDEV-weed:cornerPed", function(pPed, pCornerCoords, pVehicle)
    local ped = NetworkGetEntityFromNetworkId(pPed)
    ActivePeds[ped] = true
    UsedPeds[ped] = true
    if NetworkHasControlOfEntity(ped) then
...
RegisterNetEvent("FIXDEV-weed:cleanCornerPeds")
AddEventHandler("FIXDEV-weed:cleanCornerPeds", function()
    UsedPeds = {}
end)
...
function stopCornering()
    ...
    RPC.execute("FIXDEV-weed:stopCorner")
end
