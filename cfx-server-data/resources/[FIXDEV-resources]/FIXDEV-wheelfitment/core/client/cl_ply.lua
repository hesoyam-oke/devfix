DecorRegister("FIXDEV-wheelfitment_applied", 2)
DecorRegister("FIXDEV-wheelfitment_w_width", 1)
DecorRegister("FIXDEV-wheelfitment_w_fl", 1)
DecorRegister("FIXDEV-wheelfitment_w_fr", 1)
DecorRegister("FIXDEV-wheelfitment_w_rl", 1)
DecorRegister("FIXDEV-wheelfitment_w_rr", 1)
--[[
cl_ply.lua
]] -- #[Local Variables]#--
...
function SyncWheelFitment()
    --if isPlyWhitelisted then
        ...
        if didPlyAdjustFitments then
            if not DecorExistOn(plyVeh, "FIXDEV-wheelfitment_applied") then
                DecorSetBool(plyVeh, "FIXDEV-wheelfitment_applied", true)
            end

            DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_width", roundNum(GetVehicleWheelWidth(plyVeh), 2))
            DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_fl", roundNum(GetVehicleWheelXOffset(plyVeh, 0), 2))
            DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_fr", roundNum(GetVehicleWheelXOffset(plyVeh, 1), 2))
            DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_rl", roundNum(GetVehicleWheelXOffset(plyVeh, 2), 2))
            DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_rr", roundNum(GetVehicleWheelXOffset(plyVeh, 3), 2))

            local vin = exports["FIXDEV-garages"]:NearVehicle("sittingplate")
            RPC.execute("FIXDEV-wheelfitment_sv:saveWheelfitment", vin, currentFitmentsToSet)
        ...
        --RPC.execute("FIXDEV-wheelfitment_sv:setIsWheelFitmentInUse", false)
...
function checkVehicleFitment()
    ...
    for _, veh in ipairs(vehicles) do
        ...
        if isNear(plyPos, GetEntityCoords(veh), 150) then
            if DecorExistOn(veh, "FIXDEV-wheelfitment_applied") then
                vehiclesToCheckFitment[#vehiclesToCheckFitment + 1] = {
                    vehicle = veh,
                    w_width = DecorGetFloat(veh, "FIXDEV-wheelfitment_w_width"),
                    w_fl = DecorGetFloat(veh, "FIXDEV-wheelfitment_w_fl"),
                    w_fr = DecorGetFloat(veh, "FIXDEV-wheelfitment_w_fr"),
                    w_rl = DecorGetFloat(veh, "FIXDEV-wheelfitment_w_rl"),
                    w_rr = DecorGetFloat(veh, "FIXDEV-wheelfitment_w_rr")
                }
            end
        end
    end
...
Citizen.CreateThread(function()
	...
	exports["FIXDEV-polyzone"]:AddBoxZone("FIXDEV-wheelfitment:zone2", vector3(135.88, -3030.43, 7.04), 6.4, 4.0, {heading = 0, minZ = 6.04, maxZ = 9.04, data = {id = "wheel_fitment_zone"}})
end)
...
AddEventHandler("FIXDEV-polyzone:enter", function(zone, data)
    if zone ~= "FIXDEV-wheelfitment:zone2" then return end
    Citizen.CreateThread(function()
        local plyPed = PlayerPedId()
        inZone = true
        while inZone do
                ...
                if not isMenuShowing then
                    ...
                    if isNear(plyPos, cfg_wheelFitmentPos, 2.0) then
                        ...
                        if IsControlJustReleased(1, 38) then -- Key: E
                            ...
                            -- RPC.execute("FIXDEV-wheelfitment_sv:setIsWheelFitmentInUse", true)
...
AddEventHandler("FIXDEV-polyzone:exit", function(zone)
    if zone ~= "FIXDEV-wheelfitment:zone2" then
        return
    end
    inZone = false
end)
...
AddEventHandler("FIXDEV-admin:currentDevmode", function(pDevmode)
    devmode = pDevmode
end)

-- #[Event Handlers]#--
RegisterNetEvent("FIXDEV-wheelfitment_cl:applySavedWheelFitment")
AddEventHandler("FIXDEV-wheelfitment_cl:applySavedWheelFitment", function(wheelFitments, plyVeh)
    ...
    DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_width", wheelFitments.width)
    DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_fl", wheelFitments.fl)
    DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_fr", wheelFitments.fr)
    DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_rl", wheelFitments.rl)
    DecorSetFloat(plyVeh, "FIXDEV-wheelfitment_w_rr", wheelFitments.rr)

    performVehicleCheck = true
    checkVehicleFitment()
end)

RegisterNetEvent("FIXDEV-wheelfitment_cl:forceMenuClose")
AddEventHandler("FIXDEV-wheelfitment_cl:forceMenuClose", function()
    if isMenuShowing then
        local plyPed = PlayerPedId()
        local plyVeh = GetVehiclePedIsIn(plyPed, false)
...
RegisterCommand("leavefitment", function()
    TriggerEvent("FIXDEV-wheelfitment_cl:forceMenuClose")
end)
