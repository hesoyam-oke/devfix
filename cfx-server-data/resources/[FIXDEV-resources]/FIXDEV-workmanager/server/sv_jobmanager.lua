...
function NPX.Jobs.CanBecomeJob(self, user, job, callback)
    ...
    -- if NPX.Jobs.ValidJobs[job].requireDriversLicense and not exports["FIXDEV-police"]:CheckLicense(characterId, "Drivers License") then
    ...
    if not hexId or not characterId or not src then callback(false, "Id's don't exist") return end
        ...
        TriggerEvent("FIXDEV-jobmanager:attemptBecomeJob", src, characterId, function(allowed, reason)
...
function NPX.Jobs.SetJob(self, user, job, notify, callback)
    ...
    NPX.Jobs:CanBecomeJob(user, job, function(allowed, reason, rank) 
        ...
        TriggerClientEvent("FIXDEV-jobmanager:playerBecameJob", src, job, name, false)
        TriggerClientEvent("FIXDEV-jobmanager:playerBecomeEvent", src, job, name, notify)
                ...
                for src,data in pairs(tbl) do
                    ...
                    if user then
                        if tonumber(curTime) == tonumber(data.lastPayCheck) or tonumber(data.lastPayCheck) >= 480 then
                            ...
                            exports["FIXDEV-base"]:AddLog("Job Pay", user, "User recieved paycheck, amount: " .. tostring(payCheck))
                        else
...
AddEventHandler('jobssystem:jobs', function(job, src)
    ...
    local jobs = exports["FIXDEV-base"]:getModule("JobManager")
    local user = exports["FIXDEV-base"]:getModule("Player"):GetUser(src)
