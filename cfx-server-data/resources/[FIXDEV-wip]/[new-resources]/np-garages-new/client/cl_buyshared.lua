        ...
        while nearBuy do
            ...
            if job == 'police' or job == 'state' or job == 'sheriff' then
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('FIXDEV-garages:shared:carmenu:open')
                end
            end
        end
    end)
...
RegisterNetEvent("FIXDEV-garages:shared:carmenu:open", function()
    TriggerEvent('FIXDEV-context:sendMenu', {
        ...
        },
        {
			...
			txt = "",
			params = {
                event = "FIXDEV-garages:carmenu:list",
                args = {
...
RegisterNetEvent("FIXDEV-garages:carmenu:list", function(params)
    ...
    TriggerEvent('FIXDEV-context:sendMenu', {
        ...
        },
        {
            ...
            params = {
                event = "FIXDEV-garages:carmenu:list:vehicles",
                args = {
                    job = exports["isPed"]:isPed("myjob"),
                    choice = true
                }
...
RegisterNetEvent("FIXDEV-garages:carmenu:list:vehicles", function(params)
    local selected_job = tostring(params.job)
    ...
    if selected_job == "police" or selected_job == "state" or selected_job == "sheriff" then
        TriggerEvent('FIXDEV-context:sendMenu', {
            ...
            },
            {
            	...
            	params = {
                    event = "FIXDEV-garages:carmenu:attempt:buy",
            		args = {
            			vehicle = "npolvic",
                        price = "20000",
                        shared = p_choice
...
RegisterNetEvent("FIXDEV-garages:carmenu:attempt:buy", function(params)
    local success, receipt = RPC.execute("FIXDEV-garages:attempt:buy", tonumber(params.price))
...
function spawn_veh(car, shared, price)
    ...
    RPC.execute("FIXDEV-garages:purchase:car", car, plate, shared, job, price)
end