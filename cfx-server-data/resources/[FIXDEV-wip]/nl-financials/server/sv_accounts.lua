...
AddEventHandler("SpawnEventsServer", function()
    ...
    local account = exports["FIXDEV-base"]:getChar(src, "bankid")
    if account == 0 then
        ...
        if accountId then
            ...
            if created then
                ...
                if affectedRows and affectedRows ~= 0 then
                    exports["FIXDEV-base"]:setChar(src, "bankid", accountId)
                    TriggerClientEvent("FIXDEV-base:setChar", src, "bankid", accountId)
                end
            end
        end
    end
...
RPC.register("FIXDEV-financials:getAccounts", function(src)
    if not src then return {} end

    local cid = exports["FIXDEV-base"]:getChar(src, "id")
    if not cid then return {} end

    local _groups = exports["FIXDEV-groups"]:groupsBanks(cid)
    local groups = "0"
