...
RPC.register("FIXDEV-financials:bankWithdraw", function(src, pAccountId, pAmount, pComment)
    if not src then
    ...
    local cid = exports["FIXDEV-base"]:getChar(src, "id")
    if not cid then
    ...
    TriggerClientEvent("FIXDEV-financials:ui", src, "cash", "+", pAmount, (cash + pAmount))

    return true, "Succeeded in withdraw $" .. pAmount
end)

RPC.register("FIXDEV-financials:bankDeposit", function(src, pAccountId, pAmount, pComment)
    if not src then
        return false, "Server id dont fount"
    end
...
RPC.register("FIXDEV-financials:bankTransfer", function(src, pSenderAccount, pReceiverAccount, pAmount, pComment)
    if not src then
        return false, "Server id dont fount"
    end
    ...
    local userAccountId = exports["FIXDEV-base"]:getChar(src, "bankid")
    if pSenderAccount == userAccountId then
        TriggerClientEvent("FIXDEV-phone:notification", src, "fas fa-university", "Bank", "You transferred $" .. pAmount .. " to Account ID: " .. pReceiverAccount, 3000)
    end

    local receiverSid = exports["FIXDEV-base"]:getSidWithAccountId(pReceiverAccount)
    if receiverSid ~= 0 then
        TriggerClientEvent("FIXDEV-phone:notification", receiverSid, "fas fa-university", "Bank", "You received a transfer in the amount of $" .. pAmount .. " from Account ID: " .. pSenderAccount, 3000)
    end

    return true, "Succeeded in transfer $" .. pAmount .. " to Account ID:" .. pReceiverAccount
end)

RPC.register("FIXDEV-financials:bankTransactions", function(src, pAccountId)
