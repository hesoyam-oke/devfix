...
function updateCash(src, type, amount)
    ...
    TriggerClientEvent("FIXDEV-financials:ui", src, "cash", type, amount, getCash(src))
...
RegisterNetEvent("FIXDEV-financials:giveCash")
AddEventHandler("FIXDEV-financials:giveCash", function(pPlayer, pAmount)
    local src = source
...
RPC.register("FIXDEV-financials:getCash", function(src)
    return getCash(src)
end)